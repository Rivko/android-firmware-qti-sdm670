...
public class SignExtSampleCase extends AbstractSampleCase {
  private final FidoCryptoClientActivity fidoCryptoClientActivity;

  public SignExtSampleCase(FidoCryptoClientActivity fidoCryptoClientActivity) {
  ...
  private byte[] getSignCommand(byte[] keyHandle, byte authIndex, byte[] appId) {
    ...
    String extId = "EXT1";
    byte[] extensionId = extId.getBytes();
    ...
    int bufferSize = signCmdTag.length + authTag.length + finalChallenge.length + keyHandleAccessToken.length
            ...
            + extensionId.length + fidoCryptoClientActivity.extensionReturned.length + 12;
    ...
    byteBuffer.putShort((short)(extensionId.length +
      fidoCryptoClientActivity.extensionReturned.length + 8));
    byteBuffer.putShort(TAG_EXTENSION_ID);
    byteBuffer.putShort((short) (extensionId.length));
    byteBuffer.put(extensionId);
    byteBuffer.putShort(TAG_EXTENSION_DATA);
    byteBuffer
        .putShort((short) (fidoCryptoClientActivity.extensionReturned.length));
    byteBuffer.put(fidoCryptoClientActivity.extensionReturned);
  ...
  public void test(View v, byte authIndex) throws RemoteException {
    if (fidoCryptoClientActivity.encapsulatedResult == null) {
      Toast.makeText(v.getContext(), "verify user token first",
