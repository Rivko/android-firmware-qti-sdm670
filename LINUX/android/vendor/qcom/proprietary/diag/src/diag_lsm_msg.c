...
Copyright (c) 2007-2011, 2013-2015, 2018 Qualcomm Technologies, Inc.
All Rights Reserved.
...
void msg_send_var (const msg_const_type *pconst_blk, uint32 num_args, ...)
{
	...
	memset(&arg_list, 0, sizeof(va_list));
...
void msg_update_mask(unsigned char *ptr, int len)
{
	...
	for (i = 0; i < MSG_MASK_TBL_CNT && (read_len + sizeof(diag_msg_mask_update_t) < len); i++, mask_ptr++) {
