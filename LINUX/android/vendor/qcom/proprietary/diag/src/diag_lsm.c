...
{
	static unsigned char send_buf[USER_SPACE_DATA];
	static unsigned int cmd_size = 0;
	...
	*(int *)send_buf = USER_SPACE_DATA_TYPE;
	...
	for (i = 0; i < num_bytes; i++) {
		if (buf[i] == CONTROL_CHAR) {
			if (hdlc_disabled && (end != start) && !cmd_size) {
			} else {
				copy_bytes = end-start+1;
				if (cmd_size + offset + copy_bytes >= USER_SPACE_DATA) {
					DIAG_LOGE("diag: In %s, command too large, dropping pkt\n", __func__);
					return success;
				}
				memcpy(send_buf+offset+cmd_size, buf+start, copy_bytes);
				diag_send_data(send_buf, copy_bytes+offset+cmd_size);
				start = i+1;
				end = i+1;
				cmd_size = 0;
				continue;
...
void diag_switch_logging(int requested_mode, char *dir_location_msm)
{
	...
	if (err) {
		DIAG_LOGD("diag:%s: Using library defined connection parameters, con_all = %d\n",
			 __func__, params_con.diag_con_all);
	} else {
