...
public class CarrierConfigFragmentL extends RadioPreferenceFragmentL
    ...
    private void findDependencyCarriers(Carrier carrier, ArrayList<Carrier> list) {
        ...
        if (!TextUtils.isEmpty(carriername)) {
            ...
            while (iterator.hasNext()) {
                Entry<String, RadioGroupPreferenceCategory> entry = iterator.next();
                RadioButtonPreference preference = (RadioButtonPreference) entry.getValue()
                        .findPreference(carriername);
                if (preference != null) {
                    Carrier info = (Carrier)preference.getTag();
                    list.add(info);
                    findDependencyCarriers(info, list);
...
//            }
            ...
            case REQUEST_UPDATE_NV_ITEMS:
                ...
                try {
                    if (Utils.isFileBasedSwitchEnabled() || Utils.isPresetMode()) {
                        ArrayList<Carrier> carrierlist = getCheckedCarrierList();
    ...
    private void travelCarriersStorage_fileBased(CarriersStorage storage) {
        ...
        RadioGroupPreferenceCategory category =
                new RadioGroupPreferenceCategory(getActivity());
        ...
        for (Carrier carrier : storage.mListCarrier) {
            if (!carrier.mName.equals("Default") &&
                    TextUtils.isEmpty(carrier.getHierarchy())) continue;
            ...
            preference.setTitle(carrier.mName);
            preference.setTag(carrier);
            category.addPreference(preference);

            if (mCurrentPreferenceKey.get(mCurrentPreferenceKey.size() - 1)
                    .equals(carrier.mName)) {
