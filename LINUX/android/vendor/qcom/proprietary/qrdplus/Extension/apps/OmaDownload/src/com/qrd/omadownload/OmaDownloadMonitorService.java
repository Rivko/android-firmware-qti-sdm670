...
public class OmaDownloadMonitorService extends Service {
    ...
    private static final int RESPONSE_MESSAGE = 111;// a constant to recognize message
    private static final int RETRY_TIME = 5000;// 5 seconds retry time
    final String[] ddTagMapping = new String[] {
            "objecturi", "size", "type",
                ...
                if (downloadListCursor != null
                            ...
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    postInstallNotification(mContext,
                                            mLastObjectInstallNotifyUrl,
                                            statusReport, objectUri, null);
                                }
                            }.setObjectUri(ddObjectUri)).start();
                        }
                    }
                    ...
                    if (Downloads.Impl.isStatusCompleted(status)
                        ...
                        if (parseDDFile(mContext, filename, i)) {
                            Log.i(TAG,"Parse dd file returned true");
                            notifyConfirmWin(mContext, i);
                        } else {
                            Log.i(TAG,"Parse dd file returned false");
                        }
            ...
            for (int i = mmMonitorList.size() - 1; i >= 0; i--) {
                if (((MMMonitorInfo) mmMonitorList.get(i)).mmUri != null) {
                    ...
                    if (!(OmaDownloadActivity.sIsOmaDlErrorHandlingEnabled)) {
                        ...
                        Uri mObjectUri = ((MMMonitorInfo) mmMonitorList.get(i)).mmUri;
                        Cursor mmObjectMonitorCur = mContext
                                .getContentResolver()
                                .query(mmObjectUri,
                                        new String[] {
                        ...
                        if (mmObjectMonitorCur != null
                            ...
                            if (isNonAcceptableType) {
                                Log.i(TAG,"Non acceptable content");
                                ...
                                Log.i(TAG,"Mimetype did not match");
                            ...
                            if (mmNotifyUrl != null) {
                                ...
                                new Thread(new Runnable() {
                                    ...
                                    public void run() {
                                        postInstallNotification(mContext,
                                                mmNotifyUrl, statusReport, objectUri, null);
                                    }
                                }.setObjectUri(mObjectUri)).start();
                            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && intent.getExtra(OmaDownloadActivity.EXTRA_CONTENT_URI) != null) {
            ddUriMonitorList
                    .add(new DDMonitorInfo((Uri.parse((String) intent
                            .getExtra(OmaDownloadActivity.EXTRA_DATA))),((String) intent
                            .getExtra(OmaDownloadActivity.EXTRA_CONTENT_URI))));
           ContentMonitorObserver contentObserver = new ContentMonitorObserver(
                    Uri.parse((String) intent
                            .getExtra(OmaDownloadActivity.EXTRA_CONTENT_URI)));
    ...
    private boolean parseDDFile(Context ctx, String filename, int index) {
                    ...
                    case XmlPullParser.START_TAG:
                        ...
                        switch (tag) {
                            case OBJECTURI:
                                ...
                                if (((DDMonitorInfo) ddUriMonitorList.get(index)).mObjectURL == null) {
                                    if ((Uri.parse(tempObjectUrl)).isRelative()) {
                                        ...
                                        ((DDMonitorInfo) ddUriMonitorList.get(index)).mObjectURL = tempObjectUrl;
                                }
                            ...
                            case TYPE:
                                ...
                                Log.e(TAG, "TYPE...." + type);
                                if(mLastObjectMimeType
                                        .contains(OmaDownloadActivity.MIMETYPE_DRM_CONTENT)) {
                                    ((DDMonitorInfo) ddUriMonitorList.get(index)).mObjectType = OmaDownloadActivity.MIMETYPE_DRM_CONTENT;
                                } else {
        ...
        if (OmaDownloadActivity.sIsOmaDlErrorHandlingEnabled) {
            if (((DDMonitorInfo) ddUriMonitorList.get(index)).mObjectVersion != null) { // not
                                                                                        // starting
                                                                                        // with 1.0
    ...
    private void notifyConfirmWin(Context ctx, int index) {
        ...
        if (mLastObjectMimeType.toString() != null)
            target.putExtra(OmaDownloadActivity.EXTRA_OBJECTALLMIMETYPE,
                    mLastObjectMimeType.toString());
    ...
    private void postInstallNotification(Context ctx, String postUrl) {
        ...
        try {
            ...
            Log.e(TAG, "Server response " + nErr);
        } catch (Exception e) {
            Log.e(TAG, "Post Failed " + e);
        }
    }

    public static void postInstallNotification(final Context ctx,
            String postUrl, int statusReport, Uri mObjecturl, RetryResponse retry) {
        ...
        try {
            URL serverUrl = new URL(postUrl);
            final HttpURLConnection conn = (HttpURLConnection) serverUrl
                    .openConnection();
            ...
            conn.setDoOutput(true);
            conn.setDoInput(true);
            conn.setConnectTimeout(RETRY_TIME);
            conn.setReadTimeout(RETRY_TIME);
            ...
            if (response >= 200 && response < 300) {// success
                ...
                Log.e(TAG, "Server error, error code is " + response);
                deleteMMFile(ctx, mObjecturl);
            }
            conn.setRequestProperty("Connection", "close");

        } catch (SocketTimeoutException se) {
            Log.e(TAG, "Server Socket Timeout Exception ....." + se);
            if ((ctx.getResources().getBoolean(R.bool.enableRetryResponse))) {
                if (retry == null) {
                    retry = new RetryResponse();
                    retry.postUrl = postUrl;
                    retry.statusReport = statusReport;
                    retry.objectUrl = mObjecturl.toString();
                }
                ...
                mResponseHandler.sendMessageDelayed(message, RETRY_TIME);
            }
        } catch (Exception e) {

            Log.e(TAG, "Post Failed " + e.getStackTrace());
            deleteMMFile(ctx, mObjecturl);
        }
        return;
    }
    ...
    // delete MM file
    private static void deleteMMFile(final Context ctx, Uri mObjecturl) {
        Log.d(TAG, "Error, deleting media file");
        ...
        if (null == mObjecturl) {
            ...
            ctx.getContentResolver().delete(mObjecturl, null, null);
        }
    }

    /* Handler class to send post message at 5seconds interval */
    private static class ResponseHandler extends Handler {

        @Override
        public void handleMessage(final Message msg) {
            if (msg.what == RESPONSE_MESSAGE) {
                final RetryResponse retry = (RetryResponse) msg.obj;
                Log.d(TAG, " handleMessage, retry count = " + retry.retryCount);
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        if (retry != null) {
                            ...
                            if (retry.retryCount < 3) {
                                postInstallNotification(mContext, retry.postUrl,
                                        retry.statusReport,
                                        Uri.parse(retry.objectUrl), retry);
                            }
                        }
                    }
                }).start();
    ...
    /* RetryResponse class to save the retry values */
