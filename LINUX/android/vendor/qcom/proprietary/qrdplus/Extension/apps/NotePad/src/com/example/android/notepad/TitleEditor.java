 ...
 * window containing an EditText. NOTE: Notice that the provider operations in
 * this Activity are taking place on the UI thread. This is not a good practice.
 * It is only done here to make the code more readable. A real application
 * should use the {@link android.content.AsyncQueryHandler} or
 * {@link android.os.AsyncTask} object to perform operations asynchronously on a
 * separate thread.
 */
public class TitleEditor extends Activity {
    ...
    // A Cursor object that will contain the results of querying the provider
    // for a note.
    ...
    public void onCreate(Bundle savedInstanceState) {
         ...
         * until the query completes. In a sample app, going against a simple
         * provider based on a local database, the block will be momentary, but
         * in a real app you should use android.content.AsyncQueryHandler or
        ...
        mCursor = managedQuery(
                ...
                null, // No selection criteria are used, so no where columns are
                      // needed.
                null, // No where columns are used, so no where values are
                      // needed.
     ...
     * stack, OR when it is first starting. Displays the current title for the
     * selected note.
     */
    @Override
    protected void onResume() {
        ...
        // Verifies that the query made in onCreate() actually worked. If it
        // worked, then the
        // Cursor object is not null. If it is *empty*, then mCursor.getCount()
        // == 0.
        if (mCursor != null) {
     ...
     * saved. The Android application model is predicated on the idea that
     * "save" and "exit" aren't required actions. When users navigate away from
     * an Activity, they shouldn't have to go back to it to complete their work.
     * The act of going away should save everything and leave the Activity in a
     * state where Android can destroy it if necessary. Updates the note with
     * the text currently in the text box.
     */
    @Override
    protected void onPause() {
        ...
        if (mCursor != null) {
            ...
            /*
             * Updates the provider with the note's new title. Note: This is
            ...
            getContentResolver().update(
                    ...
                    values, // The values map containing the columns to update
                            // and the values to use.
                    null, // No selection criteria is used, so no "where"
                          // columns are needed.
                    null // No "where" columns are used, so no "where" values
                         // are needed.
                    );
