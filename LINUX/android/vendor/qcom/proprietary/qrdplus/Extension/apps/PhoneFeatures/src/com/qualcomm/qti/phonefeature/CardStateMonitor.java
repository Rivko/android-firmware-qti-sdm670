...
public class CardStateMonitor extends Handler {
    ...
    static class CardInfo {
        ...
        boolean isNetworkLocked() {
            return !(mAppState == null)
                    && mAppState.equals(AppState.APPSTATE_SUBSCRIPTION_PERSO.toString());
        }
    ...
    private String[] mSimStatus = new String[Constants.PHONE_COUNT];
    ...
    public void handleRadioTechnologyChanged(int phoneId) {
        UiccCard uiccCard = getUiccCard(phoneId);
        logd("handleRadioTechnologyChanged on slot = " + phoneId + ", card state = "
    ...
    public void handleSimStateChanged(int phoneId, String simStatus) {
        mSimStatus[phoneId] = simStatus;
        logd("handleSimStateChanged on slot = " + phoneId
                + ", simStatus = " + simStatus);
        if (isSpnAvailable(phoneId)) {
    ...
    private void notifyAllCardsAvailableIfNeed() {
        for (int index = 0; index < Constants.PHONE_COUNT; index++) {
            if (!mCards[index].isCardAvailable()
                    || ((!isSpnAvailable(index)) && (!mCards[index].isNetworkLocked())))
                return;
        }
        mAllCardsInfoAvailableRegistrants.notifyRegistrants();
    }

    private boolean isSpnAvailable (int slotId) {
        if (isCardDeactivated(slotId)){
            return true;
        }
        if (mSimStatus[slotId] != null) {
            return IccCardConstants.INTENT_VALUE_ICC_LOADED.equals(mSimStatus[slotId])
                   || IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(mSimStatus[slotId]);
        }
        return false;
    }
    ...
    boolean isCardDeactivated(int cardIndex) {
        ...
        SubscriptionInfo sir = SubscriptionManager.from(mContext)
                .getActiveSubscriptionInfoForSimSlotIndex(cardIndex);
    ...
    int getProvisionPref(int slotId) {
        ...
        try {
            ...
            logd("RemoteException @getProvisionPref" + ex + " slotId " + slotId);
        } catch (NullPointerException ex) {
            logd("NullPointerException @getProvisionPref" + ex + " slotId " + slotId);
        }
