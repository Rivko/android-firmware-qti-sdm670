...
public class DateViews implements RemoteViewsService.RemoteViewsFactory,
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == MSG_NOTIFY_CHANGE
                    ...
                    && mLunarLoadingFinished
                    && mEventLoadingFinished
                    && mLastMonthLunarLoadingFinished
                    && mNextMonthLunarLoadingFinished) {
                if (Utility.DEBUG) {
    ...
    private boolean mLastMonthLunarLoadingFinished;
    private final LunarLoader mLastMonthLunarLoader;
    private LunarLoader.LunarCursor mLastMonthLunarCursor;
    private OnLoadCompleteListener<Cursor> mLastMonthLunarListener = new
        ...
        public void onLoadComplete(Loader<Cursor> loader, Cursor data) {
            ...
            mLastMonthLunarLoadingFinished = true;
            mLastMonthLunarCursor = (LunarLoader.LunarCursor) data;
            mHandler.removeMessages(MSG_NOTIFY_CHANGE);
            mHandler.sendEmptyMessageDelayed(MSG_NOTIFY_CHANGE, MSG_DELAY);
        }
    };

    private boolean mNextMonthLunarLoadingFinished;
    private final LunarLoader mNextMonthLunarLoader;
    private LunarLoader.LunarCursor mNextMonthLunarCursor;
    private OnLoadCompleteListener<Cursor> mNextMonthLunarListener = new
            OnLoadCompleteListener<Cursor>() {
        ...
        public void onLoadComplete(Loader<Cursor> loader, Cursor data) {
            ...
            mNextMonthLunarLoadingFinished = true;
            mNextMonthLunarCursor = (LunarLoader.LunarCursor) data;
            mHandler.removeMessages(MSG_NOTIFY_CHANGE);
            mHandler.sendEmptyMessageDelayed(MSG_NOTIFY_CHANGE, MSG_DELAY);
        }
    };
    ...
    public DateViews(Context context, int[] appWidgetIds) {
        ...
        mLastMonthLunarLoader = new LunarLoader(mContext);
        mLastMonthLunarLoader.registerListener(LUNAR_LOADER, mLastMonthLunarListener);
        mNextMonthLunarLoader = new LunarLoader(mContext);
        mNextMonthLunarLoader.registerListener(LUNAR_LOADER, mNextMonthLunarListener);
    ...
    private void loadCursor(int field, Time time) {
        ...
        default:
            if (field > FIELD_EVENT) {
                ...
                mLastMonthLunarLoadingFinished = false;
                Time lastTime = new Time();
                lastTime.year = time.year;
                if(time.month == 0){
                    lastTime.month = time.month - 1 + 12;
                    lastTime.year = time.year - 1;
                } else {
                    lastTime.month = time.month - 1;
                }
                mLastMonthLunarLoader.load(lastTime);
                mNextMonthLunarLoadingFinished = false;
                Time nextTime = new Time();
                nextTime.year = time.year;
                if(time.month == 11){
                    nextTime.month = time.month + 1 - 12;
                    nextTime.year = time.year + 1;
                }else {
                    nextTime.month = time.month + 1;
                }
                mNextMonthLunarLoader.load(nextTime);
            }
        }
    }
    ...
    private boolean isLunarCursorValid() {
        return showLunar() && mLunarCursor != null && !mLunarCursor.isClosed()
                && mLastMonthLunarCursor != null && !mLastMonthLunarCursor.isClosed()
                && mNextMonthLunarCursor != null && !mNextMonthLunarCursor.isClosed();
    }
    ...
    private void initView(RemoteViews view) {
        ...
        view.setTextColor(R.id.date, mRes.getColor(R.color.black));
        view.setTextViewText(R.id.lunar, null);
        view.setTextColor(R.id.lunar, mRes.getColor(R.color.black));
    }
    ...
    private void bindBasicViewAt(RemoteViews view, Calendar calendar) {
        ...
        if (Utility.DEBUG) {
            ...
            if (day_of_week == Calendar.SATURDAY
                ...
                view.setTextColor(R.id.date, mRes.getColor(R.color.title));
            }
            if(month != mMonth ){
                view.setTextColor(R.id.date, mRes.getColor(R.color.grey));
            }
    ...
    private void bindLunarViewAt(RemoteViews view, Calendar calendar) {
        ...
        if (month < mMonth && year == mYear) {
            int day = calendar.get(Calendar.DAY_OF_MONTH);
            if(mLastMonthLunarCursor.isClosed()){
                return;
            }
            mLastMonthLunarCursor.moveToPosition(day - 1);
            view.setTextViewText(R.id.lunar, mLastMonthLunarCursor.getLunar());
            view.setTextColor(R.id.lunar, mRes.getColor(R.color.grey));
        }
        if (month > mMonth && year < mYear) {
            int day = calendar.get(Calendar.DAY_OF_MONTH);
            if(mLastMonthLunarCursor.isClosed()){
        ...
        if (month > mMonth & year == mYear) {
            int day = calendar.get(Calendar.DAY_OF_MONTH);
            if(mNextMonthLunarCursor.isClosed()){
                return;
            }
            mNextMonthLunarCursor.moveToPosition(day - 1);
            view.setTextViewText(R.id.lunar, mNextMonthLunarCursor.getLunar());
            view.setTextColor(R.id.lunar, mRes.getColor(R.color.grey));
        }
    ...
    public RemoteViews getViewAt(int position) {
        ...
        calendar.clear();
        calendar.setFirstDayOfWeek(Utility.getStartDayOfWeek(mContext));
        calendar.set(Calendar.YEAR, mYear);
        calendar.set(Calendar.MONTH, mMonth);
        calendar.set(Calendar.DAY_OF_WEEK,
                (position + Utility.getStartDayOfWeek(mContext)) % 7);
        calendar.set(Calendar.WEEK_OF_MONTH, (position / 7) + 1);
        RemoteViews view;
        if(showLunar()){
            view = new RemoteViews(mContext.getPackageName(), R.layout.date);
        }else {
            view = new RemoteViews(mContext.getPackageName(), R.layout.date_nolunar);
        }
    ...
    public RemoteViews getLoadingView() {
        RemoteViews views;
        if(showLunar()){
            views = new RemoteViews(mContext.getPackageName(), R.layout.date);
        }else {
            views = new RemoteViews(mContext.getPackageName(), R.layout.date_nolunar);
        }
        initView(views);
        return views;
    }
