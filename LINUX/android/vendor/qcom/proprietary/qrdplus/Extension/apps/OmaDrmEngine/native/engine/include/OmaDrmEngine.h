 ...
 */
class OmaDrmEngine: public android::DrmEngineBase {

public:
    OmaDrmEngine();
    virtual ~OmaDrmEngine();
...
protected:
    ...
    status_t onSetPlaybackStatus(int uniqueId,
            ...
            int64_t position);
#else
    status_t onSetPlaybackStatus(int uniqueId, DecryptHandle* decryptHandle,
     ...
     *     DRM_ERROR_CANNOT_HANDLE for failure and DRM_NO_ERROR for success
     */
    status_t onFinalizeDecryptUnit(int uniqueId, DecryptHandle* decryptHandle,
            int decryptUnitId);
     ...
     * @retval -1 Failure.
     */
    ssize_t onRead(int uniqueId, DecryptHandle* decryptHandle, void* pBuffer,
            int numBytes);
    ...
    off64_t onLseek(int uniqueId,
            ...
            int whence);
#else
    off_t onLseek(int uniqueId, DecryptHandle* decryptHandle, off_t offset,
            int whence);
#endif
    ...
    ssize_t onPread(int uniqueId,
            ...
            off64_t offset);
#else
    ssize_t onPread(int uniqueId, DecryptHandle* decryptHandle, void* buffer,
...
private:
    ...
    public:
        ...
        ConvertSession() {
            uniqueId = 0;
            memset(&output, 0, sizeof(OmaDrmConv_Output_t));
        }
    ...
    IDrmEngine::OnInfoListener *infoListenerClient;
    int infoListenerClientId = -1;

    int checkAndGetDrmRights(int fileDesc,
            OmaDrmRightsManager_Rights_t *omaDrmRights);

    int consumeDrmRights(int fileDesc);

    int checkForAction(int fileDesc, String8 path);

    int fillExtendedDataRights(KeyedVector<String8, String8> *extendedData,
            OmaDrmRightsManager_Rights_t *rights);

    int fillExtendedDataRightsPermission(
            KeyedVector<String8, String8> *extendedData,
            OmaDrmRightsManager_RightsPermission_t *permission);

    int getStaticFileMimetypes(const String8& path, String8& mimeString);

    int showExpiredDialog(int fileDesc);
     ...
     */
    static int getConvertedStatus(OmaDrmConv_Status_t status);
};
