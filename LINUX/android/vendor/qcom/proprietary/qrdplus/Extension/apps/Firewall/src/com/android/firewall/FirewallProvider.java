...
public class FirewallProvider extends ContentProvider {
    ...
    private static final int DEVICE_API_BLOCKED_CALLOG = 12;
    ...
    static {
        ...
        s_urlMatcher.addURI("com.android.firewall",
                "deviceapi-blocked-calllog", DEVICE_API_BLOCKED_CALLOG);
    }
    ...
    private static final String METHOD_IS_FORBIDDEN = "isForbidden";
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            RcsMessageProviderUtils.createDeviceApiBlockedCallogView(db);
            initDatabase(db);
        }
    ...
    public Cursor query(Uri url, String[] projectionIn, String selection,
        ...
        // do nothing
            ...
            case DEVICE_API_BLOCKED_CALLOG: {
                SQLiteDatabase db = mOpenHelper.getReadableDatabase();
                Cursor ret = db.query(
                        RcsMessageProviderConstants.DeviceApiViews.BLOCKED_CALLLOGS,
                        projectionIn, selection, selectionArgs, null, null, sort);
    ...
    public Uri insert(Uri url, ContentValues initialValues) {
        ...
        switch (match) {
            case BLACKLIST_ALL: {
                ...
                if (!validateAndPrepareNormalizedNumber(values)) {
                    Log.d(TAG, "blacklist: Invalid number");
                    return null;
                }
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        switch (match) {
            case BLACKLIST_ALL: {
                count = deleteNumber(db, BLACKLIST_TABLE, where, whereArgs);
                if (count > 0) {
                    getContext().getContentResolver().notifyChange(Blacklist.CONTENT_URI, null);
                }
                break;
            ...
            case WHITELIST_ALL: {
                count = deleteNumber(db, WHITELIST_TABLE, where, whereArgs);
                if (count > 0) {
                    getContext().getContentResolver().notifyChange(Whitelist.CONTENT_URI, null);
                }
                break;
    ...
    private int deleteNumber(SQLiteDatabase db, String table, String where, String[] whereArgs) {
        int count = 0;
        if (whereArgs != null) {
            String orgNum = whereArgs[0];
            whereArgs = new String[] {
                getNomalizeNumber(orgNum)
            };
            count = db.delete(table, where, whereArgs);
            if (Config.LOGD)
                Log.d(TAG, "deleteNumber count " + count);
            if (count <= 0) {
                String minMatchNumber = PhoneNumberUtils.toCallerIDMinMatch(orgNum);
                String minNumber= new StringBuffer(minMatchNumber).reverse().toString();
                where = "number" + " LIKE '%" + minNumber + "'";
                whereArgs = null;
                count = db.delete(table, where, whereArgs);
            }
        }
    ...
    private String getNomalizeNumber(String number) {
        ...
        String el64Num =  PhoneNumberUtils.formatNumberToE164(number, numberLocale.getCountry());
        if (el64Num != null) {
            el64Num = el64Num.replaceAll("[\\-\\/ ]", "");
        }
        return el64Num != null ? el64Num : number;
    }
    ...
    public Bundle call(String method, String arg, Bundle extras) {
        ...
        if (METHOD_IS_FORBIDDEN.equals(method)) {
            extras.putBoolean(METHOD_IS_FORBIDDEN, isForbidden(subscription, number));
            return extras;
        }
        return null;
    }
    ...
    public boolean isForbidden(int subscription, String number) {
        ...
        String minNumber= new StringBuffer(minMatchNumber).reverse().toString();
        boolean result = isForbiddenNumber(subscription, minNumber);
        Log.d(TAG, "minNumber = " + minNumber + " number = " + number + " result" + result);
        return result;
    }
    ...
    private boolean validateAndPrepareNormalizedNumber(ContentValues cv) {
        if (cv.containsKey(Blacklist.NUMBER)) {
            String number = cv.getAsString(Blacklist.NUMBER);
            ...
            if (el64Num != null) {
                el64Num = el64Num.replaceAll("[\\-\\/ ]", "");
                cv.put(Blacklist.NUMBER, el64Num);
            }
            return true;
        }
        return false;
