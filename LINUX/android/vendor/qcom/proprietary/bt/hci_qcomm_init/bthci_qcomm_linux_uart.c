...
extern int trans_fd;    /* JMF: shoddy global */
...
int bt_hci_pfal_init_transport ( bt_hci_transport_device_type bt_hci_transport_device )
{
  ...
  if (tcsetattr(trans_fd, TCSANOW, &term) < 0)
  {
    BTHCI_QCOMM_ERROR("init_uart: Error while getting attributes\n");
    close(trans_fd);
    return -1;
...
int bt_hci_qcomm_nwrite(uint8 *buf, int size)
{
  ...
  do
  {
    nwrite = write(trans_fd, (buf + tx_bytes), (size - tx_bytes));
    if (nwrite < 0) 
    {
...
int bt_hci_qcomm_nread(uint8 *buf, int size)
{
  ...
  {
    FD_ZERO (&infids);
    FD_SET (trans_fd, &infids);
    timeout.tv_sec = 3;
    timeout.tv_usec = 0;  /* half second is a long time at 115.2 Kbps */

    if (select (trans_fd + 1, &infids, NULL, NULL, &timeout) < 1)
    {
       fprintf(stderr, "bt_hci_qcomm_nread: read BT SoC timed out.\n");
       return rx_bytes;
    }

    nread = read(trans_fd, (buf + rx_bytes), (size - rx_bytes));
    if (nread < 0) 
    {
...
boolean bt_hci_qcomm_pfal_changebaudrate (unsigned long new_baud)
{
  ...
  {  
...
#if defined(BOTHER)
    if (ioctl(trans_fd, TCGETS2, &term2) == -1)
    {
    ...
    if (ioctl(trans_fd, TCSETS2, &term2) == -1)
    {
      perror("bt_hci_qcomm_pfal_changebaudrate: TCGETS2:");
      return FALSE;
    }
    ...
    if (tcsetattr(trans_fd, TCSADRAIN, &term) < 0) /* don't change speed until last write done */
    {
