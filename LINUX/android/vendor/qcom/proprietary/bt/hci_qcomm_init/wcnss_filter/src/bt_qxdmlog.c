...
#define UART_IPC_LOG_COLLECTION_OLD_PATH  "/sys/kernel/debug/ipc_logging/msm_serial_hs/log_cont"
...
#define UART_IPC_LOG_TX_FILE_NAME    ".uart_tx/log_cont"
#define UART_IPC_LOG_RX_FILE_NAME    ".uart_rx/log_cont"
#define UART_IPC_LOG_STATE_FILE_NAME    ".uart_state/log_cont"
#define UART_IPC_LOG_PWR_FILE_NAME    ".uart_pwr/log_cont"
...
bool is_force_hcidump_enabled() {
    ...
    ret = property_get("wc_transport.force_hcidump", value, NULL);
    ALOGI("%s: property_get: wc_transport.force_hcidump: %d", __func__, ret);
...
static int is_unified_logging_enabled() {
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("unified_bt_logging", value, "false");
...
static int dump_UART_logs(char* spath, char* dpath, int buffer_size, int water_mark) {
    ...
    do {
        ...
        if (ret <= 0) {
            ALOGE("%s: Error reading s file: %d (%s)", __func__, ret, strerror(errno) );
            break;
        }
