...
{
  (void) tcflush(trans_fd, TCIOFLUSH); /* so UART doesn't hang when we close */
#if defined(BT_QSOC_POWER_ONOFF_PATH)
...
#define BT_QSOC_TAG_17_TX_SCO_BUFS_HWS 0x00
  ...
  */
  ...
  (void) ioctl(trans_fd, TIOCMGET, &flags);

  flags &= ~TIOCM_RTS;
  if (ioctl(trans_fd, TIOCMSET, &flags) < 0)
  {
    status = FALSE;
...
int main(int argc, char *argv[])
{
  ...
  /* Powerup BTS402x */
    if (!bt_hci_qcomm_powerup()) /* success */
      ...
      trans_fd = bt_hci_pfal_init_transport(bt_hci_transport_device);
