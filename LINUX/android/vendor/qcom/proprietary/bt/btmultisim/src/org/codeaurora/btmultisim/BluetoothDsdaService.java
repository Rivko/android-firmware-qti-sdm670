...
public class BluetoothDsdaService extends Service {
   ...
   //At any time we can update only one active and one held
    ...
    // match with Telephony call state
    final static int NEW = 0;
    final static int CONNECTING = 1;
    final static int DIALING = 3;
    final static int RINGING = 4;
    final static int ACTIVE = 5;
    final static int ON_HOLD = 6;
    final static int DISCONNECTED = 7;
    final static int DISCONNECTING = 9;
    ...
    private int getActiveSubscription() {
        ...
        try {
            Log.w(TAG,"getActiveSubscription()");
            subId = getIExtTelephony().getActiveSubscription();
    ...
    private void switchToOtherActiveSub(int sub) {
        try {
            Log.w(TAG,"switchToOtherActiveSub()");
            getIExtTelephony().switchToActiveSub(sub);
        } catch (RemoteException ex) {
            Log.e("TelephonyConnectionService", "Exception: " + ex);
        } catch (NullPointerException ex) {
            Log.e("TelephonyConnectionService", "Exception: " + ex);
    ...
    */
    ...
    private class BluetoothSub {
        ...
        private BluetoothSub(int PhoneId) {
            ...
            mForegroundCallState = NEW;
            mRingingCallState = NEW;
            mRingNumber = "";
            mRingNumberType = 0;
        ...
        private void handleSubscriptionCallStateChange(int foregroundCallState,
            ...
            if (mForegroundCallState == DISCONNECTING) {
                Log.d(TAG, "Call disconnecting,wait before update");
                return;
            } else {
                mActive = (mForegroundCallState == ACTIVE) ? 1 : 0;
            }
    ...
    private int getBluetoothCallStateForUpdate(int ringingState, int foregroundState) {
        ...
        if (ringingState == RINGING) {
            bluetoothCallState = CALL_STATE_INCOMING;
        } else if (foregroundState == DIALING) {
    ...
    private final IBluetoothDsdaService.Stub mBinder = new IBluetoothDsdaService.Stub() {
        ...
        public boolean canDoCallSwap() {
            int active = getActiveSubscription();
