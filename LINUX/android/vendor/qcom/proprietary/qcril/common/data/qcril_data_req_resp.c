...
)
{
  ...
  {
    QCRIL_LOG_DEBUG( "%s", "EXIT with SKIP REATTACH" );
    return RIL_E_SKIP_LTE_REATTACH;
  }
  else if (QCRIL_LTE_ATTACH_SUCCESS == reti)
  {
    QCRIL_LOG_DEBUG("%s","Comparing modem attached params with",
                         "ril requested apn & ip_family");
    if( qcril_data_retrieve_and_compare_modem_attached_apn( ril_apn,
                                                            ril_ipfamily )
                                             == RIL_E_SKIP_LTE_REATTACH )
    {
      QCRIL_LOG_DEBUG( "%s", "EXIT with SKIP REATTACH" );
      return RIL_E_SKIP_LTE_REATTACH;
    }
...
)
{
  ...
  else
  {
    ...
    {
      QCRIL_LOG_DEBUG( "[%s]: Skipping Reattach",__func__);
      return RIL_E_SKIP_LTE_REATTACH;
    }
    else
    {
      QCRIL_LOG_DEBUG( "[%s]: Returning Success",__func__);
      return RIL_E_SUCCESS;
    }
  }
}
...
)
{
  ...
  {
    QCRIL_LOG_DEBUG("[%s] : modem attached apn is [%s] &",
                    "modem attached ipfamily is [%d]",__func__,
                     modem_attach_params->apn_string,
                     modem_attach_params->ip_type);
  ...
  QCRIL_LOG_DEBUG("[%s] :Invalid modem attached apn or ipfamily",__func__);
  return false;
}
