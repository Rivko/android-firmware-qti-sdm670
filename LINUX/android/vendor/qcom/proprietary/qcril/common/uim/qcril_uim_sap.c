...
)
{
  ...
  qmi_result = qcril_uim_queue_send_request(req_type,
...
)
{
  ...
  if (original_req_ptr != NULL)
  {
    ...
    /* Also update the client ID, so we can reference the right
       callback in the modem response */
    original_req_ptr->data.sap_info.client_id = client_id;
    original_req_ptr->data.sap_info.sap_error = error_code;
...
)
{
  ...
  err_code = (params_ptr->qmi_rsp_data.qmi_err_code == QMI_NO_ERR) ?
...
{
  qcril_uim_original_request_type   * orig_request_ptr = NULL;
  uint8                               client_id        = QCRIL_UIM_SAP_MAX_CLIENTS;
  ...
  if (orig_request_ptr == NULL)
  {
    QCRIL_LOG_ERROR("Invalid input, NULL orig_request_ptr");
    return;
  }
  ...
  qcril_sap_response_api[client_id].resp_handler(orig_request_ptr->request_id,
                                                 orig_request_ptr->data.sap_info.sap_error,
                                                 QMI_INTERNAL_ERR,
                                                 orig_request_ptr->token,
                                                 NULL,
                                                 0);
...
(
  qmi_uim_sap_connection_ind_msg   * ind_data_ptr
)
{
  ...
  if (ind_data_ptr->sap_connection_event_valid == FALSE ||
      active_client_id >= QCRIL_UIM_SAP_MAX_CLIENTS ||
      qcril_sap_response_api[active_client_id].ind_handler == NULL)
  {
    QCRIL_LOG_ERROR("Invalid active client : %d or  event not valid %d",
               active_client_id, ind_data_ptr->sap_connection_event_valid);
    return;
  }

  instance_id = qmi_ril_get_process_instance_id();
  if (instance_id >= QCRIL_MAX_INSTANCE_ID ||
      instance_id != qcril_uim_sap_convert_slot_type_to_instance_id(ind_data_ptr->sap_connection_event.slot))
  {
  ...
  qcril_sap_response_api[active_client_id].ind_handler(ind_data_ptr->sap_connection_event.sap_state);
