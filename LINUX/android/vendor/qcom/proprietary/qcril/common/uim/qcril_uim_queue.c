...
)
{
  ...
  switch(request_type)
  {
    ...
    case QCRIL_UIM_REQUEST_GBA:
      memcpy(&request_ptr->params.gba_init,
             param_data_ptr,
             sizeof(qmi_uim_gba_params_type));
      QCRIL_UIM_DUPLICATE(request_ptr->params.gba_init.session_info.aid.data_ptr,
      ...
      QCRIL_UIM_DUPLICATE(request_ptr->params.gba_init.naf_id.fqdn.data_ptr,
    ...
    case QCRIL_UIM_REQUEST_GBA_IMPI:
      memcpy(&request_ptr->params.gba_impi,
             param_data_ptr,
             sizeof(qmi_uim_gba_impi_params_type));
      QCRIL_UIM_DUPLICATE(request_ptr->params.gba_impi.session_info.aid.data_ptr,
    ...
    case QCRIL_UIM_REQUEST_SWITCH_SLOT_EXT:
      memcpy(&request_ptr->params.phy_slot_map,
             param_data_ptr,
             sizeof(qmi_uim_physical_slot_mapping_type));
      break;

    case QCRIL_UIM_REQUEST_SET_APDU_BEHAVIOR:
      memcpy(&request_ptr->params.set_apdu_behavior,
             param_data_ptr,
    ...
    case QCRIL_UIM_REQUEST_SET_TERMINAL_CAPABILITY:
      memcpy(&request_ptr->params.terminal_capability,
             param_data_ptr,
      ...
      {
        QCRIL_UIM_DUPLICATE(request_ptr->params.terminal_capability.tlvs[i].value.data_ptr,
    ...
    case QCRIL_UIM_REQUEST_REMOTE_UNLOCK:
      memcpy(&request_ptr->params.remote_unlock_req,
             param_data_ptr,
             sizeof(qmi_uim_remote_unlock_params_type));
      QCRIL_UIM_DUPLICATE(request_ptr->params.remote_unlock_req.simlock_data.data_ptr,
...
)
{
  ...
  switch(qcril_uim_queue_head_ptr->request_type)
  {
    ...
    case QCRIL_UIM_REQUEST_GBA:
      QCRIL_UIM_FREE_IF_NOT_NULL(qcril_uim_queue_head_ptr->params.gba_init.session_info.aid.data_ptr);
      QCRIL_UIM_FREE_IF_NOT_NULL(qcril_uim_queue_head_ptr->params.gba_init.naf_id.fqdn.data_ptr);
      break;

    case QCRIL_UIM_REQUEST_GBA_IMPI:
      QCRIL_UIM_FREE_IF_NOT_NULL(qcril_uim_queue_head_ptr->params.gba_impi.session_info.aid.data_ptr);
      break;
    ...
    case QCRIL_UIM_REQUEST_SET_TERMINAL_CAPABILITY:
      for (i = 0; i < qcril_uim_queue_head_ptr->params.terminal_capability.num_tlvs && i < QMI_UIM_TERMINAL_CAPABILITY_TLVS_MAX; i++)
      {
        QCRIL_UIM_FREE_IF_NOT_NULL(qcril_uim_queue_head_ptr->params.terminal_capability.tlvs[i].value.data_ptr);
      }
      break;

    case QCRIL_UIM_REQUEST_REMOTE_UNLOCK:
      QCRIL_UIM_FREE_IF_NOT_NULL(qcril_uim_queue_head_ptr->params.remote_unlock_req.simlock_data.data_ptr);
      break;
...
)
{
  ...
  switch(request_type)
  {
    ...
    case QCRIL_UIM_REQUEST_GBA:
      ret = qcril_qmi_uim_gba_request(qmi_handle,
    ...
    case QCRIL_UIM_REQUEST_GBA_IMPI:
      ret = qcril_qmi_uim_gba_impi(qmi_handle,
    ...
    case QCRIL_UIM_REQUEST_GET_PHYSICAL_SLOT_MAPPING:
      ret = qcril_qmi_uim_get_physical_slot_mapping(qmi_handle,
                                                   callback_function_ptr,
                                                   (void*)original_request_ptr,
                                                   NULL);
      break;
    ...
    case QCRIL_UIM_REQUEST_SWITCH_SLOT_EXT:
      ret = qcril_qmi_uim_switch_slot_ext(qmi_handle,
    ...
    case QCRIL_UIM_REQUEST_SET_APDU_BEHAVIOR:
      ret = qcril_qmi_uim_set_apdu_behavior(qmi_handle,
    ...
    case QCRIL_UIM_REQUEST_GET_SLOTS_STATUS:
      ret = qcril_qmi_uim_get_slots_status(qmi_handle,
                                           callback_function_ptr,
                                           (void*)original_request_ptr,
                                           NULL);
      break;
    ...
    case QCRIL_UIM_REQUEST_GET_TERMINAL_CAPABILITY:
      ret = qcril_qmi_uim_get_terminal_capability(qmi_handle,
                                                  (const qmi_uim_terminal_capability_params_type*)param_data_ptr,
                                                  callback_function_ptr,
                                                  (void*)original_request_ptr,
                                                  NULL);
      break;
    ...
    case QCRIL_UIM_REQUEST_SET_TERMINAL_CAPABILITY:
      ret = qcril_qmi_uim_set_terminal_capability(qmi_handle,
                                                  (const qmi_uim_terminal_capability_params_type*)param_data_ptr,
                                                  callback_function_ptr,
                                                  (void*)original_request_ptr,
                                                  NULL);
    ...
    case QCRIL_UIM_REQUEST_REMOTE_UNLOCK:
      ret = qcril_qmi_uim_remote_unlock(qmi_handle,
  ...
  if (ret < 0)
  {
    if (qcril_uim_queue_head_ptr->original_request_ptr != NULL)
    {
      ...
      qcril_uim_response(qcril_uim_queue_head_ptr->original_request_ptr->instance_id,
                         ...
                         QCRIL_UIM_RIL_E_MODEM_ERR, NULL, 0, TRUE, "error in QMI function");
...
)
{
  ...
  while(qcril_uim_queue_head_ptr != NULL)
  {
    if (qcril_uim_queue_head_ptr->original_request_ptr != NULL)
    {
      qcril_uim_response(qcril_uim_queue_head_ptr->original_request_ptr->instance_id,
                         qcril_uim_queue_head_ptr->original_request_ptr->token,
                         QCRIL_UIM_RIL_E_INTERNAL_ERR, NULL, 0, TRUE, "Dropping request");
      qcril_free(qcril_uim_queue_head_ptr->original_request_ptr);
      qcril_uim_queue_head_ptr->original_request_ptr = NULL;
    }
    qcril_uim_queue_remove_head();
