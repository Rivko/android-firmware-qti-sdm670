  ...
  for (;;)
  {
    ...
    do
    {
      if ( ( NULL != ( ev = qcril_event.list.next ) && ( ev != &qcril_event.list ) ) )
      {
        ...
        if(ev->event_id > QCRIL_EVT_OEM_TEE_REQ_BASE && ev->event_id < QCRIL_EVT_OEM_TEE_REQ_MAX)
        {
            //oem req must free it
            if(ev->data)
            {
                QtiTelephonyService_RILOEMMessageId msg_id = (QtiTelephonyService_RILOEMMessageId) qcril_qmi_oem_proto_map_event_to_request(ev->event_id);
                QCRIL_LOG_INFO("freeing request message for %s(id: %d)", qcril_qmi_oem_proto_get_msg_log_str(msg_id, QtiTelephonyService_RILOEMMessageType_RIL_OEM_MSG_REQUEST), msg_id);
                const pb_field_t* pb_fields = qcril_qmi_oem_proto_get_msg_pb_fields(msg_id, QtiTelephonyService_RILOEMMessageType_RIL_OEM_MSG_REQUEST);
                if(pb_fields)
                {
        ...
        if (ev->event_id > QCRIL_EVT_IMS_SOCKET_REQ_BASE &&
            ev->event_id < QCRIL_EVT_IMS_SOCKET_REQ_MAX) // ims request free it
        {
            if (ev->data)
            {
                ims_MsgId msg_id = (ims_MsgId) qcril_qmi_ims_map_event_to_request(ev->event_id);
