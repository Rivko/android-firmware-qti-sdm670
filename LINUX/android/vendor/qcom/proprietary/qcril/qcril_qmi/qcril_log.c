...
/* Constants for Unsolicited Response Filter (URF) logging */
#define QCRIL_LOG_URF_SIGNAL_STRENGTH_NAME             "SIGNAL_STRENGTH"
#define QCRIL_LOG_URF_FULL_NETWORK_STATE_NAME          "FULL_NETWORK_STATE"
#define QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_NAME  "DATA_CALL_DORMANCY_CHANGED"

static const char qcril_log_urf_filter_names[][sizeof(QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_NAME)] = {
  #define QCRIL_LOG_URF_SIGNAL_STRENGTH_IDX             0
  QCRIL_LOG_URF_SIGNAL_STRENGTH_NAME,

  #define QCRIL_LOG_URF_FULL_NETWORK_STATE_IDX          1
  QCRIL_LOG_URF_FULL_NETWORK_STATE_NAME,

  #define QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_IDX  2
  QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_NAME,
};

static const size_t qcril_log_urf_num_filters = sizeof(qcril_log_urf_filter_names) / sizeof(qcril_log_urf_filter_names[0]);

static const char qcril_log_urf_on[]    = "ON";
static const char qcril_log_urf_off[]   = "OFF";
static const char qcril_log_urf_sep1[]  = " - "; // sep1 separates filter name and state e.g."SIGNAL_STRENGTH - ON"
static const char qcril_log_urf_sep2[]  = " | "; // sep2 separates consecutive filter name/state pairs e.g."SIGNAL_STRENGTH - ON | FULL_NETWORK_STATE - ON"

static const size_t qcril_log_urf_max_filter_state_str_size = sizeof(qcril_log_urf_off);

// See below for an example of a filter status string.
// "SIGNAL_STRENGTH - %s | FULL_NETWORK_STATE - %s | DATA_CALL_DORMANCY_CHANGED - %s"
const size_t qcril_log_urf_filter_status_str_size =
  (
      sizeof(qcril_log_urf_sep1)
    + qcril_log_urf_max_filter_state_str_size
    + sizeof(qcril_log_urf_sep2)
  )
  * sizeof(qcril_log_urf_filter_names[0]) / sizeof(qcril_log_urf_filter_names[0][0])
  + sizeof(qcril_log_urf_filter_names);
/* Constants for Unsolicited Response Filter (URF) logging */
...
)
{
  ...
  else
  {
    ...
    switch( event_id )
    {
      ...
      case QCRIL_EVT_CM_CARD_STATUS_CHANGE_IND:
        return "QCRIL_EVT_CM_CARD_STATUS_CHANGE_IND";
      ...
      case QCRIL_EVT_INTERNAL_UIM_SAP_REQ:
        return "QCRIL_EVT_INTERNAL_UIM_SAP_REQ";
      ...
      case QCRIL_EVT_UIM_RMT_QMI_SRVC_UP:
        return "QCRIL_EVT_UIM_RMT_QMI_SRVC_UP";

      case QCRIL_EVT_UIM_LPA_QMI_COMMAND_CALLBACK:
        return "QCRIL_EVT_UIM_LPA_QMI_COMMAND_CALLBACK";

      case QCRIL_EVT_UIM_HTTP_QMI_TRANSACTION_COMPLETED_CALLBACK:
        return "QCRIL_EVT_UIM_HTTP_QMI_TRANSACTION_COMPLETED_CALLBACK";

      case QCRIL_EVT_UIM_HTTP_QMI_INDICATION_CALLBACK:
        return "QCRIL_EVT_UIM_HTTP_QMI_INDICATION_CALLBACK";

      case QCRIL_EVT_UIM_HTTP_QMI_HTTP_SRVC_UP:
        return "QCRIL_EVT_UIM_HTTP_QMI_HTTP_SRVC_UP";
      ...
      case QCRIL_EVT_DATA_ASYNC_CALLBACK:
        return "QCRIL_EVT_DATA_ASYNC_CALLBACK";

      case QCRIL_EVT_DATA_SET_RAT_PREF_CALLBACK:
        return "DATA_SET_RAT_PREF_CALLBACK";
      ...
      case QCRIL_EVT_HOOK_SET_LOCAL_CALL_HOLD_v01:
        return "QCRIL_EVT_HOOK_SET_LOCAL_CALL_HOLD_v01";
      ...
      case QCRIL_EVT_HOOK_UNSOL_CSG_ID_CHANGE_IND:
        return "QCRIL_EVT_HOOK_UNSOL_CSG_ID_CHANGE_IND";

      case QCRIL_EVT_HOOK_GET_CSG_ID:
        return "QCRIL_EVT_HOOK_GET_CSG_ID";
      ...
      case QCRIL_EVT_HOOK_UNSOL_RAT_RAC_CHANGE_IND:
        return "QCRIL_EVT_HOOK_UNSOL_RAT_RAC_CHANGE_IND";

      case QCRIL_EVT_HOOK_GET_RAT_RAC:
        return "QCRIL_EVT_HOOK_GET_RAT_RAC";
      ...
      case QCRIL_EVT_RIL_REQUEST_MANUAL_PROVISIONING:
        return "QCRIL_EVT_RIL_REQUEST_MANUAL_PROVISIONING";

      case QCRIL_EVT_QMI_PROV_UPDATE_ICCID:
        return "QCRIL_EVT_QMI_PROV_UPDATE_ICCID";
      ...
      case QCRIL_EVT_HOOK_EMBMS_UNSOL_GET_INTERESTED_TMGI_LIST:
          return "QCRIL_EVT_HOOK_EMBMS_UNSOL_GET_INTERESTED_TMGI_LIST";
      ...
      case QCRIL_EVT_QMI_RIL_MODEM_RESTART_CHECK_IF_SERVICE_UP:
          return "QCRIL_EVT_QMI_RIL_MODEM_RESTART_CHECK_IF_SERVICE_UP";

      case QCRIL_EVT_QMI_RIL_SERVICE_DOWN:
          return "QCRIL_EVT_QMI_RIL_SERVICE_DOWN";

      case QCRIL_EVT_QMI_RIL_MODEM_RESTART_CHECK_IF_SSR_EXCL_SERVICE_UP:
          return "QCRIL_EVT_QMI_RIL_MODEM_RESTART_CHECK_IF_SSR_EXCL_SERVICE_UP";

      case QCRIL_EVT_QMI_RIL_SSR_EXCL_SERVICE_DOWN:
          return "QCRIL_EVT_QMI_RIL_SSR_EXCL_SERVICE_DOWN";
      ...
      case QCRIL_EVT_QMI_PROV_ACTIVATE_FOLLOW_UP:
          return "QCRIL_EVT_QMI_PROV_ACTIVATE_FOLLOW_UP";

      case QCRIL_EVT_QMI_PROV_DEACTIVATE_FOLLOW_UP:
          return "QCRIL_EVT_QMI_PROV_DEACTIVATE_FOLLOW_UP";
      ...
      case QCRIL_EVT_QMI_NAS_BAR_FAKE_GCELL:
          return "QCRIL_EVT_QMI_NAS_BAR_FAKE_GCELL";

      case QCRIL_EVT_QMI_VOICE_AM_LCH_COMPLETE:
          return "QCRIL_EVT_QMI_VOICE_AM_LCH_COMPLETE";
      ...
      case QCRIL_EVT_QMI_DMS_HANDLE_ASYNC_CB:
          return "QCRIL_EVT_QMI_DMS_HANDLE_ASYNC_CB";

      case QCRIL_EVT_QMI_DMS_HANDLE_INDICATIONS:
          return "QCRIL_EVT_QMI_DMS_HANDLE_INDICATIONS";

      case QCRIL_EVT_DATA_DDS_STATUS_FOLLOW_UP:
          return "QCRIL_EVT_DATA_DDS_STATUS_FOLLOW_UP";

      case QCRIL_EVT_DATA_NEW_DDS_INFO:
          return "QCRIL_EVT_DATA_NEW_DDS_INFO";
      ...
      case QCRIL_EVT_QMI_PDC_HANDLE_INDICATIONS:
          return "QCRIL_EVT_QMI_PDC_HANDLE_INDICATIONS";

      case QCRIL_EVT_HOOK_DATA_GO_DORMANT:
          return "QCRIL_EVT_HOOK_DATA_GO_DORMANT";

      case QCRIL_EVT_HOOK_GET_DATA_CALL_PROFILE:
          return "QCRIL_EVT_HOOK_GET_DATA_CALL_PROFILE";
      ...
      case QCRIL_EVT_PBM_GET_PB_CAPACITY:
          return "QCRIL_EVT_PBM_GET_PB_CAPACITY";

      case QCRIL_EVT_PBM_ASYNC_CALLBACK:
          return "QCRIL_EVT_PBM_ASYNC_CALLBACK";
      ...
      case QCRIL_EVT_HOOK_SLOT_GET_CARD_STATE_REQ:
          return "QCRIL_EVT_HOOK_SLOT_GET_CARD_STATE_REQ";

      case QCRIL_EVT_HOOK_SLOT_GET_ATR_REQ:
          return "QCRIL_EVT_HOOK_SLOT_GET_ATR_REQ";

      case QCRIL_EVT_HOOK_SLOT_SEND_APDU_REQ:
          return "QCRIL_EVT_HOOK_SLOT_SEND_APDU_REQ";

      case QCRIL_EVT_HOOK_SLOT_SET_APDU_BEHAVIOR_REQ:
          return "QCRIL_EVT_HOOK_SLOT_SET_APDU_BEHAVIOR_REQ";

      case QCRIL_EVT_HOOK_SWITCH_SLOT_REQ:
          return "QCRIL_EVT_HOOK_SWITCH_SLOT_REQ";

      case QCRIL_EVT_HOOK_GET_PHYSICAL_SLOT_MAPPING_REQ:
          return "QCRIL_EVT_HOOK_GET_PHYSICAL_SLOT_MAPPING_REQ";

      case QCRIL_EVT_HOOK_SLOT_CARD_POWER_REQ:
          return "QCRIL_EVT_HOOK_SLOT_CARD_POWER_REQ";

      case QCRIL_EVT_HOOK_UNSOL_CARD_STATE_CHANGE_IND:
          return "QCRIL_EVT_HOOK_UNSOL_CARD_STATE_CHANGE_IND";

      case QCRIL_EVT_HOOK_ENTER_DEPERSONALIZATION_REQ:
          return "QCRIL_EVT_HOOK_ENTER_DEPERSONALIZATION_REQ";

      case QCRIL_EVT_HOOK_GET_TERMINAL_CAPABILITY_LPA_TLV_REQ:
          return "QCRIL_EVT_HOOK_GET_TERMINAL_CAPABILITY_LPA_TLV_REQ";

      case QCRIL_EVT_HOOK_SET_TERMINAL_CAPABILITY_LPA_TLV_REQ:
          return "QCRIL_EVT_HOOK_SET_TERMINAL_CAPABILITY_LPA_TLV_REQ";
      ...
      case QCRIL_EVT_HOOK_GET_ADN_RECORD:
          return "QCRIL_EVT_HOOK_GET_ADN_RECORD";

      case QCRIL_EVT_HOOK_UPDATE_ADN_RECORD:
          return "QCRIL_EVT_HOOK_UPDATE_ADN_RECORD";
      ...
      case QCRIL_EVT_REQUEST_MBN_HW_INIT:
          return "QCRIL_EVT_REQUEST_MBN_HW_INIT";

      case QCRIL_EVT_REQUEST_MBN_HW_DISABLED_INIT:
          return "QCRIL_EVT_REQUEST_MBN_HW_DISABLED_INIT";

      case QCRIL_EVT_REQUEST_MBN_HW_GET_SELECTED_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_HW_GET_SELECTED_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_HW_DEACTIVATE_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_HW_DEACTIVATE_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_HW_DELETE_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_HW_DELETE_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_HW_LOAD_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_HW_LOAD_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_HW_SELECT_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_HW_SELECT_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_HW_ACTIVATE_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_HW_ACTIVATE_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_SW_DISALBLE_MODEM_UPDATE:
          return "QCRIL_EVT_REQUEST_MBN_SW_DISALBLE_MODEM_UPDATE";

      case QCRIL_EVT_REQUEST_VERIFY_MBN_SW_INIT:
          return "QCRIL_EVT_REQUEST_VERIFY_MBN_SW_INIT";

      case QCRIL_EVT_REQUEST_MBN_SW_INIT:
          return "QCRIL_EVT_REQUEST_MBN_SW_INIT";

      case QCRIL_EVT_REQUEST_MBN_SW_GET_SELECTED_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_SW_GET_SELECTED_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_SW_GET_CONFIG_INFO:
          return "QCRIL_EVT_REQUEST_MBN_SW_GET_CONFIG_INFO";

      case QCRIL_EVT_REQUEST_MBN_SW_DEACTIVATE_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_SW_DEACTIVATE_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_SW_DELETE_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_SW_DELETE_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_SW_LOAD_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_SW_LOAD_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_SW_SELECT_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_SW_SELECT_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_SW_ACTIVATE_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_SW_ACTIVATE_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_SW_CLEANUP_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_SW_CLEANUP_CONFIG";

      case QCRIL_EVT_REQUEST_MBN_SW_COUNT_PENDING_CONFIGS:
          return "QCRIL_EVT_REQUEST_MBN_SW_COUNT_PENDING_CONFIGS";

      case QCRIL_EVT_REQUEST_MBN_SW_QUERY_PENDING_CONFIG:
          return "QCRIL_EVT_REQUEST_MBN_SW_QUERY_PENDING_CONFIG";

      case QCRIL_EVT_QMI_RIL_PDC_DELETE_ALL:
          return "QCRIL_EVT_QMI_RIL_PDC_DELETE_ALL";
      ...
      case QCRIL_EVT_OEM_MSG_GBA_INIT:
          return "QCRIL_EVT_OEM_MSG_GBA_INIT";

      case QCRIL_EVT_OEM_MSG_GET_IMPI:
          return "QCRIL_EVT_OEM_MSG_GET_IMPI";

      case QCRIL_EVT_HOOK_UICC_POWER_REQ:
          return "QCRIL_EVT_HOOK_UICC_POWER_REQ";
      ...
      case QCRIL_EVT_IMS_SOCKET_REQ_SUPP_SVC_STATUS:
          return "QCRIL_EVT_IMS_SOCKET_REQ_SUPP_SVC_STATUS";
      ...
      case QCRIL_EVT_IMS_SOCKET_REQ_SET_IMS_CONFIG:
          return "QCRIL_EVT_IMS_SOCKET_REQ_SET_IMS_CONFIG";

      case QCRIL_EVT_IMS_SOCKET_REQ_GET_IMS_CONFIG:
          return "QCRIL_EVT_IMS_SOCKET_REQ_GET_IMS_CONFIG";

      case QCRIL_EVT_IMS_SOCKET_REQ_SET_GEO_LOC:
          return "QCRIL_EVT_IMS_SOCKET_REQ_SET_GEO_LOC";

      case QCRIL_EVT_IMS_SOCKET_REQ_EXPLICIT_CALL_TRANSFER:
          return "QCRIL_EVT_IMS_SOCKET_REQ_EXPLICIT_CALL_TRANSFER";

      case QCRIL_EVT_IMS_SOCKET_REQ_GET_VOWIFI_CALL_QUALITY:
          return "QCRIL_EVT_IMS_SOCKET_REQ_GET_VOWIFI_CALL_QUALITY";

      case QCRIL_EVT_IMS_SOCKET_REQ_SET_VOLTE_PREF:
          return "QCRIL_EVT_IMS_SOCKET_REQ_SET_VOLTE_PREF";

      case QCRIL_EVT_IMS_SOCKET_REQ_GET_VOLTE_PREF:
          return "QCRIL_EVT_IMS_SOCKET_REQ_GET_VOLTE_PREF";

      case QCRIL_EVT_IMS_SOCKET_REQ_GET_HANDOVER_CONFIG:
          return "QCRIL_EVT_IMS_SOCKET_REQ_GET_HANDOVER_CONFIG";

      case QCRIL_EVT_IMS_SOCKET_REQ_SET_HANDOVER_CONFIG:
          return "QCRIL_EVT_IMS_SOCKET_REQ_SET_HANDOVER_CONFIG";

      case QCRIL_EVT_IMS_SOCKET_REQ_CANCEL_MODIFY_CALL:
          return "QCRIL_EVT_IMS_SOCKET_REQ_CANCEL_MODIFY_CALL";

      case QCRIL_EVT_QMI_IMSA_HANDLE_INDICATIONS:
          return "QCRIL_EVT_QMI_IMSA_HANDLE_INDICATIONS";

      case QCRIL_EVT_IMS_SOCKET_REQ_GET_VOPS_INFO:
          return "QCRIL_EVT_IMS_SOCKET_REQ_GET_VOPS_INFO";

      case QCRIL_EVT_IMS_SOCKET_REQ_GET_SSAC_INFO:
          return "QCRIL_EVT_IMS_SOCKET_REQ_GET_SSAC_INFO";

      case QCRIL_EVT_IMS_SOCKET_REQ_SEND_RTT_MSG:
          return "QCRIL_EVT_IMS_SOCKET_REQ_SEND_RTT_MSG";

      case QCRIL_EVT_HOOK_LTE_DIRECT_DISC_REQ:
          return "QCRIL_EVT_HOOK_LTE_DIRECT_DISC_REQ";

      case QCRIL_EVT_HOOK_UNSOL_LTE_DIRECT_DISC:
          return "QCRIL_EVT_HOOK_UNSOL_LTE_DIRECT_DISC";

      case QCRIL_EVT_LTE_DIRECT_DISC_REQ_INITIALIZE:
          return "QCRIL_EVT_LTE_DIRECT_DISC_REQ_INITIALIZE";

      case QCRIL_EVT_LTE_DIRECT_DISC_REQ_GET_DEVICE_CAPABILITY:
          return "QCRIL_EVT_LTE_DIRECT_DISC_REQ_GET_DEVICE_CAPABILITY";

      case QCRIL_EVT_LTE_DIRECT_DISC_REQ_TERMINATE:
          return "QCRIL_EVT_LTE_DIRECT_DISC_REQ_TERMINATE";

      case QCRIL_EVT_LTE_DIRECT_DISC_REQ_GET_SERVICE_STATUS:
          return "QCRIL_EVT_LTE_DIRECT_DISC_REQ_GET_SERVICE_STATUS";

      case QCRIL_EVT_LTE_DIRECT_DISC_REQ_PUBLISH:
          return "QCRIL_EVT_LTE_DIRECT_DISC_REQ_PUBLISH";

      case QCRIL_EVT_LTE_DIRECT_DISC_REQ_CANCEL_PUBLISH:
          return "QCRIL_EVT_LTE_DIRECT_DISC_REQ_CANCEL_PUBLISH";

      case QCRIL_EVT_LTE_DIRECT_DISC_REQ_SUBSCRIBE:
          return "QCRIL_EVT_LTE_DIRECT_DISC_REQ_SUBSCRIBE";

      case QCRIL_EVT_LTE_DIRECT_DISC_REQ_CANCEL_SUBSCRIBE:
          return "QCRIL_EVT_LTE_DIRECT_DISC_REQ_CANCEL_SUBSCRIBE";

      case QCRIL_EVT_HOOK_UNSOL_SPEECH_CODEC_INFO:
        return "QCRIL_EVT_HOOK_UNSOL_SPEECH_CODEC_INFO";
...
/*===========================================================================
 ...
 FUNCTION: qcril_log_urf_mask_to_status

===========================================================================*/
/*!
    @brief
    This function takes an unsolicited response filter mask received as part
    of the RIL_REQUEST_SET_UNSOLICITED_RESPONSE_FILTER message, and returns
    a string containing the name and state (i.e. enabled or disabled) of
    each unsol response filter.

    e.g.

    Input:  0x03
    Output: "SIGNAL_STRENGTH - ON | FULL_NETWORK_STATE - ON | DATA_CALL_DORMANCY_CHANGED - OFF"

    @return
    A string containing the name and state of all possible unsol response
    filters. In the event of an error, an empty string is returned.
*/
/*=========================================================================*/
void qcril_log_urf_mask_to_status(int mask, char* const filter_status_str,
                                  const size_t filter_status_str_len)
{
  const char* filter_state[qcril_log_urf_num_filters];
  ...
  if (QCRIL_LOG_URF_SIGNAL_STRENGTH_IDX >= qcril_log_urf_num_filters)
  {
    // TODO: Log a message explaining why
    return;
  }

  if (mask & RIL_UR_SIGNAL_STRENGTH)
  {
    filter_state[QCRIL_LOG_URF_SIGNAL_STRENGTH_IDX] = qcril_log_urf_on;
  }
  else
  {
    filter_state[QCRIL_LOG_URF_SIGNAL_STRENGTH_IDX] = qcril_log_urf_off;
  }

  /* RIL_UnsolicitedResponseFilter.RIL_UR_FULL_NETWORK_STATE */
  if (QCRIL_LOG_URF_FULL_NETWORK_STATE_IDX >= qcril_log_urf_num_filters)
  {
    // TODO: Log a message explaining why
    return;
  }

  if (mask & RIL_UR_FULL_NETWORK_STATE)
  {
    filter_state[QCRIL_LOG_URF_FULL_NETWORK_STATE_IDX] = qcril_log_urf_on;
  }
  else
  {
    filter_state[QCRIL_LOG_URF_FULL_NETWORK_STATE_IDX] = qcril_log_urf_off;
  }

  /* RIL_UnsolicitedResponseFilter.RIL_UR_DATA_CALL_DORMANCY_CHANGED */
  if (QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_IDX >= qcril_log_urf_num_filters)
  {
    // TODO: Log a message explaining why
    return;
  }

  if (mask & RIL_UR_DATA_CALL_DORMANCY_CHANGED)
  {
    filter_state[QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_IDX] = qcril_log_urf_on;
  }
  else
  {
    filter_state[QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_IDX] = qcril_log_urf_off;
  }

  // e.g. "SIGNAL_STRENGTH - ON | FULL_NETWORK_STATE - ON | DATA_CALL_DORMANCY_CHANGED - OFF"
  QCRIL_SNPRINTF(filter_status_str, filter_status_str_len, "%s%s%s%s%s%s%s%s%s%s%s",
      qcril_log_urf_filter_names[QCRIL_LOG_URF_SIGNAL_STRENGTH_IDX],
      qcril_log_urf_sep1,
      filter_state[QCRIL_LOG_URF_SIGNAL_STRENGTH_IDX],
      qcril_log_urf_sep2,

      qcril_log_urf_filter_names[QCRIL_LOG_URF_FULL_NETWORK_STATE_IDX],
      qcril_log_urf_sep1,
      filter_state[QCRIL_LOG_URF_FULL_NETWORK_STATE_IDX],
      qcril_log_urf_sep2,

      qcril_log_urf_filter_names[QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_IDX],
      qcril_log_urf_sep1,
      filter_state[QCRIL_LOG_URF_DATA_CALL_DORMANCY_CHANGED_IDX]
  );
}/* qcril_log_urf_mask_to_status */

/*===========================================================================

 FUNCTION: qcril_log_device_state_type_enum_to_str
...
}/* qcril_log_device_state_type_enum_to_str */
