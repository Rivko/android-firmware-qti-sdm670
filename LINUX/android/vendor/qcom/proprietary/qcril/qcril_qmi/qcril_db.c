...
)
{
    ...
    if(qmi_ril_get_process_instance_id() == QCRIL_DEFAULT_INSTANCE_ID)
    {
        ...
        else
        {
            ...
            //happen again after device exit from encryption
...
{
    const char get_vendor_version_sql[] =
        "SELECT VALUE from vendordb." QCRIL_PROPERTIES_TABLE_NAME " where PROPERTY = 'qcrildb_version'";
    const char get_main_version_sql[] =
        "SELECT VALUE from main." QCRIL_PROPERTIES_TABLE_NAME " where PROPERTY = 'qcrildb_version'";
    const char *get_version_sql = vendor? get_vendor_version_sql : get_main_version_sql;
    int sql_size = vendor ? sizeof(get_vendor_version_sql) : sizeof(get_main_version_sql);
    int version = -1;
    int rc = -1;
    sqlite3_stmt *stmt = NULL;
    do {
        rc = sqlite3_prepare_v2(qcril_db_handle,
                get_version_sql,
                sql_size,
                &stmt,
                NULL);
...
int qcril_upgrade_file_sort(const struct dirent **a, const struct dirent **b)
{
    ...
    int ret = 0;
    sscanf(a[0]->d_name, "%d%ms", &va, &namea);
    sscanf(b[0]->d_name, "%d%ms", &vb, &nameb);
    ret = vb - va;
    if (!ret)
    {
...
*/
/*=========================================================================*/
void qcril_db_upgrade(int cur_version, int new_version)
{
    int rc;
    int num_files;
...
{
    const char presql[] = "ATTACH DATABASE \"" QCRIL_PREBUILT_DB_NAME "\" AS vendordb;"
        "PRAGMA main.locking_mode = EXCLUSIVE;";
    const char postsql[] =
        "DETACH DATABASE vendordb;"
    ...
    do {
        ...
        if (rc != SQLITE_OK)
        {
            QCRIL_LOG_ERROR("Unable to attach " QCRIL_PREBUILT_DB_NAME ": %s", error);
            sqlite3_free(error);
            break;
        }
    ...
    if (rc != SQLITE_OK)
    {
        QCRIL_LOG_ERROR("Unable to run postsql: %s", error);
        sqlite3_free(error);
    }
