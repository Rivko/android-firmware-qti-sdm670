...
)
{
    ...
    if ( instance_id < QCRIL_MAX_INSTANCE_ID )
    {
        ...
        qcril_arb.data_sys_status[ QCRIL_DEFAULT_INSTANCE_ID ] = *data_sys_status;
        if(data_sys_status->network_info_len > QCRIL_ARB_NETWORK_INFO_LEN )
        {
          qcril_arb.data_sys_status[ QCRIL_DEFAULT_INSTANCE_ID ].network_info_len = QCRIL_ARB_NETWORK_INFO_LEN;
        }
...
void qcril_qmi_get_pref_data_tech(qcril_arb_pref_data_type *pref_data)
{
    ...
    if( pref_data )
    {
        ...
        if( TRUE == qcril_arb.is_current[cur_instance] )
        {
            ...
            if( TRUE == qcril_arb.is_dsd[cur_instance] )
            {
                ...
                if(qmi_ril_is_feature_supported(QMI_RIL_FEATURE_RIL_DATA_REG_STATE_CONDITIONAL_REPORT))
                {
                    ...
                    {
                        wlan_status = qcril_qmi_imsa_ims_registered_wlan_status();
                        ...
                        {
                            wlan_status = qcril_qmi_imss_client_provisioning_wlan_status();
            ...
            else
            {
                ...
                for( iter_i=0; iter_i < qcril_arb.data_sys_status[cur_instance].network_info_len; iter_i++ )
                {
                        ...
                        qcril_arb.data_sys_status[cur_instance].network_info[iter_i].network )
                    {
                        ...
                        pref_data->data_sys_status_info.rat_mask = (qmi_wds_db_rat_mask)(qmi_wds_cdma_rat_mask)qcril_arb.data_sys_status[ cur_instance ].network_info[iter_i].rat_mask;
                        pref_data->data_sys_status_info.db_so_mask = (qmi_wds_db_so_mask)(int)qcril_arb.data_sys_status[ cur_instance ].network_info[iter_i].so_mask;
                        break;
                    }
                }
        ...
        if( ( TRUE == pref_data->is_current ) && report_dsd_status)
        {
...
uint8_t qcril_arb_decide_data_rat_to_report(uint32_t *index)
{
    ...
    qcril_qmi_imss_fetch_wifi_call_pref(&wifi_call_preference_valid, &wifi_call_preference);
    ...
    if(!wifi_call_valid || (QCRIL_IMS_SETTING_WFC_ON != wifi_call) )
    {
    ...
    {
        if(wifi_call_preference_valid && ((QCRIL_IMS_SETTING_WFC_WLAN_PREFERRED == wifi_call_preference) ||
           (QCRIL_IMS_SETTING_WFC_WLAN_ONLY == wifi_call_preference))
          )
        {
