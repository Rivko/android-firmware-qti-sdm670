...
public class CallInfo extends PreferenceActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (this instanceof PreferenceActivity) {
            addPreferencesFromResource(R.layout.call_show);
            PreferenceScreen root = getPreferenceScreen();
            mType = (PreferenceCategory) root.findPreference(REQ_INFO);
            TeleUpdate doReq = new TeleUpdate(this);
            List<TeleUpdate.Request> appCall = doReq.getPh();
            List<Preference> recentPrefs = new ArrayList<>(appCall.size());
            for (final TeleUpdate.Request request : appCall) {
                Preference pref = new Preference(this);
                pref.setIcon(request.icon);
                pref.setTitle(request.label);
                pref.setSummary(request.call);
                Intent intent = new Intent(this, DetialShow.class);
