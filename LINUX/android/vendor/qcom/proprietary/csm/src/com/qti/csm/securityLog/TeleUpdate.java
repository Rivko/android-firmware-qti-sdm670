...
class TeleUpdate {
    ...
    private static final int[] CHECK_PH = new int[]{
            AppOpsManager.OP_CALL_PHONE,
    };
    private static final int[] CHECK_SM = new int[]{
            AppOpsManager.OP_SEND_SMS,
    };

    public TeleUpdate(Context context) {
        mContext = context;
    ...
    private Request doRequest(long now,
        ...
        try {
            ...
            request = new Request(packageName,
                    userHandle,
                    icon, appLabel,
                    infoTag,
                    posCall, uid);
    ...
    public List<Request> getPh() {
        AppOpsManager myManager =
                (AppOpsManager) mContext.getSystemService(Context.APP_OPS_SERVICE);
        List<AppOpsManager.PackageOps> appOps = myManager.getPackagesForOps(CHECK_PH);
    ...
    public List<Request> getSm() {
        AppOpsManager myManager =
                (AppOpsManager) mContext.getSystemService(Context.APP_OPS_SERVICE);
        List<AppOpsManager.PackageOps> appOps = myManager.getPackagesForOps(CHECK_SM);
        final int getCount = appOps != null ? appOps.size() : 0;
        ArrayList<Request> requests = new ArrayList<>(getCount);
        final long now = System.currentTimeMillis();
        final UserManager um = (UserManager) mContext.getSystemService(Context.USER_SERVICE);
    ...
    public static class Request {
        ...
        private Request(String packageName,
