...
*/
/*=========================================================================*/
static int qmi_test_main(qmi_client_type *clnt, qmi_txn_handle *txn,
              int test_length, int data_size_bytes, int test_duration_ms)
{
  uint32_t rc;

  printf("\n\n\nQMI TEST STARTED...\n");
  rc = qmi_test_clnt_common_async_invalid_msg_test(clnt, txn, test_length,
          data_size_bytes, test_duration_ms);
  if(rc == 0)
  {
    printf("Test Passed\n");
  }
...
static uint32_t test_length = 5;
static uint32_t data_size_bytes;
static uint32_t test_duration_ms = 5000;
...
{
  printf("\n*************************************************************\n");
  printf("Usage: %s -q -t <val> -i <val> -n <val> -s <val> -e <val>"
      "\nPARAMS can be:\n"
...
static void parse_command(int argc, char *const argv[])
{
  ...
  while ((command = getopt_long(argc, argv, "qht:i:n:s:e:", longopts, NULL)) != -1)
  {
    switch (command)
    {
      case 'q':
...
int main(int argc, char *argv[])
{
 ...
 /* Setup defaults */
  ...
  rc = qmi_test_main(&clnt, &txn, test_length, data_size_bytes, test_duration_ms);
  if(rc == 0)
  {
    printf("PASS\n");
  }
