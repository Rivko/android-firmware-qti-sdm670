...
static void pid_timer_expired(int id)
{
	/* Notify the waiting thread */
	pthread_mutex_lock(&wait_mutex);
...
static int handle_sampling_adjust(struct pid_algo_t *pid_info, uint32_t algo_idx, uint32_t sampling)
{
	...
	if(s_idx >= sampling_group_count) {
		...
		sampling_groups[s_idx].timer_id = timer_register_event(s_idx, pid_timer_expired);
	...
	for (sample_id = 0; sample_id < sampling_group_count; sample_id++) {
		...
		for (idx = 0; idx < pid_clnt_cnt; idx++) {
			...
			if ((temp >= pid_algo_clnt[idx].active_set_point) ||
					pid_algo_clnt[idx].start_pid) {
				dbgmsg(
				"%s: PID Id %s, Prev Adj. %dkHz, E0 %.1fC, "
				"E1 %.1fC, E2 %.1fC",__func__,
				pid_algo_clnt[idx].desc,
				pid_algo_clnt[idx].prev_therm_adjustment,
				E0, E1, E2);
				proportional = E0 - E1;
				integral = E0 * Ts;
			...
			dbgmsg(
			"%s: PID Id %s, Sensor %s, Device %s, Calc freq %dkHz",
			__func__, pid_algo_clnt[idx].desc,
			...
			thermalmsg(LOG_LVL_INFO, LOG_TRACE,
				"%s: %s: %s=%s %s=%d",
				CPU_FREQ_PRE_REQ, pid_algo_clnt[idx].desc,
				DEVICE,	pid_algo_clnt[idx].setting->device,
				FREQUENCY, req.value);
			applied_lvl = device_clnt_request(
					pid_algo_clnt[idx].dev_clnt, &req);
			if (applied_lvl > 0) {
				dbgmsg(
				"%s: PID Id %s, Sensor %s, Device %s, Applied freq %dkHz",
				__func__, pid_algo_clnt[idx].desc,
				pid_algo_clnt[idx].setting->sensor,
				pid_algo_clnt[idx].setting->device,
				applied_lvl);
				thermalmsg(LOG_LVL_INFO, LOG_TRACE,
					"%s: %s: %s=%s %s=%d",
					CPU_FREQ_POST_REQ,
					pid_algo_clnt[idx].desc, DEVICE,
					pid_algo_clnt[idx].setting->device,
					FREQUENCY, applied_lvl);
				pid_algo_clnt[idx].applied_freq = applied_lvl;
			} else {
				msg(
				"%s: PID Id %s, Error setting dev op value\n.",
				__func__, pid_algo_clnt[idx].desc);
			}
		}
		pthread_mutex_lock(&wait_mutex);
...
static void *pid_algo_monitor(void *data)
{
	...
	for(i = 0; i < sampling_group_count; i++) {
		...
		sampling_groups[i].timer_id = timer_register_event(i, pid_timer_expired);
...
error_handler:
		if (err) {
			...
			pid_algo_clnt[pid_clnt_cnt].num_of_lvls =
				dev_info.num_of_levels;
			pid_algo_clnt[pid_clnt_cnt].start_pid = 0;
			...
			if (i >= MAX_PID_INSTANCES_SUPPORTED ||
				...
				sampling_group_count >= MAX_PID_INSTANCES_SUPPORTED)) {
