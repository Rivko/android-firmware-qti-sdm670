...
static pthread_mutex_t ts_clnt_mtx = THERMAL_RECURSIVE_MUTEX_INITIALIZER;
...
static int update_active_thresh(struct sensors_mgr_sensor_info *sensor_mgr)
{
	...
	dbgmsg("%s: %s Active(%d), Hi(%d) %d, Lo(%d) %d, desc(%d) Interval(%d) %d\n",
	       ...
	       active->thresh.descending_threshold,
