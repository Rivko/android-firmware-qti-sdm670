...
static struct alias_map_t alias_map_8226[] = {
	{"tsens_tz_sensor5", "cpu0-1" },
	{"tsens_tz_sensor2", "cpu2-3" },
	{"tsens_tz_sensor3", "pop_mem" },
};

static struct alias_map_t alias_map_8926[] = {
	{"tsens_tz_sensor5", "cpu0-1" },
	{"tsens_tz_sensor1", "cpu2-3" },
	{"tsens_tz_sensor3", "pop_mem" },
};

static struct sensor_info tsens_sensors_v1[] = {
	{
...
static struct sensor_info tsens_sensors_v2[] = {
	{
		.name = "tsens_tz_sensor0",
		.setup = tsens_sensors_setup,
		.shutdown = tsens_sensors_shutdown,
...
static char *find_alias(const char * sensor_name)
{
	...
	if (init == 0) {
		switch (therm_get_msm_id()) {
			case THERM_MSM_8226:
				arr = alias_map_8226;
				arr_len = ARRAY_SIZE(alias_map_8226);
				break;
			case THERM_MSM_8926:
				arr = alias_map_8926;
				arr_len = ARRAY_SIZE(alias_map_8926);
				break;
			default:
		...
		case THERM_MSM_8226:
			switch (therm_get_msm_version()) {
				case THERM_VERSION_V1:
					if (add_tgt_sensors_set(tsens_sensors_v1,
						   ARRAY_SIZE(tsens_sensors_v1)) != 0) {
						msg("%s: Error adding TSENS TS\n", __func__);
						ret_val = -(EFAULT);
					}
					break;
				default:
					if (add_tgt_sensors_set(tsens_sensors_v2,
						   ARRAY_SIZE(tsens_sensors_v2)) != 0) {
						msg("%s: Error adding TSENS TS\n", __func__);
						ret_val = -(EFAULT);
					}
					break;
