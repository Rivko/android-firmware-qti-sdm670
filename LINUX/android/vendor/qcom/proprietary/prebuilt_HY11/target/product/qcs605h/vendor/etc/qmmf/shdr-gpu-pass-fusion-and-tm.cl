...
kernel void shdr_fusion_and_tm(
  ...
  if ((get_global_id(0) == (1458 / KERNEL_X_SIZE)) &&
      (get_global_id(1) == (1852 / KERNEL_Y_SIZE))) {
    dbg_en = true;
  }
  ...
  // TODO : implement bayern pattern in the intermediate image
  FLOAT data[8] = {
      quad0_L.s0, quad0_L.s1, quad1_L.s0, quad1_L.s1,
      quad0_L.s2, quad0_L.s3, quad1_L.s2, quad1_L.s3,
  };

  for (offset = 0; offset < 8; offset++) {
    FLOAT pixel_in;
    pixel_in = data[offset];

    FLOAT pow_tmp =
        pow((pixel_in * max_fusion_input_val_r), hdr_bayer_gtm_gamma);
    luma_q0 = pow_tmp * max_fusion_input_val;
    ...
    tmp = gain * pow_tmp * max_tm_output_val;
