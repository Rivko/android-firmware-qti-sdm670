...
typedef struct
{
  ...
  boolean is_decode_time_valid; /*Flag to indicate if decode time is valid             */
  boolean is_time_valid;    /* Flag to indicate if time is valid                       */
  uint64 capture_time_ms;   /* Frame capture system time in millisecs                  */
...
typedef struct MUX_HEVC_TimingHRD_params
{
    uint32  nHEVCTimeScale;
    uint32  nNumUnitsInTick;
    bool    bHrdManagementValid; /*hrd_management_valid_flag*/
    bool    bPictureAndTimingInfo; /*picture_and_timing_info_present_flag*/
}MUX_HEVC_TimingHRD_params_type;
...
typedef enum {
  ...
  MUX_STREAM_AUDIO_MPEGH,          /* MPEGH 3D audio codec           */
...
typedef enum {
  MUX_STREAM_GENERIC_XML,  /* XML Format for Widock */
  MUX_STREAM_GENERIC_UNK    /* Unknown type of generic data */
} MUX_stream_generic_type;

/* This structure contains type-specific information about a generic stream. */
typedef struct {
  MUX_stream_generic_type format;
} MUX_stream_generic_subtype;
...
typedef union {
  ...
  MUX_stream_generic_subtype generic; /* MUX_STREAM_GENERIC */
...
typedef enum {
  ...
  MUX_STREAM_GENERIC, /* MUX_stream_generic_type */
...
typedef struct {
  MUX_method_type    method;     /* This must be VIDEO_METHOD_CLIENT      */
  video::iMultipleIovDataAllocator *pMultipleIovDataAllocator; /* iMultipleIovdata allocation and release interface */
