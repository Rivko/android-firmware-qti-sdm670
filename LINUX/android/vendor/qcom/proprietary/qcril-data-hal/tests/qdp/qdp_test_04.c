...
int qdp_test_04(void)
{
  ...
  QDP_TEST_LOG("qdp_test_04: 3gpp/3gpp2 profile look up with ip6 apn [%s]ENTRY",
                QDP_TEST_APN_NAME);
  ...
  do
  {
    ...
    QDP_MALLOC(params[QDP_RIL_NAI], (strlen(QDP_TEST_APN_NAME)+1)*sizeof(char));
...
int main(void)
{
  if (QDP_SUCCESS == qdp_test_04())
  {
    QDP_TEST_LOG("%s","QDP_TEST_04 PASS");
  }
  else
  {
    QDP_TEST_LOG("%s","QDP_TEST_04 FAIL");
  }
