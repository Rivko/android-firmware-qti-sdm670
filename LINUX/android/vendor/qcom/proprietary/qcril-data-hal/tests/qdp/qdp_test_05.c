...
int qdp_test_05(void)
{
  ...
  QDP_TEST_LOG("qdp_test_05: 3gpp/3gpp2 profile look up with ip6 apn [%s]/nai [%s] ENTRY",
                QDP_TEST_APN_NAME, QDP_TEST_NAI);
  ...
  do
  {
    ...
    QDP_MALLOC(params[QDP_RIL_APN], (strlen(QDP_TEST_APN_NAME)+1)*sizeof(char));
    ...
    QDP_MALLOC(params[QDP_RIL_NAI], (strlen(QDP_TEST_NAI)+1)*sizeof(char));
    ...
    QDP_MALLOC(params[QDP_RIL_IP_FAMILY], (strlen(QDP_RIL_IP_4_6)+1)*sizeof(char));
...
int main(void)
{
  if (QDP_SUCCESS == qdp_test_05())
  {
    QDP_TEST_LOG("%s","QDP_TEST_05 PASS");
  }
  else
  {
    QDP_TEST_LOG("%s","QDP_TEST_05 FAIL");
  }
