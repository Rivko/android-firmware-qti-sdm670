...
int qdp_test_09(void)
{
  ...
  QDP_TEST_LOG("qdp_test_09: 3gpp/3gpp2 profile look up with ipv4v6 apn [%s]ENTRY",
                QDP_TEST_APN_NAME);
  ...
  do
  {
    ...
    strlcpy(params[QDP_RIL_IP_FAMILY],QDP_RIL_IP_4_6, strlen(QDP_RIL_IP_4_6)+1);
...
int main(void)
{
  if (QDP_SUCCESS == qdp_test_09())
  {
    QDP_TEST_LOG("%s","QDP_TEST_09 PASS");
  }
  else
  {
    QDP_TEST_LOG("%s","QDP_TEST_09 FAIL");
  }
