...
class LastDataCallFailCauseTranslator : public legacy_translator<RIL_REQUEST_LAST_DATA_CALL_FAIL_CAUSE> {
public:
    std::shared_ptr<SolicitedMessage<generic_callback_payload>> translate_request(const ril_request_type &request)
    {
        ...
        return std::make_shared<rildata::RilRequestLastDataCallFailCauseMessage>(params);

    }
};

static LastDataCallFailCauseTranslator lastdatacallfailcause_translator;

QCRIL_ANDROID_DECLARE_REQUEST_WITH_CONV(RIL_REQUEST_LAST_DATA_CALL_FAIL_CAUSE, void *, int, lastdatacallfailcause_translator);
QCRIL_ANDROID_DEFINE_REQUEST_WITH_CONV(RIL_REQUEST_LAST_DATA_CALL_FAIL_CAUSE);
