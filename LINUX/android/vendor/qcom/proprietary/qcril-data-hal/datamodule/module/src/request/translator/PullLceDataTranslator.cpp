...
class PullLceDataTranslator : public legacy_translator<RIL_REQUEST_PULL_LCEDATA> {
public:
    std::shared_ptr<SolicitedMessage<generic_callback_payload>> translate_request(const ril_request_type &request)
    {
        ...
        return std::make_shared<rildata::RilRequestPullLceDataMessage>(params);
    }
};


static PullLceDataTranslator PullLceData_translator;

QCRIL_ANDROID_DECLARE_REQUEST_WITH_CONV(RIL_REQUEST_PULL_LCEDATA, void *,
                                            RIL_LceDataInfo, PullLceData_translator);
QCRIL_ANDROID_DEFINE_REQUEST_WITH_CONV(RIL_REQUEST_PULL_LCEDATA);
