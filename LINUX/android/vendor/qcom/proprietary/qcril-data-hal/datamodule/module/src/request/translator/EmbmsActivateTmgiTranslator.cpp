...
class EmbmsActivateTmgiTranslator : public legacy_translator<QCRIL_REQ_HOOK_EMBMS_ACTIVATE_TMGI> {
public:
    std::shared_ptr<SolicitedMessage<generic_callback_payload>> translate_request(const ril_request_type &request)
    {
        ...
        return std::make_shared<rildata::RilRequestEmbmsActivateTmgiMessage>(params);

    }
};


static EmbmsActivateTmgiTranslator EmbmsActivateTmgi_translator;

#if (QCRIL_RIL_VERSION >= 12)
    QCRIL_ANDROID_DECLARE_REQUEST_WITH_CONV(HOOK_EMBMS_ACTIVATE_TMGI, embms_activate_tmgi_req_msg_v01,
                                            embms_activate_tmgi_resp_msg_v01, EmbmsActivateTmgi_translator);
    QCRIL_ANDROID_DEFINE_REQUEST_WITH_CONV(HOOK_EMBMS_ACTIVATE_TMGI);
#else
    /*TODO*/
#endif
#endif
