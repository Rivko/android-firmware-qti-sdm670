...
class EmbmsSendIntTmgiListTranslator : public legacy_translator<QCRIL_REQ_HOOK_EMBMS_UNSOL_GET_INTERESTED_TMGI_LIST> {
public:
    std::shared_ptr<SolicitedMessage<generic_callback_payload>> translate_request(const ril_request_type &request)
    {
        ...
        return std::make_shared<rildata::RilRequestEmbmsSendIntTmgiListMessage>(params);

    }
};


static EmbmsSendIntTmgiListTranslator EmbmsSendIntTmgiList_translator;

#if (QCRIL_RIL_VERSION >= 12)
    QCRIL_ANDROID_DECLARE_REQUEST_WITH_CONV(HOOK_EMBMS_UNSOL_GET_INTERESTED_TMGI_LIST, embms_get_interested_tmgi_list_resp_msg_v01,
                                            embms_get_interested_tmgi_list_resp_msg_v01, EmbmsSendIntTmgiList_translator);
    QCRIL_ANDROID_DEFINE_REQUEST_WITH_CONV(HOOK_EMBMS_UNSOL_GET_INTERESTED_TMGI_LIST);
#else
    /*TODO*/
#endif
#endif
