...
static const uint32_t USF_RX_EPOS_DATA_FORMAT = 0;
static const uint32_t USF_RX_ECHO_DATA_FORMAT = 1;
...
static int usf_parameter_check_mux_params(char* cfgFileName,
                                          us_all_info* paramsStruct);
                                    ...
                                    FormStruct* formStruct)
{
  ...
  if (0 != usf_parameter_check_general(cfgFileName,
      ...
      0 != usf_parameter_check_common_parameter(cfgFileName,
                                                paramsStruct) ||
      0 != usf_parameter_check_mux_params(cfgFileName,
                                          paramsStruct))
  {
    return 1;
  }
                                          ...
                                          FormStruct* formStruct)
{
  ...
  // usf_rx_data_format
  char expected[MAX_VALUE_LEN];
  if (USF_RX_EPOS_DATA_FORMAT      != paramsStruct->usf_rx_data_format &&
      USF_RX_ECHO_DATA_FORMAT      != paramsStruct->usf_rx_data_format &&
      ...
      USF_RX_SYNC_GESTURE_DATA_FORMAT != paramsStruct->usf_rx_data_format)
  {
    snprintf(expected,
             MAX_VALUE_LEN,
             "%d or %d or %d or %d or %d",
             USF_RX_EPOS_DATA_FORMAT,
             USF_RX_ECHO_DATA_FORMAT,
...
/**
 * This function checks that the MUX cfg parameters are valid.
 *
 * @param cfgFileName The path to the cfg file currently used.
 * @param paramsStruct A struct containing the cfg file parameters to be validated
 *
                                          ...
                                          us_all_info *paramsStruct)
{
  ...
  // usf_mux_enabled
  if (0 != paramsStruct->usf_mux_enabled &&
      1 != paramsStruct->usf_mux_enabled)
  {
    print_error_message_i(cfgFileName,
                          "usf_mux_enabled",
                          "0 or 1",
  ...
  {
    // MUX disabled, no need to verify other params
    return 0;
  }
  ...
  {
    uint32_t channel = paramsStruct->usf_mux_state[i];
    if (channel != 0 && channel != 1)
    {
      snprintf(paramName, MAX_VALUE_LEN, "usf_mux_state[%d]", i);
      print_error_message_i(cfgFileName,
                            paramName,
                            "0 or 1",
                            (int)channel);
    ...
    if (channel == 1)
    {
      if (i == 0 || paramsStruct->usf_mux_state[i - 1] == 1)
      {
        snprintf(paramName, MAX_VALUE_LEN, "usf_mux_state[%d]", i);
        print_error_message_i(cfgFileName,
                              paramName,
                              "C2(1) must follow C1(0) channel",
                              (int)channel);
      }
    }
  }
                                               ...
                                               us_all_info *paramsStruct)
{
  ...
  if (0 == strcmp(daemonName,"tester") && 0 != paramsStruct->usf_rx_transparent_data_size) {
    int result = usf_parse_transparent_data(paramsStruct->usf_rx_transparent_data,
                                            &frame,
                                            &group);
    if (0 != result) {
      return result;
  ...
  // usf_rx_pattern_size
  // it is ok to have empty RX pattern when MUX enabled
  if (!paramsStruct->usf_mux_enabled || paramsStruct->usf_rx_pattern_size != 0)
  {
                                        ...
                                        us_all_info* paramsStruct)
{
  ...
  if (0 == strcmp(daemonName,"tester") && 0 != paramsStruct->usf_tx_transparent_data_size) {
    int result = usf_parse_transparent_data(paramsStruct->usf_tx_transparent_data,
                                            &group,
                                            &skip);
    if (0 != result)
    {
  ...
  // touch_zone_border_diff
  if (!strcmp(daemonName,"digitalpen"))
  {
    Vector on_act_zone_area_border, touch_zone_border_diff;
    ...
    if (on_act_zone_area_border.get_element(X) < touch_zone_border_diff.get_element(X) ||
        on_act_zone_area_border.get_element(Y) < touch_zone_border_diff.get_element(Y) ||
        on_act_zone_area_border.get_element(Z) < touch_zone_border_diff.get_element(Z) )
    {
      snprintf(expected,
               MAX_VALUE_LEN,
               "less (or equal) X: %lf  Y: %lf Z: %lf",
               touch_zone_border_diff.get_element(X),
               touch_zone_border_diff.get_element(Y),
               touch_zone_border_diff.get_element(Z));
      print_error_message(cfgFileName,
                        "touch_zone_border_diff",
                        expected,
                                        ...
                                        us_all_info* paramsStruct)
{
  ...
  if (TX_DATA_FORMAT3 != paramsStruct->usf_tx_data_format && // sync gesture format
      TX_DATA_FORMAT2 != paramsStruct->usf_tx_data_format && // sync proximity format
  ...
  if (0 == strcmp(daemonName,"tester") && 0 != paramsStruct->usf_tx_transparent_data_size)
  {
    ...
    {
       uint32_t frame;
       result = usf_parse_transparent_data(paramsStruct->usf_tx_transparent_data,
                                           &frame,
                                           &group,
                                           &skip);
    }
  ...
  if (0 == strcmp(daemonName, "sync_gesture") ||
      0 == strcmp(daemonName, "proximity"))
  {
...
/**
  See description in header file.
*/
int usf_parse_transparent_data (char const* transparent_data,
                                uint32_t* frame,
                                uint16_t* group,
                                uint16_t* skip)
{
                                            ...
                                            us_all_info* paramsStruct)
{
  ...
  if (0 == paramsStruct->usf_persist_save_timeout)
  {
    LOGE("%s: Persist save timeout value is not set", __FUNCTION__);
    return 1;
  }

  if (0 == paramsStruct->usf_persist_buf_size)
  {
    LOGE("%s: Persist buffer size value is not set", __FUNCTION__);
    return 1;
  }

  if (0 == paramsStruct->usf_persist_file[0])
  {
    LOGE("%s: Persist file save location is not set", __FUNCTION__);
    return 1;
  }
                                           ...
                                           us_all_info* paramsStruct)
{
  ...
  if (0 != strcmp(paramsStruct->usf_adapter_lib,
                  "") &&
      0 == (paramsStruct->usf_event_dest & EVENT_DEST_OUT) &&
      0 != (paramsStruct->usf_event_dest & EVENT_DEST_UAL))
  {
    snprintf(expected,
             MAX_VALUE_LEN,
  ...
  else if (0 == (paramsStruct->usf_event_dest & EVENT_DEST_OUT) &&
           0 == (paramsStruct->usf_event_dest & EVENT_DEST_UAL))
  {
    snprintf(expected,
             MAX_VALUE_LEN,
