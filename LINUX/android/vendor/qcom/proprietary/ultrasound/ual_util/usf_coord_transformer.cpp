                                    ...
                                    transformation_properties &trans_props)
{
  ...
  // Calculate touch zone distances
  touch_zone_area_border.vector_multiply_element_by_element(trans_props.scaling_factor);
  trans_props.touch_zone_area_border = touch_zone_area_border;
  trans_props.touch_zone_area_border.set_element(Z, -touch_zone_area_border.get_element(Z));
                              ...
                              transformation_properties off_screen_trans_prop)
{
  ...
  * 1. On screen zone
  * 2. Off screen zone
  * 3. On screen touch border
  * 4. On screen active zone border
  * 5. Off screen active zone border
  * 6. Not on any active zone
  */
  ...
  else
  {
    ...
    else if (TOUCH_ZONE_BORDER == on_screen_zone)
    {
      result = ON_SCREEN_TOUCH_BORDER;
    }
                ...
                transformation_properties trans_props)
{
  ...
  if (point.get_element(X) >=
      (0.0 - trans_props.touch_zone_area_border.get_element(X)) &&
      point.get_element(X) <=
      (TSC_LOGICAL_MAX_X + trans_props.touch_zone_area_border.get_element(X)) &&
      point.get_element(Y) >=
      (0.0 - trans_props.touch_zone_area_border.get_element(Y)) &&
      point.get_element(Y) <=
      (TSC_LOGICAL_MAX_Y + trans_props.touch_zone_area_border.get_element(Y)) &&
      point.get_element(Z) <=
      (trans_props.hover_max_range + trans_props.touch_zone_area_border.get_element(Z)))
  {
    return TOUCH_ZONE_BORDER;
  }
                                  ...
                                  Matrix &result_matrix)
{
  ...
  else if (0 == point_end_x.get_element(Y))
  {
    new_axis_x.set_element(X, (-nr.get_element(Y)-nr.get_element(Z)*0.0) / nr.get_element(X));
    new_axis_x.set_element(Y, 1.0);
