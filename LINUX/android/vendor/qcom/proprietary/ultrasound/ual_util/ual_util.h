...
const uint16_t MAX_ALGO_TRANSPARENT_DATA_SIZE = 50000;
...
#define OUTPUT_TYPE_PROXIMITY_EVENT_SHIFT 1
...
#define SW_TESTER_POINT1      0
#define SW_TESTER_POINT2      1
#define SW_TESTER_POINT3      2
#define SW_TESTER_POINT4      3
#define SW_TESTER_POINT5      4
#define SW_TESTER_POINT_COUNT 5
...
typedef struct {
    ...
    uint32_t usf_rx_dev_arbitration;
    uint32_t usf_rx_dev_arbitration_audio_devices;
    uint32_t usf_rx_dev_arbitration_default_audio_device;
    uint32_t usf_rx_ramp_down_timeout;
    ...
    char usf_gesture_fw_lib[FILE_PATH_MAX_LEN];
    ...
    // MUX support
    uint32_t usf_mux_enabled;
    uint16_t usf_mux_state[USF_MAX_PORT_NUM];
    int16_t usf_mux_c1_sample;
    int16_t usf_mux_c2_sample;
    int16_t usf_mux_sync_delay_compensation;
    ...
    char usf_epos_touch_zone_border_diff[FILE_PATH_MAX_LEN];
    ...
    uint32_t usf_epos_wait_before_sniffing_mode;
    ...
    // Private Gesture
    uint32_t usf_event_dest;
    uint32_t usf_gesture_event_out_port;
    char usf_keys[FILE_PATH_MAX_LEN];
    uint32_t usf_app_lib_bypass;
    uint32_t usf_post_mortem_enable;
    uint32_t usf_post_mortem_frame_count;
    uint32_t usf_algo_transparent_data_size;
    char usf_algo_transparent_data[MAX_ALGO_TRANSPARENT_DATA_SIZE];

    // Private DSP calculated events daemons
    uint32_t usf_output_type;
    uint32_t usf_persist_save_timeout;
    uint32_t usf_persist_buf_size;
    char usf_persist_file[FILE_PATH_MAX_LEN];
    ...
    char usf_tester_position[SW_TESTER_POINT_COUNT][FILE_PATH_MAX_LEN];
    char usf_tester_power_min_scales[SW_TESTER_POINT_COUNT][FILE_PATH_MAX_LEN];
    char usf_tester_power_max_scales[SW_TESTER_POINT_COUNT][FILE_PATH_MAX_LEN];
    char usf_tester_power_thresholds[SW_TESTER_POINT_COUNT][FILE_PATH_MAX_LEN];
    char usf_tester_quality_min_scales[SW_TESTER_POINT_COUNT][FILE_PATH_MAX_LEN];
    char usf_tester_quality_max_scales[SW_TESTER_POINT_COUNT][FILE_PATH_MAX_LEN];
    char usf_tester_quality_thresholds[SW_TESTER_POINT_COUNT][FILE_PATH_MAX_LEN];
...
typedef struct
{
  ...
  // The screen info - size: in X_IND, Y_IND
  int screen_size[2];
} FormStruct;
...
==============================================================================*/
/**
  ual_util_tx_mux_config function configures the deMUXing operation.
  call AFTER configuring TX stream (ual_util_tx_config) but BEFORE
  configuring RX stream (ual_util_rx_config).
  Will do nothing if MUX is disabled in the configuration.
  Returns 0 for success, -1 for failure or 1 if MUX disabled in the configuration.
*/
...
==============================================================================*/
/**
 Return the number of actual TX channels that will be opened on the device
 TX stream. If no MUXing is configured, it is simply usf_tx_port_count. Otherwise
 it may be smaller if MUXing is configured because we put 2 diluted channels on
 a single physical channel.
 The function assumes usf_mux_state (MUX channel config) is validated.
*/
