                                     ...
                                     FILE *frameFileName)
{
  ...
  case RAW_IN_FORMAT:
    ret =  usf_parse_transparent_data(paramsStruct->usf_tx_transparent_data,
                                      &frame,
                                      &group,
                                      &skip);
    break;
  ...
  // note: WAV descriptions are not strings, not null-terminated, but are
  // a binary format where some of the fields are human-readable.
  // memcpy is appropriate here, even though the source is a constant
  // string.
  memcpy(wave_header.chunkID , "RIFF", WAVE_DESCRIPTION);
  memcpy(wave_header.format , "WAVE", WAVE_DESCRIPTION);
  memcpy(wave_header.subChunk1ID , "fmt ", WAVE_DESCRIPTION);
  ...
  memcpy(wave_header.subChunk2ID , "data", WAVE_DESCRIPTION);
...
==============================================================================*/
/**
 * Static function. Definition is above.
 */
