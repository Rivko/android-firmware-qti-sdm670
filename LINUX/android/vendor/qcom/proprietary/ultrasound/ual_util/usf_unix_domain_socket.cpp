...
int ControlUnSocket::thread_func()
{
  ...
  while (true)
  {
    ...
    if (retval == 0)
    {
      LOGE("%s: recv() failed, connection closed by peer",
           __FUNCTION__);
      return -1;
    }
...
void ControlUnSocket::print_config_message(config_parameters_t *cp)
{
  ...
  LOGD("VERIFY event_type: %d", cp->event_type);
  LOGD("VERIFY mouse_sensitivity: %d", cp->mouse_sensitivity);
}
...
void ControlUnSocket::update_dynamic_config_params(config_parameters_t *cp)
{
  ...
  m_dconfig->event_type = cp->event_type;
  m_dconfig->mouse_sensitivity = cp->mouse_sensitivity;
...
bool ControlUnSocket::update_config(config_parameters_t *cp)
{
  ...
  if (validate_dynamic_config())
  {
    ...
    LOGD("GOT NEW CONFIG: on_screen_hover_range:%d, off_screen_hover_range: %d, off_screen_mode:%d, on_screen_destination:%d, off_screen_destination:%d, off screen origin(%lf,%lf,%lf), endX(%lf,%lf,%lf), endY(%lf,%lf,%lf), eraser_button_index:%d, eraser_button_mode:%d, show_on_screen_hover_icon:%d, show_off_screen_hover_icon:%d, event_type:%d, mouse_sensitivity:%d",
         ...
         m_dconfig->event_type,
         m_dconfig->mouse_sensitivity);
  }
  else
  {
...
)
{
  ...
  while ((nbytes = send(client_fd,
                        arr + (size - bytes_left),
                        bytes_left,
                        MSG_NOSIGNAL)) < bytes_left)
  {
    if (0 >= nbytes)
    {
      LOGE("%s: write() failed, %s",
...
*/
int DataUnSocket::send_proximity_event(int proximity,
                                       int distance,
                                       int event_source)
{
  int buff[3];
