...
/*==============================================================================
  FUNCTION:  ual_util_get_screen_info
==============================================================================*/
/**
  This function provides the screen size either from Android framework or
  configuratuion file.
  Also the function provides the screen orientation either
  from Android framework or set it to 0.
*/
bool ual_util_get_screen_info(int &x_size,
                              int &y_size,
                              int &orientation)
{
  bool rc = true;
  struct android::DisplayInfo dInfo;
  android::sp<android::IBinder> display(
            android::SurfaceComposerClient::getBuiltInDisplay(
            android::ISurfaceComposer::eDisplayIdMain));
  int ret = android::SurfaceComposerClient::getDisplayInfo(display, &dInfo);
  LOGD("%s: Display info: ret=%d w=%d; h=%d; orientation=%d; fps=%f; "
  ...
  if (ret)
  {
    if (0 == formStruct.screen_size[X_IND])
    {
      LOGE("%s: There is no display info",
           __FUNCTION__);
      rc = false;
    }
    else
    {
      x_size = formStruct.screen_size[X_IND];
      y_size = formStruct.screen_size[Y_IND];
      orientation = 0;
      LOGD("%s: Display config info: w=%d; h=%d; orientation=%d",
           __FUNCTION__,
           x_size,
           y_size,
           orientation);
                         ...
                         paramStruct *paramInfo)
{
  ...
  lineLen = strlen(line);
  for (i = 0; i < lineLen && i < FILE_PATH_MAX_LEN; i++)
  {
    if (line[i] == ' ' ||
        line[i] == '\t' ||
        line[i] == '\n')
                                         ...
                                         const char *type)
{
  ...
  unsigned int expected_max_size = 0;
  int is_algo = 0;
  ...
  else
  {
    ...
    is_algo = strncmp(type, "algo", strlen("algo") + 1);
    expected_max_size = (is_algo == 0) ? MAX_ALGO_TRANSPARENT_DATA_SIZE : MAX_TRANSPARENT_DATA_SIZE;

    if (expected_max_size < transparent_data_size)
    {
    ...
    *ps_transparent_data_size = transparent_data_size;
    ...
    const int DATA_STR_LEN = 4*MAX_ALGO_TRANSPARENT_DATA_SIZE; // Max size is 4* since each byte is at most "255,"
    char transparent_data_string[DATA_STR_LEN] = {0};
    int offset = 0;
    int bytesRemaining = DATA_STR_LEN;
    for (unsigned int i = 0; i < *ps_transparent_data_size; ++i)
    {
        int retval = snprintf(&transparent_data_string[offset],
                              bytesRemaining,
                              "%d,",
                              transparent_data[i]);
        if (retval < 0 || retval >= bytesRemaining) {
            LOGE("%s: error translating transparent_data_size",
                 __FUNCTION__);
            free(transparent_data);
            return -1;
        }
    ...
    if (offset >= 1) {
        transparent_data_string[offset-1] = '\0'; // Remove last ','
    }
                             ...
                             us_all_info *paramsStruct)
{
  ...
  while (NULL != line)
  {
    ...
    else if (strcmp(paramName, "usf_rx_dev_arbitration") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_rx_dev_arbitration);
      LOGD("%s: usf_rx_dev_arbitration is: %d",
           __FUNCTION__,
           paramsStruct->usf_rx_dev_arbitration);
    }
    else if (strcmp(paramName, "usf_rx_dev_arbitration_audio_devices") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_rx_dev_arbitration_audio_devices);
      LOGD("%s: usf_rx_dev_arbitration_audio_devices is: %d",
           __FUNCTION__,
           paramsStruct->usf_rx_dev_arbitration_audio_devices);
    }
    else if (strcmp(paramName, "usf_rx_dev_arbitration_default_audio_device")
              == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_rx_dev_arbitration_default_audio_device);
      LOGD("%s: usf_rx_dev_arbitration_default_audio_device is: %d",
           __FUNCTION__,
           paramsStruct->usf_rx_dev_arbitration_default_audio_device);
    }

    else if (strcmp(paramName, "usf_rx_ramp_down_timeout") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_rx_ramp_down_timeout);
      LOGD("%s: usf_rx_ramp_down_timeout is: %d",
                 __FUNCTION__,
                 paramsStruct->usf_rx_ramp_down_timeout);
    }
    ...
    // MUX params
    else if (strcmp(paramName, "usf_mux_enabled") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_mux_enabled);
      LOGD("%s: usf_mux_enabled is: %d",
           __FUNCTION__,
           paramsStruct->usf_mux_enabled);
    }
    else if (strcmp(paramName, "usf_mux_state") == 0)
    {
      uint32_t num_elements = 0;
      ret = ual_util_cfg_parse_uint16_array(temp,
                                            &paramsStruct->usf_mux_state[0],
                                            USF_MAX_PORT_NUM,
                                            &num_elements);
      if (ret == 0)
      {
        if (num_elements != paramsStruct->usf_tx_port_count)
        {
          LOGE("%s: usf_mux_state must have usf_tx_port_count elements. Got %u elements",
               __FUNCTION__, num_elements);
          ret = -1;
        }
      }
      LOGD("%s: usf_mux_state has %u entries",
           __FUNCTION__,
           num_elements);
      for (i = 0; i < (int)num_elements; i++)
      {
        LOGD("%s: usf_mux_state[%d]=%hu",
             __FUNCTION__, i, paramsStruct->usf_mux_state[i]);
      }
    }
    else if (strcmp(paramName, "usf_mux_c1_sample") == 0)
    {
      ret = sscanf(temp,
                   "%hd",
                   &paramsStruct->usf_mux_c1_sample);
      LOGD("%s: usf_mux_c1_sample is: %hd",
           __FUNCTION__,
           paramsStruct->usf_mux_c1_sample);
    }
    else if (strcmp(paramName, "usf_mux_c2_sample") == 0)
    {
      ret = sscanf(temp,
                   "%hd",
                   &paramsStruct->usf_mux_c2_sample);
      LOGD("%s: usf_mux_c2_sample is: %hd",
           __FUNCTION__,
           paramsStruct->usf_mux_c2_sample);
    }
    else if (strcmp(paramName, "usf_mux_sync_delay_compensation") == 0)
    {
      ret = sscanf(temp,
                   "%hd",
                   &paramsStruct->usf_mux_sync_delay_compensation);
      LOGD("%s: usf_mux_sync_delay_compensation is: %hd",
           __FUNCTION__,
           paramsStruct->usf_mux_sync_delay_compensation);
    }
    ...
    else if (strcmp(paramName, "usf_epos_touch_zone_border_diff") == 0)
    {
      strlcpy(paramsStruct->usf_epos_touch_zone_border_diff,
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_epos_touch_zone_border_diff is: %s",
           __FUNCTION__,
           paramsStruct->usf_epos_touch_zone_border_diff);
    }
    ...
    else if (strcmp(paramName, "usf_epos_wait_before_sniffing_mode") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_epos_wait_before_sniffing_mode);
      LOGD("%s: usf_epos_wait_before_sniffing_mode is: %d",
           __FUNCTION__,
           paramsStruct->usf_epos_wait_before_sniffing_mode);
    }
    ...
    // Gesture params
    else if (strcmp(paramName, "usf_event_dest") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_event_dest);
      LOGD("%s: usf_event_dest is: %d",
           __FUNCTION__,
           paramsStruct->usf_event_dest);
    }
    ...
    else if (strcmp(paramName, "usf_keys") == 0)
    {
      strlcpy(paramsStruct->usf_keys,
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_keys is: %s",
           __FUNCTION__,
    ...
    else if (strcmp(paramName, "usf_gesture_fw_lib") == 0)
    {
      strlcpy(paramsStruct->usf_gesture_fw_lib,
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_gesture_fw_lib is: %s",
           __FUNCTION__,
           paramsStruct->usf_gesture_fw_lib);
    }
    else if (strcmp(paramName, "usf_app_lib_bypass") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_app_lib_bypass);
      LOGD("%s: usf_app_lib_bypass is: %d",
           __FUNCTION__,
           paramsStruct->usf_app_lib_bypass);
    }
    else if (strcmp(paramName, "usf_post_mortem_enable") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_post_mortem_enable);
      LOGD("%s: usf_post_mortem_enable is: %d",
           __FUNCTION__,
           paramsStruct->usf_post_mortem_enable);
    }
    else if (strcmp(paramName, "usf_post_mortem_frame_count") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_post_mortem_frame_count);
      LOGD("%s: usf_post_mortem_frame_count is: %d",
           __FUNCTION__,
           paramsStruct->usf_post_mortem_frame_count);
    }
    ...
    else if (strcmp(paramName,"usf_persist_save_timeout") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_persist_save_timeout);
      LOGD("%s: usf_persist_save_timeout is: %d",
           __FUNCTION__,
           paramsStruct->usf_persist_save_timeout);
    }
    else if (strcmp(paramName,"usf_persist_buf_size") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_persist_buf_size);
      LOGD("%s: usf_persist_buf_size is: %d",
           __FUNCTION__,
           paramsStruct->usf_persist_buf_size);
    }
    else if (strcmp(paramName,"usf_persist_file") == 0)
    {
      strlcpy(paramsStruct->usf_persist_file,
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_persist_file is: %s",
           __FUNCTION__,
    ...
    // Point 1
    else if (strcmp(paramName, "usf_tester_position_point1") == 0)
    {
      strlcpy(paramsStruct->usf_tester_position[SW_TESTER_POINT1],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_position_point1 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_position[SW_TESTER_POINT1]);
    }
    else if (strcmp(paramName,"usf_tester_power_min_scales_point1") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT1],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_min_scales_point1 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT1]);
      paramsStruct->usf_sw_calib_is_tester_mode = true;
    }
    else if (strcmp(paramName,"usf_tester_power_max_scales_point1") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT1],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_max_scales_point1 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT1]);
    }
    else if (strcmp(paramName,"usf_tester_power_thresholds_point1") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT1],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_thresholds_point1 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT1]);
    }
    else if (strcmp(paramName,"usf_tester_quality_min_scales_point1") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT1],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_min_scales_point1 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT1]);
    }
    else if (strcmp(paramName,"usf_tester_quality_max_scales_point1") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT1],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_max_scales_point1 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT1]);
    }
    else if (strcmp(paramName,"usf_tester_quality_thresholds_point1") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT1],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_thresholds_point1 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT1]);
    }
    // Point 2
    else if (strcmp(paramName, "usf_tester_position_point2") == 0)
    {
      strlcpy(paramsStruct->usf_tester_position[SW_TESTER_POINT2],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_position_point2 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_position[SW_TESTER_POINT2]);
    }
    else if (strcmp(paramName,"usf_tester_power_min_scales_point2") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT2],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_min_scales_point2 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT2]);
      paramsStruct->usf_sw_calib_is_tester_mode = true;
    }
    else if (strcmp(paramName,"usf_tester_power_max_scales_point2") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT2],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_max_scales_point2 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT2]);
    }
    else if (strcmp(paramName,"usf_tester_power_thresholds_point2") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT2],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_thresholds_point2 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT2]);
    }
    else if (strcmp(paramName,"usf_tester_quality_min_scales_point2") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT2],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_min_scales_point2 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT2]);
    }
    else if (strcmp(paramName,"usf_tester_quality_max_scales_point2") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT2],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_max_scales_point2 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT2]);
    }
    else if (strcmp(paramName,"usf_tester_quality_thresholds_point2") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT2],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_thresholds_point2 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT2]);
    }
    // Point 3
    else if (strcmp(paramName, "usf_tester_position_point3") == 0)
    {
      strlcpy(paramsStruct->usf_tester_position[SW_TESTER_POINT3],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_position_point3 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_position[SW_TESTER_POINT3]);
    }
    else if (strcmp(paramName,"usf_tester_power_min_scales_point3") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT3],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_min_scales_point3 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT3]);
      paramsStruct->usf_sw_calib_is_tester_mode = true;
    }
    else if (strcmp(paramName,"usf_tester_power_max_scales_point3") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT3],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_max_scales_point3 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT3]);
    }
    else if (strcmp(paramName,"usf_tester_power_thresholds_point3") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT3],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_thresholds_point3 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT3]);
    }
    else if (strcmp(paramName,"usf_tester_quality_min_scales_point3") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT3],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_min_scales_point3 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT3]);
    }
    else if (strcmp(paramName,"usf_tester_quality_max_scales_point3") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT3],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_max_scales_point3 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT3]);
    }
    else if (strcmp(paramName,"usf_tester_quality_thresholds_point3") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT3],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_thresholds_point3 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT3]);
    }
    // Point 4
    else if (strcmp(paramName, "usf_tester_position_point4") == 0)
    {
      strlcpy(paramsStruct->usf_tester_position[SW_TESTER_POINT4],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_position_point4 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_position[SW_TESTER_POINT4]);
    }
    else if (strcmp(paramName,"usf_tester_power_min_scales_point4") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT4],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_min_scales_point4 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT4]);
      paramsStruct->usf_sw_calib_is_tester_mode = true;
    }
    else if (strcmp(paramName,"usf_tester_power_max_scales_point4") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT4],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_max_scales_point4 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT4]);
    }
    else if (strcmp(paramName,"usf_tester_power_thresholds_point4") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT4],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_thresholds_point4 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT4]);
    }
    else if (strcmp(paramName,"usf_tester_quality_min_scales_point4") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT4],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_min_scales_point4 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT4]);
    }
    else if (strcmp(paramName,"usf_tester_quality_max_scales_point4") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT4],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_max_scales_point4 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT4]);
    }
    else if (strcmp(paramName,"usf_tester_quality_thresholds_point4") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT4],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_thresholds_point4 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT4]);
    }
    // Point 5
    else if (strcmp(paramName, "usf_tester_position_point5") == 0)
    {
      strlcpy(paramsStruct->usf_tester_position[SW_TESTER_POINT5],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_position_point5 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_position[SW_TESTER_POINT5]);
    }
    else if (strcmp(paramName,"usf_tester_power_min_scales_point5") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT5],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_min_scales_point5 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_min_scales[SW_TESTER_POINT5]);
      paramsStruct->usf_sw_calib_is_tester_mode = true;
    }
    else if (strcmp(paramName,"usf_tester_power_max_scales_point5") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT5],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_max_scales_point5 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_max_scales[SW_TESTER_POINT5]);
    }
    else if (strcmp(paramName,"usf_tester_power_thresholds_point5") == 0)
    {
      strlcpy(paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT5],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_power_thresholds_point5 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_power_thresholds[SW_TESTER_POINT5]);
    }
    else if (strcmp(paramName,"usf_tester_quality_min_scales_point5") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT5],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_min_scales_point5 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_min_scales[SW_TESTER_POINT5]);
    }
    else if (strcmp(paramName,"usf_tester_quality_max_scales_point5") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT5],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_max_scales_point5 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_max_scales[SW_TESTER_POINT5]);
    }
    else if (strcmp(paramName,"usf_tester_quality_thresholds_point5") == 0)
    {
      strlcpy(paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT5],
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_tester_quality_thresholds_point5 is: %s",
           __FUNCTION__,
           paramsStruct->usf_tester_quality_thresholds[SW_TESTER_POINT5]);
    }
    // Pairing params
                           ...
                           char* patternDirPath)
{
  ...
  if (0 == *paramsStruct->usf_rx_pattern)
  {
    LOGW("%s: Empty pattern file name",
         __FUNCTION__);
    return -1;
  }
                        ...
                        char* clientName)
{
  ...
  int num_actual_channels, temp_res;
  bool rc = false;
  ...
  num_actual_channels = ual_util_mux_get_num_physical_tx_channels(paramsStruct);
  rc = ual_configure_TX_with_mux(&tx_info, num_actual_channels);
  if (rc != 1 &&
  ...
  // MUX configuration is optional
  temp_res = ual_util_tx_mux_config(paramsStruct);
  if (temp_res < 0)
  {
    LOGW("%s: ual_util_tx_mux_config failed",
         __FUNCTION__);
    return -1;
  }
...
int ual_util_rx_config (us_all_info *paramsStruct,
                        char* clientName,
                        aud_dev_arbi_event_cb_t rxStateEvtCb)
{
  int ret;
  uint32_t temp_port0, temp_port1, temp_port2, temp_port3;
  uint8_t transparentRxData[TRANSPARENT_DATA_MAX_SIZE] = {0};
  bool rc = false;
  ual_rx_dev_arbi_config_type rxDevArbiConfig, *pRxDevArbiConfig = NULL;
  ...
  if (rxStateEvtCb)
  {
    rxDevArbiConfig.rx_state_evt_cb = rxStateEvtCb;
    rxDevArbiConfig.rx_dev_arbi_enabled_audio_devices =
      paramsStruct->usf_rx_dev_arbitration_audio_devices;
    rxDevArbiConfig.rx_dev_arbi_default_audio_device =
      paramsStruct->usf_rx_dev_arbitration_default_audio_device;
    pRxDevArbiConfig = &rxDevArbiConfig;
  }

  rc = ual_configure_RX(&rx_info, pRxDevArbiConfig, paramsStruct->usf_rx_ramp_down_timeout);
  if (1 != rc &&
      UAL_MODE_IDLE_ALL_DATA_PATH != paramsStruct->ual_work_mode)
  {
...
/*==============================================================================
  FUNCTION:  ual_util_tx_mux_config
==============================================================================*/
int ual_util_tx_mux_config (us_all_info *paramsStruct)
{
  ...
  if (!paramsStruct->usf_mux_enabled)
  {
    LOGW("%s: MUXing disabled, nothing to do",
         __FUNCTION__);
    return 1;
  }

  // configure the ADSP service to enable MUXing
  usm_param_id_endec_svc_demux_setup_cfg_t svcDemuxCfg;
  svcDemuxCfg.demux_setup_cfg_version = USM_API_VERSION_ENDEC_SVC_DEMUX_SETUP;
  svcDemuxCfg.demux_enable = (uint16_t)paramsStruct->usf_mux_enabled;
  svcDemuxCfg.c1_mux_sample = paramsStruct->usf_mux_c1_sample;
  svcDemuxCfg.c2_mux_sample = paramsStruct->usf_mux_c2_sample;
  svcDemuxCfg.sync_delay_compensation = paramsStruct->usf_mux_sync_delay_compensation;

  stream_param.module_id = USM_MODULE_ID_ENDEC_SVC;
  stream_param.param_id = USM_PARAM_ID_ENDEC_SVC_DEMUX_SETUP;
  stream_param.pbuf = (uint8_t*)(&svcDemuxCfg);
  stream_param.buf_size = sizeof(svcDemuxCfg);

  if (!ual_set_TX_param(stream_param))
  {
    LOGE("%s: ual_set_TX_param failed for USM_PARAM_ID_ENDEC_SVC_DEMUX_SETUP",
         __FUNCTION__);
    return -1;
  }

  // configure the ADSP algorithm library with the MUXing channel configuration
  usm_param_id_mux_epos_lib_mux_cfg_t libDemuxCfg;
  libDemuxCfg.mux_epos_lib_mux_cfg_version = USM_API_VERSION_MUX_EPOS_LIB_MUX_CONFIG;
  memset(libDemuxCfg.channels, 0, sizeof(libDemuxCfg.channels));
  for (i=0; i<paramsStruct->usf_tx_port_count; i++)
  {
    libDemuxCfg.channels[i] = (uint8_t)paramsStruct->usf_mux_state[i];
  }

  stream_param.module_id = USM_MODULE_ID_MUX_EPOS_LIB;
  stream_param.param_id = USM_PARAM_ID_MUX_EPOS_LIB_MUX_CONFIG;
  stream_param.pbuf = (uint8_t*)(&libDemuxCfg);
  stream_param.buf_size = sizeof(libDemuxCfg);

  if (!ual_set_TX_param(stream_param))
  {
    LOGE("%s: ual_set_TX_param failed for USM_PARAM_ID_MUX_EPOS_LIB_MUX_CONFIG",
         __FUNCTION__);
    return -1;
  }

  LOGI("%s: MUX setup completed successfully", __FUNCTION__);
  return 0;
}
...
/*==============================================================================
  FUNCTION:  ual_util_mux_get_num_physical_tx_channels
==============================================================================*/
int ual_util_mux_get_num_physical_tx_channels (us_all_info *paramsStruct)
{
  int i, result;

  if (!paramsStruct->usf_mux_enabled)
  {
      return paramsStruct->usf_tx_port_count;
  }

  result = paramsStruct->usf_tx_port_count;
  for (i = 0; i < paramsStruct->usf_tx_port_count; i++)
  {
    if (paramsStruct->usf_mux_state[i] == 1)
    {
      result--;
    }
  }
