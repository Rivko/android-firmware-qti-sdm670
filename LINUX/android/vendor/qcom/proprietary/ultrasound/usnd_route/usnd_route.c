/*
**
** Copyright (c) 2013-2015 Qualcomm Technologies, Inc. All Rights ReservedA
...
#define MAX_TRACKED_MIXER_CONTROLS  100
...
static void path_print(struct usnd_route *ur, struct mixer_path *path)
{
    ...
    for (i = 0; i < path->length; i++) {
        ...
        for (j = 0; j < path->setting[i].num_values; j++)
            ALOGE("    id=%d value=%ld", j, path->setting[i].value[j]);
    }
}
                            ...
                            struct mixer_setting *setting)
{
    ...
    if (find_ctl_index_in_path(path, setting->ctl_index) != -1) {
        ...
        ALOGE("Control '%s' index %d already exists in path '%s'",
              mixer_ctl_get_name(ctl), (int)setting->ctl_index, path->name);
        return -1;
    }
    ...
    path->setting[path_index].value = malloc(setting->num_values * sizeof(long));
    /* copy all values */
    memcpy(path->setting[path_index].value, setting->value,
           setting->num_values * sizeof(long));
                          ...
                          struct mixer_value *mixer_value)
{
    ...
    if (path_index < 0) {
        ...
        path->setting[path_index].value = malloc(num_values * sizeof(long));
...
static int path_apply(struct usnd_route *ur, struct mixer_path *path)
{
    ...
    for (i = 0; i < path->length; i++) {
        ...
        memcpy(ur->mixer_state[ctl_index].new_value, path->setting[i].value,
               path->setting[i].num_values * sizeof(long));
...
static int path_reset(struct usnd_route *ur, struct mixer_path *path)
{
    ...
    for (i = 0; i < path->length; i++) {
        ...
        memcpy(ur->mixer_state[ctl_index].new_value,
               ...
               ur->mixer_state[ctl_index].num_values * sizeof(long));
    }
...
static long mixer_enum_string_to_value(struct mixer_ctl *ctl, const char *string)
{
    unsigned long i;

    /* Search the enum strings for a particular one */
    for (i = 0; i < mixer_ctl_get_num_enums(ctl); i++) {
        const char *enum_string = mixer_ctl_get_enum_string(ctl, i);
        if (NULL != enum_string)
        {
...
/* find mixer control in list and return its index. If does not exist, add it and store
   its initial value. Return true on success and fills in out_ctl_index,
   false if failed */
static bool find_mixer_control_in_list(struct usnd_route* ur, struct mixer_ctl* ctl,
                                       unsigned int *out_ctl_index)
{
    unsigned int ctl_index, i;
    bool found = false;
    unsigned int num_values;
    enum mixer_ctl_type type;

    if (out_ctl_index == NULL) {
        ALOGE("%s: ctl_index NULL", __FUNCTION__);
        return false;
    }

    if (ctl == NULL) {
        ALOGE("%s: ctl is NULL", __FUNCTION__);
        return false;
    }
    ...
    if (found) {
        *out_ctl_index = ctl_index;
        return true;
    }

    /* add a new tracked mixer control */
    if (ur->num_mixer_ctls >= MAX_TRACKED_MIXER_CONTROLS) {
        ALOGE("%s: too many tracked mixer controls", __FUNCTION__);
        return false;
    }

    ctl_index = ur->num_mixer_ctls;
    i = ctl_index;
    num_values = mixer_ctl_get_num_values(ctl);
    ...
    if ((type != MIXER_CTL_TYPE_BOOL) && (type != MIXER_CTL_TYPE_INT) &&
        ...
        ALOGE("%s: unsupported mixer control", __FUNCTION__);
        return false;
    }

    ur->mixer_state[i].old_value = malloc(num_values * sizeof(long));
    ur->mixer_state[i].new_value = malloc(num_values * sizeof(long));
    ur->mixer_state[i].reset_value = malloc(num_values * sizeof(long));
    if (ur->mixer_state[i].old_value == NULL ||
        ur->mixer_state[i].new_value == NULL ||
        ur->mixer_state[i].reset_value == NULL) {
        ALOGE("%s: fail to allocate mixer values for mixer_state[%d]",
              __FUNCTION__, (int)i);
                      ...
                      const XML_Char **attr)
{
    ...
    if (strcmp(tag_name, "path") == 0) {
        ...
        /* locate the mixer ctl in the list */
        found = find_mixer_control_in_list(ur, ctl, &ctl_index);
        if (!found) {
            ALOGE("Problem getting index of control '%s' - skipping", attr_name);
            goto done;
        }
...
{
    ur->num_mixer_ctls = 0;
    ur->mixer_state = malloc(MAX_TRACKED_MIXER_CONTROLS * sizeof(struct mixer_state));
    if (!ur->mixer_state)
        return -1;
...
static void save_mixer_state(struct usnd_route *ur)
{
    ...
    for (i = 0; i < ur->num_mixer_ctls; i++) {
        ...
        memcpy(ur->mixer_state[i].reset_value, ur->mixer_state[i].new_value,
               ur->mixer_state[i].num_values * sizeof(long));
    }
}
...
int usnd_ramp_down_rx_vol(struct usnd_route *ur, const char *name, int ramp_down_timeout)
{
    struct mixer_path *path;
    unsigned int i;
    unsigned int ctl_index;
    const char * ramp_down_ctl_name = "Digital Volume";
    long *reset_val = 0;
    ...
    for (i = 0; i < path->length; i++) {
        ctl_index = path->setting[i].ctl_index;
        if (strstr(mixer_ctl_get_name(ur->mixer_state[ctl_index].ctl), ramp_down_ctl_name) != NULL) {
            // Ramp down rx volume
            ALOGD("%s: Ramp down rx volume to zero with timeout %d", __FUNCTION__, ramp_down_timeout);
            ur->mixer_state[ctl_index].new_value[0] = 0;
            update_ctl(ur, ctl_index);
            usleep(ramp_down_timeout);
        }
