...
public class DigitalPenManager {
    ...
    private final class OffScreenDimensionsReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            synchronized (DigitalPenManager.this) {
                ScreenDimensionsListener listener = offScreenDimensionsListenerRef.get();
                if (listener != null) {
                    final float[] origin = intent
                            .getFloatArrayExtra(EventInterface.INTENT_DIMENSION_KEY_ORIGIN);
                    final float[] endX = intent
                            .getFloatArrayExtra(EventInterface.INTENT_DIMENSION_KEY_END_X);
                    final float[] endY = intent
                            .getFloatArrayExtra(EventInterface.INTENT_DIMENSION_KEY_END_Y);
                    listener.onScreenDimensionsUpdated(origin, endX, endY);
                }
            }
        }
    ...
    private static final String VERSION = "307.0";
    ...
    public class Settings {
        ...
        /**
         * Sets the input type for the Android events generated by the
         * framework.
         *
         * @param input the new input type
         */
        public Settings setInputType(InputType input) {
            appSettings.putSerializable(AppInterfaceKeys.INPUT_TYPE, input);
            return this;
        }

        /**
         * Get the current input type for the Android events generated by the
         * framework.
         *
         * @return the current input type
         */
        public InputType getInputType() {
            return (InputType) appSettings.getSerializable(AppInterfaceKeys.INPUT_TYPE);
        }

        /**
         * Sets the mouse sensitivity value
         *
         * @param ms the new mouse sensitivity value
         */
        public Settings setMouseSensitivity(MouseSensitivity ms) {
            appSettings.putSerializable(AppInterfaceKeys.MOUSE_SENSITIVITY, ms);
            return this;
        }
         ...
         *
         * @return the current mouse sensitivity value
         */
        public MouseSensitivity getMouseSensitivity() {
            return (MouseSensitivity) appSettings
    ...
    /**
     * The listener interface for getting current dimensions. Clients interested
     * in this information should add a listener with
     * {@link DigitalPenManager#setOffScreenDimensionsListener(ScreenDimensionsListener)}
     * .
     */
    public interface ScreenDimensionsListener {
        /**
         * Called when screen dimensions are available. All values are the
         * distance in {x,y,z} from the origin defined for the device in units
         * of mm.
         *
         * @param origin the origin of the screen
         * @param endX the ending x position of the screen
         * @param endY the ending y position of the screen
         */
        void onScreenDimensionsUpdated(float[] origin, float[] endX, float[] endY);
     ...
     * Mouse speed setting
     *
     * @see Settings#setMouseSensitivity(DigitalPenManager.MouseSensitivity)
     */
    ...
    /**
     * How Android input events will be generated by the underlying framework.
     *
     * @see Settings#setInputType(DigitalPenManager.InputType)
     */
    public enum InputType {
        /**
         * The Android events will be generated as with a stylus.
         */
        STYLUS,

        /**
         * The Android events will appear to be generated by a mouse. This input
         * type only works when the off screen mode is
         * {@link OffScreenMode#DUPLICATE}
         */
        MOUSE
    }
    ...
    private static final WeakReference<OnSideChannelDataListener> NULL_DATA_LISTENER_REF = new WeakReference<OnSideChannelDataListener>(
            null);
    private ActivityLifecycleHandler activityLifecycleHandler;
    private static IDigitalPenService service = null;
    private WeakReference<OnSideChannelDataListener> allAreaDataListenerRef = NULL_DATA_LISTENER_REF;
    private WeakReference<OnSideChannelDataListener> onScreenAreaDataListenerRef = NULL_DATA_LISTENER_REF;
    private WeakReference<OnSideChannelDataListener> offScreenAreaDataListenerRef = NULL_DATA_LISTENER_REF;
    ...
    synchronized protected void handleDataCallback(DigitalPenData data) {
        ...
        OnSideChannelDataListener listener = null;
        switch (data.getRegion()) {
            case DigitalPenData.REGION_ALL:
                listener = allAreaDataListenerRef.get();
                break;
            case DigitalPenData.REGION_ON_SCREEN:
                listener = onScreenAreaDataListenerRef.get();
                break;
            case DigitalPenData.REGION_OFF_SCREEN:
                listener = offScreenAreaDataListenerRef.get();
                break;
            default:
    ...
    static {
        ...
        DEFAULT_APP_CONFIG.putSerializable(AppInterfaceKeys.INPUT_TYPE, InputType.STYLUS);
        DEFAULT_APP_CONFIG.putSerializable(AppInterfaceKeys.MOUSE_SENSITIVITY,
                MouseSensitivity.MEDIUM);

        PEN_CAPABLE_RES_ID = Resources.getSystem().getIdentifier("config_digitalPenCapable",
                "bool", "android");
    }
    ...
    private BroadcastReceiver penEventReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (type) {
                case BATTERY_STATE:
                {
                    BatteryStateChangedListener listener = batteryStateChangedListenerRef.get();
                    if (listener != null) {
                ...
                case MIC_BLOCKED:
                {
                    MicBlockedListener listener = micBlockedListenerRef.get();
                    if (listener != null) {
                        cachedMicsBlocked = params[0];
                        listener.onMicBlocked(params[0]);
                    }
                    break;
                }
                case POWER_STATE_CHANGED:
                {
                    PowerStateChangedListener listener = powerStateChangedListenerRef.get();
                    if (listener != null) {
                ...
                case BACKGROUND_SIDE_CHANNEL_CANCELED:
                {
                    ...
                    BackgroundSideChannelCanceledListener listener = backgroundSideChannelCanceledListenerRef
                            .get();
                    if (listener != null) {
                        listener.onBackgroundSideChannelCanceled(Area.OFF_SCREEN_BACKGROUND);
                    }
                    break;
                }
                case UNKNOWN:
    ...
    static private WeakReference<MicBlockedListener> NULL_MIC_BLOCKED_LISTENER_REF = new WeakReference<MicBlockedListener>(null);

    private WeakReference<MicBlockedListener> micBlockedListenerRef = NULL_MIC_BLOCKED_LISTENER_REF;

    static private WeakReference<PowerStateChangedListener> NULL_POWER_STATE_CHANGED_LISTENER_REF = new WeakReference<PowerStateChangedListener>(null);

    private WeakReference<PowerStateChangedListener> powerStateChangedListenerRef = NULL_POWER_STATE_CHANGED_LISTENER_REF;

    private PowerState cachedCurrentPowerState = PowerState.OFF;

    private PowerState cachedLastPowerState = PowerState.OFF;

    private static final WeakReference<BatteryStateChangedListener> NULL_BATTERY_STATE_CHANGED_LISTENER_REF = new WeakReference<BatteryStateChangedListener>(null);
    ...
    private WeakReference<BatteryStateChangedListener> batteryStateChangedListenerRef = NULL_BATTERY_STATE_CHANGED_LISTENER_REF;

    private static final WeakReference<BackgroundSideChannelCanceledListener> NULL_BACKGROUND_SIDE_CHANNEL_CANCELED_LISTENER_REF = new WeakReference<BackgroundSideChannelCanceledListener>(
            null);

    private WeakReference<BackgroundSideChannelCanceledListener> backgroundSideChannelCanceledListenerRef = NULL_BACKGROUND_SIDE_CHANNEL_CANCELED_LISTENER_REF;
    ...
    private static final WeakReference<ScreenDimensionsListener> NULL_SCREEN_DIMENSIONS_LISTENER_REF = new WeakReference<ScreenDimensionsListener>(
            null);
    private WeakReference<ScreenDimensionsListener> offScreenDimensionsListenerRef = NULL_SCREEN_DIMENSIONS_LISTENER_REF;
    ...
    private static boolean isBasicServiceSupported(String serviceName) {
        ...
        try {
            if (PEN_CAPABLE_RES_ID == 0 || !systemResources.getBoolean(PEN_CAPABLE_RES_ID)) {
    ...
    public boolean releaseApplication() {
        if (!released) {
            ...
            if (offScreenDimensionsReceiver != null) {
                registeredApplication.unregisterReceiver(offScreenDimensionsReceiver);
            }
    ...
    public void registerSideChannelEventListener(Area area, OnSideChannelDataListener listener) {
        ...
        WeakReference<OnSideChannelDataListener> listenerRef = new WeakReference<OnSideChannelDataListener>(listener);
        switch (area) {
            case ALL:
                allAreaDataListenerRef = listenerRef;
                break;
            case OFF_SCREEN_BACKGROUND:
                isBackgroundListenerRegistered = true;
                offScreenAreaDataListenerRef = listenerRef;
                break;
            case OFF_SCREEN:
                ...
                break;
            case ON_SCREEN:
                onScreenAreaDataListenerRef = listenerRef;
                break;
            default:
                break;
        }
    ...
    public void unregisterSideChannelEventListener(Area area) {
        ...
        switch (area) {
            case ALL:
                allAreaDataListenerRef = NULL_DATA_LISTENER_REF;
                break;
            case OFF_SCREEN_BACKGROUND:
                if (!isBackgroundListenerRegistered) {
                ...
                isBackgroundListenerRegistered = false;
                offScreenAreaDataListenerRef = NULL_DATA_LISTENER_REF;
                break;
            case OFF_SCREEN:
                if (isBackgroundListenerRegistered) {
                ...
                break;
            case ON_SCREEN:
                onScreenAreaDataListenerRef = NULL_DATA_LISTENER_REF;
                break;
            default:
                break;
        }
    ...
    public void setMicBlockedListener(MicBlockedListener listener) {
        if (listener == null) {
            micBlockedListenerRef = NULL_MIC_BLOCKED_LISTENER_REF;
        } else {
            micBlockedListenerRef = new WeakReference<MicBlockedListener>(listener);
    ...
    public void setPowerStateChangedListener(PowerStateChangedListener listener) {
        if (listener == null) {
            powerStateChangedListenerRef = NULL_POWER_STATE_CHANGED_LISTENER_REF;
        } else {
            powerStateChangedListenerRef = new WeakReference<PowerStateChangedListener>(listener);
    ...
    public void setBatteryStateChangedListener(BatteryStateChangedListener listener) {
        if (listener == null) {
            batteryStateChangedListenerRef = NULL_BATTERY_STATE_CHANGED_LISTENER_REF;
        } else {
            batteryStateChangedListenerRef = new WeakReference<BatteryStateChangedListener>(listener);
            listener.onBatteryStateChanged(cachedBatteryState);
        }
    }
    ...
    public void setBackgroundSideChannelCanceledListener(
        ...
        if (listener == null) {
            backgroundSideChannelCanceledListenerRef = NULL_BACKGROUND_SIDE_CHANNEL_CANCELED_LISTENER_REF;
        } else {
            backgroundSideChannelCanceledListenerRef = new WeakReference<BackgroundSideChannelCanceledListener> (listener);
        }
    }

    /**
     * Sets the listener for when the off-screen display dimensions change. It
     * will be called back immediately upon registering with current dimensions,
     * and if they change thereafter.
     *
     * @param listener the screen dimensions listener, or null to unregister.
     */
    public void setOffScreenDimensionsListener(ScreenDimensionsListener listener) {
        synchronized (this) {
            if (listener == null) {
                if (offScreenDimensionsReceiver != null) {
                    registeredApplication.unregisterReceiver(offScreenDimensionsReceiver);
                    offScreenDimensionsReceiver = null;
                }
                offScreenDimensionsListenerRef = NULL_SCREEN_DIMENSIONS_LISTENER_REF;
            } else {
                offScreenDimensionsListenerRef = new WeakReference<ScreenDimensionsListener>(listener);
                offScreenDimensionsReceiver = new OffScreenDimensionsReceiver();
                registeredApplication.registerReceiver(offScreenDimensionsReceiver,
                        new IntentFilter(
                                EventInterface.INTENT_OFF_SCREEN_DIMENSIONS));
            }
        }
    }
