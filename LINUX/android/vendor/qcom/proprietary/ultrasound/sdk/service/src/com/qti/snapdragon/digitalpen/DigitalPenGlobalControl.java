...
public class DigitalPenGlobalControl {
    ...
    public static class DigitalPenGlobalSettings {
        ...
        public enum InputType {
            STYLUS(DigitalPenConfig.DP_INPUT_TYPE_STYLUS),
            MOUSE(DigitalPenConfig.DP_INPUT_TYPE_MOUSE);

            private final byte code;

            private InputType(byte code) {
                this.code = code;
            }

            private static InputType fromCode(byte code) {
                for (InputType mode : InputType.values()) {
                    if (mode.code == code) {
                        return mode;
                    }
                }
        ...
        public enum MouseSensitivity {
            SLOW(DigitalPenConfig.DP_MOUSE_SENSITIVITY_SLOW),
            MEDIUM(DigitalPenConfig.DP_MOUSE_SENSITIVITY_MEDIUM),
            FAST(DigitalPenConfig.DP_MOUSE_SENSITIVITY_FAST);

            private final byte code;

            private MouseSensitivity(byte code) {
                this.code = code;
            }

            private static MouseSensitivity fromCode(byte code) {
                for (MouseSensitivity mode : MouseSensitivity.values()) {
                    if (mode.code == code) {
                        return mode;
                    }
                }
        ...
        public DigitalPenGlobalSettings setDefaultInputType(InputType inputType) {
            config.setInputType(inputType.code);
            return this;
        }

        public InputType getInputType() {
            return InputType.fromCode(config.getInputType());
        }
        ...
        public void setMouseSensitivity(MouseSensitivity mouseSensitivity) {
            config.setMouseSensitivity(mouseSensitivity.code);
        }

        public MouseSensitivity getMouseSensitivity() {
            return MouseSensitivity.fromCode(config.getMouseSensitivity());
        }
