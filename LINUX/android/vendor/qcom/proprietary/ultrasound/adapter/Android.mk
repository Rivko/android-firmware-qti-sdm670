...
LOCAL_C_INCLUDES        := $(LOCAL_PATH)/../ual \
                           $(LOCAL_PATH)/../ual_util \
                           $(LOCAL_PATH)/../../mm-gestures/gesture-bus \
                           $(LOCAL_PATH)/../../mm-gestures/gesture-core \
                           $(LOCAL_PATH)/../calculators/stubs
LOCAL_SRC_FILES         := gs_dynamic_lib_proxy.cpp \
                           gs_bus_adapter.cpp \
...
# ---------------------------------------------------------------------------------
#                       Make the Shared library (libproxsockadapter)
# ---------------------------------------------------------------------------------

include $(CLEAR_VARS)


LOCAL_MODULE            := libproxsockadapter
LOCAL_MODULE_TAGS       := optional
LOCAL_CFLAGS            := -DLOG_NIDEBUG=0

LOCAL_C_INCLUDES        := $(LOCAL_PATH)/../ual \
                           $(LOCAL_PATH)/../ual_util
LOCAL_SRC_FILES         := proximity_socket_adapter.cpp
LOCAL_SHARED_LIBRARIES  := liblog \
                           libual \
                           libualutil
