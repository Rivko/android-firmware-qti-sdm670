 ...
 status(DISCONNECT)
{
  ...
  // Init GS lib
  if (!gs_lib_proxy.open_lib(gesture_fw_lib_path))
  {
    LOGE("%s: Init GS dynamic lib failed.",
         __FUNCTION__);
    status = FAILURE;
    return;
  }

  client = gs_lib_proxy.createGsBusClient();
}

/*============================================================================
  DESTRUCTOR:
============================================================================*/
GsBusAdapter::~GsBusAdapter()
{
  gs_lib_proxy.deleteGsBusClient(client);
}
