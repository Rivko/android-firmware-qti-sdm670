...
public class BarChartView extends LinearLayout {
    ...
    public void updateConstantValues(BarConstants[] barsConstants) {
        if (barsConstants.length != mBars.size()) {
            Log.e(this.toString(),
                    "Submitted constant length not equal to the number of bars in the chart");
            return;
        }

        for (int i = 0; i < barsConstants.length; ++i) {
            mBars.get(i).updateConstantValues(barsConstants[i]);
        }
    }
    ...
    private class BarView extends LinearLayout {
        ...
        private BarConstants mBarConstants;
        ...
        public BarView(Context context, BarConstants barConstants) {
            ...
            setConstantText(barConstants);
        }

        private void setConstantText(BarConstants barConstants) {
        ...
        public void updateConstantValues(BarConstants barConstants) {
            mBarConstants = barConstants;
            setConstantText(mBarConstants);
        }
