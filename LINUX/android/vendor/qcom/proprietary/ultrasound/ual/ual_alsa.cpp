...
// For earpiece concurrency, allows adding a suffix to RX topology for selecting different
// runtime topologies based on opened audio device
static const char *s_rx_topology_suffix = NULL;
...
bool ual_alsa_open()
{
  const char *MIXER_XML_PATH = "/data/usf/mixer/mixer_paths.xml";
...
)
{
  ...
  if (UAL_RX_DIRECTION == direction)
  {
    ...
    // Ramp down rx volume
    int err = usnd_ramp_down_rx_vol(s_usnd_route, topology_name, ramp_down_timeout);
    if (err)
    {
      LOGE("%s: usnd_ramp_down_rx_vol [dir=%d] failed with error %d",
           __FUNCTION__,
           direction,
           err);
    }
...
==============================================================================*/
/**
   Open sound device for ultrasound, supports MUX configuration
  (different number of actual diluted channels from physical channels)
*/
int ual_alsa_open_device_with_mux
(
...
)
{
  ...
  if (UAL_RX_DIRECTION == direction)
  {
    if (s_rx_topology_suffix != NULL)
    {
      size_t copied_size = strlcat(topology_name, s_rx_topology_suffix, MAX_TOPOLOGY_NAME_SIZE);
      if (copied_size >= MAX_TOPOLOGY_NAME_SIZE) {
        LOGE("%s: not enough room to copy the RX topology suffix %s",
             __FUNCTION__,
             s_rx_topology_suffix);
        return -1;
      }
      LOGD("%s: added RX topology suffix. topology_name is now %s",
           __FUNCTION__,
           topology_name);
    }
  }
  ...
  config.channels = (num_actual_channels >= 0) ? num_actual_channels : us_xx_info.port_cnt;
...
{
    return ual_alsa_open_device_with_mux(us_xx_info, direction, -1);
} // ual_alsa_open_device

/*==============================================================================
  FUNCTION:  ual_alsa_set_rx_topology_suffix
==============================================================================*/
void ual_alsa_set_rx_topology_suffix(const char *suffix)
{
    s_rx_topology_suffix = suffix;
}
