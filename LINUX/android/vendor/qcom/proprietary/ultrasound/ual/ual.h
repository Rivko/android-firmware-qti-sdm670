...
/**
 RX arbitration configuration
 */
typedef struct
{
  // callback for RX state changes
  aud_dev_arbi_event_cb_t rx_state_evt_cb;
  // mask of audio devices to enable for RX arbitration
  uint32_t rx_dev_arbi_enabled_audio_devices;
  // audio device used by default when there are no active audio
  // devices
  uint32_t rx_dev_arbi_default_audio_device;
} ual_rx_dev_arbi_config_type;
...
/**
  Configure US TX device (& stream).
  Supports MUXed setup, in which one physical channel may contain 2 diluted TX
  channels. In a MUXed setup the number of physical TX channels may be smaller than
  the tx port_count which is used by calculators.
*/
...
==============================================================================*/
/**
  Update the mask of active RX devices for arbitration. Used later to check
  if we need to switch topology (mixer path)
*/
...
==============================================================================*/
/**
  Return true if we need to switch ultrasound RX topology (mixer path)
  After last change of active audio device
*/
...
==============================================================================*/
/**
  Return the current mask of active audio devices as maintained
  by arbitration notifications
*/
