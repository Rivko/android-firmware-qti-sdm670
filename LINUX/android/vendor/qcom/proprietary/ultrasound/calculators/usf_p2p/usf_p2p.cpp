...
/**
  P2P daemon name
*/
static const char* DAEMON_NAME =  "usf_p2p";
...
void p2p_lib_init (us_all_info *paramsStruct)
{
  ...
  p2p_config->TxTransparentDataSize = paramsStruct->usf_algo_transparent_data_size;
  memcpy(p2p_config->TxTransparentData,
         paramsStruct->usf_algo_transparent_data,
         p2p_config->TxTransparentDataSize);
  ...
  {
      p2pContext.m_last_lib_result.userPos[i].distance = QC_US_P2P_DISTANCE_NOT_PRESENT;
  }
...
/**
  ...
  @return 0 on success, other value on failure
*/
int p2p_send_rx_pattern_to_dsp()
{
  int ret = 0;
...
void get_p2p_output_decision(short *packet,
                                 ...
                                 P2POutput& output)
{
  output.posResult = QC_US_P2P_POS_IDLE;
  output.dataXferResult = QC_US_P2P_DATA_XFER_IDLE;
  ...
  for (i=0; i<paramsStruct->usf_p2p_num_users; i++)
  {
      ...
      if (i != paramsStruct->usf_p2p_user_index)
      {
          ...
          LOGI("%s: send_p2p_event userIndex %d peerId %d angle %d distance %d",
               ...
               (int)p2p_output.userPos[i].azimuth, (int)p2p_output.userPos[i].distance);
          ret = m_p2p_data_socket->send_p2p_event(paramsStruct->usf_p2p_user_index,
                                                  (int)peerId,
                                                  p2p_output.userPos[i].azimuth,
                                                  p2p_output.userPos[i].distance);
          if (0 > ret)
...
/*==============================================================================
  FUNCTION:  inject_sff_config_params_to_tx_transparent_data
==============================================================================*/
/**
...
void init(us_all_info *paramsStruct)
{
 ...
 (void)p2p_get_new_rx_pattern();
 (void)p2p_send_rx_pattern_to_dsp();
}
