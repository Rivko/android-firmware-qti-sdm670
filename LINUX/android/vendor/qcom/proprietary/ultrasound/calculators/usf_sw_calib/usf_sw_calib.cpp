...
/*----------------------------------------------------------------------------
  Defines
----------------------------------------------------------------------------*/
#define LINK_CFG_FILE_LOCATION "/data/usf/sw_calib/usf_sw_calib_proxy.cfg"
#define CLIENT_NAME "sw_calib"
#define DAEMON_NAME "usf_sw_calib"
...
/**
 * Scaling factor of the points after transformation that are
 * being sent to the application
 */
static const int CALIBRATION_POINT_SCALING_FACTOR = 1000;
...
typedef enum
{
  ...
  TESTER_MSG_TYPE_POINT_POSITION =      6,
  TESTER_MSG_TYPE_NUM_CONSTANTS =       7,
  TESTER_MSG_TYPE_MEASUREMENT_POWER =   7,
  TESTER_MSG_TYPE_MEASUREMENT_QUALITY = 8,
  TESTER_MSG_TYPE_INTERFERENCE =        9,
} tester_msg_t;
...
sw_calib_result process_frame(uint8_t *next_packet)
{
  ...
  if (!paramsStruct.usf_sw_calib_is_tester_mode) {
      ...
      {
        LOGE("%s: Sending point status: %d channel: %d point (%d,%d)->(%lf,%lf) point num:%d",
             __FUNCTION__,
             status.status,
             status.channel,
             status.x,
             status.y,
             transformed.get_element(X),
             transformed.get_element(Y),
             status.point_number);
...
void send_tester_statistics() {
    ...
    for (int mic_num = 0; mic_num < paramsStruct.usf_tx_port_count; mic_num++) {
        ...
        // Sending -1 as the point index, since it is ignored when sending statistics.
        sck->send_sw_calib_tester_event(TESTER_MSG_TYPE_MEASUREMENT_POWER, mic_num, -1, statistics_container.power_mean);
        sck->send_sw_calib_tester_event(TESTER_MSG_TYPE_MEASUREMENT_QUALITY, mic_num, -1, statistics_container.peak2rms_mean[0]);
        if (statistics_container.spur_frequency_mean != 0 &&
            statistics_container.spur_power_mean > SPUR_POWER_MEAN_INTERFERENCE_THRESHOLD) {
            sck->send_sw_calib_tester_event(TESTER_MSG_TYPE_INTERFERENCE, mic_num, -1, 0);
        }
    }
}
...
void send_constants()
{
    char (* tester_constants[])[FILE_PATH_MAX_LEN] = {
                                 ...
                                 paramsStruct.usf_tester_position,
                               };
    tester_msg_t constants_tester_msg_types[] = {
                                       ...
                                       TESTER_MSG_TYPE_POINT_POSITION,
                                     };
    for (int j = 0; j < SW_TESTER_POINT_COUNT; j++)
    {
      for (int i = 0; i < TESTER_MSG_TYPE_NUM_CONSTANTS; ++i)
      {
        int constant_dims = (TESTER_MSG_TYPE_POINT_POSITION == constants_tester_msg_types[i]) ?
                            2 : paramsStruct.usf_tx_port_count;
        double tester_constant[constant_dims];

        fill_array_from_string(tester_constant, tester_constants[i][j], constant_dims);

        if (TESTER_MSG_TYPE_POINT_POSITION == constants_tester_msg_types[i])
        {
          Vector transformed = transform_calibration_point(tester_constant[X], tester_constant[Y]);
          transformed.vector_mult_by_scalar(CALIBRATION_POINT_SCALING_FACTOR);
          LOGE("%s: Sending point  (%lf,%lf)->(%lf,%lf) ",
             __FUNCTION__,
             tester_constant[X],
             tester_constant[Y],
             transformed.get_element(X),
             transformed.get_element(Y));
          tester_constant[X] = transformed.get_element(X);
          tester_constant[Y] = transformed.get_element(Y);
        }

        for (int index = 0; index < constant_dims; ++index)
        {
          sck->send_sw_calib_tester_event(constants_tester_msg_types[i], index, j,
                                          tester_constant[index]);
        }
      }
    }
}
