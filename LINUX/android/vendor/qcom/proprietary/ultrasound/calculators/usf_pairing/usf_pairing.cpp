...
#define DAEMON_NAME "usf_pairing"
...
static const int32_t NO_SERIES_FOUND_YET = -1;

static const int32_t INVALID_SERIES_NUM = -2;
/**
...
/**
  ...
  best_match_not_validated is for the tester application,
  that needs pairing even if it didn't pass validation.
*/
static pairing_result find_series(int32_t* best_match_not_validated)
{
  int rc;
  ...
  static int32_t current_series_number = NO_SERIES_FOUND_YET;
    ...
    for (int b = 0; b < num_of_buffers; ++b)
    {
      ...
      for (int f = 0; f < num_of_frames; ++f)
      {
        ...
        for (int m = 0; m < paramsStruct.usf_tx_port_count; ++m)
        {
          ...
          // This condition's purpose is to make pairing without validation
          // possible, if there is such a need. It doesn't interrupt the flow,
          // and the result is being sent on timeout events.
          if (fb_info.SeriesFound)
          {
            // This condition means, that if different series number
            // was found twice, than we have an error.
            current_series_number =
              (current_series_number == NO_SERIES_FOUND_YET) ||
              (current_series_number == fb_info.CurrentSeriesNumber) ?
              fb_info.CurrentSeriesNumber :
              INVALID_SERIES_NUM;

            *best_match_not_validated = current_series_number;
          }
...
int main()
{
  ...
  int32_t best_match_not_validated = NO_SERIES_FOUND_YET;
  int rc;
  ...
  while (daemon_run &&
         0 < timeout_msecs)
  {
    res = find_series(&best_match_not_validated);
  ...
  if (0 >= timeout_msecs)
  {
    sck->send_pairing_event(PAIRING_TIMEOUT, best_match_not_validated);
    if (best_match_not_validated != NO_SERIES_FOUND_YET &&
        best_match_not_validated != INVALID_SERIES_NUM)
    {
      LOGW("%s: Found pen series - but did not pass validation",
           __FUNCTION__);
    }
    else
    {
