...
/**
   This is to store the previous mouse pointer locations.
 */
static int x_prev = 0;
                               ...
                               region_zone_t rz)
{
  ...
  // Sometimes, when the pen is held inside work-zone, close to the border,
  // the library returns X-Y coordinate in the touch zone border.
  // Therefore, the touch-zone border is treated the same as the work-zone.
  case ON_SCREEN_TOUCH_BORDER:
    // Disable ts only if send_to_ual is enabled
...
int epos_exit (int status)
{
  ...
  property_thread_exit((char *)DAEMON_NAME);
...
void init_transform_settings(us_all_info *paramsStruct)
{
  ...
  // Initialize touch zone with active zone size
  Vector on_touch_zone_area_border;
  on_touch_zone_area_border = on_act_zone_area_border;
  Vector touch_zone_border_diff;
  touch_zone_border_diff = paramsStruct->usf_epos_touch_zone_border_diff;
  on_touch_zone_area_border.vector_substract(touch_zone_border_diff);
...
==============================================================================*/
/**
 * Helper function to calculate mouse movement points
 *
*/
static bool calc_mouse(
    bool pen_down,
    int x,
    int y,
    int32_t type,
...
)
{
  if (pen_down)
  {
    ...
    int orientation = 0;
    float mouse_sensitivity = eposParams.m_mouse_sensitivity;
    bool rc = ual_util_get_screen_info(screen_x, screen_y, orientation);
    if(!rc)
    {
      // no screen information, discard mouse movement
      remove_last_event();
      return false;
    }
    double trans_factor_x = screen_x * mouse_sensitivity / TSC_LOGICAL_MAX_X;
    double trans_factor_y = screen_y * mouse_sensitivity / TSC_LOGICAL_MAX_Y;
    switch (orientation)
    {
      case SCREEN_ROTATION_0:
        x_rel = x - x_prev;
        y_rel = y - y_prev;
        x_rel = x_rel * trans_factor_x;
        y_rel = y_rel * trans_factor_y;
      break;
      case SCREEN_ROTATION_90:
        x_rel = y - y_prev;
        y_rel = x_prev - x;
        x_rel = x_rel * trans_factor_y;
        y_rel = y_rel * trans_factor_x;
      break;
      ...
      pEvent->event_data.mouse_event.rels[X_IND] = x_rel;
      pEvent->event_data.mouse_event.rels[Y_IND] = y_rel;
      pEvent->event_data.mouse_event.rels[Z_IND] = 0;
    }
  else
  {
    pEvent->event_data.mouse_event.rels[X_IND] = 0;
    pEvent->event_data.mouse_event.rels[Y_IND] = 0;
    pEvent->event_data.mouse_event.rels[Z_IND] = 0;
  }
  pEvent->event_data.mouse_event.buttons_states =
      (get_buttons_state_bitmap(&(s_work_params.paramsStruct), type)
          & EPOS_TYPE_VALID_BIT) ? USF_BUTTON_LEFT_MASK : 0;
...
)
{
  ...
  switch (event_type_ind)
  {
    ...
    case USF_MOUSE_EVENT_IND:
      return calc_mouse(pen_down, x, y, type, pEvent);
...
static void get_battery_state(void *InWorkspace)
{
  ...
  {
    // Translate number to Q15 format
    // We add 1 << (Q15_SHIFT - 1 ) which is 0.5 in Q15 format
    // in order to round the number right.
    battery_level = (battery_level * 100 + (1 << (Q15_SHIFT - 1 )) )
                    / (1 << Q15_SHIFT);
  }

  if (battery_level <=
      (int)s_work_params.paramsStruct.usf_epos_battery_low_level_threshold)
  {
...
{
  if (OFF_SCREEN_MODE_DUPLICATE == eposParams.m_off_screen_mode &&
      OFF_SCREEN_WORK_ZONE == rz && eposParams.m_event_type == USF_MOUSE_EVENT)
  {
    return USF_MOUSE_EVENT_IND;
  }
...
void update_config_parameters()
{
  ...
  eposParams.m_mouse_sensitivity = eposParams.m_dconfig.mouse_sensitivity;
  ...
  LOGD("Config was updated: on screen ual:%d, off screen ual:%d, on screen socket:%d, off screen socket:%d, event type:%d, off screen mode:%d, eraser button index:%d, eraser button mode:%d, on screen hover icon mode: %d, off screen hover icon mode: %d, touch range: %d, all events side channel: %d, on screen mapping: %d, off screen mapping: %d, mouse sensitivity: %d",
       ...
       eposParams.m_mouse_sensitivity);
}
...
static void usf_handle_zone(uint32_t frames_counter)
{
  ...
  { // no active zone power saving is enabled
    ...
    // Wait for timeout before entering sniffing mode
    static double s_wait_before_sniffing_mode =
      current_time_msec + s_work_params.paramsStruct.usf_epos_wait_before_sniffing_mode;

    if (s_work_params.bActZone)
    {
      // preparing for the next "no active zone" situation
      s_empty_frames_counter = 0;
      s_wait_before_sniffing_mode = current_time_msec + s_work_params.paramsStruct.usf_epos_wait_before_sniffing_mode;
    ...
    else
    {
      ...
      {
        // Wait before entering sniffing mode
        if (current_time_msec > s_wait_before_sniffing_mode)
        {
...
int main(void)
{
  ...
  if (0 != property_thread_create((char *)DAEMON_NAME))
  {
