                            ...
                            P2POutput *pOutput)
{
  ...
  counter++;
  pOutput->dataXferResult = QC_US_P2P_DATA_XFER_IDLE;
  if (counter % 10 == 0)
  {
      pOutput->posResult = QC_US_P2P_POS_UPDATED;
      for (i=0; i<QC_US_P2P_MAX_USERS; i++)
      {
          pOutput->userPos[i].azimuth = (i*90 + counter) % 360;
          pOutput->userPos[i].distance = 10;
      }
  }
  else
  {
      pOutput->posResult = QC_US_P2P_POS_IDLE;
  }
