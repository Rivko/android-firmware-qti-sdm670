...
#define QMI_WDS_GET_CAPABILITIES_MSG_ID                   0x00A9
...
#define QMI_WDS_EMBMS_LIST_IND_SAI_LIST_EX_TLV_ID       0x12
...
#define QMI_WDS_EMBMS_ACTIVATE_REQ_EARFCNLIST_EX_TLV_ID 0x14
...
#define QMI_WDS_EMBMS_ACT_DEACT_REQ_EARFCNLIST_EX_TLV_ID   0x14
...
/* Get Capabilities message TLV IDs */
#define QMI_WDS_GET_CAPABILITIES_EMBMS_EARFCN_EX_TLV_ID     0x10
...
static int embms_32_bit_freq = FALSE;
...
)
{
  ...
  while (rx_buf_len > 0)
  {
    ...
    if( embms_32_bit_freq == TRUE )
    {
      /* 32- bit frequencies - process only extended TLV */
      switch(type)
      {
        ...
        break;
        case QMI_WDS_EMBMS_LIST_IND_SAI_LIST_EX_TLV_ID:
        {
          ind_data->sai_list.param_mask |=
          ...
          {
            READ_32_BIT_VAL(value_ptr, freq);
          ...
          {
             QMI_DEBUG_MSG_1("available_sai_list:%d", ind_data->sai_list.available_sai_list[temp_index] );
          }
          for (temp_index = 0; temp_index < (ind_data->sai_list.camped_sai_list_len); temp_index ++)
          {
             QMI_DEBUG_MSG_1("camped_sai_list:%d", ind_data->sai_list.camped_sai_list[temp_index]);
          }
          for (temp_index = 0; temp_index < (ind_data->sai_list.num_sai_per_group_len); temp_index ++)
          {
             QMI_DEBUG_MSG_1("num_sai_per_group:%d", ind_data->sai_list.num_sai_per_group[temp_index]);
          }
    ...
    else /* 16 bit frequency values */
    {
      /* 16 - bit frequencies - process only standard TLV */
      switch(type)
      {
        case QMI_WDS_EMBMS_LIST_IND_SAI_LIST_TRANX_ID_TLV_ID:
        {
...
/*===========================================================================
  FUNCTION  qmi_wds_get_capabilities
===========================================================================*/
/*!
@brief
  Query the modem to get the EARFCN integer bit length and use the response
  to configure the global embms_earfcn_bit_mask value

@return
  0 if query was sucessful, < 0 if not.
...
)
{
  ...
  */
  ...
  tmp_value = 1;
  /* Write extended_embms_frequency tlv value */
  if (qmi_util_write_std_tlv (&tmp_msg_ptr,
                              &msg_size,
                              QMI_WDS_GET_CAPABILITIES_EMBMS_EARFCN_EX_TLV_ID,
                              1,
                              (void *)&tmp_value) < 0)
  {
    return QMI_INTERNAL_ERR;
  ...
  rc = qmi_service_send_msg_sync (user_handle,
                                  QMI_WDS_SERVICE,
                                  QMI_WDS_GET_CAPABILITIES_MSG_ID,
                                  QMI_SRVC_PDU_PTR(msg),
  ...
  while (msg_size > 0)
  {
    ...
    switch (type)
    {
      case QMI_WDS_GET_CAPABILITIES_EMBMS_EARFCN_EX_TLV_ID:
      {
        unsigned char temp;
        READ_8_BIT_VAL (value_ptr,temp);
        if(temp == 1)
        {
          embms_32_bit_freq = TRUE;
        }
        break;
      }
      default:
...
)
{
  ...
  unsigned char *tmp_msg_ptr;
  int rc;
  static int embms_earfcn_caps_queried = FALSE;

  /* Get the modem configuration by querying modem */
  if( embms_earfcn_caps_queried )
  {
    embms_earfcn_caps_queried = TRUE;
    rc = qmi_wds_get_capabilities(user_handle, qmi_err_code);
  }
...
)
{
  ...
  */
  ...
  if(params->param_mask & QMI_WDS_EMBMS_TMGI_ACTIVATE_EARFCNLIST_PARAM)
  {
    ...
    fcnlist_size = embms_32_bit_freq ?
                   fcnlist_len * sizeof (unsigned int) + 1 :
                   fcnlist_len * sizeof (unsigned short) + 1;
    if (NULL == (tmp_buf = malloc(fcnlist_size)))
    {
      return QMI_INTERNAL_ERR;
    }
    ...
    if( embms_32_bit_freq == TRUE )
    {
      /* 32-bit earfcn list processing */
      /* copy fcnlist */
      memcpy(tmp_buf, params->earfcn_list, fcnlist_len * sizeof(unsigned int));
      ...
      rc =qmi_util_write_std_tlv (&tmp_msg_ptr,
                                  &msg_size,
                                  QMI_WDS_EMBMS_ACTIVATE_REQ_EARFCNLIST_EX_TLV_ID,
                                  fcnlist_size,/* earfcnlist TLV size earfcnlist_len + earfcnlist */
                                  (void *)tmp_buf);
    }
    else
    {
      /* Legacy 16 bit frequency value processing */
      unsigned short earfcn_short_list[QMI_WDS_EMBMS_TMGI_EARFCNLIST_SIZE];
      int i=0;
      memset(earfcn_short_list, 0, sizeof(short)*QMI_WDS_EMBMS_TMGI_EARFCNLIST_SIZE);

      /* iterate and copy to a 16 bit integer array from a 32 bit integer array */
      for(i=0; i<fcnlist_len; ++i)
      {
        earfcn_short_list[i] = params->earfcn_list[i];
      }
      /* copy fcnlist */
      memcpy(tmp_buf, earfcn_short_list, fcnlist_len * sizeof(unsigned short));
      /* get back to location when memory is allocated */
      tmp_buf--;
  ...
  */
  ...
  if(params->param_mask & QMI_WDS_EMBMS_TMGI_ACT_DEACT_EARFCNLIST_PARAM)
  {
    ...
    if( embms_32_bit_freq == TRUE )
    {
      ...
      rc =qmi_util_write_std_tlv (&tmp_msg_ptr,
                                  &msg_size,
                                  QMI_WDS_EMBMS_ACT_DEACT_REQ_EARFCNLIST_EX_TLV_ID,
                                  fcnlist_size,/* earfcnlist TLV size earfcnlist_len + earfcnlist */
                                  (void *)tmp_buf);
    }
    else
...
)
{
  ...
  /* mnc_includes_pcs_digit */
   READ_8_BIT_VAL (rx_buf, tmp_char);
   params->mnc_includes_pcs_digit = (qmi_wds_bool_type) tmp_char;
