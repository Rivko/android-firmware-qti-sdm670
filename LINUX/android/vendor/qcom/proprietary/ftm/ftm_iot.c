...
void *iot_thr_func(void *hdl)
{
    ...
    while(1) {
        ...
        memset(new_iot_ftm_rsp2_pkt, 0, MAX_BUF_SIZE);
        sem_wait(&iot_sem);
...
void *ftm_iot_dispatch(void *iot_ftm_pkt, int pkt_len, void *hdl)
{
    ...
    memset(new_iot_ftm_pkt, 0, (sizeof(ftm_iot_req_pkt_type) + pkt_len + NHDLC_TERM_SIZE));
    ...
    memset(new_iot_ftm_rsp_pkt, 0, MAX_BUF_SIZE);
    ...
    payload_ptr = &(new_iot_ftm_pkt->payload);
    *( payload_ptr + pkt_len) = NHDLC_TERM;
    ...
    ftm_iot_flash_ptr = &(new_iot_ftm_pkt->payload);
