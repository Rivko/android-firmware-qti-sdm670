...
def encrypt_with_private_key(message, private_key, use_pss=False, pss_digest_algorithm=None):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.encrypt_with_private_key(message, private_key, use_pss, pss_digest_algorithm)

def decrypt_with_public_key(encypted_message, public_key, image_hash, use_pss=False, use_dsa=False, pss_digest_algorithm=None):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.decrypt_with_public_key(encypted_message, public_key, image_hash, use_pss, use_dsa, pss_digest_algorithm)
...
def create_root_certficate(root_cert_params, root_key_pair, days, configfile, serial_num, use_pss, hash_algorithm):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.create_root_certficate(root_cert_params, root_key_pair, days, configfile, serial_num, use_pss, hash_algorithm)

def create_certificate(certificate_params, certificate_key_pair, CACertificate, CA_key_pair, days=7300, configfile="opensslroot.cfg", serial_num=1, extfile_name="v3.ext", use_pss=False, hash_algorithm='sha256'):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.create_certificate(certificate_params, certificate_key_pair, CACertificate, CA_key_pair, days, configfile, serial_num, extfile_name, use_pss, hash_algorithm)
...
def get_hash_algorithm_from_certicate_chain(certificate_chain_blob):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.get_hash_algorithm_from_certicate_chain(certificate_chain_blob)

def get_public_key_from_private_key(private_key, use_dsa=False):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.get_public_key_from_private_key(private_key, use_dsa)
...
def cert_uses_pss(certificate, form):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.cert_uses_pss(certificate, form)

def cert_uses_dsa(certificate, form):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.cert_uses_dsa(certificate, form)

def cert_version_check(certificate, version):
    openssl_service = openssl_implementation_discovery.OpenSSL_Service()
    return openssl_service.openssl_implementation.cert_version_check(certificate, version)

# Module enums
from discovery.defines import MOD_OPENSSL, MOD_CRYPTO_CBC, MOD_CRYPTO_CCM

def are_available(modules):
    openssl_implementation_discovery.are_available(modules)
