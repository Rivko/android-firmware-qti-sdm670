...
from sectools.features.isc.builder import msmjtagid_parse as msm_jtag_parser
...
DEFAULT_MSM_JTAG_MAPPING_FILE = c_path.join('${BUILD_ROOT}', 'build', 'msm_jtag_mapping.xml')
...
def build(env,
    ...
    if _isPolicyRun(env, disable_featureflags, build_policy, source_path) is True:
...
def _execute_sec_image_policy(env, target_base_dir,
    ...
    for i in range(0, num_targets):
        ...
        target_str = os.path.join(policy_target_base_dir, msmid_list[i], sign_id, source_name)
...
def get_msm_jtag_dict_from_file(env, msmid,
    msm_jtag_mapping_file=DEFAULT_MSM_JTAG_MAPPING_FILE, file_type="xml"):
...
def _execute_postprocess_policy(env,
    ...
    for install_policy in install_policies:
        ...
        if install_file_name is not None:
            target_file_path = c_path.join(install_directory_processed,
                                        install_file_name)
        else:
            filename = os.path.basename(sectools_builder_output[0])
            target_file_path = c_path.join(install_directory_processed, filename)
