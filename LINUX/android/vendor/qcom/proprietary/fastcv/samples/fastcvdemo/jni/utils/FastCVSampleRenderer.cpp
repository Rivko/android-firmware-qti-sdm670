...
void drawColorCorners( uint32_t* corners, uint32_t numCorners, uint32_t colorByte1, uint32_t colorByte2 )
{
   ...
   for( uint32_t  k=0; k<numCorners; k++ )
   {
           ...
           i< GREEN_PIXEL_HEIGHT_WIDTH + curcornerx; i++)
      {
         ...
         {
            //processing on RGB data
            state.renderBufRGB565[ 2* (j*state.renderBufWidth +i)] = colorByte1;
            state.renderBufRGB565[ 2* (j*state.renderBufWidth +i) + 1] = colorByte2;
         }
      }
   }
}
...
void drawScoreCorners( uint32_t* corners, uint32_t numCorners, uint32_t* scores )
{
   ...
   for( uint32_t  k=0; k<numCorners; k++ )
   {
      ...
      if(scores[k]<=40)
          scores[k] = 200;
      else if(scores[k] > 40 && scores[k] <= 50 )
          scores[k] = 230;
      else if(scores[k] > 50)
          scores[k] = 31;
           ...
           i< GREEN_PIXEL_HEIGHT_WIDTH + curcornerx; i++)
      {
              ...
              j< GREEN_PIXEL_HEIGHT_WIDTH +curcornery; j++)
         {
            //processing on RGB data
            state.renderBufRGB565[ 2* (j*state.renderBufWidth +i)] = 0;
            state.renderBufRGB565[ 2* (j*state.renderBufWidth +i) + 1] = scores[k];
...
{
    for(uint32_t i = 0; i < edgeWidth; i++){
        for(uint32_t j = 0; j < edgeHeight; j++){

            if(edgeBuffer[(j*state.renderBufWidth +i)] == 255){
                state.renderBufRGB565[ 2 * (j*state.renderBufWidth +i)] = 0;
                state.renderBufRGB565[ 2 * (j*state.renderBufWidth +i) + 1] = 31;

                //make lines more pronounced
                j++;
                state.renderBufRGB565[ 2 * (j*state.renderBufWidth +i)] = 0;
                state.renderBufRGB565[ 2 * (j*state.renderBufWidth +i) + 1] = 31;
            }
...
void colorConvertYUV420ToRGB565Renderer( const uint8_t* srcYUV420,
                                          ...
                                          uint32_t*      dstRGB565 )
{
   fcvColorYUV420toRGB565u8(srcYUV420, srcWidth, srcHeight, dstRGB565);
}
