...
int main()
{
	...
	if (!bInit_Success) {
		ALOGE("SSR: Diag_LSM_Init() failed\n");
		goto err_exit1;
	}
	...
	if (!ssr_bus) {
		ALOGE("SSR: Failed to open subsys_prefix dir");
		goto err_exit2;
	}
	...
	if (pfd_num == 0) {
		...
		if (!pil_bus) {
			ALOGE("SSR: Failed to open pil_prefix dir");
			goto err_exit2;
		}
	...
	if (pfd_num == 0) {
		ALOGE("SSR: No match subsystem found\n");
		goto err_exit2;
	}
