...
#include <sys/uio.h>
...
#if !defined(RMTS_LE_BUILD) && !defined(RMTS_FSM_BUILD)
	#include "cutils/log.h"
	#include "common_log.h"
	#include "mdm_detect.h"

#elif defined(RMTS_FSM_BUILD) /* Is an FSM build */
	...
	#include "fsminitdWd.h"
	#include "assertApi.h"
	#include "logOsal.h"
	#include "binlogSocket.h"
	#include "qmiServApi.h"
	#include "fdWatchApi.h"

	static unsigned int remotefs_debug_level = QCT_LOG_LEVEL_DEBUG;

	#define remotefs_fmt(fmt) "Remotefs Server: " fmt

	/* Use binary log */
	/* Binary log module ID: 24 */
	#define REMOTEFS_BINLOG_MODULE_ID 0x1800

	#define FSM_LOG_MESSAGE(level, fmt, args...)                       \
		if (level <= remotefs_debug_level)                             \
		{                                                              \
			do {                                                       \
				char buf[200];                                         \
				snprintf(buf, sizeof(buf), remotefs_fmt(fmt), ##args); \
				qctLogOsalOutput(level, buf);                          \
			} while( 0 );                                              \
		}

	#define LOGI(...) FSM_LOG_MESSAGE(QCT_LOG_LEVEL_INFO, __VA_ARGS__)
	#define LOGV(...) FSM_LOG_MESSAGE(QCT_LOG_LEVEL_NOTICE, __VA_ARGS__)
	#define LOGE(...) FSM_LOG_MESSAGE(QCT_LOG_LEVEL_ERROR, __VA_ARGS__)
	#define LOGW(...) FSM_LOG_MESSAGE(QCT_LOG_LEVEL_WARNING, __VA_ARGS__)
...
#else
#define KMSGI(format, args...)
#define KMSGE(format, args...)
#endif
...
#define RMTS_LOG_CLNT(format, args...) \
	do { \
		if (client) { \
			LOGI("%s: %s: " format "\n", \
...
/* Track QCSI requests */
#define RMTS_LOG_REQ(format, args...) \
	do { \
		if (client) { \
...
#elif defined(RMTS_FSM_BUILD)
static int dropPrivs(void)
{
...
#ifdef RMTS_FSM_BUILD
static void *remotefs_thread(void *unused);
...
#endif
{
	...
	if(ret)
	{
		LOGE("Error dropping privileges\n");
		exit(1);
	}
...
int main(int argc, char **argv)
{
	pthread_attr_t attr;
	pthread_t tid = 0;
	QmiServHdl_t service_handle = 0;

	/* No junk */
	memset(&attr, 0, sizeof(attr));

	/* qctClientBinLogInit will exit automatically if binary log unix domain
	 * socket coud not be obtained.
	 */
	qctClientBinLogInit(REMOTEFS_BINLOG_MODULE_ID);
	...
	{
		LOGE("Unable to pthread_attr_setdetachstate!\n");
		exit(1);
	}

	/* Start the remotefs thread */
	if (pthread_create(&tid, &attr, remotefs_thread, NULL) != 0)
	{
		LOGE("Unable to pthread_create!\n");
		exit(1);
	}

	if (pthread_attr_destroy(&attr) != 0)
	{
		LOGE("Unable to pthread_attr_destroy!\n");
		exit(1);
	}

	usleep(200000);

	/* Start ourselves as a QMI service that the init mgr can communicate with
	 * for watchdog events
	 */
	ASSERT_DIE(
		qmiServiceStart(
			fidwd_get_service_object_v01(),
			...
			&service_handle) == 0,
		-1,
		"qmiServiceStart() failed\n" );

	/* Should run forever */
	watchLoop();

	qmiServiceStop(service_handle);
