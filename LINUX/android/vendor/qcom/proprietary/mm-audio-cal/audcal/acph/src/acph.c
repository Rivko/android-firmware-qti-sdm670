 ...
 * None.
 *
 * Copyright (c) 2010-2017 by Qualcomm Technologies, Inc.
 * All Rights Reserved.
 ...
 *--------     ---     ----------------------------------------------------------
 *07/12/16  mahadevk   Updated ACPH_BUFFER_LENGTH to 0x9C40
 *08/05/15  mahadevk   Added acph_create_partial_suc_resp API for handling partial success case
 *05/28/15     San     Update ACPH buffer length to 20000 bytes as part of AudioSphere Interfaces.
 *05/28/14     mh      SW migration from 32-bit to 64-bit architecture
 *05/28/10     ayin    initial draft
 ********************************************************************************
 */
/* $Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acph/src/acph.c#12 $ */
/*
   -------------------------------
   |Include Files                |
   -------------------------------
...
#define ACPH_BUFFER_LENGTH          0x9C40
...
/**
 * FUNCTION : acph_create_partial_suc_resp
 *
 * DESCRIPTION : Create a response buffer with the specified response data
 * and request command id.
 *
 ...
 * SIDE EFFECTS : None
 */
static void acph_create_partial_suc_resp(uint32_t error_code, uint32_t data_length,
                          uint8_t *req_buf_ptr,
                          uint8_t *resp_buf_ptr,
                          uint32_t *resp_buf_length
                          )
{
   ...
   if(error_code != ACPH_SUCCESS)
   {
	   suc_flag = ACPH_SUC_FLAG_FALSE;
	   result_len = ACPH_ERROR_CODE_LENGTH;
   }
   *resp_buf_length = ACPH_HEADER_LENGTH + ACPH_SUC_FLAG_LENGTH + data_length + result_len;
   if (NULL == resp_buf_ptr)
   {
      /**not initialized*/
      ACDB_DEBUG_LOG("resp buffer ptr address[%p]\n", (void *)resp_buf_ptr);
   ...
   resp_data_length = ACPH_SUC_FLAG_LENGTH + data_length + result_len;
   /**copy command id */
   ACDB_MEM_CPY(resp_buf_ptr,ACPH_COMMAND_ID_LENGTH, req_buf_ptr, ACPH_COMMAND_ID_LENGTH);
   /**copy response data length */
   ACDB_MEM_CPY(resp_buf_ptr + ACPH_COMMAND_ID_LENGTH,ACPH_DATA_LENGTH_LENGTH,
   ...
   // write the error code if its an error case
   if(error_code != ACPH_SUCCESS)
   {
     memmove(resp_buf_ptr + ACPH_HEADER_LENGTH + ACPH_SUC_FLAG_LENGTH + ACPH_ERROR_CODE_LENGTH,resp_buf_ptr + ACPH_HEADER_LENGTH + ACPH_SUC_FLAG_LENGTH,data_length);
	   //In case of error move the current contents of respbuf by 4 bytes
	   // and then do the copy of error code.

	   // check for the resp buffer size
     ACDB_MEM_CPY(resp_buf_ptr + ACPH_HEADER_LENGTH + ACPH_SUC_FLAG_LENGTH,ACPH_ERROR_CODE_LENGTH,
          &error_code,
          ACPH_ERROR_CODE_LENGTH);
	   // Insert the result value in the location (resp_buf_ptr + seven bytes)
   }
}
                          ...
                          )
{
   ...
   *resp_buf_ptr = acph_main_buffer;
   ...
   if (g_pCmdTbl != NULL)
   {
      ...
      else if(result == ACPH_PARTIAL_SUCCESS)
      {
         acph_create_partial_suc_resp(result, *resp_buf_length,req_buf_ptr,*resp_buf_ptr,resp_buf_length);
      }
