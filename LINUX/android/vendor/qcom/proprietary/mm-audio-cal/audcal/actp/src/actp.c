 ...
 */
void actp_diag_init(
        void (*pfnCallback_Function)(uint8_t*, uint32_t, uint8_t**, uint32_t*)
        )
{
    (void)Diag_LSM_Init(NULL);
#ifndef ACDB_WIN32_SIM
        ...
        )
{
    ...
    if (NULL != req_ptr)
    {
        ...
        ACDB_MEM_CPY((void *)&request,sizeof(diag_pkt_req_struct), (void *)req_ptr,pkt_len);
        /**Get command code*/
        ...
        else
        {
            ...
            else
            {
                ...
                ACDB_MEM_CPY(rsp_diag_pkt_ptr,actp_rsp_pkt_len,actp_rsp_ptr,actp_rsp_pkt_len);
                        ...
                        )
{
    ...
    atp_receive_diag_pkt(atp_diag_req_pkt_ptr,&actp_rsp_ptr,&actp_phone_context);

     if(NULL == actp_rsp_ptr)
    {
        printf("rsp_ptr is NULL :( ");
    }
    else
    {
        actp_rsp_pkt_len = DIAG_REQ_PKT_SIZE + sizeof(diag_pkt_header_struct);
        ACDB_MEM_CPY(atp_diag_rsp_pkt_ptr,actp_rsp_pkt_len,actp_rsp_ptr,actp_rsp_pkt_len);
