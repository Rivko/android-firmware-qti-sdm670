 ...
 *
 *  $Header: //source/qcom/qct/multimedia2/Audio/audcal4/ACDBSimulator/main/latest/audtp/src/audtp.c#7 $
 *  $DateTime: 2017/01/24 14:40:42 $ $Author: mahadevk $
 *
        ...
        )
{
    ...
    while(remaining_buffer_length > 0 )
    {
        ...
        ACDB_MEM_CPY(atp_frame_ptr->data_ptr,atp_data_size,buffer_ptr,frame_length);
        ...
        )
{
    ...
    ACDB_MEM_CPY( dest_loc_ptr,(buf_cntxt_ptr->buffer_length-frame_ptr->header.frame_offset), frame_ptr->data_ptr, frame_ptr->header.frame_length );
        ...
        )
{
    ...
    *atp_diag_pkt = (diag_pkt_req_struct* )malloc(sizeof(diag_pkt_req_struct));
    ...
    ACDB_MEM_CPY((*atp_diag_pkt)->payload_req,sizeof((*atp_diag_pkt)->payload_req),&atp_frame_ptr->header,headersize);

    //copy frame data to payload
    ACDB_MEM_CPY((*atp_diag_pkt)->payload_req+headersize,(sizeof((*atp_diag_pkt)->payload_req)-headersize),atp_frame_ptr->data_ptr,
        ...
        )
{
    ...
    *atp_frame_ptr = (atp_frame_struct* )malloc(sizeof(atp_frame_struct));
    ...
    ACDB_MEM_CPY(&(*atp_frame_ptr)->header,headersize,atp_diag_pkt->payload_resp,headersize);
    ...
    /**copy frame data*/
    ACDB_MEM_CPY((*atp_frame_ptr)->data_ptr,(*atp_frame_ptr)->header.frame_length, atp_diag_pkt->payload_resp+headersize,
            (*atp_frame_ptr)->header.frame_length);
    return;
}
#endif
        ...
        )
{
    ...
    *atp_frame_ptr = (atp_frame_struct* )malloc(sizeof(atp_frame_struct));
    ...
    ACDB_MEM_CPY(&(*atp_frame_ptr)->header,headersize,atp_diag_pkt->payload_req,headersize);
    (*atp_frame_ptr)->data_ptr = NULL ;
    if ( (*atp_frame_ptr)->header.frame_length > 0 )
    {
        ...
        /**copy frame data*/
        ACDB_MEM_CPY((*atp_frame_ptr)->data_ptr,(*atp_frame_ptr)->header.frame_length, atp_diag_pkt->payload_req+headersize,
                (*atp_frame_ptr)->header.frame_length);
    }
    return;
}
        ...
        )
{
    ...
    if ( context_ptr->is_next_frame_gnf == TRUE )
    {
        ...
        ACDB_MEM_CPY(atp_resp_frame_ptr,atp_frame_header_length,
        ...
        ACDB_MEM_CPY(atp_resp_frame_ptr->data_ptr,atp_resp_frame_ptr->header.frame_length,
        ...
        )
{
    ...
    ACDB_MEM_CPY(&(*atp_diag_pkt)->payload_resp[0],headersize,&atp_frame_ptr->header,headersize);
    /**copy frame data*/
    ACDB_MEM_CPY(&(*atp_diag_pkt)->payload_resp[headersize],atp_frame_ptr->header.frame_length,atp_frame_ptr->data_ptr, atp_frame_ptr->header.frame_length);
    return;
}
