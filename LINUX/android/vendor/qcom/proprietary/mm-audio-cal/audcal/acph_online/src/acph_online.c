 ...
 ********************************************************************************
 */
/* $Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acph_online/src/acph_online.c#18 $ */
/*
   -------------------------------
   |Include Files                |
   -------------------------------
...
static int32_t get_table_indices_count(uint32_t tblId, uint32_t *noOfTableIndices, uint32_t *nonModuleTblFound)
{
	switch(tblId)
	{
	   case AUDPROC_GAIN_INDP_TBL:
						  ...
						  )
{
	...
	ACDB_MEM_CPY((void *)&tblId,sizeof(tblId),req_buf_ptr + offset,sizeof(tblId));
	offset += sizeof(uint32_t);
	result = get_table_indices_count(tblId,&noOfTableIndices,&nonModuleTblFound);
	if(result != ACPH_SUCCESS)
	{
