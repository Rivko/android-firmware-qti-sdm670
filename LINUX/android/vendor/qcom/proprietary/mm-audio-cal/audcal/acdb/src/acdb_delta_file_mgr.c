...
int32_t AcdbDeltaDataCmdSaveData()
{
   ...
   AcdbDeltaFileHeaderInfoV1 deltaFileHeaderInfoV1;
   AcdbDeltaFileHeaderInfoV2 deltaFileHeaderInfoV2;
	...
	for(idx=0;idx<gDeltaDataFileInfo.noOfFiles;idx++)
	{
      ...
      {
		   header_offset = sizeof(AcdbDeltaFileHeader) + sizeof(AcdbDeltaFileHeaderInfoV1) + sizeof(uint32_t);
      }
      else
      {
		...
		if(newFileBuf != NULL)
		{
         ...
         if(gDeltaDataFileInfo.fInfo[idx].nFileInfo.cplInfo == INF)
         {
            ...
            offset += sizeof(deltaHeaderInfo);
			   deltaFileHeaderInfoV1.versMajor = gDeltaDataFileInfo.fInfo[idx].nFileInfo.maj;
			   deltaFileHeaderInfoV1.versMinor = gDeltaDataFileInfo.fInfo[idx].nFileInfo.min;
			   deltaFileHeaderInfoV1.versRevision = gDeltaDataFileInfo.fInfo[idx].nFileInfo.rev;
            ACDB_MEM_CPY(newFileBuf + offset, sizeof(deltaFileHeaderInfoV1), &deltaFileHeaderInfoV1, sizeof(deltaFileHeaderInfoV1));
            offset += sizeof(deltaFileHeaderInfoV1);
         }
         else
         {
            deltaHeaderInfo.fileMajor = ACDB_DELTA_FILE_VERSION_MAJOR;
			   deltaHeaderInfo.fileMinor = ACDB_DELTA_FILE_VERSION_MINOR_V2;
            ACDB_MEM_CPY(newFileBuf + offset, sizeof(deltaHeaderInfo), &deltaHeaderInfo, sizeof(deltaHeaderInfo));
            offset += sizeof(deltaHeaderInfo);
