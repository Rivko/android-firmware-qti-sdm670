...
$Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_data_mgr.c#14 $
...
int32_t GetMidPidCalibTableSizeWithIid(uint32_t tblId,uintptr_t nLookupKey,ContentDefWithInstanceTblType cdefTbl,ContentDataOffsetsTblType cdotTbl,AcdbDataInfo dataPoolChnk,
	uint32_t *pSize)
{
	AcdbDataType cData;
	uint32_t i=0;
...
int32_t GetMidPidCalibTableWithIid(uint32_t tblId,uintptr_t nLookupKey,ContentDefWithInstanceTblType cdefTbl,ContentDataOffsetsTblType cdotTbl,AcdbDataInfo dataPoolChnk,
	uint8_t *pDstBuff, uint32_t nDstBuffLen,uint32_t *pDstBytesUsed)
{
	uint32_t nMemBytesLeft=0;
	AcdbDataType cData;
...
int32_t GetModuleMetaInfoWithoutPadding(uint32_t tblId,uintptr_t nLookupKey,ContentDefWithOnlyParamTblType cdefTbl,ContentDataOffsetsTblType cdotTbl,AcdbDataInfo dataPoolChnk,
	uint8_t *pDstBuff, uint32_t nDstBuffLen,uint32_t *pDstBytesUsed)
{
	uint32_t nMemBytesLeft=0;
	AcdbDataType cData;
...
int32_t GetModuleMetaInfoSizeWithoutPadding(uint32_t tblId,uintptr_t nLookupKey,ContentDefWithOnlyParamTblType cdefTbl,ContentDataOffsetsTblType cdotTbl,AcdbDataInfo dataPoolChnk,
	uint32_t *pSize)
{
	AcdbDataType cData;
	uint32_t i=0;
	...
	uint8_t *pDstBuff, uint32_t nDstBuffLen,uint32_t *pDstBytesUsed)
{
	...
	for(i=0 ;i< cdefTbl.nLen;i++)
	{
		uint32_t nMemBytesRequired = 0;
		uint32_t nPaddedBytes = 0;
		WdspCalHdrWithInstanceIDFormatType hdr;
		...
		nMemBytesRequired = sizeof(WdspCalHdrWithInstanceIDFormatType) + cData.nLen + nPaddedBytes;
...
int32_t GetIdcalibTableSize(uint32_t tblId,uintptr_t nLookupKey,ContentDefWithOnlyParamTblType cdefTbl,ContentDataOffsetsTblType cdotTbl,AcdbDataInfo dataPoolChnk,
	uint32_t *pSize)
{
	AcdbDataType cData;
	uint32_t i=0;
	...
	for(i=0 ;i< cdefTbl.nLen;i++)
	{
		...
		nSizeRequired += (sizeof(WdspCalHdrWithInstanceIDFormatType) + cData.nLen + nPaddedBytes);
	}
	*pSize = nSizeRequired;
	return ACDB_SUCCESS;
}
