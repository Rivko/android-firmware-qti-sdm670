...
$Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_init.c#11 $

when who what, where, why
---------- --- -----------------------------------------------------
2016-06-09 Updated acdb_init() to ignore validation for .qwsp file, if present in the build.
...
const char* ACDB_AMDB_AVS_FILE_NAME ="adsp_avs_config.acdb";
const char* WKSP_FILE_NAME_EXT = ".qwsp";

int32_t acdb_init (const char* pFilename,uint8_t **pBfr,uint32_t *pFileSize,uint32_t *pVerMaj,uint32_t *pVerMin, uint32_t *pRevVer, uint32_t *pVerCplInfo)
{
	...
	if(strstr(pFilename, ACDB_AMDB_AVS_FILE_NAME) != NULL)// Verify if AMDB file loaded, if so validate the AMDB format
	{
		if(ACDB_PARSE_SUCCESS != IsAcdbAMDBFileValid(*pBfr,*pFileSize))
			result = ACDB_INIT_FAILURE;
	}
	else if(strstr(pFilename, WKSP_FILE_NAME_EXT) != NULL)  // Verify if workspace file loaded
    {
	...
	else
	{
		...
		if(AcdbFileGetSWVersion(*pBfr,*pFileSize,pVerMaj,pVerMin,pRevVer,pVerCplInfo)!=ACDB_PARSE_SUCCESS)
			return ACDB_INIT_FAILURE;
	}
...
int32_t acdb_delta_init (const char* pFilename,uint32_t fileNameLen, uint32_t *pDeltaFileExists, uint8_t **pBfr,uint32_t *pFileSize,uint32_t *pVerMaj,uint32_t *pVerMin, uint32_t *pRevVer, uint32_t *pVerCplInfo)
{
	// acdb_init module should be able to handle both the zipped and unzipped format of acdb file
	int32_t result = ACDB_INIT_SUCCESS;
	...
	*pFileSize = deltaFileSize;
	...
	if(ACDB_UTILITY_INIT_SUCCESS == deltaFileExists)
	{
     ...
     if(AcdbDeltaFileGetSWVersion(*pBfr,*pFileSize,pVerMaj,pVerMin,pRevVer, pVerCplInfo)!=ACDB_PARSE_SUCCESS)
			return ACDB_INIT_FAILURE;
