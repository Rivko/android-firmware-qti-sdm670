...
$Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_parser.c#9 $
...
int32_t IsAcdbAMDBFileValid(const uint8_t *pFileBuf,const uint32_t nFileSize)
{
	...
	if(pFileProp->acdb_type != ACDB_AMDB_FILE_TYPE)
	{
		return ACDB_PARSE_INVALID_FILE;
	}
	if(nFileSize != (pFileProp->acdb_data_len + sizeof(AcdbFileProperties)))
...
int32_t AcdbFileGetSWVersion(const uint8_t *pFileBuf,const uint32_t nFileSize,uint32_t *pMajVer,uint32_t *pMinVer,uint32_t *pRevVer, uint32_t *pCplVer)
{
	uint8_t *pChkBuf = NULL;
	uint32_t nChkBufSize=0;
	AcdbSwVersion *swver = NULL;
	AcdbSwVersionV2 *swverV2 = NULL;

	int32_t result = AcdbFileGetChunkData(pFileBuf,nFileSize,ACDB_CHUNKID_ACSWVER2,&pChkBuf,&nChkBufSize);
	if(result != ACDB_PARSE_SUCCESS)
	{
		if(result==ACDB_PARSE_CHUNK_NOT_FOUND)//old version of file there is no cplInfo in it.
		{
	...
	else//latest version, verify minimum compatible version
	{
		if(nChkBufSize != sizeof(AcdbSwVersionV2))
			return ACDB_PARSE_FAILURE;
		swverV2  = (AcdbSwVersionV2 *)pChkBuf;
		*pMajVer = swverV2->major;
		*pMinVer = swverV2->minor;
		*pRevVer = swverV2->rev;
      *pCplVer = swverV2->cplInfo;
	}
	return ACDB_PARSE_SUCCESS;
}
