...
$Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_file_mgr.c#30 $
...
* Type Declarations
*--------------------------------------------------------------------------- */
typedef struct _AcdbDataFileInfo AcdbDataFileInfo;
#include "acdb_begin_pack.h"
struct _AcdbDataFileInfo
{
	uint32_t noOfFiles;
...
;
typedef struct _AcdbDevProp AcdbDevProp;
#include "acdb_begin_pack.h"
struct _AcdbDevProp
{
	uint32_t devid;
...
#include "acdb_end_pack.h"
;
typedef struct _AcdbDevPropLut AcdbDevPropLut;
#include "acdb_begin_pack.h"
struct _AcdbDevPropLut
{
	uint32_t devPropLen;
	AcdbDevProp *devProp;
}
#include "acdb_end_pack.h"
;
typedef struct _AcdbGlbProp AcdbGlbProp;
#include "acdb_begin_pack.h"
struct _AcdbGlbProp
{
	uint32_t pid;
...
#include "acdb_end_pack.h"
;
typedef struct _AcdbGlbPropLut AcdbGlbPropLut;
#include "acdb_begin_pack.h"
struct _AcdbGlbPropLut
{
	uint32_t glbPropLen;
	AcdbGlbProp *glbProp;
}
...
int32_t AcdbDataGetDataPtr(AcdbCmdFileInfo fileInfo,uint32_t dataoffset,AcdbDataInfo *pDataInfo)
{
	...
	nDataLen = READ_UInt32(pChkBuf+dataoffset);
...
int32_t AcdbDataGetDevPropData(AcdbDevPropInfo *dpInfo)
{
	...
	for(i=0;i<gDataFileInfo.noOfFiles;i++)
	{
		...
		devPropLut.devPropLen = READ_UInt32(pChkBuf);
...
int32_t AcdbDataGetGlobalPropData(AcdbGlbalPropInfo *glbInfo)
{
	...
	for(i=0;i<gDataFileInfo.noOfFiles;i++)
	{
        ...
        //Found Global.acdb file, initialize result to ACDB_DATA_NOT_FOUND
        result = ACDB_DATA_NOT_FOUND;
		glbPropLut.glbPropLen = READ_UInt32(pChkBuf);
		...
		if(nDataFound == 1 || result == ACDB_DATA_NOT_FOUND)
			break;
	}
...
int32_t AcdbDataGetFileTypeInfo(char * fileName, AcdbFileInfo *pFileInfo)
{
   ...
   uint32_t cplInfo = INF;
	...
	for(i=0;i<gDataFileInfo.noOfFiles;i++)
	{
		...
		{
			// get maj, min, rev, cpl info, swp info.
			AcdbFileGetSWVersion(gDataFileInfo.fInfo[i].pFileBuf,gDataFileInfo.fInfo[i].nFileSize,&major, &minor, &revision, &cplInfo);

			pFileInfo->maj = major;
			pFileInfo->min = minor;
			pFileInfo->rev = revision;
         pFileInfo->cplInfo = cplInfo;
			...
			else if(ACDB_PARSE_SUCCESS == IsAcdbAMDBFileValid(gDataFileInfo.fInfo[i].pFileBuf, gDataFileInfo.fInfo[i].nFileSize))
			{
				pFileInfo->fileType = ACDB_AMDB_TYPE;
			}
			else if(strstr(gDataFileInfo.fInfo[i].chFileName, WKSP_FILE_NAME_EXT) != NULL)
			{
...
int32_t AcdbDataGetCodecTblsInfo(AcdbTableCmd *tblCmd,AcdbTableInfo *tblInfo)
{
	...
	case ADIE_CODEC_TBL:
		...
		break;
	case WDSP_DYN_MOD_INFO_TBL_ID:
		lutId = ACDB_WDSP_CHUNKID_DYNAMICMODULEINFOLUT;
		cdefId = ACDB_WDSP_CHUNKID_DYNAMICMODULEINFOCDFT;
		cdotId = ACDB_WDSP_CHUNKID_DYNAMICMODULEINFOCDOT;
		break;
	case WDSP_MODULE_CONN_TBL_ID:
		lutId = ACDB_WDSP_CHUNKID_MODULECONNLUT;
		cdefId = ACDB_WDSP_CHUNKID_MODULECONNCDFT;
		cdotId = ACDB_WDSP_CHUNKID_MODULECONNCDOT;
		break;
	case WDSP_MOD_INFO_DATA_TBL_ID:
		lutId = ACDB_WDSP_CHUNKID_MODULEINFOLUT;
		cdefId = ACDB_WDSP_CHUNKID_MODULEINFOCDFT;
		cdotId = ACDB_WDSP_CHUNKID_MODULEINFOCDOT;
		break;
	case WDSP_SUBGRAPH_CONN_TBL_ID:
		lutId = ACDB_WDSP_CHUNKID_SUBGRAPHCONNLUT;
		cdefId = ACDB_WDSP_CHUNKID_SUBGRAPHCONNCDFT;
		cdotId = ACDB_WDSP_CHUNKID_SUBGRAPHCONNCDOT;
		break;
	case WDSP_MODULE_CAL_TBL_ID:
		lutId = ACDB_WDSP_CHUNKID_MODULECALBLUT;
		cdefId = ACDB_WDSP_CHUNKID_MODULECALBCDFT;
		cdotId = ACDB_WDSP_CHUNKID_MODULECALBCDOT;
		break;
	default:
		tblFound = 0;
	}
	...
	{
		uint32_t i=0;
		// the table request is from codec table
		for(i=0;i<gDataFileInfo.noOfFiles;i++)
		{
...
int32_t WdspGetGetSubgraphList(WdspGetSubgraphListQry *dpInfo)
{
	int32_t result = ACDB_ERROR;
	AcdbDevPropLut devPropLut;
	uint8_t *pChkBuf = NULL;
	...
	if(dpInfo == NULL)
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Received NULL input for WdspGetGetSubgraphList\n");
		return ACDB_ERROR;
	}
	if(gDataFileInfo.noOfFiles == 0)
	{
	...
	for(i=0;i<gDataFileInfo.noOfFiles;i++)
	{
		...
		result = AcdbFileGetChunkData(gDataFileInfo.fInfo[i].pFileBuf,gDataFileInfo.fInfo[i].nFileSize,ACDB_CHUNKID_WDSP_DEVPROPLUT,&pChkBuf,&nChkBufSize);
		if(result != ACDB_PARSE_SUCCESS)
		{
			result = ACDB_ERROR;
			continue;
		...
		{
			if(dpInfo->devId == devPropLut.devProp[idx].devid &&
				USECASESUBGRAPHINFO_PID == devPropLut.devProp[idx].pid)
			{
				result = AcdbDataGetDataPtr(gDataFileInfo.fInfo[i],devPropLut.devProp[idx].dataoffset,&dpInfo->dataInfo);
				nDataFound = 1;
				break;
...
int32_t WdspGetSubgraphDefinition(WdspGetSubgraphDefnQry *dpInfo)
{
	int32_t result = ACDB_ERROR;
	AcdbDevPropLut devPropLut;
	uint8_t *pChkBuf = NULL;
	uint32_t nChkBufSize=0;
	uint32_t i = 0,idx=0, index=0;
	if(dpInfo == NULL)
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Received NULL input for WdspGetGetSubgraphList\n");
		return ACDB_ERROR;
	...
	{
		int8_t nDataFound = 0;
		AcdbDataInfo *tempData = (AcdbDataInfo*) malloc(sizeof(AcdbDataInfo));
		...
		for(idx=0;idx<devPropLut.devPropLen;idx++)
		{
			if(dpInfo->devId == devPropLut.devProp[idx].devid &&
				USECASESUBGRAPHINFO_PID == devPropLut.devProp[idx].pid)
			{
				result = AcdbDataGetDataPtr(gDataFileInfo.fInfo[i],devPropLut.devProp[idx].dataoffset,tempData);
				nDataFound = 1;
				break;
			}
		}
	...
	//Here data found is for all usecase IDs, we have to find for one specific usecase ID.
			uint8_t *buffer = tempData->pData;
			uint32_t numOfUsecaseIds = READ_UInt32(buffer);
			uint32_t offset = sizeof(uint32_t);
			int8_t subgraphFound = 0;
			uint32_t numBytesRemaining = tempData->nDataLen;
			for(index = 0; index < numOfUsecaseIds;index++)
			{
				uint32_t usecaseID;
				uint32_t sgtCount;
				usecaseID = READ_UInt32(buffer + offset);
				offset += sizeof(uint32_t); //For usecase ID which is read from buffer.
				if(usecaseID == dpInfo->usecaseId)
				{
					sgtCount = READ_UInt32(buffer + offset);
					offset += sizeof(uint32_t); //For sgtcount which is read from buffer.
					for(idx = 0; idx < sgtCount ;idx ++)
					{
						uint32_t midpidCount=0;
						uint32_t subgraphID = READ_UInt32(buffer + offset);
						offset += sizeof(uint32_t);
						if(subgraphID == dpInfo->subgraphId)
						{
							dpInfo->dataInfo.pData = buffer + offset;
							midpidCount = READ_UInt32(buffer + offset);
							dpInfo->dataInfo.nDataLen = (midpidCount * 2 * sizeof(uint32_t))+sizeof(uint32_t);
							subgraphFound = 1;
							break;
						}
						else
						{
							midpidCount = READ_UInt32(buffer + offset);
							offset += sizeof(uint32_t);
							offset += (midpidCount*sizeof(uint32_t) * 2); //Payload contains array of MID and IID of length midpidCount
						}
					}
					if(subgraphFound == 1)
						break;
				...
				else
				{
					...
					{
						uint32_t midpidCount=0;
						READ_UInt32(buffer + offset); //Skipping subgraph ID
						offset += sizeof(uint32_t);
						midpidCount = READ_UInt32(buffer + offset);
						offset += sizeof(uint32_t);
						offset += (midpidCount*sizeof(uint32_t) * 2); //Payload contains array of MID and IID of length midpidCount
...
int32_t WdspGetFrameworkIds(WdspFrameworkIds *dpInfo)
{
	int32_t result = ACDB_ERROR;
	AcdbGlbPropLut glblPropLut;
	uint8_t *pChkBuf = NULL;
	uint32_t nChkBufSize=0;
	uint32_t i = 0,idx=0;
	if(dpInfo == NULL)
	...
	for(i=0;i<gDataFileInfo.noOfFiles;i++)
	{
		...
		result = AcdbFileGetChunkData(gDataFileInfo.fInfo[i].pFileBuf,gDataFileInfo.fInfo[i].nFileSize,ACDB_CHUNKID_WDSP_GLOBALPROPLUT,&pChkBuf,&nChkBufSize);
		if(result != ACDB_PARSE_SUCCESS)
		{
			result = ACDB_ERROR;
			continue;
		}
		glblPropLut.glbPropLen = READ_UInt32(pChkBuf);
		glblPropLut.glbProp = (AcdbGlbProp *)(pChkBuf + sizeof(glblPropLut.glbPropLen));
		for(idx=0;idx<glblPropLut.glbPropLen;idx++)
		{
			if(SUPPORTEDFWKIDS_PID == glblPropLut.glbProp[idx].pid)
			{
				result = AcdbDataGetDataPtr(gDataFileInfo.fInfo[i],glblPropLut.glbProp[idx].dataoffset,&dpInfo->dataInfo);
				nDataFound = 1;
				break;
			}
		}
	...
	if(result != ACDB_SUCCESS)
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Failed to get framework IDs");
	}
	return result;
}
	...
	uint32_t nOutputSize)
{
	...
	case ACDBDATACMD_GET_FILE_NAME:
		...
		break;
	case ACDBDATACMD_GET_WDSP_SUBGRAPHLIST:
		{
			if(pInput == NULL || nInputSize != sizeof(WdspGetSubgraphListQry))
			{
				ACDB_DEBUG_LOG("ACDBFILE_MGR: Received invalid input/output params\n");
				return ACDB_BADPARM;
			}
			...
			{
				WdspGetSubgraphListQry *pDInfo = (WdspGetSubgraphListQry *)pInput;
				result = WdspGetGetSubgraphList(pDInfo);
			}
		}
		break;
	case ACDBDATACMD_GET_WDSP_SUBGRAPH_DEFINITION:
		{
			if(pInput == NULL || nInputSize != sizeof(WdspGetSubgraphDefnQry))
			{
				ACDB_DEBUG_LOG("ACDBFILE_MGR: Received invalid input/output params\n");
				return ACDB_BADPARM;
			}
			...
			{
				WdspGetSubgraphDefnQry *pDInfo = (WdspGetSubgraphDefnQry *)pInput;
				result = WdspGetSubgraphDefinition(pDInfo);
			}
		}
		break;
	case ACDBDATACMD_GET_WDSP_FRAMEWORKIDS:
		{
			if(pInput == NULL || nInputSize != sizeof(WdspFrameworkIds))
			{
				ACDB_DEBUG_LOG("ACDBFILE_MGR: Received invalid input/output params\n");
				return ACDB_BADPARM;
			}
			...
			{
				WdspFrameworkIds *pDInfo = (WdspFrameworkIds *)pInput;
				result = WdspGetFrameworkIds(pDInfo);
			}
		}
		break;
	}
