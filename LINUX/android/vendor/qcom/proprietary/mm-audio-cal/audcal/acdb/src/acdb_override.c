...
$Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_override.c#23 $
	...
	)
{
		...
		&& pFileDataBuf != NULL ) //nFileDataLen can be Zero for VP3
	{
		...
		{//If data node exist on default,check table node on heap
			...
			result = FindSecondaryKeyNodeFromCVDKeyOnHeapEx((AcdbDataLookupCVDKeyType*) pKey,
			...
			if (result == ACDB_SUCCESS)
			{
				...
				{//Free Topology Node
					uint32_t fReeTblResult = ACDB_HEAP_NOTFREE_NODE;
					result = FreeTopologyNodeEx((uint32_t*) pModuleId,
						(uint32_t*) pParamId,
						(AcdbDynamicSecondaryKeyType*) pSecondaryKeyNode,
						(uint32_t*) &fReeTblResult
						);
		...
		{//Data not member of static data
		 ...
		 if (result == ACDB_SUCCESS && pDataNode!= NULL)
			{
				AcdbDynamicSecondaryKeyType *pSecondaryKeyNode = NULL;
				result = GetSecondaryNodeFromCVDKeyOnHeapEx((AcdbDataLookupCVDKeyType*) pKey,
						(AcdbDynamicTblNodeTypeEx*) g_pTblEx,
						(AcdbDynamicSecondaryKeyType**) &pSecondaryKeyNode
						);
				if (result == ACDB_SUCCESS)
...
int32_t Acdb_sys_reset(void)
{
   ...
   if (g_pTblEx != NULL)
   {
      ...
      while (pCur != NULL)
      {
         ...
         {
            AcdbDynamicPrimaryKeyType *pPrimaryCur;
            ...
            while (pPrimaryCur != NULL)
            {
               ...
               {
                  AcdbDynamicSecondaryKeyType *pSecondaryCur = pPrimaryCur->pSecondaryNode->pTblHead;
   ...
   if(nonModuleTbl == 0)
   {
      if(pInput->nTblId==VOCPROC_DYNAMIC_TBL ||pInput->nTblId==VOCPROC_STATIC_TBL)
      {
         ...
         while(pCur != NULL)
         {
            ...
            {
               AcdbDynamicPrimaryKeyType *pCurPrimaryKey = pCur->pPrimaryKeyNode->pTblHead;
               while (pCurPrimaryKey != NULL)
               {
                  AcdbDynamicSecondaryKeyType *pCurSecondaryKey = pCurPrimaryKey->pSecondaryNode->pTblHead;
                  while(pCurSecondaryKey != NULL)
                  {
                     AcdbDynamicTopologyType *pCurTop = pCurSecondaryKey->pTopologyNode->pTopHead;
                     while(pCurTop != NULL)
                     {
                        pOut->nNoOfEntries++;
   ...
   if(nonModuleTbl == 0)
   {
      if(pInput->nTblId==VOCPROC_DYNAMIC_TBL ||pInput->nTblId==VOCPROC_STATIC_TBL)
      {
         ...
         while(pCur != NULL)
         {
            if(pCur->pKey->nTableId == pInput->nTblId)
            {
               AcdbDynamicPrimaryKeyType *pPrimaryKeyNode = pCur->pPrimaryKeyNode->pTblHead;
               while (pPrimaryKeyNode != NULL)
               {
                  AcdbDynamicSecondaryKeyType *pSecondaryKeyNode = pPrimaryKeyNode->pSecondaryNode->pTblHead;
                  while(pSecondaryKeyNode != NULL)
                  {
                     AcdbDynamicTopologyType *pCurTop = pSecondaryKeyNode->pTopologyNode->pTopHead;
                     while(pCurTop != NULL)
                     {
