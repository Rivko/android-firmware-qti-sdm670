...
/*===========================================================================
    ...
    $Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_delta_parser.c#7 $
...
int32_t IsAcdbDeltaFileValid(const uint8_t *pFileBuf,const uint32_t nFileSize)
{
   ...
   if(nFileSize < sizeof(AcdbDeltaFileHeader) + sizeof(uint32_t))
      return ACDB_PARSE_INVALID_FILE;
   ...
   if(!((fileMajor == 0 && fileMinor == 0) || (fileMajor == 0 && fileMinor == 1))) // could only parse 0 , 0 file version, add additional versions here in future.
   {
      return ACDB_PARSE_INVALID_FILE;
   }
   if(fileMajor == 0 && fileMinor == 0) // maj, min, rev
   {
      offset += 3 * sizeof(uint32_t);
   }
   else
   {
      offset += 4 * sizeof(uint32_t); // maj, min, rev and cplInfo.
   }
...
int32_t AcdbDeltaFileGetSWVersion(const uint8_t *pFileBuf,const uint32_t nFileSize,uint32_t *pMajVer,uint32_t *pMinVer,uint32_t *pRevVer, uint32_t *pVerCplInfo)
{
   ...
   {
      if(nFileSize < (sizeof(AcdbDeltaFileHeader) + sizeof(AcdbDeltaFileHeaderInfoV1) + sizeof(uint32_t)))
         return ACDB_PARSE_FAILURE;
   ...
   else if(fileMajor == 0 && fileMinor == 1)
   {
      if(nFileSize < (sizeof(AcdbDeltaFileHeader) + sizeof(AcdbDeltaFileHeaderInfoV2) + sizeof(uint32_t)))
         return ACDB_PARSE_FAILURE;
      ACDB_MEM_CPY(pMajVer,sizeof(uint32_t),pFileBuf + offset,sizeof(uint32_t));
      offset += sizeof(uint32_t);
      ...
      ACDB_MEM_CPY(pVerCplInfo,sizeof(uint32_t),pFileBuf + offset,sizeof(uint32_t));
      offset += sizeof(uint32_t);
   }
