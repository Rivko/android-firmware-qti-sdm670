 ...
 * and max of uint16_t since we are using
 * get_param_V5 version.
 */
#define MAX_PARAM_SIZE			57344
...
#define AFE_API_VERSION_LOOPBACK_CONFIG	1
#define LB_MODE_SIDETONE	2
/* ADM rtac info */
...
struct afe_sidetone_get_param_resp {
	uint16_t	enable;
	uint16_t	gain;
}__packed;
...
struct afe_sidetone_gain_payload {
	uint16_t 	rx_port_id;
	uint16_t 	gain;
}__packed;

struct afe_loopback_payload{
...
struct afe_st_set_gain_param_payload {
	struct ACPH_SET_AFE_DATA_req    	afe_set;
	struct afe_sidetone_gain_payload	gain_payload;
} __packed;

struct afe_st_set_enable_param_payload {
	struct ACPH_SET_AFE_DATA_req	afe_set;
	struct afe_loopback_payload	lb_payload;
} __packed;
...
static uint32_t	rtac_buf[MAX_PARAM_SIZE/sizeof(uint32_t)];
...
int32_t get_audio_copp_id(uint32_t topo_id, uint32_t app_type, uint32 acdb_id, uint32_t *copp_id)
{
        int32_t result;
        uint32_t i,num=0;
        ...
        result = get_adm_info();
        if(result < 0) {
                LOGD("[%s] finding copp id failed %d\n", __func__, result);
                goto done;
        }

        result = -1;
        for(i=0; i < rtac_adm_data.num_of_dev; i++) {
                if((rtac_adm_data.device[i].topology_id == topo_id) &&
                   (rtac_adm_data.device[i].app_type == app_type) &&
                   (rtac_adm_data.device[i].acdb_dev_id == acdb_id))
                {
		    num++;
                    copp_id[num] = rtac_adm_data.device[i].copp;
                    result = 0;
                }
        }
done:
...
int32_t get_audio_popp_id(uint32_t topo_id, uint32_t app_type, uint32_t acdb_id, uint32_t *popp_id)
{
        int32_t result;
        uint32_t i,j,num=0;
        ...
        result = get_adm_info();
        if(result < 0) {
                LOGD("[%s] finding popp id failed %d\n", __func__, result);
                goto done;
        }

        result = -1;
        for(i=0; i<rtac_adm_data.num_of_dev; i++) {
                LOGE(" app 0x%x acdb 0x%x", rtac_adm_data.device[i].app_type, rtac_adm_data.device[i].acdb_dev_id);
                if((rtac_adm_data.device[i].app_type == app_type) &&
                   (rtac_adm_data.device[i].acdb_dev_id == acdb_id))
                {
                    for(j=0; j<rtac_adm_data.device[i].num_of_popp; j++) {
                        LOGE(" topo 0x%x ", rtac_adm_data.device[i].popp[j].popp_topology);
                        if(rtac_adm_data.device[i].popp[j].popp_topology == topo_id) {
			    num++;
                            popp_id[num] = rtac_adm_data.device[i].popp[j].popp;
                            result = 0;
                        }
                    }
                }
	...
	popp_id[0] = num;
        if(result < 0) {
                LOGE("[%s] finding active popp id failed, apptype %d, acdb %d, topo %d",
                       __func__, app_type, acdb_id, topo_id);
        }
        return result;
}
...
int32_t get_voice_info()
{
	...
	if (ret < 0) {
		LOGE("ACDB RTAC -> ERROR: AUDIO_GET_RTAC_VOICE_INFO errno: %d\n", errno);
			...
			uint32_t *bytes_filled)
{
	...
	for(i=0; i < rtac_adm_data.num_of_dev; i++) {
		aud_top[(i*5+1)] = rtac_adm_data.device[i].topology_id;
		aud_top[(i*5+2)] = rtac_adm_data.device[i].app_type;
		aud_top[(i*5+3)] = rtac_adm_data.device[i].copp;
		aud_top[(i*5+4)] = rtac_adm_data.device[i].copp;
		aud_top[(i*5+5)] = rtac_adm_data.device[i].num_of_popp;
	}

	*bytes_filled = (i*5+1) * sizeof(uint32_t);
done:
	return ret;
}

int32_t get_aud_device(uint32_t *aud_dev,
			uint32_t *bytes_filled)
{
	int ret = 0;
	uint32_t i;
	...
	aud_dev[0] = rtac_adm_data.num_of_dev;
	for(i=0; i < rtac_adm_data.num_of_dev; i++) {
		aud_dev[(i*6+1)] = rtac_adm_data.device[i].acdb_dev_id;
		aud_dev[(i*6+2)] = rtac_adm_data.device[i].topology_id;
		aud_dev[(i*6+3)] = rtac_adm_data.device[i].app_type;
		aud_dev[(i*6+4)] = rtac_adm_data.device[i].copp;
		aud_dev[(i*6+5)] = rtac_adm_data.device[i].copp;
		aud_dev[(i*6+6)] = rtac_adm_data.device[i].num_of_popp;
	}

	*bytes_filled = (i*6+1) * sizeof(uint32_t);
done:
	return ret;
}
			...
			uint32_t *bytes_filled)
{
	...
	aud_strm[0] = 0;
	for(i=0; i < rtac_adm_data.num_of_dev; i++) {
                /* Return popp handles only if copp handle matches. */
                if (aud_strm_info->copp_handle != rtac_adm_data.device[i].copp)
                    continue;

		for(j=0; j < rtac_adm_data.device[i].num_of_popp; j++) {
			if (aud_strm[0] == 0) {
				aud_strm[aud_strm[0]*3+1] = rtac_adm_data.device[i].popp[j].popp;
				aud_strm[aud_strm[0]*3+2] = rtac_adm_data.device[i].popp[j].popp_topology;
				aud_strm[aud_strm[0]*3+3] = rtac_adm_data.device[i].popp[j].app_type;
				aud_strm[0]++;
				continue;
			}
			for(k=0; k < aud_strm[0]; k++) {
				if (rtac_adm_data.device[i].popp[j].popp == aud_strm[k*3+1])
					break;
			}
			if (k == aud_strm[0]) {
				aud_strm[aud_strm[0]*3+1] = rtac_adm_data.device[i].popp[j].popp;
	...
	*bytes_filled = (aud_strm[0]*3+1) * sizeof(uint32_t);
done:
	return ret;
}

int32_t get_aud_afe_port_id(ACPH_CMD_QUERY_AUD_AFE_ACTIVE_PORT_ID_V2_req *aud_afe_info,
			uint32_t *aud_afe,
			uint32_t *bytes_filled)
{
	int ret = 0;
	...
	for(i=0; i < rtac_adm_data.num_of_dev; i++) {
		if (aud_afe_info->aud_copp_handle == rtac_adm_data.device[i].copp) {
			break;
		} else if (i == rtac_adm_data.num_of_dev-1) {
			LOGE("ACDB RTAC -> ERROR: %s, copp 0x%x does not match 0x%x in rtac driver\n",
				__func__, aud_afe_info->aud_copp_handle, rtac_adm_data.device[i].copp);
			/* default to 0 */
			i = 0;
			break;
		}
	...
	aud_afe[0] = rtac_adm_data.device[i].afe_port;
	aud_afe[1] = rtac_adm_data.device[i].afe_topology;
...
int32_t get_voc_afe_port_id_v2(ACPH_CMD_QUERY_VOC_AFE_ACTIVE_PORT_IDS_V2_req *voc_afe_info,
			uint32_t *voc_afe,
			uint32_t *bytes_filled)
{
	int ret = 0;
	...
	for(i=0; i < rtac_voice_data.num_of_voice_combos; i++) {
		if (voc_afe_info->voc_copp_handle == rtac_voice_data.voice[i].cvp_handle) {
			...
			LOGE("ACDB RTAC -> ERROR: get_voc_afe_port_id_v2, copp 0x%x does not match 0x%x in rtac driver\n",
				voc_afe_info->voc_copp_handle, rtac_voice_data.voice[i].cvp_handle);
			/* default to 0 */
			i = 0;
			break;
	...
	voc_afe[0] = rtac_voice_data.voice[i].tx_afe_port;
	voc_afe[1] = rtac_voice_data.voice[i].tx_afe_topology;
	voc_afe[2] = rtac_voice_data.voice[i].rx_afe_port;
	voc_afe[3] = rtac_voice_data.voice[i].rx_afe_topology;

	*bytes_filled = 4 * sizeof(uint32_t);
done:
	return ret;
}

int32_t get_voc_vc_devices(ACPH_CMD_QUERY_VOC_VC_DEVICES_req *voc_devices_info,
			   uint32_t *voc_devices_data,
			   uint32_t *bytes_filled)
{
	int ret = 0;
	uint32_t i;
	...
	ret = get_voice_info();
	if (ret < 0) {
		LOGE("ACDB RTAC -> Failed to fetch data from rtac driver!");
		goto err;
	}

	for(i=0; i < rtac_voice_data.num_of_voice_combos; i++) {
		if (voc_devices_info->copp_handle == rtac_voice_data.voice[i].cvp_handle) {
			voc_devices_data[0] = 1;
			voc_devices_data[1] = rtac_voice_data.voice[i].rx_acdb_id;
			voc_devices_data[2] = rtac_voice_data.voice[i].tx_acdb_id;
			goto done;
		}
	}

	LOGE("ACDB RTAC -> ERROR: Either cvp handle (0x%x) do not match in rtac driver or there is no active voice call.", voc_devices_info->copp_handle);
err:
	voc_devices_data[0] = 0;
	voc_devices_data[1] = 0;
	voc_devices_data[2] = 0;
done:
	*bytes_filled = 3 * sizeof(uint32_t);
	return ret;
}
...
void pack_audio_get_rtac_data(ACPH_CMD_RTC_GET_CAL_DATA_req *rtac_info, uint32_t out_buf_size)
{
	...
	buf->aud_get.param_max_size = (uint16_t)MAX_PARAM_SIZE;
...
void pack_afe_get_rtac_data(ACPH_GET_AFE_DATA_req *rtac_info, uint32_t out_buf_size)
{
	...
	buf->afe_get.payload_size = (uint16_t)MAX_PARAM_SIZE;
}
...
void pack_voice_get_rtac_data(ACPH_CMD_RTC_GET_CAL_DATA_req *rtac_info, uint32_t out_buf_size)
{
	...
	buf->voc_get.mem_size = (uint16_t)MAX_PARAM_SIZE;
...
void pack_afe_sidetone_set_rtac_gain_data(ACPH_SET_AFE_SIDETONE_GAIN_req *rtac_info,
		uint32_t in_buf_size __unused, uint32_t out_buf_size)
{
	struct afe_st_set_gain_param_payload  params;
	memset((void*)&params, 0, sizeof(struct afe_st_set_gain_param_payload));
	params.afe_set.tx_afe_port_id = rtac_info->tx_afe_port_id;
	params.afe_set.rx_afe_port_id = rtac_info->rx_afe_port_id;
	params.afe_set.module_id = AFE_MODULE_LOOPBACK;
	params.afe_set.parameter_id = AFE_PARAM_ID_LOOPBACK_GAIN_PER_PATH;
	params.afe_set.length = sizeof(params.gain_payload);
	params.gain_payload.rx_port_id  = rtac_info->rx_afe_port_id;
	params.gain_payload.gain  = rtac_info->gain;
	pack_afe_set_rtac_data((ACPH_SET_AFE_DATA_req *)&params, sizeof(params),  out_buf_size);
	LOGD("%s,Rx port ID:0x%x,Gain to be set:0x%x", __func__, rtac_info->rx_afe_port_id,
	      rtac_info->gain);
}

void pack_afe_sidetone_set_rtac_enable_data(ACPH_SET_AFE_SIDETONE_GAIN_req *rtac_info,
		uint32_t in_buf_size __unused, uint32_t out_buf_size)
{
	struct afe_st_set_enable_param_payload  params;
	memset((void*)&params, 0, sizeof(struct afe_st_set_enable_param_payload));
	params.afe_set.tx_afe_port_id = rtac_info->tx_afe_port_id;
	params.afe_set.rx_afe_port_id = rtac_info->rx_afe_port_id;
	params.afe_set.module_id = AFE_MODULE_LOOPBACK;
	params.afe_set.parameter_id = AFE_PARAM_ID_LOOPBACK_CONFIG;
	params.afe_set.length = sizeof(params.lb_payload);
	params.lb_payload.routing_mode  = LB_MODE_SIDETONE;
	params.lb_payload.enable  = rtac_info->enable;
	params.lb_payload.loopback_cfg_minor_version  = AFE_API_VERSION_LOOPBACK_CONFIG;
	params.lb_payload.dst_port_id  = rtac_info->rx_afe_port_id;
	params.lb_payload.reserved     = 0;
	pack_afe_set_rtac_data((ACPH_SET_AFE_DATA_req *)&params, sizeof(params),  out_buf_size);
	LOGD("%s,Rx port ID:0x%x, Enable:%d", __func__, rtac_info->rx_afe_port_id,
	      rtac_info->enable);
}
...
static int32_t get_afe_rtac(struct ACPH_GET_AFE_DATA_req *rtac_info, uint32_t in_buf_size __unused,
		uint32_t *rtac_data, uint32_t out_buf_size, uint32_t *bytes_filled)
{
	...
	if (is_afe_port_exist(rtac_info->afe_port_id)) {
		...
		LOGE("ACDB RTAC -> Error: get_afe_rtac, AFE port ID %d not found!\n", rtac_info->afe_port_id);
		ret = ACPH_FAILURE;
	}
	return ret;
}

static int32_t get_afe_sidetone_rtac(struct ACPH_GET_AFE_SIDETONE_GAIN_req *rtac_info,
		uint32_t in_buf_size __unused, uint32_t *rtac_data, uint32_t out_buf_size,
		uint32_t *bytes_filled)
{
	int32_t ret = 0;
	struct afe_sidetone_get_param_resp         sidetone_rsp;
	struct ACPH_GET_AFE_DATA_req               afe_get;
	uint8_t *offset;

	memset((void*)&sidetone_rsp, 0, sizeof(struct afe_sidetone_get_param_resp));
	afe_get.afe_port_id = rtac_info->tx_afe_port_id;
	/* Check if AFE port exists */
	if (is_afe_port_exist(rtac_info->tx_afe_port_id)) {
		afe_get.module_id = AFE_MODULE_LOOPBACK;
		afe_get.parameter_id = AFE_PARAM_ID_LOOPBACK_GAIN_PER_PATH;
		pack_afe_get_rtac_data(&afe_get, out_buf_size);
		ret = send_get_rtac(rtac_data, out_buf_size, bytes_filled, AUDIO_GET_RTAC_AFE_CAL);
		if(ret < 0) {
			LOGE("%s ERROR param id=0x%x ret=%d\n", __func__, afe_get.parameter_id,
			      ret);
			goto done;
		}
		offset = (uint8_t *)rtac_data + sizeof(struct afe_get_param_resp_payload);
		sidetone_rsp.gain = ((struct afe_sidetone_gain_payload *)offset)->gain;

		afe_get.parameter_id = AFE_PARAM_ID_LOOPBACK_CONFIG;
		pack_afe_get_rtac_data(&afe_get, out_buf_size);
		ret = send_get_rtac(rtac_data, out_buf_size, bytes_filled, AUDIO_GET_RTAC_AFE_CAL);
		if(ret < 0) {
			LOGE("%s ERROR param id=0x%x ret=%d\n", __func__, afe_get.parameter_id,
		...
		offset = (uint8_t *)rtac_data + sizeof(struct afe_get_param_resp_payload);
		sidetone_rsp.enable = ((struct afe_loopback_payload *)offset)->enable;

		LOGD("%s sidetone_rsp.gain = 0x%x sidetone_rsp.enable = %d\n", __func__,
		      sidetone_rsp.gain, sidetone_rsp.enable);

		*bytes_filled = sizeof(struct afe_sidetone_get_param_resp);
		memcpy(rtac_data, &sidetone_rsp, sizeof(struct afe_sidetone_get_param_resp));
	} else {
		LOGD("%s -> Error: AFE port ID %d not found!\n", __func__,
		      rtac_info->tx_afe_port_id);
		ret = ACPH_FAILURE;
                *bytes_filled = 0;
	}
...
int32_t get_rtac(uint32_t service_id, uint32_t *rtac_info, uint32_t in_buf_size __unused,
		uint32_t *rtac_data, uint32_t out_buf_size, uint32_t *bytes_filled)
{
	int ret = 0;
	uint32_t i, j;
...
static int32_t set_afe_rtac(struct ACPH_SET_AFE_DATA_req *rtac_info, uint32_t in_buf_size,
		uint32_t *rtac_data __unused, uint32_t out_buf_size )
{
...
static int32_t set_afe_sidetone_rtac(struct ACPH_SET_AFE_SIDETONE_GAIN_req *rtac_info,
		uint32_t in_buf_size, uint32_t *rtac_data __unused, uint32_t out_buf_size )
{
	uint32_t afe_port_id;
	int32_t ret;

	afe_port_id = rtac_info->tx_afe_port_id;

	/* Check if AFE port exists */
	if (is_afe_port_exist(afe_port_id)) {
		pack_afe_sidetone_set_rtac_gain_data(rtac_info, in_buf_size, out_buf_size);
		ret = send_rtac(AUDIO_SET_RTAC_AFE_CAL);
		if(ret < 0) {
			goto done;
		}
		pack_afe_sidetone_set_rtac_enable_data(rtac_info, in_buf_size, out_buf_size);
		ret = send_rtac(AUDIO_SET_RTAC_AFE_CAL);

	} else {
		LOGE("%s Error: set_afe_sidetone_rtac, AFE port ID %d not found!\n", __func__,
		      afe_port_id);
		ret = ACPH_FAILURE;
	}
				...
				uint32_t resp_buf_len, uint32_t *resp_buf_bytes_filled)
{
	...
	case ACPH_CMD_QUERY_DSP_RTC_VERSION:
            {
                ACPH_CMD_QUERY_DSP_RTC_VERSION_rsp tmp;
                // to avoid alingment issue, store the version info in local struct vaiable
                // and do byte copy to response buffer i.e. resp_buf_ptr
                tmp.dsp_rtc_major_version = RTAC_MAJOR_VERSION;
                tmp.dsp_rtc_minor_version = RTAC_MINOR_VERSION;
                memcpy(resp_buf_ptr, (uint8_t *)&tmp, sizeof(ACPH_CMD_QUERY_DSP_RTC_VERSION_rsp));
                *resp_buf_bytes_filled = sizeof (ACPH_CMD_QUERY_DSP_RTC_VERSION_rsp);
		goto done;
            }
	case ACPH_CMD_QUERY_AUD_TOPOLOGY_COPP_HANDLES_V2:
		ret = get_aud_topology((uint32_t *)resp_buf_ptr,
					resp_buf_bytes_filled);
		goto done;
	case ACPH_CMD_QUERY_AUD_DEVICE_COPP_HANDLES_V2:
		ret = get_aud_device((uint32_t *)resp_buf_ptr,
					resp_buf_bytes_filled);
		goto done;
	case ACPH_CMD_QUERY_AUD_COPP_STREAM_HANDLES_V3:
	...
	case ACPH_SET_AFE_DATA:
		ret = set_afe_rtac((ACPH_SET_AFE_DATA_req *)req_buf_ptr,
				...
				resp_buf_len);
		goto done;
	case ACPH_CMD_QUERY_AUD_AFE_ACTIVE_PORT_ID_V2:
		ret = get_aud_afe_port_id((ACPH_CMD_QUERY_AUD_AFE_ACTIVE_PORT_ID_V2_req *)req_buf_ptr,
					(uint32_t *)resp_buf_ptr,
					resp_buf_bytes_filled);
		goto done;
	case ACPH_CMD_QUERY_VOC_AFE_ACTIVE_PORT_IDS:
		ret = get_voc_afe_port_id((ACPH_CMD_QUERY_VOC_AFE_ACTIVE_PORT_IDS_req *)req_buf_ptr,
					...
					resp_buf_bytes_filled);
		goto done;
	case ACPH_CMD_QUERY_VOC_AFE_ACTIVE_PORT_IDS_V2:
		ret = get_voc_afe_port_id_v2((ACPH_CMD_QUERY_VOC_AFE_ACTIVE_PORT_IDS_V2_req *)req_buf_ptr,
					(uint32_t *)resp_buf_ptr,
					resp_buf_bytes_filled);
		goto done;
	case ACPH_CMD_QUERY_VOC_VC_DEVICES:
		ret = get_voc_vc_devices((ACPH_CMD_QUERY_VOC_VC_DEVICES_req *)req_buf_ptr,
					(uint32_t *)resp_buf_ptr,
					resp_buf_bytes_filled);
		goto done;
	case  ACPH_GET_AFE_SIDETONE_GAIN:
		ret = get_afe_sidetone_rtac((ACPH_GET_AFE_SIDETONE_GAIN_req *)req_buf_ptr,
						req_buf_len,
						(uint32_t *)resp_buf_ptr,
						resp_buf_len,
						resp_buf_bytes_filled);
	...
	case ACPH_SET_AFE_SIDETONE_GAIN:
		ret = set_afe_sidetone_rtac((ACPH_SET_AFE_SIDETONE_GAIN_req *)req_buf_ptr,
						req_buf_len,
						(uint32_t *)resp_buf_ptr,
						resp_buf_len);
		goto done;
