...
public class SettingsActivity extends Activity implements CompoundButton.OnCheckedChangeListener {
    ...
    private Switch mUiOpagueTransferOnOff;
    private EditText mUiHistBuffTimeEdit;
    private RelativeLayout mLayoutOpague;
    private RelativeLayout mLayoutHistBuffTime;
    private View mUiSeparatorAboveHist;
    private View mUiSeparatorBelowHist;
    ...
    private void initializeUserInterface() {
        ...
        if (Global.getIsSupportFR44168(this)) {
            mUiOpagueTransferOnOff = (Switch)findViewById(R.id.settings_opague_data_transfer_onoff);
            mLayoutOpague = (RelativeLayout)findViewById(R.id.settings_opague_data);
            mUiHistBuffTimeEdit = (EditText)findViewById(R.id.settings_hist_buff_time_edit);

            //Setting opague data transfer on off
            mUiOpagueTransferOnOff.setChecked(Global.getInstance().getConfigOpagueTransferONOFF(this)
                    == Global.CTRL_OPAGUE.OPAGUE_TRANSFER_ON.ordinal());
            mUiOpagueTransferOnOff.setOnCheckedChangeListener(this);
            mLayoutOpague.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    boolean checked = !mUiOpagueTransferOnOff.isChecked();
                    mUiOpagueTransferOnOff.setChecked(checked);
                }
            });

            //Setting hist buffer time
            mUiHistBuffTimeEdit.setText(String.valueOf(
                    Global.getInstance().getConfigHistBufTime(this)));
            mUiHistBuffTimeEdit.addTextChangedListener(new TextWatcher() {
                @Override
                ...
                public void afterTextChanged(Editable s) {
                    try {
                        Global.getInstance().setOpagueSharedPreference(getApplicationContext(),
                                Global.KEY_CONFIG_CTRL_HIST_BUF_TIME,
            ...
            mLayoutOpague = (RelativeLayout)findViewById(R.id.settings_opague_data);
            mLayoutOpague.setVisibility(View.GONE);
            mLayoutHistBuffTime = (RelativeLayout)findViewById(R.id.settings_hist_buff_time);
            mLayoutHistBuffTime.setVisibility(View.GONE);
            mUiSeparatorAboveHist = findViewById(R.id.separator_above_hist_buffer_time);
            mUiSeparatorAboveHist.setVisibility(View.GONE);
            mUiSeparatorBelowHist = findViewById(R.id.separator_below_hist_buffer_time);
            mUiSeparatorBelowHist.setVisibility(View.GONE);
        }
    }
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        switch (buttonView.getId()) {
            case R.id.settings_opague_data_transfer_onoff:
                Global.getInstance().setOpagueSharedPreference(getApplicationContext(),
                        Global.KEY_CONFIG_OPAGUE_TRANSFER_ONOFF,
                        isChecked?Global.CTRL_OPAGUE.OPAGUE_TRANSFER_ON.ordinal()
                                :Global.CTRL_OPAGUE.OPAGUE_TRANSFER_OFF.ordinal());
