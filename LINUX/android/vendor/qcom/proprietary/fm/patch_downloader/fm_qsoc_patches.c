...
#define CAL_FILE_PATH  "/data/misc/fm/Riva_fm_cal"
...
int get_cal_data_bahama( int i2c_fd )
{
    ...
    cal_file = open("/data/misc/fm/fm_cal", O_RDWR | O_CREAT, S_IRUSR | S_IRGRP);
    if (cal_file < 0)
    {
                    return cal_file;
    }
...
int get_bt_soc_type()
{
    ...
    if (ret != 0) {
        ...
        if (!strncasecmp(bt_soc_type, "rome", sizeof("rome"))) {
            ...
            printf( "qcom.bluetooth.soc not set, so using default.\n");
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
#ifdef ANDROID
	debug_file = fopen("/data/misc/fm/fm_wa_mode", "w+");
#else
	debug_file = fopen("/var/log/fm_wa_mode", "w+");
		...
		fprintf(debug_file,"qcom.bluetooth.soc = %d\n", transport);
		if( transport == BT_SOC_SMD ){
	...
	if(atoi(argv[2]) == 3) {
#ifdef ANDROID
		debug_file = fopen("/data/misc/fm/fm_dacconfig_mode", "w+");
#else
	...
	transport = get_bt_soc_type();
	if( transport  == BT_SOC_SMD ){
		cal_fp = fopen("/data/misc/fm/Riva_fm_cal","r" );
		if (!cal_fp ||
...
#ifdef ANDROID
	debug_file = fopen("/data/misc/fm/fm_dld_enable", "w+");
#else
	...
	is_marimba = chip_is_Marimba(chipVersion);
        ...
        cal_file = open("/data/misc/fm/fm_cal", O_RDONLY);
        if (cal_file < 0) {
