...
#define SLEEP_MS 100
const uint32_t PAUSE_TIMEOUT = 5000; // in ms

void *get_omx_component_factory_fn(void)
{
    return (new COmxApeDec);
}

// omx_cmd_queue destructor
COmxApeDec::omx_cmd_queue::~omx_cmd_queue()
{
    // Nothing to do
}

// omx cmd queue constructor
COmxApeDec::omx_cmd_queue::omx_cmd_queue(): m_read(0),m_write(0),m_size(0)
{
    memset(m_q, 0, sizeof(omx_event)*OMX_CORE_CONTROL_CMDQ_SIZE);
}

// omx cmd queue insert
bool COmxApeDec::omx_cmd_queue::insert_entry(unsigned long p1,
...
bool COmxApeDec::omx_cmd_queue::pop_entry(unsigned long *p1,
        unsigned long *p2, unsigned char *id)
{
    bool ret = true;
    if (m_size > 0)
...
bool COmxApeDec::omx_cmd_queue::get_msg_id(unsigned char *id)
{
    if(m_size > 0)
    {
        *id = m_q[m_read].id;
...
void COmxApeDec::wait_for_event()
{
    int rc;
    struct timespec ts;
...
void COmxApeDec::event_complete()
{
    pthread_mutex_lock(&m_event_lock);
...
void COmxApeDec::wait_for_flush_event()
{
...
void COmxApeDec::event_flush_complete()
{
    pthread_mutex_lock(&m_flush_cmpl_lock);
...
void COmxApeDec::process_event_cb(void *client_data, unsigned char id)
{
    /* To remove warning for unused variable to keep prototype same */
    (void)id;
    DEBUG_PRINT("PE:Waiting for event's...");
    COmxApeDec  *pThis = (COmxApeDec *) client_data;
    pThis->process_events(pThis);
}

void COmxApeDec::process_events(COmxApeDec *client_data)
{
    int rc = 0;
    ...
    /* To remove warning for unused variable to keep prototype same */
    (void)client_data;
    // This loop waits indefinitely till an EVENT is received.
    ...
    while(1)
    {
        ...
        if(errno == ENETRESET)
        {
            DEBUG_PRINT_ERROR("In SSR, post error to omx component");
        ...
        {
            DEBUG_PRINT("EventThread exiting rc[%d] errno[%d]", rc, errno);
            return;
        }
                    ...
                    if(bufHdr)
                    {
                        if(drv_event.event_payload.aio_buf.data_len > sizeof(META_OUT)) {
                            ...
                            if(!bufHdr->nFilledLen)
                                bufHdr->nOffset = 0;
#if DUMPS_ENABLE
                            DEBUG_PRINT("ASYNC READ log to local buff %d", pfd);
...
/* ======================================================================
   ...
   None.
   ========================================================================== */
COmxApeDec::COmxApeDec(): m_volume(25),
    m_app_data(NULL),
    m_eos_bm(0),
    m_flush_cnt(0),
    m_first_ape_header(0),
    m_is_alloc_buf(0),
    drv_inp_buf_cnt(0),
    drv_out_buf_cnt(0),
    nNumInputBuf(0),
    ...
    nTimestamp(0),
    pcm_feedback(1),
    output_buffer_size (OMX_APE_OUTPUT_BUFFER_SIZE),
    ...
    m_is_event_done(0),
    m_flush_cmpl_event(0),
    m_drv_bPaused(OMX_FALSE),
#ifdef NT_PAUSE_TIMEOUT_ENABLED
    m_wakelock_voted(false),
#endif
    m_ipc_to_in_th(NULL),
    ...
    suspensionPolicy(OMX_SuspensionDisabled)
{
    ...
    pthread_mutex_init(&m_audio_drive_lock, NULL);
#ifdef NT_PAUSE_TIMEOUT_ENABLED
    pthread_mutex_init(&m_wakelock_lock, NULL);
#endif
...
COmxApeDec::~COmxApeDec()
{
    ...
    pthread_mutex_destroy(&m_audio_drive_lock);
#ifdef NT_PAUSE_TIMEOUT_ENABLED
    pthread_mutex_destroy(&m_wakelock_lock);
#endif
    ...
    DEBUG_PRINT("OMX APE component destroyed\n");
    return;
}

bool COmxApeDec::aio_write(msm_audio_aio_buf *audio_aio_buf)
{
    bool rc = true;
...
#if DUMPS_ENABLE
    DEBUG_PRINT("ASYNC WRITE log to local buff");
    write(bfd, ((char *)audio_aio_buf->buf_addr + sizeof(META_IN)), (audio_aio_buf->data_len - sizeof(META_IN)));
#endif
    return rc;

}
bool COmxApeDec::aio_read(msm_audio_aio_buf *audio_aio_buf)
{
    bool rc = true;
...
bool COmxApeDec::prepare_for_ebd(OMX_BUFFERHEADERTYPE *bufHdr)
{
    bool rc = true;
    ...
    if (bufHdr->nFlags & OMX_BUFFERFLAG_EOS)
    {
        if (bufHdr->nFilledLen)
        {
            ...
            /* Asynchronous write call to the APE driver */
...
bool COmxApeDec::audio_register_ion(struct mmap_info *ion_buf)
{
    bool rc = true;
    struct msm_audio_ion_info audio_ion_buf;
    /* Register the mapped ION buffer with the APE driver */
...
bool COmxApeDec::audio_deregister_ion(struct mmap_info *ion_buf)
{
    bool rc = true;
    struct msm_audio_ion_info audio_ion_buf;
    if (!ion_buf)
...
/**
  ...
  @return none
 */
void COmxApeDec::buffer_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
...
void COmxApeDec::frame_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
    META_OUT *pmeta_out = NULL;
    DEC_META_OUT *dec_meta_out = NULL;
    OMX_BUFFERHEADERTYPE *buffer = bufHdr;
...
void COmxApeDec::process_out_port_msg(void *client_data, unsigned char id)
{
    unsigned long p1;
    unsigned long p2;
    unsigned char ident;
...
loopback_out:
    ...
    {
        pthread_mutex_unlock(&pThis->m_outputlock);
        DEBUG_PRINT("OUT: Sleeping out thread state[%d] totq[%d] coutq %d\n",\
    ...
    {
        DEBUG_PRINT("No flush/port reconfig qsize=%d tot_qsize=%d",\
                qsize,tot_qsize);
        pthread_mutex_unlock(&pThis->m_outputlock);
    ...
    else if(state == OMX_StatePause)
    {
        ...
        if (!pThis->m_cout_q.m_size)
        {
            ...
            {
                pthread_mutex_unlock(&pThis->m_outputlock);
                DEBUG_PRINT("OUT: SLEEPING OUT THREAD\n");
                pThis->out_th_sleep();
                goto loopback_out;
            }
            else
    ...
    {
        id = ident;
        DEBUG_PRINT("OUT->state[%d]ident[%d]cout[%d]fbd[%d]dataq[%d]\n",\
                pThis->m_state,
                ident,
                pThis->m_cout_q.m_size,
        ...
        else if(id == OMX_COMPONENT_GENERATE_COMMAND)
        {
            ...
            {
                DEBUG_PRINT("Executing FLUSH command on Output port\n");
                pThis->execute_output_omx_flush();

            }
            else
            {
                DEBUG_PRINT("Invalid command[%d]\n",p1);
            }
        }
        else
        {
    ...
    {
        DEBUG_PRINT("ERROR: OUT--> Empty OUTPUTQ\n");
    }

    return;
}

void COmxApeDec::process_command_msg(void *client_data, unsigned char id)
{
    unsigned long p1;
    unsigned long p2;
    unsigned char ident;
        ...
        if (pThis->m_cb.EventHandler)
        {
            if (p1 == OMX_CommandStateSet)
            {
                        ...
                        (state == OMX_StateLoaded))
                {
                    ...
                    {
                        DEBUG_PRINT("WAKE UP IN THREADS\n");
                        pThis->in_th_wakeup();
                    ...
                    {
                        DEBUG_PRINT("WAKE UP OUT THREADS\n");
                        pThis->out_th_wakeup();
    ...
    {
        DEBUG_PRINT("CMD OMX_COMPONENT_PORTSETTINGS_CHANGED");
        pThis->m_cb.EventHandler(&pThis->m_cmp,
                pThis->m_app_data,
...
void COmxApeDec::process_in_port_msg(void *client_data, unsigned char id)
{
...
loopback_in:
    ...
    DEBUG_PRINT("Input-->state[%d]cin[%d]ebd[%d]data[%d]",state,\
            pThis->m_cin_q.m_size,
            pThis->m_ebd_q.m_size,
            pThis->m_din_q.m_size);

    if(tot_qsize ==0) {
        DEBUG_PRINT("IN-->BREAKING FROM IN LOOP");
    ...
    else if ((state == OMX_StatePause))
    {
        if((!pThis->m_cin_q.m_size))
        {
            ...
            DEBUG_PRINT("IN: Sleeping in thread state[%d] cinq[%d]", \
    ...
    {
        id = ident;
        DEBUG_PRINT("Input->state[%d]id[%d]cin[%d]ebdq[%d]dataq[%d]\n",\
        ...
        else if(id == OMX_COMPONENT_GENERATE_COMMAND)
        {
            if(p1 == OMX_CommandFlush)
            {
                DEBUG_PRINT(" Executing FLUSH command on Input port\n");
    ...
    {
        DEBUG_PRINT("Error-->No messages in queue\n");
    }
    return ;
}
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE COmxApeDec::component_init(OMX_STRING role)
{
    ...
    /* Open the APE device in asynchronous mode */
    if(!strcmp(role,"OMX.qcom.audio.decoder.ape"))
    {
        pcm_feedback = 1;
    ...
    if(0 == pcm_feedback)
    {
        m_drv_fd = open("/dev/msm_ape",O_WRONLY | O_NONBLOCK);
    }
    else
    {
        DEBUG_PRINT("Opening /dev/msm_ape in rdwr mode\n");
        m_drv_fd = open("/dev/msm_ape", O_RDWR | O_NONBLOCK);
    }
    if (0 > m_drv_fd)
    {
    ...
    if(!m_ipc_to_in_th)
    {
        m_ipc_to_in_th = omx_ape_thread_create(process_in_port_msg, this,
                (char *) "INPUT_THREAD");
        if(!m_ipc_to_in_th)
        {
    ...
    if(!m_ipc_to_cmd_th)
    {
        m_ipc_to_cmd_th = omx_ape_thread_create(process_command_msg, this,
                (char *) "COMMAND_THREAD");
        if(!m_ipc_to_cmd_th)
        {
    ...
    if(pcm_feedback)
    {
        if(!m_ipc_to_out_th)
        {
            m_ipc_to_out_th = omx_ape_thread_create(process_out_port_msg, this,
                    (char *) "OUTPUT_THREAD");
            if(!m_ipc_to_out_th)
            {
...
#if DUMPS_ENABLE
    bfd = open("/data/vendor/misc/audio/bitstreamdump.dat", O_RDWR|O_CREAT, 0666);
    if (bfd < 0) {
        DEBUG_PRINT_ERROR("cannot open bitstreamdump file");
    }
    pfd = open("/data/vendor/misc/audio/pcmdump.dat", O_RDWR|O_CREAT, 0666);
    if (pfd < 0) {
        DEBUG_PRINT_ERROR("cannot open pcmdump file");
    }
#endif
    return eRet;
}
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxApeDec::get_component_version(OMX_IN OMX_HANDLETYPE hComp,
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxApeDec::send_command(OMX_IN OMX_HANDLETYPE hComp,
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxApeDec::send_command_proxy(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_COMMANDTYPE  cmd,
        OMX_IN OMX_U32       param1,
        OMX_IN OMX_PTR      cmdData)
{
    ...
    if(cmd == OMX_CommandStateSet)
    {
        ...
        else if(m_state == OMX_StateIdle)
        {
            if(eState == OMX_StateLoaded)
            {
                if(release_done(OMX_ALL))
                {
                    ...
                    {
                        omx_ape_thread_stop(m_ipc_to_event_th);
                        m_ipc_to_event_th = NULL;
                    }
            ...
            else if(eState == OMX_StateExecuting)
            {
                if(!m_ipc_to_event_th)
                {
                    ...
                    m_ipc_to_event_th = omx_ape_event_thread_create(
        ...
        else if(m_state == OMX_StateExecuting)
        {
            if(eState == OMX_StateIdle)
            {
                ...
                if(pcm_feedback)
                {
                    execute_omx_flush(OMX_ALL); // Flush all ports
                }
                else
                {
                    execute_omx_flush(0);
        ...
        else if(m_state == OMX_StatePause)
        {
            ...
            if((eState == OMX_StateExecuting || eState == OMX_StateIdle))
            {
                ...
                {
                    is_in_th_sleep = false;
                    DEBUG_PRINT("SCP: WAKING UP IN THREAD\n");
                    in_th_wakeup();
                }
                if(is_out_th_sleep)
                {
                    DEBUG_PRINT("SCP: WAKING UP OUT THREAD\n");
                    out_th_wakeup();
                    is_out_th_sleep = false;
                }
            }
    ...
    else if (cmd == OMX_CommandPortEnable)
    {
        ...
        if ((param1 == OMX_CORE_OUTPUT_PORT_INDEX) || (param1 == OMX_ALL))
        {
            ...
            {
                is_in_th_sleep = false;
                DEBUG_PRINT("SCP:WAKING UP IN THREADS\n");
                in_th_wakeup();
            }
            if(is_out_th_sleep)
            {
 ...
 */
bool COmxApeDec::execute_omx_flush(OMX_IN OMX_U32 param1, bool cmd_cmpl)
{
    bool bRet = true;
    ...
    if (param1 == OMX_ALL)
    {
        ...
        while (1)
        {
            ...
            if(drv_inp_buf_cnt > 0 || drv_out_buf_cnt > 0)
            {
                ...
                if(ioctl(m_drv_fd, AUDIO_FLUSH, 0) == -1)
                    DEBUG_PRINT_ERROR("FLush:ioctl flush failed errno=%d\n",errno);
                // Polling for flush for every 10ms causing issue in SSR case where driver
                // unable to process events, so increase to 100ms
                usleep(100000);
            }
            else
            {
        ...
        // output threads
        DEBUG_PRINT("WAITING FOR FLUSH ACK's param1=%d",(signed)param1);
        wait_for_event();
    ...
    else if (param1 == OMX_CORE_INPUT_PORT_INDEX)
    {
        ...
        DEBUG_PRINT("WAITING FOR FLUSH ACK's param1=%d",(signed)param1);
        wait_for_event();
        DEBUG_PRINT(" RECIEVED FLUSH ACK FOR I/P PORT param1=%d",
                (signed)param1);
    ...
    else if (param1 == OMX_CORE_OUTPUT_PORT_INDEX)
    {
        ...
        DEBUG_PRINT("RECIEVED FLUSH ACK FOR O/P PORT param1=%d ",
                (signed)param1);
        bFlushinprogress = false;
    }
    else
...
/**
  ...
  @return bool value indicating whether flushing is carried out successfully
 */
bool COmxApeDec::execute_input_omx_flush()
{
    ...
    do
    {
        ...
        DEBUG_PRINT("Input FLUSH-->flushq[%d] ebd[%d]dataq[%d]",\
                m_cin_q.m_size,
                m_ebd_q.m_size,qsize);

        if(!tot_qsize)
        {
            DEBUG_PRINT("Input-->BREAKING FROM execute_input_flush LOOP");
        ...
        if (qsize)
        {
            ...
            if ((ident == OMX_COMPONENT_GENERATE_ETB))
            {
                ...
                DEBUG_PRINT("Flush:Input dataq=%p \n", omx_buf);
        ...
        else if((m_ebd_q.m_size))
        {
            ...
            if(ident == OMX_COMPONENT_GENERATE_BUFFER_DONE)
            {
                omx_buf = (OMX_BUFFERHEADERTYPE *) p2;
                omx_buf->nFilledLen = 0;
                DEBUG_PRINT("Flush:ctrl dataq=%p \n",omx_buf);
...
/**
  ...
  @return bool value indicating whether flushing is carried out successfully
 */
bool COmxApeDec::execute_output_omx_flush()
{
    OMX_BUFFERHEADERTYPE *omx_buf;
    unsigned long      p1=0; // Parameter - 1
    unsigned long      p2=0; // Parameter - 2
    ...
    do
    {
        ...
        DEBUG_PRINT("OUT FLUSH-->flushq[%d] fbd[%d]dataq[%d]",\
                m_cout_q.m_size,
        ...
        if(!tot_qsize)
        {
            DEBUG_PRINT("OUT-->BREAKING FROM execute_output_flush LOOP");
        ...
        else if((qsize = m_fbd_q.m_size))
        {
            ...
            if(ident == OMX_COMPONENT_GENERATE_FRAME_DONE)
            {
                ...
                DEBUG_PRINT("CALLING FROM CTRL-FBDQ FROM FLUSH");
            }
        }
        else
        {
 ...
 */
bool COmxApeDec::post_input(unsigned long p1,
        unsigned long p2,
        unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_ape_post_msg(m_ipc_to_in_th, id);
    }

    DEBUG_PRINT("PostInput-->state[%d]id[%d]cin[%d]ebdq[%d]dataq[%d]\n",\
            m_state,
            id,
            m_cin_q.m_size,
            m_ebd_q.m_size,
...
bool COmxApeDec::post_command(unsigned int p1,
        unsigned int p2,
        unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_ape_post_msg(m_ipc_to_cmd_th, id);
    }

    DEBUG_PRINT("PostCmd-->state[%d]id[%d]cmdq[%d]",
            m_state,
            id,
            m_cmd_q.m_size);
...
bool COmxApeDec::post_output(unsigned long p1,
        unsigned long p2,
        unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_ape_post_msg(m_ipc_to_out_th, id);
    }
    DEBUG_PRINT("PostOutput-->state[%d]id[%d]flushq[%d]fbdq[%d]dataq[%d]\n",\
            m_state,
            id,
            m_cout_q.m_size,
            m_fbd_q.m_size,
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxApeDec::get_parameter(OMX_IN OMX_HANDLETYPE hComp,
        ...
        OMX_INOUT OMX_PTR     paramData)
{
    ...
    switch((int)paramIndex)
    {
        case OMX_IndexParamPortDefinition:
            {
                ...
                if (0 == portDefn->nPortIndex)
                {
                    ...
                    portDefn->format.audio.eEncoding = (OMX_AUDIO_CODINGTYPE)QOMX_IndexParamAudioApe;
        ...
        case OMX_IndexParamAudioPortFormat:
            {
                ...
                {
                    portFormatType->eEncoding = (OMX_AUDIO_CODINGTYPE)QOMX_IndexParamAudioApe;
                }
        ...
        case QOMX_IndexParamAudioApe:
            {
                QOMX_AUDIO_PARAM_APETYPE *apeParam =
                    (QOMX_AUDIO_PARAM_APETYPE *) paramData;
                DEBUG_PRINT("get_parameter: OMX_IndexParamAudioApe\n");

                if (OMX_CORE_INPUT_PORT_INDEX == apeParam->nPortIndex)
                {
                    *apeParam = m_adec_param;
                }
                else
                {
                    DEBUG_PRINT_ERROR("get_parameter: OMX_IndexParamAudioApe \
                            OMX_ErrorBadPortIndex %d\n",
                            (int)apeParam->nPortIndex);
                    eRet = OMX_ErrorBadPortIndex;
                }
                break;
            }
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxApeDec::set_parameter(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_INDEXTYPE paramIndex,
        OMX_IN OMX_PTR        paramData)
{
    ...
    switch((int)paramIndex)
    {
        case QOMX_IndexParamAudioApe:
            {
                DEBUG_PRINT("set_parameter: OMX_IndexParamAudioApe");
                m_adec_param = *((QOMX_AUDIO_PARAM_APETYPE *) paramData);
                break;
            }
        case OMX_IndexParamPortDefinition:
            {
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::get_config(OMX_IN OMX_HANDLETYPE      hComp,
...
/* ======================================================================
   ...
   OMX Error None if successful.
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::set_config(OMX_IN OMX_HANDLETYPE      hComp,
        OMX_IN OMX_INDEXTYPE configIndex,
...
None
=============================================================================*/
OMX_ERRORTYPE  COmxApeDec::get_extension_index(
        ...
        OMX_OUT OMX_INDEXTYPE* indexType)
{
    ...
    if(strncmp(paramName,"OMX.Qualcomm.index.audio.ape",
                strlen("OMX.Qualcomm.index.audio.ape")) == 0){
        *indexType =(OMX_INDEXTYPE)QOMX_IndexParamAudioApe;
        DEBUG_PRINT("Extension index type - %x\n", *indexType);
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::get_state(OMX_IN OMX_HANDLETYPE  hComp,
...
/* ======================================================================
   FUNCTION
   COmxApeDec::ComponentTunnelRequest
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::component_tunnel_request(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_U32                        port,
...
void* COmxApeDec::alloc_ion_buffer(unsigned int bufsize)
{
...
void COmxApeDec::free_ion_buffer(void** ion_buffer)
{
...
/* ======================================================================
   FUNCTION
   COmxApeDec::AllocateInputBuffer
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::allocate_input_buffer(OMX_IN OMX_HANDLETYPE hComp,
        ...
        OMX_IN OMX_U32                       bytes)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxApeDec::allocate_input_buffer");
...
OMX_ERRORTYPE  COmxApeDec::allocate_output_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                        port,
        OMX_IN OMX_PTR                     appData,
        OMX_IN OMX_U32                       bytes)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxApeDec::allocate_output_buffer");
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::allocate_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                        port,
        OMX_IN OMX_PTR                     appData,
        OMX_IN OMX_U32                       bytes)
    ...
    if((eRet == OMX_ErrorNone))
    {
        ...
        if((port == OMX_CORE_OUTPUT_PORT_INDEX) && m_out_bPopulated)
        {
            if(BITMASK_PRESENT(&m_flags,OMX_COMPONENT_OUTPUT_ENABLE_PENDING))
            {
                ...
                {
                    is_out_th_sleep = false;
                    DEBUG_PRINT("AllocBuf:WAKING UP OUT THREADS\n");
                    out_th_wakeup();
                }
                if(is_in_th_sleep)
                {
                    is_in_th_sleep = false;
                    DEBUG_PRINT("AB:WAKING UP IN THREADS\n");
                    in_th_wakeup();
                }
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::use_buffer(
        OMX_IN OMX_HANDLETYPE            hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                   port,
        OMX_IN OMX_PTR                   appData,
    ...
    if((eRet == OMX_ErrorNone))
    {
        ...
        if((port == OMX_CORE_OUTPUT_PORT_INDEX) && m_out_bPopulated)
        {
            if(BITMASK_PRESENT(&m_flags,OMX_COMPONENT_OUTPUT_ENABLE_PENDING))
            {
                ...
                {
                    is_out_th_sleep = false;
                    DEBUG_PRINT("UseBuf:WAKING UP OUT THREADS\n");
                    out_th_wakeup();
                }
                if(is_in_th_sleep)
                {
                    is_in_th_sleep = false;
                    DEBUG_PRINT("UB:WAKING UP IN THREADS\n");
                    in_th_wakeup();
                }
            }
        }
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::use_input_buffer(
        OMX_IN OMX_HANDLETYPE            hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                   port,
        OMX_IN OMX_PTR                   appData,
        ...
        OMX_IN OMX_U8*                   buffer)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxApeDec::use_input_buffer");
    if(bytes < input_buffer_size)
    {
        /* return if i\p buffer size provided by client is less
           than min i\p buffer size supported by omx component */
    ...
    {
        bufHdr = (OMX_BUFFERHEADERTYPE *) calloc(sizeof(OMX_BUFFERHEADERTYPE) , 1);
        loc_bufHdr = (OMX_BUFFERHEADERTYPE *) calloc(sizeof(OMX_BUFFERHEADERTYPE) , 1);
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::use_output_buffer(
        OMX_IN OMX_HANDLETYPE            hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                   port,
        OMX_IN OMX_PTR                   appData,
        ...
        OMX_IN OMX_U8*                   buffer)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxApeDec::use_output_buffer");
    if(bytes < output_buffer_size)
    {
        /* return if o\p buffer size provided by client is less
           than min o\p buffer size supported by omx component */
    ...
    if(m_out_current_buf_count < m_out_act_buf_count)
    {
        ...
        if (bufHdr != NULL && loc_bufHdr != NULL)
        {
            ...
            /* Register the mapped PMEM buffer with the APE driver */
            if (!audio_register_ion(mmap_data)) {
                free(bufHdr);
                free(loc_bufHdr);
                *bufferHdr = NULL;
 ...
 */
bool COmxApeDec::search_input_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
    bool eRet = false;
    ...
    if(buffer && temp)
    {
        DEBUG_PRINT("search_input_bufhdr %p \n", buffer);
        eRet = true;
    }
    return eRet;
}
...
/**
  ...
  @return bool value indicating whether buffer is found
 */
bool COmxApeDec::search_output_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
    bool eRet = false;
    OMX_BUFFERHEADERTYPE *temp = NULL;
    ...
    if(buffer && temp)
    {
        DEBUG_PRINT("search_output_bufhdr %p \n", buffer);
        eRet = true;
    }
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxApeDec::free_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_U32                 port,
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxApeDec::empty_this_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_STATETYPE state;
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxApeDec::empty_this_buffer_proxy(OMX_IN OMX_HANDLETYPE hComp,
        OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if(m_first_ape_header == 0)
    {
        m_first_ape_header = 1;
        if (pcm_feedback) {
            struct msm_audio_config config;
            ioctl(m_drv_fd, AUDIO_GET_CONFIG, &config);
            config.sample_rate = m_adec_param.nSampleRate;
            config.channel_count = m_adec_param.nChannels;
            ioctl(m_drv_fd, AUDIO_SET_CONFIG, &config);
        }
        struct msm_audio_ape_config drv_config;
        ioctl(m_drv_fd, AUDIO_GET_APE_CONFIG, &drv_config);

        if (buffer->nFlags & OMX_BUFFERFLAG_CODECCONFIG) {
            DEBUG_PRINT("Populating CSD structure from buffer");
            uint8_t *ptr = buffer->pBuffer;

            memcpy(&m_adec_param.nCompatibleVersion, ptr + kKeyIndexApeCompatibleVersion,
                    sizeof(m_adec_param.nCompatibleVersion));
            memcpy(&m_adec_param.nCompressionLevel, ptr + kKeyIndexApeCompressionLevel,
                    sizeof(m_adec_param.nCompressionLevel));

            if (m_adec_param.nCompressionLevel != APE_COMPRESSION_LEVEL_FAST &&
                m_adec_param.nCompressionLevel != APE_COMPRESSION_LEVEL_NORMAL &&
                m_adec_param.nCompressionLevel != APE_COMPRESSION_LEVEL_HIGH) {
                DEBUG_PRINT_ERROR("Cannot play APE clips with compression level %d",
                                    m_adec_param.nCompressionLevel);
                return OMX_ErrorInsufficientResources;
            }

            memcpy(&m_adec_param.nFormatFlags, ptr + kKeyIndexApeFormatFlags,
                    sizeof(m_adec_param.nFormatFlags));
            memcpy(&m_adec_param.nBlocksPerFrame, ptr + kKeyIndexApeBlocksPerFrame,
                    sizeof(m_adec_param.nBlocksPerFrame));
            memcpy(&m_adec_param.nFinalFrameBlocks, ptr + kKeyIndexApeFinalFrameBlocks,
                    sizeof(m_adec_param.nFinalFrameBlocks));
            memcpy(&m_adec_param.nTotalFrames, ptr + kKeyIndexApeTotalFrames,
                    sizeof(m_adec_param.nTotalFrames));
            memcpy(&m_adec_param.nBitsPerSample, ptr + kKeyIndexApeBitsPerSample,
                    sizeof(m_adec_param.nBitsPerSample));
            memcpy(&m_adec_param.nChannels, ptr + kKeyIndexApeNumChannels,
                    sizeof(m_adec_param.nChannels));
            memcpy(&m_adec_param.nSampleRate, ptr + kKeyIndexApeSampleRate,
                    sizeof(m_adec_param.nSampleRate));
            memcpy(&m_adec_param.nSeekTablePresent, ptr + kKeyIndexApeSeekTablePresent,
                    sizeof(m_adec_param.nSeekTablePresent));

        }

        drv_config.compatibleVersion = m_adec_param.nCompatibleVersion;
        drv_config.compressionLevel = m_adec_param.nCompressionLevel;
        drv_config.formatFlags = m_adec_param.nFormatFlags;
        drv_config.blocksPerFrame = m_adec_param.nBlocksPerFrame;
        drv_config.finalFrameBlocks = m_adec_param.nFinalFrameBlocks;
        drv_config.totalFrames = m_adec_param.nTotalFrames;
        drv_config.bitsPerSample = m_adec_param.nBitsPerSample;
        drv_config.numChannels = m_adec_param.nChannels;
        drv_config.sampleRate = m_adec_param.nSampleRate;
        drv_config.seekTablePresent = m_adec_param.nSeekTablePresent;

        DEBUG_PRINT("sampleRate %d numChannels %d compatibleVersion %d compressionLevel %d formatFlags %d blocksPerFrame %d\
                    finalFrameBlocks %d totalFrames %d bitsPerSample %d seekTablePresent %d", drv_config.sampleRate,
                    drv_config.numChannels, drv_config.compatibleVersion, drv_config.compressionLevel,
                    drv_config.formatFlags, drv_config.blocksPerFrame, drv_config.finalFrameBlocks, drv_config.totalFrames,
                    drv_config.bitsPerSample, drv_config.seekTablePresent);

        ioctl(m_drv_fd, AUDIO_SET_APE_CONFIG, &drv_config);
        rc = ioctl(m_drv_fd, AUDIO_START, 0);

        if(rc < 0)
        {
            DEBUG_PRINT_ERROR("AUDIO_START FAILED for Handle %p with error %d\n", hComp, errno);
            m_first_ape_header= 0;
        ...
        else
        {
            ...
            {
                is_out_th_sleep = false;
                DEBUG_PRINT("ETBP:WAKING UP OUT THREADS\n");
                out_th_wakeup();
            }
            pthread_mutex_unlock(&m_out_th_lock_1);
            pthread_mutex_lock(&m_in_th_lock_1);
        ...
        if (buffer->nFlags & OMX_BUFFERFLAG_CODECCONFIG) {
            DEBUG_PRINT("DSP setup with CSD complete. Post EBD");
            buffer_done_cb((OMX_BUFFERHEADERTYPE *)bufHdr);
            return OMX_ErrorNone;
        }
    }
    ...
    {
         if (buffer->nFlags & OMX_BUFFERFLAG_CODECCONFIG) {
            DEBUG_PRINT("Received CSD from client again. Post EBD");
            buffer_done_cb((OMX_BUFFERHEADERTYPE *)bufHdr);
            return OMX_ErrorNone;
        }
    }
    ...
    // EOS support with dependency on Kernel and DSP
    if((bufHdr->nFlags & OMX_BUFFERFLAG_EOS))
    {
        if(new_length == 0)
        {
...
OMX_ERRORTYPE  COmxApeDec::fill_this_buffer_proxy (
        OMX_IN OMX_HANDLETYPE         hComp,
        OMX_BUFFERHEADERTYPE* buffer)
{
    int ret = 0;
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::fill_this_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_STATETYPE state;
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::set_callbacks(OMX_IN OMX_HANDLETYPE hComp,
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::component_deinit(OMX_IN OMX_HANDLETYPE hComp)
{
#if DUMPS_ENABLE
    if (bfd != -1)
        close(bfd);
    ...
    deinit_decoder();
    DEBUG_PRINT("COMPONENT DEINIT...\n");
    return OMX_ErrorNone;
}

/* ======================================================================
   FUNCTION
   COmxApeDec::deinit_decoder
   ...
   ========================================================================== */
void  COmxApeDec::deinit_decoder()
{
    ...
    if(is_in_th_sleep)
    {
        DEBUG_PRINT("Deinit:WAKING UP IN THREADS\n");
        in_th_wakeup();
        is_in_th_sleep = false;
    }
    pthread_mutex_unlock(&m_in_th_lock_1);
    ...
    if(is_out_th_sleep)
    {
        DEBUG_PRINT("Deinit:WAKING UP OUT THREADS\n");
        out_th_wakeup();
        is_out_th_sleep = false;
    }
    pthread_mutex_unlock(&m_out_th_lock_1);

    DEBUG_PRINT("Component-deinit calling omx_ape_thread_stop\
            (m_ipc_to_in_th)\n");
    if (m_ipc_to_in_th != NULL) {
        omx_ape_thread_stop(m_ipc_to_in_th);
        m_ipc_to_in_th = NULL;
    }
    ...
    if (m_ipc_to_cmd_th != NULL) {
        omx_ape_thread_stop(m_ipc_to_cmd_th);
        m_ipc_to_cmd_th = NULL;
    }

#ifdef NT_PAUSE_TIMEOUT_ENABLED
    DEBUG_PRINT("Component-deinit calling destroy_pause_timer\n");
    destroy_pause_timer();
#endif
    ...
    {
        DEBUG_PRINT_ERROR("ape device already closed\n");
    }
    ...
    if(pcm_feedback ==1)
    {
        ...
        if (m_ipc_to_out_th != NULL) {
            omx_ape_thread_stop(m_ipc_to_out_th);
            m_ipc_to_out_th = NULL;
        }
    }
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::use_EGL_image(OMX_IN OMX_HANDLETYPE hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                        port,
        OMX_IN OMX_PTR                     appData,
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxApeDec::component_role_enum(OMX_IN OMX_HANDLETYPE hComp,
        OMX_OUT OMX_U8*        role,
        OMX_IN OMX_U32        index)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    const char *cmp_role = "audio_decoder.ape";
...
/* ======================================================================
   ...
   ========================================================================== */
bool COmxApeDec::allocate_done(void)
{
...
/* ======================================================================
   ...
   ========================================================================== */
bool COmxApeDec::release_done(OMX_U32 param1)
{
    OMX_BOOL bRet = OMX_FALSE;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    if(param1 == OMX_ALL)
...
#ifdef NT_PAUSE_TIMEOUT_ENABLED
bool COmxApeDec::acquire_wakelock()
{
    bool status = true;
    int  ret;

    pthread_mutex_lock(&m_wakelock_lock);
    if(m_wakelock_voted) {
        DEBUG_PRINT("wake lock already hold\n");
        goto exit;
    }
    ret = ioctl(m_drv_fd, AUDIO_PM_AWAKE, 0);
    if (ret < 0) {
        status = false;
        DEBUG_PRINT_ERROR("fail to acquire wake lock\n");
    } else {
        m_wakelock_voted = true;
        DEBUG_PRINT("acquire wake lock successfully\n");
    }

exit:
    pthread_mutex_unlock(&m_wakelock_lock);
    return status;
}

bool COmxApeDec::release_wakelock()
{
    bool status = true;
    int  ret;
    ...
    pthread_mutex_lock(&m_wakelock_lock);
    if(!m_wakelock_voted) {
        DEBUG_PRINT("wake lock already released\n");
        goto exit;
    }

    ret = ioctl(m_drv_fd, AUDIO_PM_RELAX, 0);
    if (ret < 0) {
        status = false;
        DEBUG_PRINT_ERROR("fail to release wake lock\n");
    } else {
        m_wakelock_voted = false;
        DEBUG_PRINT("release wake lock successfully\n");
    }
...
void COmxApeDec::create_timer_thread() {
    int result;

    memset(&m_timer, 0, sizeof(struct timer));
    m_timer.info.expired = true;
    m_timer.info.kill_thread = false;
    m_timer.info.reset_timer = true;
    m_timer.pMe = this;

    pthread_attr_init(&m_timer.attr);
    pthread_attr_setdetachstate(&m_timer.attr, PTHREAD_CREATE_JOINABLE);
    pthread_mutex_init(&m_timer.mutex, NULL);
    if((pthread_cond_init(&m_timer.loop_cond, NULL) != 0)
        || (pthread_cond_init(&m_timer.kill_cond, NULL) != 0)) {
        DEBUG_PRINT_ERROR("pthread_cond_init fails\n");
        return;
    }

    result = pthread_create(&m_timer.timer_thread, &m_timer.attr,
                            pause_timeout_handler, &m_timer);
    if (result < 0) {
        DEBUG_PRINT_ERROR("Could not create timer thread!\n");
        return;
    }
}

void COmxApeDec::reset_pause_timer() {
    acquire_wakelock();

    pthread_mutex_lock(&m_timer.mutex);
    m_timer.info.expired = false;
    pthread_cond_signal(&m_timer.loop_cond);
    m_timer.info.reset_timer = true;
    pthread_mutex_unlock(&m_timer.mutex);
}

void COmxApeDec::destroy_pause_timer() {
    int result;

    m_timer.info.kill_thread = true;
    pthread_cond_signal(&m_timer.loop_cond);
    pthread_cond_signal(&m_timer.kill_cond);
    result = pthread_join(m_timer.timer_thread, NULL);
    if (result < 0) {
        DEBUG_PRINT_ERROR("Fail to join timer thread!\n");
        return;
    }
    pthread_mutex_destroy(&m_timer.mutex);
    pthread_cond_destroy(&m_timer.loop_cond);
    pthread_cond_destroy(&m_timer.kill_cond);
}

void *COmxApeDec::pause_timeout_handler(void *p_data) {
    struct timeval    now, last;
    struct timer      *p_ctxt = (struct timer *)p_data;
    struct timer_info *timer_info = (struct timer_info *)&(p_ctxt->info);
    struct timespec   ts;
    class COmxApeDec  *me = p_ctxt->pMe;

    gettimeofday(&last, NULL);

    while (!timer_info->kill_thread) {
        pthread_mutex_lock(&p_ctxt->mutex);
        gettimeofday(&now, NULL);
        int64_t diff = (int64_t)(now.tv_sec - last.tv_sec) * 1000
                        + (int64_t)(now.tv_usec - last.tv_usec) / 1000;
        if ((diff >= PAUSE_TIMEOUT) && (!timer_info->expired)) {
            pthread_mutex_lock(&me->m_audio_drive_lock);
            if (me->is_drv_paused() == OMX_FALSE) {
                ioctl(me->m_drv_fd, AUDIO_PAUSE, 1);
                me->set_drv_paused(OMX_TRUE);
                DEBUG_PRINT_ERROR("PAUSE TIMEOUT, suspend audio driver!\n");
            }
            pthread_mutex_unlock(&me->m_audio_drive_lock);
            me->release_wakelock();
            timer_info->expired = true;
        }

        if (timer_info->expired) {
            // suspend thread till signaled
            DEBUG_PRINT("suspend timer thread\n");
            pthread_cond_wait(&p_ctxt->loop_cond, &p_ctxt->mutex);
            DEBUG_PRINT("awake timer thread\n");
        }

        if (timer_info->reset_timer) {
            gettimeofday(&last, NULL);
            timer_info->reset_timer = false;
        }

        // idle for certain ms unless interrupted by kill sigal
        // that is to avoid MIPS cost in vain by intensive looping
        clock_gettime(CLOCK_REALTIME, &ts);
        ts.tv_nsec += 300 * 1000000; // 300ms
        pthread_cond_timedwait(&p_ctxt->kill_cond, &p_ctxt->mutex, &ts);
        pthread_mutex_unlock(&p_ctxt->mutex);
    }
...
void COmxApeDec::suspend_audio_driver(bool suspend) {
    if (m_drv_fd < 0) {
        DEBUG_PRINT_ERROR("invalid device handler\n");
        return;
    }

    pthread_mutex_lock(&m_audio_drive_lock);
    if ((is_drv_paused() == OMX_TRUE) && !suspend) {
        ioctl(m_drv_fd, AUDIO_PAUSE, 0);
        set_drv_paused(OMX_FALSE);
    }

    if ((is_drv_paused() == OMX_FALSE) && suspend) {
        ioctl(m_drv_fd, AUDIO_PAUSE, 1);
        set_drv_paused(OMX_TRUE);
    }
    pthread_mutex_unlock(&m_audio_drive_lock);
