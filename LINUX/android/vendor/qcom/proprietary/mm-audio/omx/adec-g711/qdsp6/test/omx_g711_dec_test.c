/*============================================================================
*
*@file omx_g711_dec_test.c
*This module contains an openamx test application for G711 component.
*
...
OMX_COMPONENTTYPE* g711_dec_handle = 0;
        ...
        case OMX_EventError:
            ...
            if(OMX_ErrorInvalidState == (OMX_ERRORTYPE)nData1)
            {
                ...
                for(bufCnt=0; bufCnt < input_buf_cnt; ++bufCnt)
                {
                    ...
                    OMX_FreeBuffer(g711_dec_handle, 0, pInputBufHdrs[bufCnt]);
                }
                if(tunnel == 0)
                {
                    for(bufCnt=0; bufCnt < output_buf_cnt; ++bufCnt)
                    {
                        ...
                        OMX_FreeBuffer(g711_dec_handle, 1, pOutputBufHdrs[bufCnt]);
                    }
                }
        ...
        OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    {
        DEBUG_PRINT("omx_g711_adec_test: bFlushing is set to TRUE \
                used_ip_buf_cnt=%d\n",used_ip_buf_cnt);
        if (used_ip_buf_cnt == 0)
        {
            ...
            DEBUG_PRINT("omx_g711_adec_test: more buffer to come back\n");
            return OMX_ErrorNone;
        }
    }
...
void signal_handler(int sig_id) {
    ...
    if (sig_id == SIGUSR1)
    {
        ...
        bFlushing = true;
        OMX_SendCommand(g711_dec_handle, OMX_CommandFlush, OMX_ALL, NULL);
    } else if (sig_id == SIGUSR2) {
        if (bPause == true) {
            DEBUG_PRINT("%s resume playback\n", __FUNCTION__);
            bPause = false;
            OMX_SendCommand(g711_dec_handle, OMX_CommandStateSet,
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        DEBUG_PRINT("ex: mm_adec_omxg711_test INPUTFILE  \
                TUNNEL FILEWRITE OUTPUTFILE\n");
    ...
    if(tunnel == 0)
    {
        if(decode_format == FORMAT_MULAW)
        {
            aud_comp = "OMX.qcom.audio.decoder.g711mlaw";
        }
        else {
            aud_comp = "OMX.qcom.audio.decoder.g711alaw";
        }
    } else {
        if(decode_format == FORMAT_MULAW)
        {
            aud_comp = "OMX.qcom.audio.decoder.tunneled.g711mlaw";
        }
        else {
            aud_comp = "OMX.qcom.audio.decoder.tunneled.g711alaw";
        }
    }
    ...
    if(bOutputEosReached || (tunnel && bInputEosReached))
    {
        ...
        {
            sleep(1);
            if(0>ioctl(m_pcmdrv_fd, AUDIO_STOP, 0))
                DEBUG_PRINT("%s:%d:AUDIO_STOP failed:\n",__func__,__LINE__);
            if(m_pcmdrv_fd >= 0)
            {
                close(m_pcmdrv_fd);
                m_pcmdrv_fd = -1;
        ...
        OMX_SendCommand(g711_dec_handle,
        ...
        result = OMX_FreeHandle(g711_dec_handle);
        if (result != OMX_ErrorNone)
        {
            DEBUG_PRINT("\nOMX_FreeHandle error. Error code: %d\n", result);
        }
        g711_dec_handle = NULL;
        /* Deinit OpenMAX */
        OMX_Deinit();
...
int Init_Decoder(OMX_STRING audio_component)
{
    ...
    omxresult = OMX_GetHandle((OMX_HANDLETYPE*)(&g711_dec_handle),
    ...
    omxresult = OMX_GetParameter(g711_dec_handle, OMX_IndexParamAudioInit,
            (OMX_PTR)&portParam);
...
int Play_Decoder()
{
    ...
    OMX_GetParameter(g711_dec_handle,OMX_IndexParamPortDefinition,
            &inputportFmt);
    ...
    OMX_SetParameter(g711_dec_handle,OMX_IndexParamPortDefinition,
            &inputportFmt);
    ...
    OMX_GetExtensionIndex(g711_dec_handle,"OMX.Qualcomm.index.audio.sessionId",
            &index);
    OMX_GetParameter(g711_dec_handle,index,&streaminfoparam);
    CONFIG_VERSION_SIZE(g711param);
    g711param.nPortIndex   =  0;
    g711param.nChannels    =  channels;  /* 1-> mono */
    g711param.nSamplingRate = samplerate;
    OMX_SetParameter(g711_dec_handle,OMX_IndexParamAudioG711,&g711param);
    DEBUG_PRINT("\nOMX_SendCommand Decoder -> IDLE\n");
    OMX_SendCommand(g711_dec_handle, OMX_CommandStateSet, OMX_StateIdle,0);
    ...
    if (tunnel == 1)
    {
        ...
        OMX_SendCommand(g711_dec_handle, OMX_CommandPortDisable,1,0);
        wait_for_event();
    }

    DEBUG_PRINT("\nOMX_SendCommand Decoder -> Executing\n");
    OMX_SendCommand(g711_dec_handle, OMX_CommandStateSet, OMX_StateExecuting,0);
    wait_for_event();
    ...
    if(tunnel == 0)
    {
        ...
        for(i=0; i < output_buf_cnt; i++) {
            ...
            if (pOutputBufHdrs[i] != NULL)
            {
                ...
                ret = OMX_FillThisBuffer(g711_dec_handle, pOutputBufHdrs[i]);
    ...
    for (i = 0;i < input_buf_cnt;i++)
    {
        ...
        else
        {
            ...
            ret = OMX_EmptyThisBuffer(g711_dec_handle, pInputBufHdrs[i]);
            if (OMX_ErrorNone != ret)
            {
        ...
        unsigned int bufCntMin, unsigned int bufSize)
{
    ...
    *pBufHdrs= (OMX_BUFFERHEADERTYPE **)
    ...
    for(bufCnt=0; bufCnt < bufCntMin; ++bufCnt) {
        ...
        error = OMX_AllocateBuffer(g711_dec_handle, &((*pBufHdrs)[bufCnt]),
        ...
        unsigned int bufCntMin, unsigned int bufSize)
{
    ...
    *pBufHdrs= (OMX_BUFFERHEADERTYPE **)
    ...
    for(bufCnt=0; bufCnt < bufCntMin; ++bufCnt) {
        ...
        error = OMX_UseBuffer(g711_dec_handle, &((*pBufHdrs)[bufCnt]),
...
static int parse_g711_header ()
{
    ...
    /* parsing input file*/
    if(fread(&g711hdr, 1, sizeof(g711hdr),inputBufferFile)!=sizeof(g711hdr))
    {
        DEBUG_PRINT("can't read g711 file header\n");
        return -1;
    }

    if ((g711hdr.audio_format != FORMAT_MULAW) && (g711hdr.audio_format != FORMAT_ALAW))
    {
        DEBUG_PRINT("g711 file is not MULAW or ALAW format it's format is %d ",
                      g711hdr.audio_format);
        return -1;
    }
    ...
    if (g711hdr.num_channels != 1) {
          DEBUG_PRINT("stereo and multi channel are not supported, channels %d"
                      , g711hdr.num_channels);
        return -1;
    }

    DEBUG_PRINT("Samplerate is %d\n", g711hdr.sample_rate);
    DEBUG_PRINT("Channel Count is %d\n", g711hdr.num_channels);
    DEBUG_PRINT("Format is %d\n", g711hdr.audio_format);
    DEBUG_PRINT("\n***************************************************************\n");

    samplerate = g711hdr.sample_rate;
    channels = g711hdr.num_channels;
    decode_format = g711hdr.audio_format;
    return 0;
}
