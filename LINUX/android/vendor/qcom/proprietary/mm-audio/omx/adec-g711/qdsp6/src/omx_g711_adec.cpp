...
{
    return (new COmxG711Dec);
}

// omx_cmd_queue destructor
COmxG711Dec::omx_cmd_queue::~omx_cmd_queue()
{
    // Nothing to do
}

// omx cmd queue constructor
COmxG711Dec::omx_cmd_queue::omx_cmd_queue(): m_read(0),m_write(0),m_size(0)
{
    memset(m_q, 0, sizeof(omx_event)*OMX_CORE_CONTROL_CMDQ_SIZE);
}

// omx cmd queue insert
bool COmxG711Dec::omx_cmd_queue::insert_entry(unsigned long p1,
...
bool COmxG711Dec::omx_cmd_queue::pop_entry(unsigned long *p1,
        unsigned long *p2, unsigned char *id)
{
    bool ret = true;
    if (m_size > 0)
...
bool COmxG711Dec::omx_cmd_queue::get_msg_id(unsigned char *id)
{
    if(m_size > 0)
    {
        *id = m_q[m_read].id;
...
void COmxG711Dec::wait_for_event()
{
    int rc;
    struct timespec ts;
...
void COmxG711Dec::event_complete()
{
    pthread_mutex_lock(&m_event_lock);
...
void COmxG711Dec::out_th_wakeup()
{
    DEBUG_DETAIL("%s",__FUNCTION__);
    pthread_mutex_lock(&m_out_th_lock);
...
void COmxG711Dec::wait_for_flush_event()
{
...
void COmxG711Dec::event_flush_complete()
{
    pthread_mutex_lock(&m_flush_cmpl_lock);
...
void COmxG711Dec::process_event_cb(void *client_data, __unused unsigned char id)
{
    DEBUG_PRINT("PE:Waiting for event's...");
    COmxG711Dec  *pThis = (COmxG711Dec *) client_data;
    pThis->process_events(pThis);
}

void COmxG711Dec::process_events(__unused COmxG711Dec *client_data)
{
    int rc = 0;
    ...
    while(1)
    {
        ...
        if(errno == ENETRESET)
        {
            ...
            m_cb.EventHandler(&m_cmp,m_app_data,OMX_EventError,OMX_ErrorHardware,
                    0, NULL );
        }
        else if(rc < 0)
        {
...
/* ======================================================================
   ...
   None.
   ========================================================================== */
COmxG711Dec::COmxG711Dec(): m_volume(25),
    m_app_data(NULL),
    m_eos_bm(0),
    m_flush_cnt(0),
    m_first_g711_header(0),
    m_is_alloc_buf(0),
    drv_inp_buf_cnt(0),
    drv_out_buf_cnt(0),
    nNumInputBuf(0),
    ...
    pcm_feedback(1),
    g711alaw(0),
    output_buffer_size (OMX_G711_OUTPUT_BUFFER_SIZE),
...
COmxG711Dec::~COmxG711Dec()
{
    ...
    DEBUG_PRINT_ERROR("OMX G711 component destroyed\n");
    return;
}

bool COmxG711Dec::aio_write(msm_audio_aio_buf *audio_aio_buf)
{
    bool rc = true;
...
bool COmxG711Dec::aio_read(msm_audio_aio_buf *audio_aio_buf)
{
    bool rc = true;
...
bool COmxG711Dec::prepare_for_ebd(OMX_BUFFERHEADERTYPE *bufHdr)
{
    bool rc = true;
    ...
    if (bufHdr->nFlags & OMX_BUFFERFLAG_EOS )
    {
        if (bufHdr->nFilledLen)
        {
            ...
            /* Asynchronous write call to the G711 driver */
...
bool COmxG711Dec::audio_register_ion(struct mmap_info *ion_buf)
{
    bool rc = true;
    struct msm_audio_ion_info audio_ion_buf;
    /* Register the mapped ION buffer with the G711 driver */
...
bool COmxG711Dec::audio_deregister_ion(struct mmap_info *ion_buf)
{
    bool rc = true;
    struct msm_audio_ion_info audio_ion_buf;
    if (!ion_buf)
...
/**
  ...
  @return none
 */
void COmxG711Dec::buffer_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
...
void COmxG711Dec::frame_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
    META_OUT *pmeta_out = NULL;
    DEC_META_OUT *dec_meta_out = NULL;
    OMX_BUFFERHEADERTYPE *buffer = bufHdr;
...
void COmxG711Dec::process_out_port_msg(void *client_data, unsigned char id)
{
    unsigned long p1;
    unsigned long p2;
    unsigned char ident;
...
void COmxG711Dec::process_command_msg(void *client_data, unsigned char id)
{
    unsigned long p1;
    unsigned long p2;
    unsigned char ident;
...
void COmxG711Dec::process_in_port_msg(void *client_data, unsigned char id)
{
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE COmxG711Dec::component_init(OMX_STRING role)
{
    ...
    /* Open the G711 device in asynchronous mode */
    if(!strcmp(role,"OMX.qcom.audio.decoder.g711mlaw"))
    {
        pcm_feedback = 1;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    }
    else if(!strcmp(role,"OMX.qcom.audio.decoder.tunneled.g711mlaw"))
    {
        pcm_feedback = 0;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    }
    else if(!strcmp(role,"OMX.qcom.audio.decoder.g711alaw"))
    {
        pcm_feedback = 1;
        g711alaw = 1;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    }
    else if(!strcmp(role,"OMX.qcom.audio.decoder.tunneled.g711alaw"))
    {
        pcm_feedback = 0;
        g711alaw = 1;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    ...
    if(g711alaw)
    {
        if(0 == pcm_feedback)
        {
            m_drv_fd = open("/dev/msm_g711alaw",O_WRONLY | O_NONBLOCK);
        }
        else
        {
            m_drv_fd = open("/dev/msm_g711alaw",O_RDWR | O_NONBLOCK);
        }
    }
    else {
        if(0 == pcm_feedback)
        {
            m_drv_fd = open("/dev/msm_g711mlaw",O_WRONLY | O_NONBLOCK);
        }
        else
        {
            m_drv_fd = open("/dev/msm_g711mlaw",O_RDWR | O_NONBLOCK);
        }
    }
    ...
    if(!m_ipc_to_in_th)
    {
        m_ipc_to_in_th = omx_g711_thread_create(process_in_port_msg, this,
                (char *) "INPUT_THREAD");
        if(!m_ipc_to_in_th)
        {
    ...
    if(!m_ipc_to_cmd_th)
    {
        m_ipc_to_cmd_th = omx_g711_thread_create(process_command_msg, this,
                (char *) "COMMAND_THREAD");
        if(!m_ipc_to_cmd_th)
        {
    ...
    if(pcm_feedback)
    {
        if(!m_ipc_to_out_th)
        {
            m_ipc_to_out_th = omx_g711_thread_create(process_out_port_msg, this,
                    (char *) "OUTPUT_THREAD");
            if(!m_ipc_to_out_th)
            {
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxG711Dec::get_component_version(__unused OMX_IN OMX_HANDLETYPE hComp,
        __unused OMX_OUT OMX_STRING          componentName,
        __unused OMX_OUT OMX_VERSIONTYPE* componentVersion,
        __unused OMX_OUT OMX_VERSIONTYPE*      specVersion,
        __unused OMX_OUT OMX_UUIDTYPE*       componentUUID)
{
    if(m_state == OMX_StateInvalid)
    {
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxG711Dec::send_command(__unused OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_COMMANDTYPE  cmd,
        OMX_IN OMX_U32       param1,
        __unused OMX_IN OMX_PTR      cmdData)
{
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxG711Dec::send_command_proxy(__unused OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_COMMANDTYPE  cmd,
        OMX_IN OMX_U32       param1,
        __unused OMX_IN OMX_PTR      cmdData)
{
    ...
    if(cmd == OMX_CommandStateSet)
    {
        ...
        else if(m_state == OMX_StateIdle)
        {
            if(eState == OMX_StateLoaded)
            {
                if(release_done(OMX_ALL))
                {
                    ...
                    {
                        omx_g711_thread_stop(m_ipc_to_event_th);
                        m_ipc_to_event_th = NULL;
                    }
            ...
            else if(eState == OMX_StateExecuting)
            {
                if(!m_ipc_to_event_th)
                {
                    ...
                    m_ipc_to_event_th = omx_g711_event_thread_create(
        ...
        else if(m_state == OMX_StatePause)
        {
            ...
            if(eState == OMX_StateExecuting)
            {
                ...
                /* Issue resume to driver to resume tunnel mode playback */
                if(!pcm_feedback)
                {
 ...
 */
bool COmxG711Dec::execute_omx_flush(OMX_IN OMX_U32 param1, bool cmd_cmpl)
{
    bool bRet = true;
    ...
    if (param1 == OMX_ALL)
    {
        ...
        while (1 )
        {
            ...
            if(drv_inp_buf_cnt > 0 || drv_out_buf_cnt > 0)
            {
                ...
                if(ioctl( m_drv_fd, AUDIO_FLUSH, 0) == -1)
                    DEBUG_PRINT("FLush:ioctl flush failed errno=%d\n",errno);
                // Polling for flush for every 10ms causing issue in SSR case where driver  2348
...
/**
  ...
  @return bool value indicating whether flushing is carried out successfully
 */
bool COmxG711Dec::execute_input_omx_flush()
{
...
/**
  ...
  @return bool value indicating whether flushing is carried out successfully
 */
bool COmxG711Dec::execute_output_omx_flush()
{
    OMX_BUFFERHEADERTYPE *omx_buf;
    unsigned long      p1=0; // Parameter - 1
    unsigned long      p2=0; // Parameter - 2
 ...
 */
bool COmxG711Dec::post_input(unsigned long p1,
        unsigned long p2,
        unsigned char id)
{
    bool bRet = false;
...
bool COmxG711Dec::post_command(unsigned int p1,
        unsigned int p2,
        unsigned char id)
{
    bool bRet = false;
...
bool COmxG711Dec::post_output(unsigned long p1,
        unsigned long p2,
        unsigned char id)
{
    bool bRet = false;
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxG711Dec::get_parameter(__unused OMX_IN OMX_HANDLETYPE hComp,
        ...
        OMX_INOUT OMX_PTR     paramData)
{
    ...
    switch((int)paramIndex)
    {
        ...
        case OMX_IndexParamAudioG711:
            {
                OMX_AUDIO_PARAM_G711TYPE *g711Param =
                    (OMX_AUDIO_PARAM_G711TYPE *) paramData;
                DEBUG_PRINT("OMX_IndexParamAudiog711\n");

                if (OMX_CORE_INPUT_PORT_INDEX== g711Param->nPortIndex)
                {
                    *g711Param = m_adec_param;
                }
                else
                {
                    DEBUG_PRINT_ERROR("get_parameter:OMX_IndexParamAudioG711 \
                            OMX_ErrorBadPortIndex %d\n",
                            (int)g711Param->nPortIndex);
                    eRet = OMX_ErrorBadPortIndex;
                }
                break;
            }
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxG711Dec::set_parameter(__unused OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_INDEXTYPE paramIndex,
        OMX_IN OMX_PTR        paramData)
{
    ...
    switch((int)paramIndex)
    {
        case OMX_IndexParamAudioG711:
            {
                m_adec_param = *((OMX_AUDIO_PARAM_G711TYPE *) paramData);
                break;
            }
        case OMX_IndexParamPortDefinition:
            {
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::get_config(OMX_IN OMX_HANDLETYPE      hComp,
...
/* ======================================================================
   ...
   OMX Error None if successful.
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::set_config(OMX_IN OMX_HANDLETYPE      hComp,
        OMX_IN OMX_INDEXTYPE configIndex,
...
None
=============================================================================*/
OMX_ERRORTYPE  COmxG711Dec::get_extension_index(
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::get_state(OMX_IN OMX_HANDLETYPE  hComp,
...
/* ======================================================================
   FUNCTION
   COmxG711Dec::ComponentTunnelRequest
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::component_tunnel_request(__unused OMX_IN OMX_HANDLETYPE hComp,
        __unused OMX_IN OMX_U32                        port,
        __unused OMX_IN OMX_HANDLETYPE        peerComponent,
        __unused OMX_IN OMX_U32                    peerPort,
        __unused OMX_INOUT OMX_TUNNELSETUPTYPE* tunnelSetup)
{
    DEBUG_PRINT_ERROR("Error: component_tunnel_request Not Implemented\n");
    return OMX_ErrorNotImplemented;
}

void* COmxG711Dec::alloc_ion_buffer(unsigned int bufsize)
{
...
void COmxG711Dec::free_ion_buffer(void** ion_buffer)
{
...
/* ======================================================================
   FUNCTION
   COmxG711Dec::AllocateInputBuffer
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::allocate_input_buffer(__unused OMX_IN OMX_HANDLETYPE hComp,
        ...
        OMX_IN OMX_U32                   bytes)
{
    ...
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxG711Dec::allocate_input_buffer");
...
OMX_ERRORTYPE  COmxG711Dec::allocate_output_buffer(__unused OMX_IN OMX_HANDLETYPE hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        __unused OMX_IN OMX_U32               port,
        OMX_IN OMX_PTR                     appData,
        OMX_IN OMX_U32                       bytes)
{
    ...
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxG711Dec::allocate_output_buffer");
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::allocate_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                        port,
        OMX_IN OMX_PTR                     appData,
        OMX_IN OMX_U32                       bytes)
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::use_buffer(
        OMX_IN OMX_HANDLETYPE            hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                   port,
        OMX_IN OMX_PTR                   appData,
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::use_input_buffer(
        ...
        OMX_IN OMX_U8*                   buffer)
{
    ...
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxG711Dec::use_input_buffer");
    if(bytes < input_buffer_size)
    {
        /* return if i\p buffer size provided by client is less
           than min i\p buffer size supported by omx component */
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::use_output_buffer(
        __unused OMX_IN OMX_HANDLETYPE   hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        __unused OMX_IN OMX_U32          port,
        OMX_IN OMX_PTR                   appData,
        ...
        OMX_IN OMX_U8*                   buffer)
{
    ...
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxG711Dec::use_output_buffer");
    if(bytes < output_buffer_size)
    {
        /* return if o\p buffer size provided by client is less
           than min o\p buffer size supported by omx component */
    ...
    if(m_out_current_buf_count < m_out_act_buf_count)
    {
        ...
        if (bufHdr != NULL && loc_bufHdr != NULL)
        {
            ...
            /* Register the mapped PMEM buffer with the G711 driver */
            if (!audio_register_ion(mmap_data))
            {
                free(bufHdr);
                free(loc_bufHdr);
 ...
 */
bool COmxG711Dec::search_input_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
    bool eRet = false;
...
/**
  ...
  @return bool value indicating whether buffer is found
 */
bool COmxG711Dec::search_output_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
    bool eRet = false;
    OMX_BUFFERHEADERTYPE *temp = NULL;
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxG711Dec::free_buffer(__unused OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_U32                 port,
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxG711Dec::empty_this_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_STATETYPE state;
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    ...
    if ( buffer != NULL )
        buffer->nVersion.nVersion = OMX_SPEC_VERSION; //temporary
    if ( (buffer != NULL) &&
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxG711Dec::empty_this_buffer_proxy(OMX_IN OMX_HANDLETYPE hComp,
        OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if(m_first_g711_header == 0)
    {
        m_first_g711_header = 1;
        if (pcm_feedback)
        {
            struct msm_audio_config config;
            config.sample_rate =
                m_adec_param.nSamplingRate;
            config.channel_count = (OMX_U16)m_adec_param.nChannels;
            if(0 > ioctl(m_drv_fd, AUDIO_SET_CONFIG, &config))
                DEBUG_PRINT("%s:%d:AUDIO_SET_CONFIG failed\n",__func__,__LINE__);
        }
        struct msm_audio_g711_dec_config drv_config;
        if (0> ioctl(m_drv_fd, AUDIO_GET_G711_DEC_CONFIG, &drv_config))
            DEBUG_PRINT("%s:%d:AUDIO_GET_G711_DEC_CONFIG failed\n",__func__,__LINE__);

        drv_config.sample_rate = m_adec_param.nSamplingRate;
        if(0> ioctl(m_drv_fd, AUDIO_SET_G711_DEC_CONFIG, &drv_config))
            DEBUG_PRINT("%s:%d:AUDIO_SET_G711_CONFIG failed\n",__func__,__LINE__);
...
OMX_ERRORTYPE  COmxG711Dec::fill_this_buffer_proxy (
        __unused OMX_IN OMX_HANDLETYPE         hComp,
        OMX_BUFFERHEADERTYPE* buffer)
{
    int ret = 0;
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::fill_this_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_STATETYPE state;
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::set_callbacks(__unused OMX_IN OMX_HANDLETYPE hComp,
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::component_deinit(__unused OMX_IN OMX_HANDLETYPE hComp)
{
#if DUMPS_ENABLE
    if (bfd != -1)
        close(bfd);
...
/* ======================================================================
   ...
   ========================================================================== */
void  COmxG711Dec::deinit_decoder()
{
    ...
    DEBUG_PRINT("Component-deinit calling omx_g711_thread_stop\
            (m_ipc_to_in_th)\n");
    ...
    {
        DEBUG_PRINT_ERROR(" g711 device already closed\n");
    }
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::use_EGL_image(__unused OMX_IN OMX_HANDLETYPE hComp,
        __unused OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        __unused OMX_IN OMX_U32                        port,
        __unused OMX_IN OMX_PTR                     appData,
        __unused OMX_IN void*                      eglImage)
{
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxG711Dec::component_role_enum(__unused OMX_IN OMX_HANDLETYPE hComp,
        OMX_OUT OMX_U8*        role,
        OMX_IN OMX_U32        index)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    const char *cmp_role = "audio_decoder.g711";
...
/* ======================================================================
   ...
   ========================================================================== */
bool COmxG711Dec::allocate_done(void)
{
...
/* ======================================================================
   ...
   ========================================================================== */
bool COmxG711Dec::release_done(OMX_U32 param1)
{
    OMX_BOOL bRet = OMX_FALSE;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    if(param1 == OMX_ALL)
...
void COmxG711Dec::suspend_audio_driver(bool suspend) {
    if (m_drv_fd < 0) {
    ...
    if ((is_drv_paused() == OMX_TRUE) && !suspend) {
        if (0> ioctl(m_drv_fd, AUDIO_PAUSE, 0))
