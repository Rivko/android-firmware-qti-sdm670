                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    {
        DEBUG_PRINT("omx_evrc_adec_test: bFlushing is set to TRUE used_ip_buf_cnt=%d\n",used_ip_buf_cnt);
...
int main(int argc, char **argv)
{
    ...
    if(tunnel == 0)
    {
        aud_comp = "OMX.qcom.audio.decoder.evrc";
    }
    else
    {
...
int Init_Decoder(OMX_STRING audio_component)
{
    ...
    OMX_STRING  role ="audio_decoder.evrc";
    ...
    OMX_STRING audiocomponent ="OMX.qcom.audio.decoder.evrc";
    int componentfound = false;

    if(tunnel == 1)
    {
        audiocomponent = "OMX.qcom.audio.decoder.tunneled.evrc";
    }
    ...
    if(total)
    {
        ...
        memset(audCompNames, 0, (sizeof(OMX_U8))*total);
        for (i = 0; i < total; ++i)
        {
            audCompNames[i] =
            ...
            memset(audCompNames[i], 0, sizeof(OMX_U8)*OMX_MAX_STRINGNAME_SIZE);
        }
...
int Play_Decoder()
{
    ...
    OMX_GetParameter(evrc_dec_handle,
   ...
   OMX_SetParameter(evrc_dec_handle,OMX_IndexParamPortDefinition,&inputportFmt);
    ...
    OMX_SetParameter(evrc_dec_handle,
        OMX_IndexParamAudioEvrc,&evrcparam);
    ...
    error = Allocate_Buffer(evrc_dec_handle, &pInputBufHdrs,
    ...
    if(tunnel == 0)
    {
        ...
        error = Allocate_Buffer(evrc_dec_handle, &pOutputBufHdrs,
...
static int open_audio_file ()
{
    ...
    if((tunnel == 0) && (filewrite == 1))
    {
        ...
        outputBufferFile = fopen("Audio_evrc.wav","wb");
