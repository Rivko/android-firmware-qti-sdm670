...
{
  return (new COmxAmrwbplusDec);
}

// omx_cmd_queue destructor
COmxAmrwbplusDec::omx_cmd_queue::~omx_cmd_queue()
{
  // Nothing to do
}

// omx cmd queue constructor
COmxAmrwbplusDec::omx_cmd_queue::omx_cmd_queue(): m_read(0),m_write(0),m_size(0)
{
  memset(m_q, 0, sizeof(omx_event)*OMX_CORE_CONTROL_CMDQ_SIZE);
}

// omx cmd queue insert
bool COmxAmrwbplusDec::omx_cmd_queue::insert_entry(unsigned long p1,
...
bool COmxAmrwbplusDec::omx_cmd_queue::pop_entry(unsigned long *p1,
    unsigned long *p2, unsigned char *id)
{
    bool ret = true;
    if (m_size > 0)
...
bool COmxAmrwbplusDec::omx_cmd_queue::get_msg_id(unsigned char *id)
{
    if(m_size > 0)
    {
        *id = m_q[m_read].id;
...
void COmxAmrwbplusDec::wait_for_event()
{
  int               rc;
  struct timespec   ts;
...
void COmxAmrwbplusDec::event_complete()
{
  pthread_mutex_lock(&m_event_lock);
...
void COmxAmrwbplusDec::in_th_sleep()
{
...
void COmxAmrwbplusDec::in_sleep()
{
...
void COmxAmrwbplusDec::in_th_wakeup()
{
  DEBUG_PRINT("%s",__FUNCTION__);
  pthread_mutex_lock(&m_in_th_lock);
...
void COmxAmrwbplusDec::out_th_sleep()
{
  DEBUG_PRINT("%s",__FUNCTION__);
...
void COmxAmrwbplusDec::out_sleep()
{
...
void COmxAmrwbplusDec::out_th_wakeup()
{
  DEBUG_PRINT("%s",__FUNCTION__);
  pthread_mutex_lock(&m_out_th_lock);
...
void COmxAmrwbplusDec::wait_for_flush_event()
{
...
void COmxAmrwbplusDec::event_flush_complete()
{
  pthread_mutex_lock(&m_flush_cmpl_lock);
...
void COmxAmrwbplusDec::process_event_cb(void *client_data, unsigned char id)
{
  /* To remove warning for unused variable to keep prototype same */
  (void)id;
  DEBUG_PRINT("PE:Waiting for event's...");
  COmxAmrwbplusDec  *pThis = (COmxAmrwbplusDec *) client_data;
  if(pThis) {
      pThis->process_events(pThis);
  }
}

void COmxAmrwbplusDec::process_events(COmxAmrwbplusDec *client_data)
{
  int rc                        = 0;
    ...
    switch(drv_event.event_type)
    {
      ...
      case AUDIO_EVENT_WRITE_DONE:
      {
        ...
        if(pcm_feedback)
        {
            ...
            (unsigned int)(drv_event.event_payload.aio_buf.data_len - sizeof(META_IN));
        }
        bufHdr = (OMX_BUFFERHEADERTYPE*)drv_event.event_payload.aio_buf.private_data;
        if(bufHdr)
        {
          bufHdr->nFilledLen = drv_event.event_payload.aio_buf.data_len;
          DEBUG_PRINT("W-D event, bufHdr[%p] buf\
                      [%p] len[0x%x]", bufHdr,
                      bufHdr->pBuffer,
                      (unsigned int)bufHdr->nFilledLen);
          post_input((unsigned long) &m_cmp, (unsigned long)bufHdr, OMX_COMPONENT_GENERATE_BUFFER_DONE);
      ...
      case AUDIO_EVENT_READ_DONE:
      {
        ...
        if(bufHdr)
        {
          if(drv_event.event_payload.aio_buf.data_len > sizeof(META_OUT)) {
              drv_event.event_payload.aio_buf.data_len =
              (unsigned int)(drv_event.event_payload.aio_buf.data_len - sizeof(META_OUT));
              ...
              if(!bufHdr->nFilledLen)
                ...
                if(pfd != NULL)
                {
                  fwrite((bufHdr->pBuffer + bufHdr->nOffset),1,bufHdr->nFilledLen,pfd);
                }
#endif
          } else {
          ...
          post_output((unsigned long) &m_cmp, (unsigned long)bufHdr, OMX_COMPONENT_GENERATE_FRAME_DONE);
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::COmxAmrwbplusDec
...
None.
========================================================================== */
COmxAmrwbplusDec::COmxAmrwbplusDec(): m_volume(25),
m_app_data(NULL),
m_eos_bm(0),
m_flush_cnt(0),
m_first_amrwbplus_header(0),
m_is_alloc_buf(0),
drv_inp_buf_cnt(0),
drv_out_buf_cnt(0),
nNumInputBuf(0),
...
nTimestamp(0),
pcm_feedback(1),
output_buffer_size (OMX_AMRWBPLUS_OUTPUT_BUFFER_SIZE),
...
COmxAmrwbplusDec::~COmxAmrwbplusDec()
{
    ...
    DEBUG_PRINT("OMX AMRWBPLUS component destroyed\n");
    return;
}

bool COmxAmrwbplusDec::aio_write(msm_audio_aio_buf *audio_aio_buf)
{
    bool rc = true;
    ...
    if(bfd != NULL)
    {
      fwrite(((char *)audio_aio_buf->buf_addr + sizeof(META_IN)),1,(audio_aio_buf->data_len - sizeof(META_IN)),bfd);
    }
#endif
    return rc;

}
bool COmxAmrwbplusDec::aio_read(msm_audio_aio_buf *audio_aio_buf)
{
    bool rc = true;
...
bool COmxAmrwbplusDec::prepare_for_ebd(OMX_BUFFERHEADERTYPE *bufHdr)
{
    bool rc = true;
    ...
    if (bufHdr->nFlags & OMX_BUFFERFLAG_EOS )
    {
        if (bufHdr->nFilledLen)
        {
            ...
            /* Asynchronous write call to the AMRWBPLUS driver */
            ...
            if (pcm_feedback)
            {
                ...
                if(pmeta_in)
                {
                    ...
                    pmeta_in->nTimeStamp.HighPart =
                        (unsigned int) (((((OMX_BUFFERHEADERTYPE*)bufHdr)->nTimeStamp) >> 32) & 0xFFFFFFFF);

                    pmeta_in->nFlags = bufHdr->nFlags;
                    audio_aio_buf.buf_addr = (OMX_U8*)pmeta_in;
                    audio_aio_buf.data_len = bufHdr->nFilledLen + (unsigned int)sizeof(META_IN);
            ...
            {
                pthread_mutex_lock(&in_buf_count_lock);
                DEBUG_PRINT_ERROR("ERROR: aio_write failed");
                drv_inp_buf_cnt--;
                pthread_mutex_unlock(&in_buf_count_lock);
...
bool COmxAmrwbplusDec::audio_register_ion(struct mmap_info *ion_buf)
{
    bool rc = true;
    struct msm_audio_ion_info audio_ion_buf;
    /* Register the mapped ION buffer with the AMR-WB+ driver */
...
bool COmxAmrwbplusDec::audio_deregister_ion(struct mmap_info *ion_buf)
{
    bool rc = true;
    struct msm_audio_ion_info audio_ion_buf;
    if (!ion_buf)
...
@return none
*/
void COmxAmrwbplusDec::buffer_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
...
void COmxAmrwbplusDec::flush_ack()
{
    int lcnt = 0;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
...
void COmxAmrwbplusDec::frame_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
    META_OUT *pmeta_out = NULL;
    DEC_META_OUT *dec_meta_out = NULL;
    OMX_BUFFERHEADERTYPE *buffer = bufHdr;
...
void COmxAmrwbplusDec::process_out_port_msg(void *client_data, unsigned char id)
{
    unsigned long p1;
    unsigned long p2;
    unsigned char ident;
...
void COmxAmrwbplusDec::process_command_msg(void *client_data, unsigned char id)
{
    unsigned long p1;
    unsigned long p2;
    unsigned char ident;
...
void COmxAmrwbplusDec::process_in_port_msg(void *client_data, unsigned char id)
{
...
@return Error status
*/
OMX_ERRORTYPE COmxAmrwbplusDec::component_init(OMX_STRING role)
{
    ...
    /* Open the AMR-WB+ device in asynchronous mode */
    if(!strcmp(role,"OMX.qcom.audio.decoder.amrwbplus"))
    {
        pcm_feedback = 1;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    }
    else if(!strcmp(role,"OMX.qcom.audio.decoder.tunneled.amrwbplus"))
    {
        pcm_feedback = 0;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    }
    ...
    if(0 == pcm_feedback)
    {
        DEBUG_PRINT("opening msm_amrwbplus driver in tunneled mode");
        m_drv_fd = open("/dev/msm_amrwbplus",O_WRONLY | O_NONBLOCK);//TO DO;Check what should be used to open the driver
    }
    else
    {
        DEBUG_PRINT("opening msm_amrwbplus driver in non-tunneled mode");
        m_drv_fd = open("/dev/msm_amrwbplus",O_RDWR | O_NONBLOCK);//TO DO;Check what should be used to open the driver
    }
    ...
    if(!m_ipc_to_in_th)
    {
        m_ipc_to_in_th = omx_amrwbplus_thread_create(process_in_port_msg, this, (char *) "INPUT_THREAD");
        if(!m_ipc_to_in_th)
        {
    ...
    if(!m_ipc_to_cmd_th)
    {
        m_ipc_to_cmd_th = omx_amrwbplus_thread_create(process_command_msg, this, (char *) "COMMAND_THREAD");
        if(!m_ipc_to_cmd_th)
        {
    ...
    if(pcm_feedback)
    {
        if(!m_ipc_to_out_th)
        {
            m_ipc_to_out_th = omx_amrwbplus_thread_create(process_out_port_msg, this, (char *) "OUTPUT_THREAD");
            if(!m_ipc_to_out_th)
            {
...
#ifdef DUMPS_ENABLE
    bfd = fopen("/data/vendor/bitstream", "w+");
    if (bfd ==NULL) {
        DEBUG_PRINT_ERROR("*********fopen failed for /data/vendor/bitstream");
    }
    pfd = fopen("/data/vendor/pcm", "w+");
    if (pfd == NULL) {
        DEBUG_PRINT_ERROR("******fopen failed for /data/vendor/pcm");
    }
#endif
    return eRet;
}
...
@return Error status
*/
OMX_ERRORTYPE  COmxAmrwbplusDec::get_component_version( OMX_IN OMX_HANDLETYPE       hComp,
...
@return Error status
*/
OMX_ERRORTYPE  COmxAmrwbplusDec::send_command(OMX_IN OMX_HANDLETYPE hComp,
...
@return error status
*/
OMX_ERRORTYPE  COmxAmrwbplusDec::send_command_proxy(OMX_IN OMX_HANDLETYPE hComp,
                                                    OMX_IN OMX_COMMANDTYPE  cmd,
                                                    OMX_IN OMX_U32       param1,
                                                    OMX_IN OMX_PTR      cmdData)
{
        ...
        else if(m_state == OMX_StateIdle)
        {
            if(eState == OMX_StateLoaded)
            {
                if(release_done(OMX_ALL))
                {
                    ...
                    {
                        omx_amrwbplus_thread_stop(m_ipc_to_event_th);
                        m_ipc_to_event_th = NULL;
                    }
            ...
            else if(eState == OMX_StateExecuting)
            {
                if(!m_ipc_to_event_th)
                {
                    ...
                    m_ipc_to_event_th = omx_amrwbplus_event_thread_create(
    ...
    else if (cmd == OMX_CommandFlush)
    {
           ...
           (param1 == OMX_ALL))
        {
          if(m_state == OMX_StateInvalid) {
              ...
              DEBUG_PRINT_ERROR("component in invalid state do not send flush cmpl event\n");
          } else {
    ...
    else if (cmd == OMX_CommandPortDisable)
    {
        ...
        if(param1 == OMX_CORE_INPUT_PORT_INDEX || param1 == OMX_ALL)
        {
            ...
            else
            {
                ...
                DEBUG_PRINT("send_command_proxy:OMX_CommandPortDisable: OMX_CORE_INPUT_PORT_INDEX \n");
...
*/
bool COmxAmrwbplusDec::execute_omx_flush(OMX_IN OMX_U32 param1, bool cmd_cmpl)
{
    bool bRet = true;
...
@return bool value indicating whether flushing is carried out successfully
*/
bool COmxAmrwbplusDec::execute_input_omx_flush()
{
...
@return bool value indicating whether flushing is carried out successfully
*/
bool COmxAmrwbplusDec::execute_output_omx_flush()
{
    OMX_BUFFERHEADERTYPE *omx_buf;
    unsigned long  p1 = 0; // Parameter - 1
    unsigned long  p2 = 0; // Parameter - 2
...
*/
bool COmxAmrwbplusDec::post_input(unsigned long p1,
                                  unsigned long p2,
                                  unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_amrwbplus_post_msg(m_ipc_to_in_th, id);
    }

    DEBUG_PRINT("PostInput-->state[%d]id[%d]cin[%d]ebdq[%d]dataq[%d]\n",\
                  m_state, id, m_cin_q.m_size, m_ebd_q.m_size, m_din_q.m_size);
...
bool COmxAmrwbplusDec::post_command(unsigned int p1,
                                    unsigned int p2,
                                    unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_amrwbplus_post_msg(m_ipc_to_cmd_th, id);
    }

    DEBUG_PRINT("PostCmd-->state[%d]id[%d]cmdq[%d]",
                  m_state, id, m_cmd_q.m_size);
...
bool COmxAmrwbplusDec::post_output( unsigned long p1,
                                    unsigned long p2,
                                    unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_amrwbplus_post_msg(m_ipc_to_out_th, id);
    }
    DEBUG_PRINT("PostOutput-->state[%d]id[%d]flushq[%d]fbdq[%d]dataq[%d]\n",\
                  m_state, id, m_cout_q.m_size, m_fbd_q.m_size, m_dout_q.m_size);
...
@return error status
*/
OMX_ERRORTYPE  COmxAmrwbplusDec::get_parameter( OMX_IN OMX_HANDLETYPE hComp,
    ...
    switch((int)paramIndex)
    {
      case OMX_IndexParamPortDefinition:
      {
            ...
            if (0 == portDefn->nPortIndex)
            {
                ...
                portDefn->format.audio.eEncoding = (OMX_AUDIO_CODINGTYPE)QOMX_IndexParamAudioAmrWbPlus;
        ...
        case OMX_IndexParamAudioPortFormat:
        {
            ...
            {
                portFormatType->eEncoding = (OMX_AUDIO_CODINGTYPE)QOMX_IndexParamAudioAmrWbPlus;
            }
        ...
        case (OMX_INDEXTYPE)QOMX_IndexParamAudioAmrWbPlus:
        {
            QOMX_AUDIO_PARAM_AMRWBPLUSTYPE *amrwbplusParam =
                (QOMX_AUDIO_PARAM_AMRWBPLUSTYPE *) paramData;
            DEBUG_PRINT("get_parameter QOMX_AUDIO_PARAM_AMRWBPLUSTYPE\n");

            if (OMX_CORE_INPUT_PORT_INDEX== amrwbplusParam->nPortIndex)
            {
                *amrwbplusParam = m_adec_param;
            }
            else
            {
                DEBUG_PRINT_ERROR("get_parameter:QOMX_AUDIO_PARAM_AMRWBPLUSTYPE \
...
@return error status
*/
OMX_ERRORTYPE  COmxAmrwbplusDec::set_parameter( OMX_IN OMX_HANDLETYPE hComp,
                                                OMX_IN OMX_INDEXTYPE paramIndex,
                                                OMX_IN OMX_PTR        paramData)
{
    ...
    switch((int)paramIndex)
    {
        ...
        {
            DEBUG_PRINT("QOMX_AUDIO_PARAM_AMRWBPLUSTYPE");
            m_adec_param = *((QOMX_AUDIO_PARAM_AMRWBPLUSTYPE *) paramData);
            DEBUG_PRINT("nSize %d nPortIndex %d nChannels %d nBitRate %d nSampleRate %d \
                         eAMRBandMode %d eAMRDTXMode %d eAMRFrameFormat %d",
                         m_adec_param.nSize,
                         m_adec_param.nPortIndex,
                         m_adec_param.nChannels,
                         m_adec_param.nBitRate,
                         m_adec_param.nSampleRate,
                         m_adec_param.eAMRBandMode,
                         m_adec_param.eAMRDTXMode,
                         m_adec_param.eAMRFrameFormat);
            break;
        }
        case OMX_IndexParamPortDefinition:
        {
            ...
            {
                DEBUG_PRINT_ERROR(" set_parameter: Bad Port idx %d", (int)portDefn->nPortIndex);
                eRet = OMX_ErrorBadPortIndex;
            }
            break;
        }
    ...
    case OMX_IndexParamAudioPcm:
        {
            ...
            if (OMX_CORE_OUTPUT_PORT_INDEX== pcmparam->nPortIndex)
            {
                ...
                DEBUG_PRINT_ERROR("set_parameter: Sampling rate %u",
                (unsigned)( pcmparam->nSamplingRate));
                DEBUG_PRINT_ERROR("set_parameter: Number of channels %d",
                (unsigned) (pcmparam->nChannels));
            }
            else
            {
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::get_config(OMX_IN OMX_HANDLETYPE      hComp,
...
OMX Error None if successful.
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::set_config(OMX_IN OMX_HANDLETYPE      hComp,
                                            OMX_IN OMX_INDEXTYPE configIndex,
...
None
=============================================================================*/
OMX_ERRORTYPE  COmxAmrwbplusDec::get_extension_index( OMX_IN OMX_HANDLETYPE      hComp,
                                                      ...
                                                      OMX_OUT OMX_INDEXTYPE* indexType)
{
    ...
    else if(strncmp(paramName,"OMX.Qualcomm.index.audio.amrwbplus",
    strlen("OMX.Qualcomm.index.audio.amrwbplus")) == 0)
    {
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::get_state( OMX_IN OMX_HANDLETYPE  hComp,
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::ComponentTunnelRequest
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::component_tunnel_request(OMX_IN OMX_HANDLETYPE hComp,
                                                          OMX_IN OMX_U32                        port,
...
void* COmxAmrwbplusDec::alloc_ion_buffer(unsigned int bufsize)
{
    ...
    ion_data->map_buf_size = (unsigned int)((bufsize + 4095) & (~4095));
...
void COmxAmrwbplusDec::free_ion_buffer(void** ion_buffer)
{
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::AllocateInputBuffer
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::allocate_input_buffer(OMX_IN OMX_HANDLETYPE hComp,
...
OMX_IN OMX_U32                       bytes)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxAmrwbplusDec::allocate_input_buffer");
...
OMX_ERRORTYPE  COmxAmrwbplusDec::allocate_output_buffer(OMX_IN OMX_HANDLETYPE hComp,
OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
OMX_IN OMX_U32                        port,
OMX_IN OMX_PTR                     appData,
OMX_IN OMX_U32                       bytes)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxAmrwbplusDec::allocate_output_buffer");
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::AllocateBuffer
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::allocate_buffer(OMX_IN OMX_HANDLETYPE hComp,
OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
OMX_IN OMX_U32                        port,
OMX_IN OMX_PTR                     appData,
OMX_IN OMX_U32                       bytes)
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::use_buffer(
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::UseInputBuffer
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::use_input_buffer(
OMX_IN OMX_HANDLETYPE            hComp,
OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
OMX_IN OMX_U32                   port,
OMX_IN OMX_PTR                   appData,
...
OMX_IN OMX_U8*                   buffer)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxAmrwbplusDec::use_input_buffer");
    if(bytes < input_buffer_size)
    {
        /* return if i\p buffer size provided by client is less
        than min i\p buffer size supported by omx component */
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::UseOutputBuffer
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::use_output_buffer(
OMX_IN OMX_HANDLETYPE            hComp,
OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
OMX_IN OMX_U32                   port,
OMX_IN OMX_PTR                   appData,
...
OMX_IN OMX_U8*                   buffer)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxAmrwbplusDec::use_output_buffer");
    if(bytes < output_buffer_size)
    {
        /* return if o\p buffer size provided by client is less
        than min o\p buffer size supported by omx component */
    ...
    if(m_out_current_buf_count < m_out_act_buf_count)
    {
        ...
        if (bufHdr != NULL && loc_bufHdr != NULL)
        {
            ...
            /* Register the mapped PMEM buffer with the AMR-WB+ driver */
            if (!audio_register_ion(mmap_data)) {
               free(bufHdr);
               free(loc_bufHdr);
               *bufferHdr = NULL;
...
*/
bool COmxAmrwbplusDec::search_input_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
    bool eRet = false;
...
@return bool value indicating whether buffer is found
*/
bool COmxAmrwbplusDec::search_output_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
    bool eRet = false;
    OMX_BUFFERHEADERTYPE *temp = NULL;
...
@return Error status
*/
OMX_ERRORTYPE  COmxAmrwbplusDec::free_buffer( OMX_IN OMX_HANDLETYPE hComp,
                                              OMX_IN OMX_U32                 port,
...
@return error status
*/
OMX_ERRORTYPE  COmxAmrwbplusDec::empty_this_buffer( OMX_IN OMX_HANDLETYPE hComp,
                                                    OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_STATETYPE state;
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
@return error status
*/
OMX_ERRORTYPE  COmxAmrwbplusDec::empty_this_buffer_proxy( OMX_IN OMX_HANDLETYPE hComp,
                                                          OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if(m_first_amrwbplus_header == 0)
    {
        m_first_amrwbplus_header = 1;
        if (pcm_feedback) {
            struct msm_audio_config config;
            ioctl(m_drv_fd, AUDIO_GET_CONFIG, &config);
            config.sample_rate = (OMX_U16)m_adec_param.nSampleRate;
            config.channel_count = (OMX_U16)m_adec_param.nChannels;
            ioctl(m_drv_fd, AUDIO_SET_CONFIG, &config);
            DEBUG_PRINT_ERROR("config.sample_rate %d config.channel_count %d",config.sample_rate,config.channel_count);
        }
        struct msm_audio_amrwbplus_config_v2 drv_config;
        ioctl(m_drv_fd, AUDIO_GET_AMRWBPLUS_CONFIG_V2, &drv_config);
        memset(&drv_config, 0x0, sizeof(struct msm_audio_amrwbplus_config_v2));
        drv_config.size_bytes = (unsigned int)sizeof(struct msm_audio_amrwbplus_config_v2);
        drv_config.num_channels   =  m_adec_param.nChannels;


        DEBUG_PRINT("size_bytes %d version %d #channels %d band mode %d dtx mode %d frame fmt %d lsf idx %d\n",
                    drv_config.size_bytes,drv_config.version,drv_config.num_channels,
                    drv_config.amr_band_mode,drv_config.amr_dtx_mode,drv_config.amr_frame_fmt,
                    drv_config.amr_lsf_idx);

        ioctl(m_drv_fd, AUDIO_SET_AMRWBPLUS_CONFIG_V2, &drv_config);
    ...
    else
    {
        ...
        /* Asynchronous write call to the amr-wb+ driver */
        audio_aio_buf.buf_len = bufHdr->nAllocLen;
        audio_aio_buf.private_data = bufHdr;
        ...
        {
            audio_aio_buf.buf_addr = (OMX_U8*)pmeta_in;
            audio_aio_buf.data_len = (OMX_U16)(new_length + sizeof(META_IN));
            audio_aio_buf.mfield_sz = (OMX_U16)sizeof(META_IN);
        }
        else
        {
...
OMX_ERRORTYPE  COmxAmrwbplusDec::fill_this_buffer_proxy ( OMX_IN OMX_HANDLETYPE         hComp,
                                                          OMX_BUFFERHEADERTYPE* buffer)
{
    int ret = 0;
    META_OUT *pmeta_out = NULL;
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::FillThisBuffer
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::fill_this_buffer(OMX_IN OMX_HANDLETYPE hComp,
                                                  OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_STATETYPE state;
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::set_callbacks(OMX_IN OMX_HANDLETYPE hComp,
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::ComponentDeInit
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::component_deinit(OMX_IN OMX_HANDLETYPE hComp)
{
#ifdef DUMPS_ENABLE
  if (bfd != NULL)
    fclose(bfd);
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::deinit_decoder
...
========================================================================== */
void  COmxAmrwbplusDec::deinit_decoder()
{
    ...
    DEBUG_PRINT("Component-deinit calling omx_amrwbplus_thread_stop\
        (m_ipc_to_in_th)\n");
    if (m_ipc_to_in_th != NULL) {
        omx_amrwbplus_thread_stop(m_ipc_to_in_th);
        m_ipc_to_in_th = NULL;
    }
    ...
    if (m_ipc_to_cmd_th != NULL) {
        omx_amrwbplus_thread_stop(m_ipc_to_cmd_th);
        m_ipc_to_cmd_th = NULL;
    }
    ...
    {
        DEBUG_PRINT_ERROR(" amrwbplus device already closed\n");
    }
    ...
    if(pcm_feedback ==1)
    {
      ...
      (m_ipc_to_out_th)\n");
        if (m_ipc_to_out_th != NULL) {
            omx_amrwbplus_thread_stop(m_ipc_to_out_th);
            m_ipc_to_out_th = NULL;
        }
    }
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::use_EGL_image(OMX_IN OMX_HANDLETYPE hComp,
OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
OMX_IN OMX_U32                        port,
OMX_IN OMX_PTR                     appData,
...
/* ======================================================================
FUNCTION
COmxAmrwbplusDec::ComponentRoleEnum
...
========================================================================== */
OMX_ERRORTYPE  COmxAmrwbplusDec::component_role_enum(OMX_IN OMX_HANDLETYPE hComp,
OMX_OUT OMX_U8*        role,
OMX_IN OMX_U32        index)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    const char *cmp_role = "audio_decoder.amrwbplus";
...
========================================================================== */
bool COmxAmrwbplusDec::allocate_done(void)
{
...
========================================================================== */
bool COmxAmrwbplusDec::release_done(OMX_U32 param1)
{
    OMX_BOOL bRet = OMX_FALSE;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    if(param1 == OMX_ALL)
...
DESCRIPTION
Checks if the current platform is 8960
...
========================================================================== */
bool COmxAmrwbplusDec::is_platform_8960()
{
    FILE *fp = NULL;
