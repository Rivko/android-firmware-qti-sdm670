...
*This module contains an openamx test application for WMA component.
*
*Copyright (c) 2010, 2014, 2017-2018 Qualcomm Technologies, Inc.
*All Rights Reserved.
                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    {
        DEBUG_PRINT("omx_wma_adec_test: bFlushing is set to TRUE \
			used_ip_buf_cnt=%d\n",used_ip_buf_cnt);
...
void signal_handler(int sig_id) {
    ...
    if (sig_id == SIGUSR1) {
        ...
        if (bPause == true) {
            ...
            bPause = false;
            OMX_SendCommand(wma_dec_handle, OMX_CommandStateSet,
...
int main(int argc, char **argv)
{
    ...
    {
        in_filename = argv[1];
        samplerate= (uint32_t)get_input_and_validate(argv[2], UINTMAX);;
        channels = (uint16_t)get_input_and_validate(argv[3], USHRTMAX);;
        BitRate= (uint32_t)atoi(argv[4]);
        BlockAlign= (uint16_t)atoi(argv[5]);
        virtual_chunk_len = (uint32_t)get_input_and_validate(argv[6], UINTMAX);
        EncodeOptions = (uint16_t)atoi(argv[7]);
        packet_size  = (uint32_t)get_input_and_validate(argv[8], UINTMAX);
        packet_padding_size = (uint32_t)get_input_and_validate(argv[9], UINTMAX);
        total_payload_data = (uint32_t)atoi(argv[10]);
        version = (uint32_t)atoi(argv[11]);
        bitspersample = (uint32_t)atoi(argv[12]);
        formattag = (uint32_t)atoi(argv[13]);
        advencopt = (uint32_t)atoi(argv[14]);
        advencopt2 = (uint32_t)atoi(argv[15]);
        tunnel  = (uint32_t)atoi(argv[16]);
        filewrite = (uint32_t)atoi(argv[17]);
        out_filename = argv[18];
        /* To Skip packet header*/
        packet_header_size = packet_size - virtual_chunk_len - packet_padding_size;
	write_buff_size = virtual_chunk_len;
        if(argc==20) {
           allocate_buf_flag = (uint32_t)atoi(argv[19]);
        }else if(argc >= 21) {
           allocate_buf_flag = (uint32_t)atoi(argv[19]);
           write_buff_size = (uint32_t)atoi(argv[20]);
	}
        if (tunnel == 1)
        {
    ...
    else
    {
        ...
        DEBUG_PRINT("ex: ./mm-adec-omxwma-test INPUTFILE SAMPLERATE CHANNELS \
	BITRATE BLOCKALIGN VIRTUALCHUNKLENGTH ENCODEOPT PACKETSIZE \
        PACKETPADDINGSIZE TOTALPAYLOADDATA VERSION BITSPERSAMPLE \
	FORMATTAG ADVENCOPT ADVENOPT2 TUNNEL FILEWRITE OUTFILE Use_Alloc_Flag buf_size\n");
        DEBUG_PRINT( " PACKETSIZE represents packet size\n");
        DEBUG_PRINT( " PACKETPADDINGSIZE represents pad bytes per packet\n");
        DEBUG_PRINT( " TOTALPAYLOADDATA represents total data payload\n");
        ...
        DEBUG_PRINT( "Use_Alloc_Flag = 0 (Use buffer) 1 (Allocate buffer)\n");
	...
	else if(version == WMAPROLOSS)
    {
        if(tunnel == 0)
        {
            aud_comp = "OMX.qcom.audio.decoder.wmaLossLess";
        }
        else
        {
            aud_comp = "OMX.qcom.audio.decoder.tunneled.wmaLossLess";
        }
     }
     else
     {
    ...
    if(bOutputEosReached || (tunnel && bInputEosReached))
    {
		...
		if(devmgr_fd >= 0)
		{
			write_devctlcmd(devmgr_fd, "-cmd=unregister_session_rx \
					-sid=", session_id_hpcm);
		}
		else
		{
...
int Play_Decoder()
{
    ...
    OMX_GetParameter(wma_dec_handle,OMX_IndexParamPortDefinition,
		    &inputportFmt);
    ...
    OMX_SetParameter(wma_dec_handle,OMX_IndexParamPortDefinition,
		    &inputportFmt);
    ...
    OMX_GetExtensionIndex(wma_dec_handle,"OMX.Qualcomm.index.audio.sessionId",
		    &index);
    ...
    if(version == WMAPRO)
    {
        OMX_GetExtensionIndex(wma_dec_handle,
			"OMX.Qualcomm.index.audio.wma10Pro",&index);
                                      ...
                                      unsigned int bufCntMin, unsigned int bufSize)
{
    ...
    *pBufHdrs= (OMX_BUFFERHEADERTYPE **)
	...
	DEBUG_PRINT(" allocate use buffer %p\n", pBuffer);
        error = OMX_UseBuffer(wma_dec_handle, &((*pBufHdrs)[bufCnt]),
...
static size_t Read_Buffer (OMX_BUFFERHEADERTYPE  *pBufHdr )
{
    ...
    temp = current_ptr % packet_size;
    // Check pointer at packer header location
    if(temp < packet_header_size) {
        //skip packet header
        temp = (packet_header_size-temp);
        fseek(inputBufferFile, (long)temp, SEEK_CUR);
        current_ptr +=temp;
        bytes_read = fread(pBufHdr->pBuffer, 1, write_buff_size, inputBufferFile);
        if (bytes_read != write_buff_size)
           return 0;
        current_ptr += write_buff_size;
    } else {
        if((temp + write_buff_size) >(packet_size-packet_padding_size)) {
	   temp = (packet_size - packet_padding_size - temp);
           if(temp) {
              bytes_read = fread(pBufHdr->pBuffer, 1,temp, inputBufferFile);
              current_ptr+=temp;
           }
           fseek(inputBufferFile,(long)(packet_padding_size+packet_header_size), SEEK_CUR);
           current_ptr+=(packet_padding_size+packet_header_size);
           bytes_read +=fread(pBufHdr->pBuffer+temp, 1,(write_buff_size-temp), inputBufferFile);
           current_ptr+=(write_buff_size-temp);
        } else if ((temp + write_buff_size) <= (packet_size-packet_padding_size)) {
           bytes_read =fread(pBufHdr->pBuffer, 1,write_buff_size, inputBufferFile);
           current_ptr+=write_buff_size;
        }
   }
   DEBUG_PRINT("cur ptr = 0x%8x, bytes_read= %zu\n", current_ptr, bytes_read);
    ...
    if(total_bytes_read >=total_payload_data)
    {
    if(total_bytes_read > total_payload_data)
        {
            pBufHdr->nFilledLen = pBufHdr->nFilledLen -
...
static int open_audio_file ()
{
    ...
    headerobjectsize = (OMX_U16)(headerobject[0] | headerobject[1] << 8);
