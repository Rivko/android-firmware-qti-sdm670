...
OMX_ERRORTYPE COmxWmaDec::component_init(OMX_STRING role)
{
    ...
    if(wmaVersion & (WMA_PRO | WMA_LOSSLESS))
    {
        if(0 == pcm_feedback)
        {
...
OMX_IN OMX_PTR        paramData)
{
    ...
    case OMX_IndexParamAudioWma:
        {
            ...
            for (int i = 0; i < WMA_STD_NUM_FREQ; i++) {
                if (((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nSamplingRate == wmaStdSampleRateTbl[i]) {
                    sr_id = i;
                    break;
                }
            }
            if ((sr_id < 0) || (((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nChannels > 2)
                    || (((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nChannels == 0)) {
                DEBUG_PRINT_ERROR("invalid sample rate or channel count\n");
                return OMX_ErrorUnsupportedSetting;
            }

            min_bitrate = wmaStdMinAvgByteRateTbl[sr_id][((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nChannels - 1];
            max_bitrate = wmaStdMaxAvgByteRateTbl[sr_id][((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nChannels - 1];
            if ((((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nBitRate > max_bitrate) ||
                    (((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nBitRate < min_bitrate)) {
                DEBUG_PRINT_ERROR("unsupported bitrate %d", ((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nBitRate);
                return OMX_ErrorUnsupportedSetting;
            }
    ...
    case QOMX_IndexParamAudioWma10Pro:
        {
            DEBUG_PRINT("OMX_IndexParamAudiowma10Pro");
            if (((wmaVersion & WMA_PRO) && ((QOMX_AUDIO_PARAM_WMA10PROTYPE *)paramData)->nBitRate > MAX_BITRATE_WMA_PRO) ||
                ((wmaVersion & WMA_LOSSLESS) && ((QOMX_AUDIO_PARAM_WMA10PROTYPE *)paramData)->nBitRate > MAX_BITRATE_WMA_LOSSLESS)) {
                DEBUG_PRINT_ERROR("unsupported bitrate %d", ((OMX_AUDIO_PARAM_WMATYPE *)paramData)->nBitRate);
                return OMX_ErrorUnsupportedSetting;
            }
...
OMX_BUFFERHEADERTYPE* buffer)
{
	...
	if(wmaVersion & (WMA_PRO | WMA_LOSSLESS))
	{
		...
		drv_config.avgbytespersecond =
			(m_adec_param_wma10Pro.nBitRate/8);
		...
		drv_config.channelmask =
			channel_out_mask_from_count(drv_config.numchannels);
		...
		drv_config.samplingrate = m_adec_param.nSamplingRate;
		drv_config.numchannels =(OMX_U16)m_adec_param.nChannels;
		drv_config.avgbytespersecond = m_adec_param.nBitRate/8;
...
/* ======================================================================
FUNCTION
channel_out_mask_from_count

DESCRIPTION
pop up channel output mask based on channel count.
...
========================================================================== */
uint32_t COmxWmaDec::channel_out_mask_from_count(OMX_U16 channel_count)
{
    uint32_t channel_mask = 0;

    switch (channel_count) {
    case 1:
        channel_mask = CHMASK_MONO;
        break;
    case 2:
        channel_mask = CHMASK_STEREO;
        break;
    case 3:
        channel_mask = CHMASK_2POINT1;
        break;
    case 4:
        channel_mask = CHMASK_QUAD;
        break;
    case 5:
        channel_mask = CHMASK_PENTA;
        break;
    case 6:
        channel_mask = CHMASK_5POINT1;
        break;
    case 7:
        channel_mask = CHMASK_6POINT1;
        break;
    case 8:
        channel_mask = CHMASK_7POINT1;
        break;
    default:
        DEBUG_PRINT_ERROR("%s: unknown channel count %d\n", __FUNCTION__, channel_count);
    }
...
#ifdef NT_PAUSE_TIMEOUT_ENABLED
bool COmxWmaDec::acquire_wakelock()
{
    bool status = true;
    int  ret;
...
bool COmxWmaDec::release_wakelock()
{
    bool status = true;
    int  ret;
...
void COmxWmaDec::create_timer_thread() {
    int result;
...
void COmxWmaDec::reset_pause_timer() {
    acquire_wakelock();
...
void COmxWmaDec::destroy_pause_timer() {
    int result;
    ...
    while (!timer_info->kill_thread) {
        ...
        if ((diff >= PAUSE_TIMEOUT) && (!timer_info->expired)) {
            ...
            if (me->is_drv_paused() == OMX_FALSE) {
                ...
                DEBUG_PRINT("PAUSE TIMEOUT, suspend audio driver!\n");
            }
...
void COmxWmaDec::suspend_audio_driver(bool suspend) {
    if (m_drv_fd < 0) {
