...
{
    return (new COmxAlacDec);
}

// omx_cmd_queue destructor
COmxAlacDec::omx_cmd_queue::~omx_cmd_queue()
{
    // Nothing to do
}

// omx cmd queue constructor
COmxAlacDec::omx_cmd_queue::omx_cmd_queue(): m_read(0),m_write(0),m_size(0)
{
    memset(m_q, 0, sizeof(omx_event)*OMX_CORE_CONTROL_CMDQ_SIZE);
}

// omx cmd queue insert
bool COmxAlacDec::omx_cmd_queue::insert_entry(unsigned long p1,
...
bool COmxAlacDec::omx_cmd_queue::pop_entry(unsigned long *p1,
        unsigned long *p2, unsigned char *id)
{
    bool ret = true;
    if (m_size > 0)
...
bool COmxAlacDec::omx_cmd_queue::get_msg_id(unsigned char *id)
{
    if(m_size > 0)
    {
        *id = m_q[m_read].id;
...
void COmxAlacDec::wait_for_event()
{
    int rc;
    struct timespec ts;
...
void COmxAlacDec::event_complete()
{
    pthread_mutex_lock(&m_event_lock);
...
void COmxAlacDec::out_th_wakeup()
{
    DEBUG_PRINT("%s",__FUNCTION__);
    pthread_mutex_lock(&m_out_th_lock);
...
void COmxAlacDec::wait_for_flush_event()
{
...
void COmxAlacDec::event_flush_complete()
{
    pthread_mutex_lock(&m_flush_cmpl_lock);
...
void COmxAlacDec::process_event_cb(void *client_data, unsigned char id)
{
    /* To remove warning for unused variable to keep prototype same */
    (void)id;
    DEBUG_PRINT("PE:Waiting for event's...");
    COmxAlacDec  *pThis = (COmxAlacDec *) client_data;
    pThis->process_events(pThis);
}

void COmxAlacDec::process_events(COmxAlacDec *client_data)
{
    int rc = 0;
...
/* ======================================================================
   ...
   None.
   ========================================================================== */
COmxAlacDec::COmxAlacDec(): m_volume(25),
    m_app_data(NULL),
    m_eos_bm(0),
    m_flush_cnt(0),
    m_first_alac_header(0),
    m_is_alloc_buf(0),
    drv_inp_buf_cnt(0),
    drv_out_buf_cnt(0),
    nNumInputBuf(0),
    ...
    nTimestamp(0),
    pcm_feedback(1),
    output_buffer_size (OMX_ALAC_OUTPUT_BUFFER_SIZE),
...
COmxAlacDec::~COmxAlacDec()
{
    ...
    DEBUG_PRINT("OMX ALAC component destroyed\n");
    return;
}

bool COmxAlacDec::aio_write(msm_audio_aio_buf *audio_aio_buf)
{
    bool rc = true;
...
bool COmxAlacDec::aio_read(msm_audio_aio_buf *audio_aio_buf)
{
    bool rc = true;
...
bool COmxAlacDec::prepare_for_ebd(OMX_BUFFERHEADERTYPE *bufHdr)
{
    bool rc = true;
    ...
    if (bufHdr->nFlags & OMX_BUFFERFLAG_EOS)
    {
        if (bufHdr->nFilledLen)
        {
            ...
            /* Asynchronous write call to the ALAC driver */
...
bool COmxAlacDec::audio_register_ion(struct mmap_info *ion_buf)
{
    bool rc = true;
    struct msm_audio_ion_info audio_ion_buf;
    /* Register the mapped ION buffer with the ALAC driver */
...
bool COmxAlacDec::audio_deregister_ion(struct mmap_info *ion_buf)
{
    bool rc = true;
    struct msm_audio_ion_info audio_ion_buf;
    if (!ion_buf)
...
/**
  ...
  @return none
 */
void COmxAlacDec::buffer_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
...
void COmxAlacDec::frame_done_cb(OMX_BUFFERHEADERTYPE *bufHdr, bool flg)
{
    META_OUT *pmeta_out = NULL;
    DEC_META_OUT *dec_meta_out = NULL;
    OMX_BUFFERHEADERTYPE *buffer = bufHdr;
...
void COmxAlacDec::process_out_port_msg(void *client_data, unsigned char id)
{
    unsigned long p1;
    unsigned long p2;
    unsigned char ident;
...
void COmxAlacDec::process_command_msg(void *client_data, unsigned char id)
{
    unsigned long p1;
    unsigned long p2;
    unsigned char ident;
...
void COmxAlacDec::process_in_port_msg(void *client_data, unsigned char id)
{
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE COmxAlacDec::component_init(OMX_STRING role)
{
    ...
    /* Open the ALAC device in asynchronous mode */
    if(!strcmp(role,"OMX.qcom.audio.decoder.alac"))
    {
        pcm_feedback = 1;
    ...
    if(0 == pcm_feedback)
    {
        m_drv_fd = open("/dev/msm_alac",O_WRONLY | O_NONBLOCK);
    }
    else
    {
        DEBUG_PRINT("Opening /dev/msm_alac in rdwr mode\n");
        m_drv_fd = open("/dev/msm_alac", O_RDWR | O_NONBLOCK);
    }
    if (0 > m_drv_fd)
    {
    ...
    if(!m_ipc_to_in_th)
    {
        m_ipc_to_in_th = omx_alac_thread_create(process_in_port_msg, this,
                (char *) "INPUT_THREAD");
        if(!m_ipc_to_in_th)
        {
    ...
    if(!m_ipc_to_cmd_th)
    {
        m_ipc_to_cmd_th = omx_alac_thread_create(process_command_msg, this,
                (char *) "COMMAND_THREAD");
        if(!m_ipc_to_cmd_th)
        {
    ...
    if(pcm_feedback)
    {
        if(!m_ipc_to_out_th)
        {
            m_ipc_to_out_th = omx_alac_thread_create(process_out_port_msg, this,
                    (char *) "OUTPUT_THREAD");
            if(!m_ipc_to_out_th)
            {
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxAlacDec::get_component_version(OMX_IN OMX_HANDLETYPE hComp,
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxAlacDec::send_command(OMX_IN OMX_HANDLETYPE hComp,
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxAlacDec::send_command_proxy(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_COMMANDTYPE  cmd,
        OMX_IN OMX_U32       param1,
        OMX_IN OMX_PTR      cmdData)
{
        ...
        else if(m_state == OMX_StateIdle)
        {
            if(eState == OMX_StateLoaded)
            {
                if(release_done(OMX_ALL))
                {
                    ...
                    {
                        omx_alac_thread_stop(m_ipc_to_event_th);
                        m_ipc_to_event_th = NULL;
                    }
            ...
            else if(eState == OMX_StateExecuting)
            {
                if(!m_ipc_to_event_th)
                {
                    ...
                    m_ipc_to_event_th = omx_alac_event_thread_create(
 ...
 */
bool COmxAlacDec::execute_omx_flush(OMX_IN OMX_U32 param1, bool cmd_cmpl)
{
    bool bRet = true;
...
/**
  ...
  @return bool value indicating whether flushing is carried out successfully
 */
bool COmxAlacDec::execute_input_omx_flush()
{
...
/**
  ...
  @return bool value indicating whether flushing is carried out successfully
 */
bool COmxAlacDec::execute_output_omx_flush()
{
    OMX_BUFFERHEADERTYPE *omx_buf;
    unsigned long      p1=0; // Parameter - 1
    unsigned long      p2=0; // Parameter - 2
 ...
 */
bool COmxAlacDec::post_input(unsigned long p1,
        unsigned long p2,
        unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_alac_post_msg(m_ipc_to_in_th, id);
    }
...
bool COmxAlacDec::post_command(unsigned int p1,
        unsigned int p2,
        unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_alac_post_msg(m_ipc_to_cmd_th, id);
    }
...
bool COmxAlacDec::post_output(unsigned long p1,
        unsigned long p2,
        unsigned char id)
{
    bool bRet = false;
    ...
    {
        bRet = true;
        omx_alac_post_msg(m_ipc_to_out_th, id);
    }
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxAlacDec::get_parameter(OMX_IN OMX_HANDLETYPE hComp,
        ...
        OMX_INOUT OMX_PTR     paramData)
{
    ...
    switch((int)paramIndex)
    {
        case OMX_IndexParamPortDefinition:
            {
                ...
                if (0 == portDefn->nPortIndex)
                {
                    ...
                    portDefn->format.audio.eEncoding = (OMX_AUDIO_CODINGTYPE)QOMX_IndexParamAudioAlac;
        ...
        case OMX_IndexParamAudioPortFormat:
            {
                ...
                {
                    portFormatType->eEncoding = (OMX_AUDIO_CODINGTYPE)QOMX_IndexParamAudioAlac;
                }
        ...
        case QOMX_IndexParamAudioAlac:
            {
                QOMX_AUDIO_PARAM_ALACTYPE *alacParam =
                    (QOMX_AUDIO_PARAM_ALACTYPE *) paramData;
                DEBUG_PRINT("get_parameter: OMX_IndexParamAudioAlac\n");

                if (OMX_CORE_INPUT_PORT_INDEX == alacParam->nPortIndex)
                {
                    *alacParam = m_adec_param;
                }
                else
                {
                    DEBUG_PRINT_ERROR("get_parameter: OMX_IndexParamAudioAlac \
                            OMX_ErrorBadPortIndex %d\n",
                            (int)alacParam->nPortIndex);
                    eRet = OMX_ErrorBadPortIndex;
                }
                break;
            }
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxAlacDec::set_parameter(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_INDEXTYPE paramIndex,
        OMX_IN OMX_PTR        paramData)
{
    ...
    switch((int)paramIndex)
    {
        case QOMX_IndexParamAudioAlac:
            {
                DEBUG_PRINT("set_parameter: OMX_IndexParamAudioAlac");
                m_adec_param = *((QOMX_AUDIO_PARAM_ALACTYPE *) paramData);
                if (m_adec_param.nSampleRate > 48000 && m_adec_param.nChannels > 2) {
                    DEBUG_PRINT_ERROR("Cannot play multichannel ALAC clip with sample rate > 48kHz");
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::get_config(OMX_IN OMX_HANDLETYPE      hComp,
...
/* ======================================================================
   ...
   OMX Error None if successful.
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::set_config(OMX_IN OMX_HANDLETYPE      hComp,
        OMX_IN OMX_INDEXTYPE configIndex,
...
None
=============================================================================*/
OMX_ERRORTYPE  COmxAlacDec::get_extension_index(
        ...
        OMX_OUT OMX_INDEXTYPE* indexType)
{
    ...
    if(strncmp(paramName,"OMX.Qualcomm.index.audio.alac",
                strlen("OMX.Qualcomm.index.audio.alac")) == 0){
        *indexType =(OMX_INDEXTYPE)QOMX_IndexParamAudioAlac;
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::get_state(OMX_IN OMX_HANDLETYPE  hComp,
...
/* ======================================================================
   FUNCTION
   COmxAlacDec::ComponentTunnelRequest
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::component_tunnel_request(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_U32                        port,
...
void* COmxAlacDec::alloc_ion_buffer(unsigned int bufsize)
{
...
void COmxAlacDec::free_ion_buffer(void** ion_buffer)
{
...
/* ======================================================================
   FUNCTION
   COmxAlacDec::AllocateInputBuffer
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::allocate_input_buffer(OMX_IN OMX_HANDLETYPE hComp,
        ...
        OMX_IN OMX_U32                       bytes)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxAlacDec::allocate_input_buffer");
...
OMX_ERRORTYPE  COmxAlacDec::allocate_output_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                        port,
        OMX_IN OMX_PTR                     appData,
        OMX_IN OMX_U32                       bytes)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxAlacDec::allocate_output_buffer");
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::allocate_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                        port,
        OMX_IN OMX_PTR                     appData,
        OMX_IN OMX_U32                       bytes)
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::use_buffer(
        OMX_IN OMX_HANDLETYPE            hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                   port,
        OMX_IN OMX_PTR                   appData,
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::use_input_buffer(
        OMX_IN OMX_HANDLETYPE            hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                   port,
        OMX_IN OMX_PTR                   appData,
        ...
        OMX_IN OMX_U8*                   buffer)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxAlacDec::use_input_buffer");
    if(bytes < input_buffer_size)
    {
        /* return if i\p buffer size provided by client is less
           than min i\p buffer size supported by omx component */
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::use_output_buffer(
        OMX_IN OMX_HANDLETYPE            hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                   port,
        OMX_IN OMX_PTR                   appData,
        ...
        OMX_IN OMX_U8*                   buffer)
{
    ...
    (void)port;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    DEBUG_PRINT("Inside COmxAlacDec::use_output_buffer");
    if(bytes < output_buffer_size)
    {
        /* return if o\p buffer size provided by client is less
           than min o\p buffer size supported by omx component */
    ...
    if(m_out_current_buf_count < m_out_act_buf_count)
    {
        ...
        if (bufHdr != NULL && loc_bufHdr != NULL)
        {
            ...
            /* Register the mapped PMEM buffer with the ALAC driver */
            if (!audio_register_ion(mmap_data)) {
                free(bufHdr);
                free(loc_bufHdr);
                *bufferHdr = NULL;
 ...
 */
bool COmxAlacDec::search_input_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
    bool eRet = false;
...
/**
  ...
  @return bool value indicating whether buffer is found
 */
bool COmxAlacDec::search_output_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
    bool eRet = false;
    OMX_BUFFERHEADERTYPE *temp = NULL;
...
/**
  ...
  @return Error status
 */
OMX_ERRORTYPE  COmxAlacDec::free_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_U32                 port,
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxAlacDec::empty_this_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_STATETYPE state;
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
/**
  ...
  @return error status
 */
OMX_ERRORTYPE  COmxAlacDec::empty_this_buffer_proxy(OMX_IN OMX_HANDLETYPE hComp,
        OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if(m_first_alac_header == 0)
    {
        m_first_alac_header = 1;
        if (pcm_feedback) {
        ...
        struct msm_audio_alac_config drv_config;
        ioctl(m_drv_fd, AUDIO_GET_ALAC_CONFIG, &drv_config);

        if (buffer->nFlags & OMX_BUFFERFLAG_CODECCONFIG) {
            DEBUG_PRINT("Populating CSD structure from buffer");
            uint8_t *ptr = buffer->pBuffer;

            memcpy(&m_adec_param.nFrameLength, ptr + kKeyIndexAlacFrameLength,
                    sizeof(m_adec_param.nFrameLength));
            memcpy(&m_adec_param.nCompatibleVersion, ptr + kKeyIndexAlacCompatibleVersion,
                    sizeof(m_adec_param.nCompatibleVersion));
            memcpy(&m_adec_param.nBitDepth, ptr + kKeyIndexAlacBitDepth,
                    sizeof(m_adec_param.nBitDepth));
            memcpy(&m_adec_param.nPb, ptr + kKeyIndexAlacPb, sizeof(m_adec_param.nPb));
            memcpy(&m_adec_param.nMb, ptr + kKeyIndexAlacMb, sizeof(m_adec_param.nMb));
            memcpy(&m_adec_param.nKb, ptr + kKeyIndexAlacKb, sizeof(m_adec_param.nKb));
            memcpy(&m_adec_param.nChannels, ptr + kKeyIndexAlacNumChannels,
                    sizeof(m_adec_param.nChannels));
            memcpy(&m_adec_param.nMaxRun, ptr + kKeyIndexAlacMaxRun,
                    sizeof(m_adec_param.nMaxRun));
            memcpy(&m_adec_param.nMaxFrameBytes, ptr + kKeyIndexAlacMaxFrameBytes,
                    sizeof(m_adec_param.nMaxFrameBytes));
            memcpy(&m_adec_param.nAvgBitRate, ptr + kKeyIndexAlacAvgBitRate,
                    sizeof(m_adec_param.nAvgBitRate));
            memcpy(&m_adec_param.nSampleRate, ptr + kKeyIndexAlacSamplingRate,
                    sizeof(m_adec_param.nSampleRate));

        }

        drv_config.frameLength = m_adec_param.nFrameLength;
        drv_config.compatVersion = m_adec_param.nCompatibleVersion;
        drv_config.bitDepth = m_adec_param.nBitDepth;
        drv_config.pb = m_adec_param.nPb;
        drv_config.mb = m_adec_param.nMb;
        drv_config.kb = m_adec_param.nKb;
        drv_config.channelCount = m_adec_param.nChannels;
        drv_config.maxRun = m_adec_param.nMaxRun;
        drv_config.maxSize = m_adec_param.nMaxFrameBytes;
        drv_config.averageBitRate = m_adec_param.nAvgBitRate;
        drv_config.sampleRate = m_adec_param.nSampleRate;
        drv_config.channelLayout = m_adec_param.nChannelLayoutTag;

        DEBUG_PRINT("sampleRate %d channelCount %d frameLength %d compatVersion %d bitDepth %d maxSize %d\
                    averageBitRate %d pb %d mb %d kb %d maxRun %d channelLayout %d", drv_config.sampleRate,
                    drv_config.channelCount, drv_config.frameLength, drv_config.compatVersion,
                    drv_config.bitDepth, drv_config.maxSize, drv_config.averageBitRate, drv_config.pb,
                    drv_config.mb, drv_config.kb, drv_config.maxRun, drv_config.channelLayout);

        ioctl(m_drv_fd, AUDIO_SET_ALAC_CONFIG, &drv_config);
...
OMX_ERRORTYPE  COmxAlacDec::fill_this_buffer_proxy (
        OMX_IN OMX_HANDLETYPE         hComp,
        OMX_BUFFERHEADERTYPE* buffer)
{
    int ret = 0;
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::fill_this_buffer(OMX_IN OMX_HANDLETYPE hComp,
        OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_STATETYPE state;
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::set_callbacks(OMX_IN OMX_HANDLETYPE hComp,
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::component_deinit(OMX_IN OMX_HANDLETYPE hComp)
{
#if DUMPS_ENABLE
    if (bfd != -1)
        close(bfd);
...
/* ======================================================================
   ...
   ========================================================================== */
void  COmxAlacDec::deinit_decoder()
{
    ...
    DEBUG_PRINT("Component-deinit calling omx_alac_thread_stop\
            (m_ipc_to_in_th)\n");
    if (m_ipc_to_in_th != NULL) {
        omx_alac_thread_stop(m_ipc_to_in_th);
        m_ipc_to_in_th = NULL;
    }
    ...
    if (m_ipc_to_cmd_th != NULL) {
        omx_alac_thread_stop(m_ipc_to_cmd_th);
        m_ipc_to_cmd_th = NULL;
    }
    ...
    {
        DEBUG_PRINT_ERROR("alac device already closed\n");
    }
    ...
    if(pcm_feedback ==1)
    {
        ...
        if (m_ipc_to_out_th != NULL) {
            omx_alac_thread_stop(m_ipc_to_out_th);
            m_ipc_to_out_th = NULL;
        }
    }
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::use_EGL_image(OMX_IN OMX_HANDLETYPE hComp,
        OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
        OMX_IN OMX_U32                        port,
        OMX_IN OMX_PTR                     appData,
...
/* ======================================================================
   ...
   ========================================================================== */
OMX_ERRORTYPE  COmxAlacDec::component_role_enum(OMX_IN OMX_HANDLETYPE hComp,
        OMX_OUT OMX_U8*        role,
        OMX_IN OMX_U32        index)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    const char *cmp_role = "audio_decoder.alac";
...
/* ======================================================================
   ...
   ========================================================================== */
bool COmxAlacDec::allocate_done(void)
{
...
/* ======================================================================
   ...
   ========================================================================== */
bool COmxAlacDec::release_done(OMX_U32 param1)
{
    OMX_BOOL bRet = OMX_FALSE;
    DEBUG_PRINT("%s %p\n",__FUNCTION__,this);
    if(param1 == OMX_ALL)
...
#ifdef NT_PAUSE_TIMEOUT_ENABLED
bool COmxAlacDec::acquire_wakelock()
{
    bool status = true;
    int  ret;
...
bool COmxAlacDec::release_wakelock()
{
    bool status = true;
    int  ret;
...
void COmxAlacDec::create_timer_thread() {
    int result;
...
void COmxAlacDec::reset_pause_timer() {
    acquire_wakelock();
...
void COmxAlacDec::destroy_pause_timer() {
    int result;
...
void COmxAlacDec::suspend_audio_driver(bool suspend) {
    if (m_drv_fd < 0) {
