...
bool COmxAacDec::omx_cmd_queue::insert_entry(unsigned long p1,
...
bool COmxAacDec::omx_cmd_queue::pop_entry(unsigned long *p1,
        unsigned long *p2, unsigned char *id)
{
    bool ret = true;
    if (m_size > 0)
...
bool COmxAacDec::omx_cmd_queue::get_msg_id(unsigned char *id)
{
    if(m_size > 0)
    {
        *id = m_q[m_read].id;
    ...
    while(1)
    {
        ...
        switch(drv_event.event_type)
        {
            ...
            case AUDIO_EVENT_WRITE_DONE:
            {
                ...
                if(pcm_feedback)
                {
                    ...
                    (unsigned int)(drv_event.event_payload.aio_buf.data_len -
                     sizeof(META_IN));
                }
		...
		if (pStrm == NULL)
		{
			...
			break;
                ...
                post_command(0,(unsigned long)pStrm,OMX_COMPONENT_STREAM_INFO);
...
None.
========================================================================== */
COmxAacDec::COmxAacDec(): m_volume(25),
                          m_app_data(NULL),
                          m_eos_bm(0),
			  m_flush_cnt(0),
...
OMX_ERRORTYPE COmxAacDec::component_init(OMX_STRING role)
{
    ...
    memset(&m_adec_config_dualmono, 0, sizeof(m_adec_config_dualmono));
    m_adec_config_dualmono.nSize = (OMX_U32)sizeof(m_adec_config_dualmono);
    ...
    {
        pcm_feedback = 1;
        strlcpy(dev_name, "/dev/msm_aac", strlen("/dev/msm_aac")+1);
    ...
    {
        pcm_feedback = 1;
        strlcpy(dev_name, "/dev/msm_multi_aac", strlen("/dev/msm_multi_aac")+1);
        mask = O_RDWR | O_NONBLOCK;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    }
    ...
    {
      DEBUG_PRINT_ERROR("component_init:dev_name is empty\n");
...
*/
bool COmxAacDec::post_input(unsigned long p1,
                            unsigned long p2,
                            unsigned char id)
{
    bool bRet = false;
...
bool COmxAacDec::post_command(unsigned long p1,
                            unsigned long p2,
                            unsigned char id)
{
    bool bRet = false;
...
bool COmxAacDec::post_output(unsigned long p1,
                            unsigned long p2,
                            unsigned char id)
{
    bool bRet = false;
    ...
    case OMX_IndexParamPortDefinition:
        {
                ...
                && (m_state != OMX_StateWaitForResources)))
            {
                DEBUG_PRINT("Set Parameter called in valid state\n");
                    ...
                    (m_inp_bEnabled == false || m_out_bEnabled == false))
            {
                DEBUG_PRINT("Allowing set parameter on a disabled port\n");
            }
            else
            {
                                    ...
                                    OMX_IN OMX_U8*                   buffer)
{
    ...
    if(m_inp_current_buf_count < m_inp_act_buf_count)
    {
        ...
        if (bufHdr != NULL && loc_bufHdr != NULL)
        {
            ...
            {
                DEBUG_PRINT_ERROR("%s[%p]ion registration failed",
                __FUNCTION__,this);
                free(bufHdr);
                free(loc_bufHdr);
                return OMX_ErrorInsufficientResources;
                                    ...
                                    OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if ((buffer->nFlags & OMX_BUFFERFLAG_EOS) &&
         buffer->nFilledLen == 0 && !bOutputPortReEnabled)
    {
         DEBUG_PRINT("\n ETBP: First Input Buffer is EOS!! \n");
         set_eos_bm(IP_PORT_BITMASK);
                                    ...
                                    OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if ((get_eos_bm() & IP_PORT_BITMASK) && !bOutputPortReEnabled)
    {
         DEBUG_PRINT("%s[%p] EOS is reached", __FUNCTION__, this);
...
OMX_ERRORTYPE COmxAacDec::set_aac_config(OMX_AUDIO_PARAM_AACPROFILETYPE  *m_adec_param_tmp)
{
    ...
    aac_config.channel_configuration = (OMX_U16)m_adec_param_tmp->nChannels;
    ...
    else
    {
        ...
        while(i < (bufHdr->nFilledLen))
        {
            if ((buf[i] == 0xFF) && ((buf[i+1] & 0xF6) == 0xF0) )
            {
                ...
                header->head.adts.fixed.channel_configuration = (OMX_U8)index;
                header->head.adts.fixed.sampling_frequency_index = (OMX_U8)sf;
                break;
            }
        ...
        if(!sync_word_found)
        {
            ...
            header->head.raw.aud_obj_type = (OMX_U8)temp;
            header->head.raw.ext_aud_obj_type = (OMX_U8)temp;
            ...
            header->head.raw.freq_index = (OMX_U8)temp;
            header->head.raw.ext_freq_index = (OMX_U8)temp;
            ...
            header->head.raw.channel_config = (OMX_U8)temp;
        ...
        Currrent release does not support AAC_SCALABLE Object */
                ...
                if(header->head.raw.channel_config == 0 && status == 0)
                {
                    ...
                    header->head.raw.channel_config = (OMX_U8)num_ch ;
                    /* byte_alignment() */
            ...
            if (ext_aud_obj_type != 5)
            {
                ...
                if (temp == 0x2b7)
                {
                    ...
                    if(ext_aud_obj_type == 5)
                    {
                        /*sbrPresentFlag*/
                        audaac_extract_bits(data, 1, &temp);
                        header->head.raw.sbr_present_flag = (OMX_U8)temp;
                        if(temp)
                        {
                            /*extensionSamplingFrequencyIndex*/
                            audaac_extract_bits(data,
                            ...
                            if (temp == 0x548)
                            {
                                /*psPresentFlag*/
                                audaac_extract_bits(data, 1, &temp);
                                header->head.raw.sbr_ps_present_flag = (OMX_U8)temp;
                                if(temp)
                                {
                    ...
                    header->head.raw.ext_aud_obj_type = (OMX_U8)ext_aud_obj_type;
                }
            }
                                ...
                                struct aac_header *aac_header_info)
{
    ...
    &temp);
    aac_header_info->head.adif.variable_bit_rate = (OMX_U8)temp;
    ...
    for(pfe_index=0; pfe_index<num_pfe+1; pfe_index++) {
        ...
        &buf8);
        aac_header_info->head.adif.aud_obj_type = (OMX_U8)buf8;

        buf8 = 0;
        audaac_extract_bits(data, AAC_SAMPLING_FREQ_INDEX_SIZE, &buf8);
        aac_header_info->head.adif.freq_index = (OMX_U8)buf8;
        ...
        if(buf8) {
            m_aac_hdr_bit_index = (unsigned int)(m_aac_hdr_bit_index + 8 - buf8);
        }
        ...
        /* Skip the comment */
        m_aac_hdr_bit_index =
        (unsigned int)(m_aac_hdr_bit_index + buf8 * AAC_COMMENT_FIELD_DATA_SIZE);
    }
    ...
    aac_header_info->head.adif.channel_config = (OMX_U8)totalChannels;
}
                                ...
                                struct aac_header *aac_header_info)
{
    ...
    aac_header_info->head.loas.aud_obj_type = (OMX_U8)value;
    ...
    aac_header_info->head.loas.freq_index = (OMX_U8)value;
    ...
    aac_header_info->head.loas.channel_config = (OMX_U8)value;
    ...
    if (LOAS_GA_SPECIFIC_CONFIG(obj_type))
    {
        ...
        if (!aac_header_info->head.loas.channel_config)
        {
            ...
            num_bits_to_skip = (OMX_U8)((num_fc * 5) + (num_sc * 5) + (num_bc * 5) +
            ...
            while (num_bits_to_skip != 0){
                if (num_bits_to_skip > 32) {
                    ...
                    num_bits_to_skip = (OMX_U8)(num_bits_to_skip - 32);
                } else {
            ...
            num_bits_to_skip = (OMX_U8)(value * 8);
...
)
{
    ...
    while(num_remaining) {
        ...
        mask = (OMX_U8)(~(0xff << bits_avail_in_byte));
