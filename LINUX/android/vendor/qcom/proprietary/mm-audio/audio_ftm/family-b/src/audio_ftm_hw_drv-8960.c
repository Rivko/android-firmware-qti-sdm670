...
static int rec_ext_loopback(void* ctxt)
{
...
#ifndef TINYALSA_LIB
    pcm_capture_file = pcm_open(flags, "hw:0,4");
    printf("Recording does not support non-Android device currently.\n");
    pcm_close(pcm);
#else
    int device_num = 0;
    if (g_capture_device >= 0)
            ...
            && num_of_capture < g_total_freq_to_test)
    {
        ...
        else if(FFT_START == g_analyze_status)
        {
            for(count = 0; count < frames; count++)
            {
                ...
                capture_power += curValue * curValue;
            }
    ...
    printf("\nPower:\t");
    if(0 == dbfs_power)
        printf("power is 0. dBFS power cannot be calculated.\n");
    else
        printf("%.2f\tdBFS", 10*log10(dbfs_power));
...
int audio_ftm_hw_loopback_en(int enable, int afe)
{
	...
	if (enable && afe) {
		...
		ret = pthread_create(&lb_rec_thread, NULL,
            (void *(* _Nonnull)(void *))rec_loopback, NULL);
		if (ret != 0)
		{
...
audio_ftm_hw_open(Aud_FTM_DevCtxt_T  *pDevCtxt)
{
	...
	if ((pDevCtxt->read_write_flag == PCM_OUT) && (pDevCtxt->bLoopbackCase != TRUE))
	{
		...
		printf("%s: flags %d rate %d channels %d period size %d", __func__, flags, pb_config.rate,
			pb_config.channels, pb_config.period_size);
