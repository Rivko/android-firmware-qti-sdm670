...
int tc_rx_handset_earphone_en_tasha(struct mixer *mxr, int enable)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    int val = enable ? 1 : 0;
    ...
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "RX INT0_1 MIX1 INP0");
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_select(ctl, "RX0");
    else
        ret = mixer_ctl_select(ctl, "ZERO");
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "RX INT0 DEM MUX");
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_select(ctl, "CLSH_DSM_OUT");
    if (ret)
        goto mixer_err;
    ...
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_select(ctl, "G_6_DB");
    if (ret)
        goto mixer_err;
...
mixer_err:
    if(!ctl)
        ...
        DALSYS_Log_Err("%s: mixer_set/mixer_select failed\n",__func__);
    return -1;
}

int tc_rx_handset_earphone_vol_tasha(struct mixer *mxr, int vol)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    printf("%s(%d)\n",__func__,vol);
    ctl = get_ctl(mxr, "RX0 Digital Volume");
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_set(ctl, vol);
    if (ret)
...
int tc_tx_handset_mic_en_tasha(struct mixer *mxr, int enable)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    int val = enable ? 1 : 0;
    ...
    if (enable)
        ret = mixer_ctl_select(ctl, "One");
    ctl = get_ctl(mxr, "ADC MUX6");
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_select(ctl, "DMIC");
    else
        ret = mixer_ctl_select(ctl, "ZERO");
    ctl = get_ctl(mxr, "DMIC MUX6");
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_select(ctl, "DMIC0");
    else
        ret = mixer_ctl_select(ctl, "ZERO");
    ctl = get_ctl(mxr, "DEC6 Volume");
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_set(ctl, 96);
    ctl = get_ctl(mxr, "AIF1_CAP Mixer SLIM TX6");
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_set(ctl, val);
...
int tc_tx_handset_mic_vol_tasha(struct mixer *mxr, int vol)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    printf("%s(%d)\n",__func__,vol);
...
int tc_lb_adie_headset_mic_headset_earphone_en_tasha(struct mixer *mxr, int enable)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    int val = enable ? 1 : 0;
    ...
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_set(ctl, 30);
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "IIR0 INP0 Volume");
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_set(ctl, 75);
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "IIR0 INP0 MUX");
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_select(ctl, "DEC6");
    ...
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "RX INT1 DEM MUX");
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_select(ctl, "CLSH_DSM_OUT");
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "RX INT2 DEM MUX");
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_select(ctl, "CLSH_DSM_OUT");
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "RX INT1_1 MIX1 INP0");
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_select(ctl, "IIR0");
    else
        ret = mixer_ctl_select(ctl, "ZERO");
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "RX INT2_1 MIX1 INP0");
    if (!ctl)
        goto mixer_err;
    if (enable)
        ret = mixer_ctl_select(ctl, "IIR0");
...
int tc_lb_adie_headset_mic_headset_earphone_vol_tasha(struct mixer *mxr, int vol)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    unsigned int decvol = 65;
    printf("%s(%d)\n",__func__,vol);
    ctl = get_ctl(mxr, "DEC6 Volume");
    if (!ctl)
        goto mixer_err;
...
int tc_rx_headset_earphone_en_tasha(struct mixer *mxr, int enable)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    int val = enable ? 1 : 0;
    ...
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "COMP1 Switch");
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_set(ctl, val);
    if (ret)
        goto mixer_err;
    ctl = get_ctl(mxr, "COMP2 Switch");
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_set(ctl, val);
    if (ret)
...
int tc_rx_headset_earphone_vol_tasha(struct mixer *mxr, int vol)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    ctl = get_ctl(mxr, "RX1 Digital Volume");
...
int tc_lb_afe_headset_mic_headset_earphone_en_tasha(struct mixer *mxr, int enable)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    int val = enable ? 1 : 0;
    ...
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_set(ctl, val);
    ctl = get_ctl(mxr, "PRI_MI2S_RX Port Mixer PRI_MI2S_TX");
    if (!ctl)
        goto mixer_err;
    ret = mixer_ctl_set(ctl, val);
...
int tc_lb_afe_headset_mic_headset_earphone_vol_tasha(struct mixer *mxr, int vol)
{
    uint32 ret = 0;
    struct mixer_ctl *ctl = 0;
    unsigned int decvol = 65;
...
static struct ftm_tc_device ftm_tc_devices_tasha_i2s[] = {
{
    NULL,
    NULL,
    PATH_NULL,
...
{
    tc_tx_handset_mic_en_tasha,
    tc_tx_handset_mic_vol_tasha,
    PATH_TX,
    65
},
{
    tc_rx_handset_earphone_en_tasha,
    tc_rx_handset_earphone_vol_tasha,
    PATH_RX,
    68
},
{
    tc_rx_headset_earphone_en_tasha,
    tc_rx_headset_earphone_vol_tasha,
    PATH_RX,
    68
},
{
    tc_lb_adie_headset_mic_headset_earphone_en_tasha,
    tc_lb_adie_headset_mic_headset_earphone_vol_tasha,
    PATH_ADIE_LB,
    68
},
{
    tc_lb_afe_headset_mic_headset_earphone_en_tasha,
    tc_lb_afe_headset_mic_headset_earphone_vol_tasha,
    PATH_AFE_LB,
    68
}
};
    ...
    int fh, int duration, char *file_name)
{
    ...
    fprintf(stderr, "size of ftm_tc_devices_tasha = %d\n",
            sizeof(ftm_tc_devices_tasha_i2s)/sizeof(ftm_tc_devices_tasha_i2s[0]));
    ...
    else if (codec == CODEC_TASHA_I2S)
        memcpy(ftm_tc_devices, ftm_tc_devices_tasha_i2s,
        sizeof(ftm_tc_devices_tasha_i2s));
