...
#define CAPTURE_BUFFER_SIZE 480
...
static unsigned playbackBufSize = 0;
static unsigned recorderFrames = 0;
static unsigned char *recorderBuffer;
static char value[100];
static unsigned numRxChannels = 2;
static unsigned numTxChannels = 1;
static unsigned bytesPerSample = 2;
        ...
        jclass clazz)
{
    ...
    SLDataFormat_PCM format_pcm = {SL_DATAFORMAT_PCM, numRxChannels, SL_SAMPLINGRATE_48,
...
jboolean Java_com_android_nativeaudiolatency_NativeAudioLatency_createAudioRecorder(JNIEnv* env, jclass clazz, jint latency) {
    ...
    SLDataFormat_PCM format_pcm = {SL_DATAFORMAT_PCM, numTxChannels, SL_SAMPLINGRATE_48,
...
void Java_com_android_nativeaudiolatency_NativeAudioLatency_startRecording(JNIEnv* env, jclass clazz, jint latency) {
    ...
    if (property_get("vendor.audio_hal.period_size", value, NULL) > 0)
        recorderFrames = atoi(value) * 2;
    else
        recorderFrames = CAPTURE_BUFFER_SIZE;
...
void Java_com_android_nativeaudiolatency_NativeAudioLatency_init(JNIEnv* env, jclass clazz) {
    ...
    if (property_get("vendor.audio_hal.period_size", value, NULL) > 0) {
        playbackBufSize = atoi(value) * bytesPerSample * numRxChannels;
        recorderFrames = atoi(value) * bytesPerSample * numTxChannels;
    }
    else {
        playbackBufSize = PLAYBACK_BUFFER_SIZE;
        recorderFrames = CAPTURE_BUFFER_SIZE;
    }

    if (playbackBufSize > PLAYBACK_BUFFER_SIZE)
        playbackBufSize = PLAYBACK_BUFFER_SIZE;
