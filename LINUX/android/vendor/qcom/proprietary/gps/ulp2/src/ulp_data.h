...
#define ULP_HIGH_ACCURACY_MIN_TRACKING_INTERVAL_MSEC                 100          // 100 msec
#define ULP_LOW_ACCURACY_MIN_TRACKING_INTERVAL_MSEC                  (5 * 1000)   // 5 seconds
#define ULP_EMERGENCY_LOW_ACCURACY_MIN_TRACKING_INTERVAL_MSEC        (2 * 1000)   // 2 seconds
...
#define ULP_ZPP_MIN_TRIGGER_INTERVAL                 60000        // 60 seconds
#define ULP_ZPP_ACCURACY_THRESHOLD                   20000        // 20000 meters
...
#define HW_FLP_PROVIDER_NAME                          "HW-FLP"
...
{
   DR_STATE_IDLE            = 0,
   DR_STATE_INITIALIZING    = 1,
   DR_STATE_HIGH            = 2,
   DR_STATE_MEDIUM          = 3,
   DR_STATE_LOW             = 4,
   DR_STATE_FAILED          = 5,
   DR_STATE_MAX_VALUE       = 0x10000000 // force enum to be 32 bit
} dr_state_e_type;

// NMEA Provider Source
typedef enum
{
   NMEA_PROVIDER_AP            = 0, // Application Processor
   NMEA_PROVIDER_MODEM         = 1  // Modem Processor
}nmea_provider_e_type;
...
typedef struct
{
   ...
   LocGpsPositionRecurrence recurrence_type;
   uint32_t              fix_interval;
   LocGpsPositionMode    position_mode;
   uint32_t              accuracy;
...
typedef struct
{
   ...
   uint32_t               zpp_accuracy_threshold;
...
// Data structure that used to save external DR provider related info, eg:
// External DR state and etc
typedef struct
{
   bool               enabled;         // External DR is enabled or not, via UI setting
   dr_state_e_type    state;           // Ext. DR current state
   bool               selected_to_run; // This flag will be set to true when GPS provider
                                       // can be invoked to satisfy the requests. This flag
                                       // is and is only determined by fix requests.
                                       // Whehter this provider will be run or not
...
typedef struct
{
   ...
   bool                    dr_provider_request_active;
   bool                    high_accuracy_request_active;
   bool                    low_accuracy_request_active;
   bool                    nlp_provider_request_active;
   bool                    zpp_request_active;
   UlpRecurrenceCriteria   quipc_recurrence_type;
   uint32_t                quipc_fix_interval;
   UlpRecurrenceCriteria   gnss_recurrence_type;
   uint32_t                gnss_fix_interval;
   UlpRecurrenceCriteria   nlp_recurrence_type;
   uint32_t                nlp_fix_interval;
   uint32_t                gnss_interval_cache;
   UlpRecurrenceCriteria   gnss_recurrence_type_cache;
   bool                    emergency_request;

   // Data related to phone context info
   bool                    phoneSettingReceived;
   ...
   dr_provider_info_s_type      dr_provider_info;    /*ULP DR Provider for external DR solution*/
   ...
   ulp_gnss_status_callback       ulp_gnss_status_cb;
   ...
   ulp_nmea_callback              ulp_nmea_cb;
   ulp_svinfo_callback            ulp_svinfo_cb;
} ulp_data_s_type;
