...
static int ulp_msg_process_send_sv_external_req (const GnssSvNotification& svNotify);
...
static int ulp_msg_process_gnss_sv_report (const GnssSvNotification& svNotify,
                                           LocSvInfoSource svSource);
...
void ulp_msg_main(void * /*context*/)
{
   ...
   while (1)
   {
      ...
      case ULP_MSG_REPORT_SV:
      {
         ...
         ulp_msg_process_gnss_sv_report (svMsg->svNotify, svMsg->svSource);
         break;
      }
      ...
      case ULP_MSG_SEND_SV_EXTERNAL_REQ:
      {
         ...
         ulp_msg_process_send_sv_external_req(svMsg->svNotify);
         break;
      }
      ...
      case ULP_MSG_REPORT_BATCHING_SESSION:
      {
          ...
          LOC_LOGV("minInterval: %u\n minDistance: %u\n mode: %u\n active: %u\n",
            batchingSessionMsg->options.minInterval,
            batchingSessionMsg->options.minDistance,
            batchingSessionMsg->options.mode,
...
int ulp_msg_process_start_req (void)
{
   ...
   do
   {
      ...
      /* If phone context setting not Received, We request phone context setting upon receiving
       first start request */
      if (!ulp_data.phoneSettingReceived && ulp_data.phoneSettingRequested == false)
      {
...
int ulp_msg_process_phone_setting_update (const UlpPhoneContextSettings* phoneSettingPtr)
{
   ...
   do
   {
      ...
      ulp_data.phoneSettingReceived = true;
...
int ulp_msg_process_network_position_report (const UlpNetworkPositionReport* networkPositionPtr)
{
   ...
   do
   {
      ...
      memset(&location_extended, 0, sizeof(GpsLocationExtended));
      ...
      if (networkPositionPtr->valid_flag & ULP_NETWORK_POSITION_HAS_ALTITUDE) {
          ulpLocation.gpsLocation.flags |= LOC_GPS_LOCATION_HAS_ALTITUDE;
          ulpLocation.gpsLocation.altitude = networkPositionPtr->position.altitudeWrtEllipsoid;
      }

      if (networkPositionPtr->valid_flag & ULP_NETWORK_POSITION_HAS_VERTICAL_UNC) {
          location_extended.flags |=  GPS_LOCATION_EXTENDED_HAS_VERT_UNC;
          location_extended.vert_unc =
                  networkPositionPtr->position.verticalUncertainity;
      }

      if (networkPositionPtr->valid_flag & ULP_NETWORK_POSITION_HAS_SPEED) {
          ulpLocation.gpsLocation.flags |= LOC_GPS_LOCATION_HAS_SPEED;
          ulpLocation.gpsLocation.speed =
                  networkPositionPtr->position.horizontalSpeed;
      }

      if (networkPositionPtr->valid_flag & ULP_NETWORK_POSITION_HAS_SPEED_UNC) {
          location_extended.flags |= GPS_LOCATION_EXTENDED_HAS_SPEED_UNC;
          location_extended.speed_unc =
                  networkPositionPtr->position.horizontalSpeedUnc;
      }

      if (networkPositionPtr->valid_flag & ULP_NETWORK_POSITION_HAS_BEARING) {
          ulpLocation.gpsLocation.flags |= LOC_GPS_LOCATION_HAS_BEARING;
          ulpLocation.gpsLocation.bearing =
                  networkPositionPtr->position.bearing;
      }

      if (networkPositionPtr->valid_flag & ULP_NETWORK_POSITION_HAS_BEARING_UNC) {
          location_extended.flags |= GPS_LOCATION_EXTENDED_HAS_BEARING_UNC;
          location_extended.bearing_unc =
                  networkPositionPtr->position.bearingUncertainity;
      }
                                           ...
                                           LocSvInfoSource svSource)
{
   ...
   do
   {
      ...
      ulp_data.loc_proxy->getAdapter()->
          reportSvEvent(svNotify, true/*fromUlp*/);

      //Send svinfo to registered clients
      if(NULL != ulp_data.ulp_svinfo_cb)
      {
         ulp_data.ulp_svinfo_cb(&svNotify, svSource);
      } else {
         LOC_LOGE("No callback function registered for -> ulp_svinfo_cb");
      }

      ret_val = ulp_brain_process_gnss_sv_report (svNotify);
                                          ...
                                          const GpsLocationExtended& locationExtended)
{
   ...
   do
   {
      ...
      locationCpy = location;
      locationCpy.gpsLocation.size = sizeof(locationCpy.gpsLocation);
      locationExtendedCpy = locationExtended;
      locationExtendedCpy.size = sizeof(locationExtendedCpy);

      LOC_LOGV("Sending GNSS Pos to External DR, Lat: %f, Long: %f",
               location.gpsLocation.latitude, location.gpsLocation.longitude);

      //Send GnssPosition to External DR for consumption
      ulp_data.loc_proxy->sendGnssPositionToDr(location,locationExtended,status,tech_mask);
...
===========================================================================*/
int ulp_msg_process_send_sv_external_req (const GnssSvNotification& svNotify)
{
   int ret_val = -1;
   GpsLocationExtended locationExtendedCpy;
   ...
   do
   {
      ...
      LOC_LOGV("Sending GNSS Sv to AP DR, #Svs: %zu ",
               svNotify.count);

      //Send GnssPosition to External DR for consumption
      ulp_data.loc_proxy->sendGnssSvToDr(svNotify);
      //Send raw svinfo to registered clients
      if(NULL != ulp_data.ulp_svinfo_cb)
      {
         ulp_data.ulp_svinfo_cb(&svNotify, ULP_SVINFO_IS_FROM_GNSS);
      }
      else {
         LOC_LOGE("No callback function registered for -> ulp_svinfo_cb");
      }
                                             ...
                                             GpsLocationExtended* extendedLocations)
{
   ...
   do
   {
      ...
      {
         LOC_LOGE("%s:%d]: invalid message, num locations = %d, tech mask ptr = %p,"
                  "locations ptr = %p, extendedLocations = %p",
                                          ...
                                          const GpsLocationExtended* locationExtendedPtr)
{
   ...
   do
   {
      ...
      if ((ULP_LOCATION_IS_FROM_HYBRID |
          ...
          LOC_LOGV ("LocTech-Value :: Vertical Accuracy: %f",  locationExtendedPtr->vert_unc);
          LOC_LOGV ("LocTech-Value :: Speed Accuracy: %f",  locationExtendedPtr->speed_unc);
          LOC_LOGV ("LocTech-Value :: Bearing Accuracy: %f",  locationExtendedPtr->bearing_unc);
          LOC_LOGV ("LocTech-Value :: UTC Time: %" PRIu64,
                  locationPtr->gpsLocation.timestamp);
      ...
      if ((ULP_LOCATION_IS_FROM_GNSS | ULP_LOCATION_IS_FROM_EXT_DR) & locationPtr->position_source)
      {
          ...
          ulp_data.loc_proxy->getAdapter()->
              reportPositionEvent(*locationPtr, gpsLocationExtended,
                                  status, tech_mask, true/*fromUlp*/);
      }
...
int ulp_msg_process_dr_set_pos_mode (uint32_t min_interval,
        LocGpsPositionRecurrence /*recurrence*/, LocPositionMode pos_mode)
{
   int ret_val = -1;
   ENTRY_LOG_CALLFLOW();
   UlpLocationCriteria native_criteria;
...
static void ulp_msg_process_nmea_report(const UlpNmea *pNmeaStr)
{
   if (pNmeaStr){
       ...
       ulp_data.loc_proxy->getAdapter()->reportNmeaEvent(pNmeaStr->nmea_str, pNmeaStr->len,
                                                             true/*fromUlp*/);
       if(NULL != ulp_data.ulp_nmea_cb)
       {
           ulp_data.ulp_nmea_cb(pNmeaStr);
       } else {
           LOC_LOGE("No callback function registered for -> ulp_nmea_cb");
       }
   }
}
