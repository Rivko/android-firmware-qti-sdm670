              ...
              UlpPhoneContextCallbacks* pPhoneContextCallbacks)
{
   ...
   ulp_data.ulp_gnss_status_cb = pEngineCallbacks->gnss_status_cb;
   ...
   ulp_data.ulp_nmea_cb =  pEngineCallbacks->nmea_cb;
   ulp_data.ulp_svinfo_cb =  pEngineCallbacks->svinfo_cb;
...
int ulp_engine_update_criteria(UlpLocationCriteria criteria)
{
    ...
    LOC_LOGD ("LocTech-Label :: ULP :: Criteria");
    LOC_LOGD("valid 0x%x action:%d, minTime:%u, minDistance:%f, singleShot:%d, "
             "provider_source:%d, horizontalAccuracy:%d, powerRequirement:%d, "
             "emergency_request:%d",
              criteria.valid_mask, criteria.action, criteria.min_interval,
              criteria.min_distance, criteria.recurrence_type,
              criteria.provider_source, criteria.preferred_horizontal_accuracy,
              criteria.preferred_power_consumption, criteria.emergency_request);
...
int ulp_send_network_position(UlpNetworkPositionReport * position_report)
{
    ENTRY_LOG();
    int ret_val = 0;
    LOC_LOGV ("LocTech-Label :: ULP :: Network Position Report In");
    LOC_LOGV ("LocTech-Value :: Position Report: network position valid flag = 0x%x, lat = %f, lon = %f, hepe = %f position source = %d\n",
               position_report->valid_flag,
               position_report->position.latitude,
               position_report->position.longitude,
               position_report->position.HEPE,
               position_report->position.pos_source);
...
/**
 * @brief Report GNSS status.
 * @details Report GNSS status.
 *
 * @param status GNSS status.
 * @return Zero for success or non zero for failure.
 */
int ulp_main_report_gnss_status (const uint16 status) {
  ulp_data.ulp_gnss_status_cb (status);
  return 0;
}
