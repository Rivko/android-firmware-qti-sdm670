...
static int threshold = ULP_ZPP_MIN_TRIGGER_INTERVAL;
static int accuracy_threshold = ULP_ZPP_ACCURACY_THRESHOLD;
...
static loc_param_s_type izat_conf_param_table[] =
{
    ...
    {"ZPP_ACCURACY_THRESHOLD",&accuracy_threshold, NULL, 'n'},
};

static loc_param_s_type gps_conf_param_table[] =
{
    {"EXTERNAL_DR_ENABLED", &ulp_data.external_dr_enabled, NULL, 'n'},
    {"NMEA_PROVIDER", &ulp_data.nmea_provider, NULL, 'n'},
};
...
int ulp_data_init ()
{
   ...
   ulp_data.phoneSettingReceived = false;
   ...
   memset (&ulp_data.dr_provider_info, 0, sizeof (dr_provider_info_s_type));
   ...
   ulp_data.dr_provider_info.last_started_time_ms     = 0;
   ulp_data.dr_provider_info.last_stopped_time_ms     = 0;
   ulp_data.dr_provider_info.last_position_received.gpsLocation.flags = 0;
   ulp_data.dr_provider_info.last_position_received_time_ms = 0;
   ...
   LOC_LOGI("%s: External DR Enabled: %d", __func__, ulp_data.external_dr_enabled);

   if(1 == ulp_data.external_dr_enabled)
   {
      ulp_data.loc_proxy = new LocAmtProxy();
      if (ulp_data.loc_proxy->DRInit((void *)ulp_data.ulp_create_thread_cb))
      {
        LOC_LOGI("%s: DRInit Successful - using AmtProxy", __func__);
      }
      else
      {
         LOC_LOGE("%s: DRInit Failed - using UlpProxy", __func__);
         delete ulp_data.loc_proxy;
         ulp_data.loc_proxy = NULL;

         //Initialize LocUlpProxy -- default path if DRInitialization fails.
         ulp_data.loc_proxy = new LocUlpProxy();
      }
   }
   else
   {
        //Initialize LocUlpProxy the default path if DRInitialization fails.
        ulp_data.loc_proxy = new LocUlpProxy();
   }
   ...
   pthread_cond_init(&ulp_data.zpp_provider_info.tCond, &condAttr);
   ...
   ulp_data.zpp_provider_info.zpp_accuracy_threshold = (long)accuracy_threshold;
   LOC_LOGI ("%s, quipc_enabled = %d, gps capabilities = 0x%lx "
               "zpp trigger threshold = %d, zpp_accuracy_threshlod = %d\n",
             __func__, ulp_data.quipc_enabled,
             ulp_data.loc_proxy->mCapabilities,
             ulp_data.zpp_provider_info.zpp_trigger_threshold,
             ulp_data.zpp_provider_info.zpp_accuracy_threshold);
...
int ulp_data_add_criteria (const UlpLocationCriteria* locationCriteriaPtr)
{
   ...
   for (i = 0; i < MAX_NUM_UNIQUE_CRITERIA; i++)
   {
      ...
      {
         //If the incoming Criteria update is for GPS provider then we want to simply update TBF
         //and recurrence attributes if an existing GPS provider criteria is present in the queue.
                                     ...
                                     const UlpLocationCriteria* criteriaPtr2)
{
   ...
   do
   {
      ...
      if ((criteriaPtr1->valid_mask & ULP_CRITERIA_HAS_EMERGENCY_TYPE) &&
          (criteriaPtr2->valid_mask & ULP_CRITERIA_HAS_EMERGENCY_TYPE))
      {
        if (criteriaPtr1->emergency_request != criteriaPtr2->emergency_request)
        {
            break;
        }
      }
