...
int  LBSApiBase::injectBtClassicDevScanData(const BtDeviceInfo& /*btDevInfo*/)
DEFAULT_IMPL(0)

int  LBSApiBase::injectBtLeDevScanData(const BtDeviceInfo& /*btDevInfo*/)
DEFAULT_IMPL(0)
...
int  LBSApiBase::batteryLevelInject(int batteryPct)
DEFAULT_IMPL(0)
...
void LBSApiBase::reportFdclInfoEvent(FdclInfo& fdclInfo)
{
    // loop through adapters, and deliver to the first handling adapter.
    TO_1ST_HANDLING_LBSADAPTERS(mLBSAdapters[i]->reportFdclInfoEvent(fdclInfo));
}

bool LBSApiBase::injectFdclData(const FdclData& fdclData)
DEFAULT_IMPL(false)

bool LBSApiBase::requestFdclData(int32_t expireInDays)
DEFAULT_IMPL(false)
