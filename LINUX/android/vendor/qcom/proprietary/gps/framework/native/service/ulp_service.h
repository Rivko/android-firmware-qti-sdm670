...
typedef enum{
    ...
    ULP_LOC_SCREEN_OFF,
    ULP_LOC_GPS_SETTINGS_OFF,
    ULP_LOC_GPS_SETTINGS_ON,
    ULP_LOC_EVENT_OBSERVER_STOP_EVENT_TX
}UlpSystemEvent;
...
#define ULP_CRITERIA_HAS_EMERGENCY_TYPE                0x00008000

#define ULP_PROVIDER_SOURCE_GNSS                       0x00000001
#define ULP_PROVIDER_SOURCE_HYBRID                     0x00000002
#define ULP_PROVIDER_SOURCE_NLP                        0x00000004
#define ULP_PROVIDER_SOURCE_DR                         0x00000008
 ...
 */
typedef void (* ulp_location_callback)(UlpLocation* location, GpsLocationExtended* locationExtended, enum loc_sess_status status);

/** Callback with status information.
 */
typedef void (* ulp_gnss_status_callback) (const LocGpsStatusValue status);

/** Callback with nmea information.
 */
typedef void (* ulp_nmea_callback) (const UlpNmea* nmea);

/** Callback with sv information.
 */
typedef void (* ulp_svinfo_callback) (const GnssSvNotification* svNotify,
                                      uint16_t source);
...
typedef struct {
    ...
    ulp_gnss_status_callback gnss_status_cb;
    ulp_create_thread create_thread_cb;
    ulp_nmea_callback nmea_cb;
    ulp_svinfo_callback svinfo_cb;
} UlpEngineCallbacks;
...
/** Request for single network position */
#define ULP_NETWORK_POS_START_SINGLE_REQUEST                (0x03)
/** Request last known location   */
#define ULP_NETWORK_POS_GET_LAST_KNOWN_LOCATION_REQUEST     (0x04)
/** Cancel request */
#define ULP_NETWORK_POS_STOP_REQUEST                        (0x05)
...
typedef struct  {
    ...
    float HEPE;
    /** Altitude wrt Ellipsoid in degrees */
    double altitudeWrtEllipsoid;
    /** Vertical error estimate in meters */
    float verticalUncertainity;
    /** Horizontal speed in meters per second */
    float horizontalSpeed;
    /** Horizontal speed error estimate in meters per second */
    float horizontalSpeedUnc;
    /** Bearing in degrees */
    float bearing;
    /** Bearing error esimate in degrees */
    float bearingUncertainity;
} UlpNetworkPosition;
...
/** altitude is valid */
#define ULP_NETWORK_POSITION_HAS_ALTITUDE         (0x08)
/** vertical uncertainity is valid */
#define ULP_NETWORK_POSITION_HAS_VERTICAL_UNC     (0x10)
/** Horizontal speed is valid */
#define ULP_NETWORK_POSITION_HAS_SPEED            (0x20)
/** Horizontal speed uncertainity is valid */
#define ULP_NETWORK_POSITION_HAS_SPEED_UNC        (0x40)
/** Bearing is valid */
#define ULP_NETWORK_POSITION_HAS_BEARING          (0x80)
/** Bearing uncertainity is valid */
#define ULP_NETWORK_POSITION_HAS_BEARING_UNC      (0x100)
