...
#define RX_SYNC_CMD_TIMEOUT_MS   90000
#define RX_CMD_TIMEOUT_MS        90000
...
typedef enum {                         // STATE :
	...
	PM_STATE,                 //
	NARTCMD_STATE,
	SAME_STATE,               //: size, SAME_STATE doesn't have an entry in the STATE table
...
typedef enum {              // EVENT
	...
	EVT_NULL,                  //
	EVT_CMDPROC_LAST_EVENT = EVT_NULL,    //
...
#define TEST_CONFIG_CAL             0x00000002
...
typedef struct _utfCmdprocessStateInfo {
	...
	uint8_t              failRetrySteps;
	uint8_t              failRetryCount;
	uint16_t             paddingFail;

	_FAIL_RETRY          timeoutRetry;
	uint8_t              timeoutRetrySteps;
	uint8_t              timeoutRetryCount;
	uint16_t             paddingTimeout;
