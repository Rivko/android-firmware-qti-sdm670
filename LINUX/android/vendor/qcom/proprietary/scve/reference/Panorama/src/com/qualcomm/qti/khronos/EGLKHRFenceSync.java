...
public class EGLKHRFenceSync {
    ...
    public static EGLSyncKHR eglCreateSyncKHR(EGLDisplay dpy, int type, int[] attrib_list) {
        long eglSync = _eglCreateSyncKHR(EGL14Helper.getEGLObjectHandle(dpy), type, attrib_list);
        return (eglSync == 0 ? EGL_NO_SYNC : EGLSyncKHR.create(eglSync));
    }

    public static boolean eglDestroySyncKHR(EGLDisplay dpy, EGLSyncKHR sync) {
        return _eglDestroySyncKHR(EGL14Helper.getEGLObjectHandle(dpy),
                EGL14Helper.getEGLObjectHandle(sync));
    }

    public static int eglClientWaitSyncKHR(EGLDisplay dpy, EGLSyncKHR sync, int flags,
            long timeout) {
        return _eglClientWaitSyncKHR(EGL14Helper.getEGLObjectHandle(dpy),
                EGL14Helper.getEGLObjectHandle(sync), flags, timeout);
    }

    public static boolean eglGetSyncAttribKHR(EGLDisplay dpy, EGLSyncKHR sync, int attribute,
            int[] value) {
        return _eglGetSyncAttribKHR(EGL14Helper.getEGLObjectHandle(dpy),
                EGL14Helper.getEGLObjectHandle(sync), attribute, value);
    }
