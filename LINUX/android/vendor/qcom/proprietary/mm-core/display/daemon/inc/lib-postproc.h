...
struct display_hw_info {
    ...
    uint32_t nPriDisplayLutMaxVal;
    uint32_t nPriDisplayLutBitDepth;
};
...
#define PCC_COEFF_IDX_C     0
#define PCC_COEFF_IDX_R     1
#define PCC_COEFF_IDX_G     2
#define PCC_COEFF_IDX_B     3
#define PCC_COEFF_IDX_RR    4
#define PCC_COEFF_IDX_GG    5
#define PCC_COEFF_IDX_BB    6
#define PCC_COEFF_IDX_RG    7
#define PCC_COEFF_IDX_GB    8
#define PCC_COEFF_IDX_RB    9
#define PCC_COEFF_IDX_RGB   10
...
struct display_pp_hist_lut_data - Structure for histogram look up table data
    ...
    data   - histogram LUT data
 */

struct display_pp_hist_lut_data{
    uint32_t ops;
    uint32_t data[MAX_HIST_LUT_ENTRIES];
};
...
/*
struct display_pp_pa_pwl_cfg - Structure used for Saturation and Value
       based PWL trapezoidal curves. These are used in combination with the
       main Hue based PWL trapezoidal curve for determining memory color
       adjustment.
    slope - floating point slope of Sat/Val PWL trapezoid side, (-32768 - 32767)
    offset - floating point offset from minimum sat/val value to top corner of
            Sat/Val trapezoid. For saturation PWL, range is (0 - 1.0)
            For value PWL, range is (0 - 255)
    left_hold/right_hold - boolean value for determining the the Sat/Val PWL weight
            If the hold value is set to true (1), then the weight of the PWL will
            be the same on the trapezoid edges as the middle of the trapezoid. If
            the hold value is set to false (0), then the weight of the PWL on the
            trapezoid edges will be equal to (pixel sat/val - minSat/Val)
*/
struct display_pp_pa_pwl_cfg
{
    float slope;
    float offset;
    uint8_t left_hold;
...
struct display_pp_mem_col_cfg - Structure for Memory Color adjustment data
    ...
    blend_gain - Weight gain for blending memory color with other PA
            features. Only uses in BLEND_EN = 1
    sat_pwl - PWL structure for saturation trapezoid
    val_pwl - PWL structure for value trapezoid
*/
struct display_pp_mem_col_cfg
{
    ...
    /* v1_7 PA parameters*/
    uint32_t blend_gain;
    struct display_pp_pa_pwl_cfg sat_pwl;
    struct display_pp_pa_pwl_cfg val_pwl;
};
...
struct display_pp_six_zone_cfg - Structure for Six Zone adjustment data
    ...
    sat_min - min sat for six zone sat adjustment (0 - 1.0)
    val_min - min val for six zone val adjustment (0 - 255)
    val_max - max val for six zone val adjustment (0 - 255)
    sat_max - max sat for six zone sat adjustment (0 - 1.0) - Only in PA v1_7
    sat_pwl - PWL structure for saturation trapezoid
    val_pwl - PWL structure for value trapezoid
*/
struct display_pp_six_zone_cfg
{
    ...
    int32_t val_min;
    int32_t val_max;
    /* sat_max - v1_7 PA parameter only */
    float sat_max;
    /* v1_7 PA parameters*/
    struct display_pp_pa_pwl_cfg sat_pwl;
    struct display_pp_pa_pwl_cfg val_pwl;
...
struct display_pp_pa_v2_cfg - Structure for Picture Adjustmemt V2 data
    ...
    mode - PA v1_7 parameter only. In PA v1_7 the above bits 12-23 are placed in
            mode rather than ops.
            The following bits are also used in PA v1_7 mode
        bit 1 - Memory protection global Hue Enable, allow pixel to be adjusted
            based on memory color weight
        bit 2 - Memory protection global Sat Enable, allow pixel to be adjusted
            based on memory color weight
        bit 3 - Memory protection global Val Enable, allow pixel to be adjusted
            based on memory color weight
        bit 4 - Memory protection global Contrast Enable, allow pixel to be adjusted
            based on memory color weight
        bit 5 - Memory protection Six Zone Enable - Pixel adjusted by six zone
            based on memory color weight.
        bit 6 - Memory protection Blend enable, allow pixel adjusted by memory color
            to be adjusted by other forms of picture adjustment
...
int display_pp_foss_supported(void);
