                                   ...
                                   unsigned long nLen)
{
    ...
    if( pIn && pOut && nLen )
    {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "HDCPManager Time Taken Decrypt = %lu", endtime-starttime);
        if(nStatus)
        {
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
...
void HDCPManager::constructCodecAndStreamType(int nStream, HDCPCodecType eCodec)
{
    ...
    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                "HDCPManager::constructCodecAndStreamType [%d] -> %d",
                nStream,eCodec);

    if (nStream == HDCP_AUDIO_TRACK_ID) // Audio Stream
    {
        // then its a LPCM
        m_nAudioStreamType = ((eCodec << 16) | DX_HDCP_STREAM_TYPE_AUDIO);
        m_bAudioCodecInfoAvailable = true;
    }
    else if (nStream == HDCP_VIDEO_TRACK_ID) // Video stream
    {
        m_nVideoStreamType = ((eCodec << 16) | DX_HDCP_STREAM_TYPE_VIDEO);
        m_bVideoCodecInfoAvailable = true;
    }
    else
    {
