...
bool CWFD_HdcpCp::m_bReAuthRequired;
  ...
  unsigned long  ulPayloadLen )
{
  ...
  MM_CriticalSection_Enter(m_encryptCriticalSectionHandle);

  if(m_bReAuthRequired)
  {
    uint32_t ret = DX_HDCP_Tsmt_ReAuthenticate(m_ucHdcpClientID,
                                               HDCP_CP_REAUTH_TIMEOUT);
    if(ret == 0)
    {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"HDCP ReAuthenticate success");
      m_bReAuthRequired = false;
    }
    else
    {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"HDCP ReAuthenticate failed");
      m_bKeepGoing = false;
      MM_CriticalSection_Leave(m_encryptCriticalSectionHandle);
      m_eHdcpSessionState = HDCP_SESSION_STATE_ERROR;
      return QC_HDCP_NON_AUTHENTICATED_CONNECTION;
    }
  }
...

{
    ...
    switch(eHdcpEvent)
    {
      ...
      case DX_HDCP_EVENT_REAUTHENTICATE_REQUEST:
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"DX_HDCP_EVENT_REAUTHENTICATE_REQUEST!!");
        CWFD_HdcpCp::m_bReAuthRequired = true;
        break;
