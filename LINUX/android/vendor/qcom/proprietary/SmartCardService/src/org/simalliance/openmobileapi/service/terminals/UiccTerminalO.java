...
public class UiccTerminalO extends Terminal {
    ...
    public String getCallingPackageName(int uid, Context context){
        ...
        Log.e(_TAG, "Caller PackageName can not be determined");
        return null;
    }
    ...
    protected int internalOpenLogicalChannel(byte[] aid, byte p2) throws CardException,
        ...
        mSelectResponse = null;
        int p2int = p2 & 0xff;
        String packageName = getCallingPackageName(Binder.getCallingUid(), mContext);
        for (int i = 1; i < channelId.length; i++) {
            if (channelId[i] == 0) {
                try {
                        response = manager.iccOpenLogicalChannel(subId[0], packageName, ByteArrayConverter.byteArrayToHexString(aid), p2int);
