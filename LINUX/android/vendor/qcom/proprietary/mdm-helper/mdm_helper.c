		...
		struct mdm_helper_ops *ops )
{
	...
	if (!strncmp(soc_name, "MDM9x25",6) ||
			!strncmp(soc_name, "MDM9x35", 7) ||
			!strncmp(soc_name, "MDM9x45", 7) ||
			!strncmp(soc_name, "MDM9x55", 7) ||
			!strncmp(soc_name, "SDX50M", 6)) {
		*ops = mdm9k_ops;
		return RET_SUCCESS;
	}
		...
		struct mdm_private_data **pdata)
{
	...
	if ((!strncmp(soc_name, "MDM9x45", 7)) ||
			!strncmp(soc_name, "MDM9x55", 7) ||
			!strncmp(soc_name, "SDX50M", 6)) {
		ALOGI("Found private data for %s", soc_name);
		*pdata = &private_data_9x45;
		free(soc_name);
		free(link_name);
		return RET_SUCCESS;
	}
