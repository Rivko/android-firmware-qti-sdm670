...
// To export the symbols to access from ATEL
...
// command to release the native handle
#define RELEASE_NATIVE_HANDLE 15
...
int cameraId = 1;
int facing = 1;
char CameraID[PROPERTY_VALUE_MAX];
camera_module_t *hw;
...
struct global_imscamera {
  ...
  ims_recording_frame_handle_callback_timestamp frame_hdl_timestamp;
  ims_custom_params_callback custom_cb;
  void *user;
};
/* Assigning all the fields to null */
struct global_imscamera global_imscamera_cb = {NULL, NULL, NULL, NULL, NULL, NULL};
...
void setCameraFPS(CameraParameters *ptr)
{
  ...
  for (i = fpsRangeList.size() - 1; i >= 0; i--) {
    ...
    if (fpsRangeList[i].hi == fps * 1000 && fpsRangeList[i].lo == fps * 1000) {
      char str[20];
  ...
  if (i < 0 && fpsRangeList.size() > 0) {
    fps = fpsRangeList[fpsRangeList.size() - 1].hi / 1000;
    ALOGE("Dint find a fps match. new fps %d", fps);
    char str[20];
    snprintf(str, sizeof(str), "%d,%d", fpsRangeList[fpsRangeList.size() - 1].lo, fpsRangeList[fpsRangeList.size() - 1].hi);
    ptr->set("preview-fps-range", str);
  }
}
...
void startPreviewInternal(ImsCameraClient *ptr)
{
  ...
  callbackParam.cropRotationParam.fps = fps;
...
//Disabling this part of the code as the custom callback will configure the size
#if 0
...
//      ALOGE("cameraHandleLoop::startCameraRecording Storemetadatainbuffers : %d",ptr->iCamera->storeMetaDataInBuffers(1));
    ALOGE("cameraHandleLoop::startCameraRecording	Storemetadatainbuffers : %d", ptr->iCamera->setVideoBufferMode(1));
...
{
  CustomCallbackParam callbackParam;
  callbackParam.type = CAMERA_WAIT_FOR_BUFFERS;
  ...
  while (1) {
    ...
    switch (camera_cmd) {
      case CAMERA_OPEN_CMD: {
          ...
          int CAMERA_TYPE_ALL = 1;
          camera_service->getNumberOfCameras(CAMERA_TYPE_ALL, &mNumberOfCameras);
          ALOGE("cameraHandleLoop::no of camera %d", mNumberOfCameras);
          CameraInfo info;
          binder::Status rc = camera_service->getCameraInfo(cameraId, &info);
          ALOGE("cameraHandleLoop::Camera id %d", cameraId);

//          ALOGE("cameraHandleLoop::return value of camera info %s",rc.toString8());
...
//          if(rc)
          ...
          if (!(rc.isOk())) {
            ...
            camera_service->connect(client, cameraId, (const String16&)*packagename, -1, -1, &ptr->iCamera);
          }
      ...
      case RELEASE_CAMERA_FRAME: {
        ...
        break;
        /*
          Case to handle the release the native handle to camera
        */
      case RELEASE_NATIVE_HANDLE: {
          native_handle_t *hdl = (native_handle_t*)cam_arg1;
          DLOG("cameraHandleLoop::Released recording handle %p", hdl);

          /* Need to release the native handle */
          if (ptr->iCamera.get() != NULL) {
            /* Releasing the recording frame handle. */
            ptr->iCamera->releaseRecordingFrameHandle(hdl);
          }
        }
        break;
        /* Happy to break the case */
...
short cameraOpen2(unsigned int cameraid, String16* apppackagename)
{
  ...
  if (ptr->state == ENGINE_INITIALIZED) {
    ALOGE("imscamera::ImsCameraOpen2 already initialised >");
    return -2;
  }
                        ...
                        void* user)
{
  ...
  if (ptr->state == ENGINE_INITIALIZED) {
    ...
    global_imscamera_cb.frame_hdl_timestamp = frame_hdl_timestamp;
  ...
  property_get("persist.radio.ims.backcamera", CameraID, "0");
  return 0;
}
...
/*===========================================================================
 ...
 FUNCTION        : recordingFrameHandleCallbackTimestamp

 ARGUEMENTS   : tstamp, native handle

 RETURN VALUE : void

 PURPOSE     : To receive the camera frames in native handle format and the timestamp

 SIDE EFFECTS   : None

 ===========================================================================*/

void BufferClient::recordingFrameHandleCallbackTimestamp(nsecs_t timestamp, native_handle_t* handle)
{
  int fd = 0;
  int size = 0;
  DLOG("our recordingFrameHandleCallbackTimestamp handle -> %p timestamp - %lu", handle, timestamp);
  ImsCameraClient *ptr = ImsCameraClient::getInstance();
  ...
  if ( handle != NULL ) {
    fd = handle->data[0];
    size = handle->data[2];
    DLOG("our recordingFrameHandleCallbackTimestamp handle-> %p fd - %d,size - %d", handle, fd, size);
  }

  pthread_mutex_lock(&callbackMutex);
  sem_timedwait(&cameraEventReleaseSem);

  if (global_imscamera_cb.frame_hdl_timestamp && (stopping != 1)) {
    processingCameraFrameCount++;
    // Inovking the callback to thin client to process the native handle
    global_imscamera_cb.frame_hdl_timestamp(timestamp,
                                            handle);
    // Releasing the native handle
    executeCommand(RELEASE_NATIVE_HANDLE, (void*)(handle), 0, 0);
    processingCameraFrameCount--;
  } else {
    ...
    if (ptr->iCamera.get() != NULL) {
      /* Releasing the recording frame handle. */
      ptr->iCamera->releaseRecordingFrameHandle(handle);
    }
  }
...
/* End of recordingFrameHandleCallbackTimestamp */
