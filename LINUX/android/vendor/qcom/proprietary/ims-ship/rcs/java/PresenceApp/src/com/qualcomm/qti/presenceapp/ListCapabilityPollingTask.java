...
public class ListCapabilityPollingTask extends AsyncTask<Void, Integer, Integer> {
   ...
   private String checkPhoneFormat(String number) {
        ...
        for (int count = 0; count <= noOfIterations; count++) {
            ...
            if (count != noOfIterations) {
                ...
                if(AppGlobalState.getPresenceService() != null && AppGlobalState.isCdByOption() == false)
                {
                    ...
                    {
                        Log.d("PRESENCE_UI","ListCapabilityPollingTask added in queue");
                    }
                }
                else if(AppGlobalState.getCdService() != null && AppGlobalState.isCdByOption())
                {
                    ...
                    Log.d("PRESENCE_UI", "ListAvailabilityFetchTask : QCDService_RequestGivenContactsCDInfo :  status.getStatusCode() "+ status.getStatusCode());
    ...
    private static Runnable listCapPollingRunnable = new Runnable() {
        ...
        public void run() {
            Log.d("PRESENCE_UI","inside listCapPollingRunnable size "+AppGlobalState.listCapPollingRequestinfo.size());
            ...
            else
            {
                if(AppGlobalState.listCapPollingRequestinfo.size() == 1)
                {
                    ...
                    {
                        Log.d("PRESENCE_UI","inside listCapPollingRunnable size 1 and request id 0");
                        sendListCapPollingFromQueue();
                    }
                    else
                    {
                        Log.d("PRESENCE_UI","inside listCapPollingRunnable size 1 and request id not 0");
                        AppGlobalState.listCapPollingRequestinfo.remove(0);
                    }
                }
            }
    ...
    public static int sendListCapPollingFromQueue()
    {
        try
        {
            Log.d("PRESENCE_UI","inside sendListCapPollingFromQueue size "+AppGlobalState.listCapPollingRequestinfo.size());
            ...
            {
                Log.d("PRESENCE_UI","sendListCapPollingFromQueue -0");
                return 0;

            }
        } catch (RemoteException e) {
            e.printStackTrace();
            Log.d("PRESENCE_UI","sendListCapPollingFromQueue -2");
            return -2;
        } catch (Exception e) {
            e.printStackTrace();
            Log.d("PRESENCE_UI","sendListCapPollingFromQueue -3");
            return -3;
        }
    }
}
