...
public class Utility {
    ...
    public static TimerTask resetSubscribeTimer(final int index ) {
        ...
        return setTimer(AppGlobalState.getTimerManager(), new Runnable() {
            public void run() {
                ...
                CapabilityPollingTask capabilityPollingTask =
                      (CapabilityPollingTask) new CapabilityPollingTask(
    ...
    if(AppGlobalState.getContacts() != null && AppGlobalState.getContacts().get(index) != null){
        String currentNum =AppGlobalState.getContacts().get(index).getPhone();

        for(String num : AppGlobalState.excludedNumberList) {
      if(currentNum!=null)
        Log.d("UTILITY: currentNum: ", currentNum);
      if(num !=null)
        Log.d("UTILITY: index num: ",num);
            if(AppGlobalState.objectValidator(currentNum) && AppGlobalState.objectValidator(num) && currentNum.startsWith(num)) {
                return true;
            }
        }
    }
    ...
    public static boolean isContactExcluded(Contact c) {
      ...
      if(c!=null && c.getPhone() != null)
          Log.d("UTILITY: c.getPhone()= ",c.getPhone().toString());
        if(num !=null)
          Log.d("UTILITY: c num: ",num);
            if( AppGlobalState.objectValidator(c) && AppGlobalState.objectValidator(c.getPhone()) && AppGlobalState.objectValidator(num) && c.getPhone().startsWith(num) ) {
                return true;
            }
        }
        return false;
