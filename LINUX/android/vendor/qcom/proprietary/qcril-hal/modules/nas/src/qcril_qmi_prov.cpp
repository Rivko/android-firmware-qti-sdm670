...
)
{
    ...
    select_uicc_sub.sub_type   = (RIL_SubscriptionType) qcril_qmi_get_modem_stack_id(); // TODO map??
...
RIL_Errno qcril_qmi_prov_evaluate_and_mark_apps_to_deactivate(void)
{
    ...
    if (UimModuleInterface::get_uim_interface()->get_card_status(
        (unsigned char)instance_id, &ril_card_status) == RIL_E_SUCCESS)
    {
        /* Mark 3gpp sim app */
...
{
    int          user_prov_pref = QCRIL_INVALID_PROV_PREF;
    RIL_UiccSubActStatus act_status = RIL_UICC_SUBSCRIPTION_ACTIVATE;
    ...
    switch(state)
    {
        ...
        ** based on user preference.*/
        case QCRIL_QMI_PROV_STATE_FM_APPLY:
        case QCRIL_QMI_PROV_STATE_CARD_UP:
            act_status = (RIL_UiccSubActStatus) user_prov_pref;
            break;
    }
...
*/
/*=========================================================================*/
RIL_Errno qcril_qmi_prov_update_db_with_user_pref(RIL_UiccSubActStatus user_prov_pref)
{
...
int qcril_qmi_nas_fetch_user_prov_pref_from_database(void)
{
    ...
    qcril_qmi_prov_update_user_pref_cache((RIL_UiccSubActStatus)pref); //NAS REFACTOR:
