...
RilRequestChangeBarringPasswordMessage::~RilRequestChangeBarringPasswordMessage() {
  if (params != nullptr) {
...
void RilRequestChangeBarringPasswordMessage::deepCopy(const ril_request_type &request) {
  char** data = nullptr;
  bool success = false;
  const size_t elements = request.payload_len / sizeof(char*); // should be 3, need check?
...
string RilRequestChangeBarringPasswordMessage::dump() {
  return mName;
}
