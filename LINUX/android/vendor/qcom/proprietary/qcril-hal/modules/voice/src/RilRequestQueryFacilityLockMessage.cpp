...
RilRequestQueryFacilityLockMessage::~RilRequestQueryFacilityLockMessage() {
  if (params != nullptr) {
...
void RilRequestQueryFacilityLockMessage::deepCopy(const ril_request_type &request) {
  char** data = nullptr;
  bool success = false;
  ...
  do {
    ...
    Log::getInstance().d("Query Facility elements:" + std::to_string(elements));
    size_t i = 0;
    for (i = 0; i < elements; i++) {
      int length;
      if ((payload[i]) && (length = strlen(payload[i]))) {
        Log::getInstance().d("index: " + std::to_string(i) + " length = " +
...
string RilRequestQueryFacilityLockMessage::dump() {
  return mName;
}
