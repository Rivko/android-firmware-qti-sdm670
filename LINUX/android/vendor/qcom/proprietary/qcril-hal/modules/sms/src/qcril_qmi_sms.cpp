  ...
  else if (raw_send_resp->resp.result == QMI_RESULT_FAILURE_V01)
  {
    ...
    if (retry_status == TRUE)
    {
      ...
      if (raw_send_resp->resp.error == QMI_ERR_INVALID_OPERATION_V01) {
        ...
        send_msg_response.messageRef = -1;
      }
    }
  ...
  do
  {
    ...
    else if ( ack_ptr->uErrorClass == RIL_CDMA_SMS_ERROR )
    {
      ...
      request_msg.wms_3gpp2_failure_information.tl_status =
        (wms_tl_cause_code_send_ack_enum_v01) ack_ptr->uSMSCauseCode;
    }
...
)
{
  ...
  do
  {
    ...
    for ( i = 0; i < bc_table_size; i++ )
    {
      ...
      (wms_service_category_enum_v01) ( (RIL_CDMA_BroadcastSmsConfigInfo **) params_ptr->data )[ i ]->service_category;
      request_msg.wms_3gpp2_broadcast_config_info[i].language =
      (wms_language_enum_v01) ( (RIL_CDMA_BroadcastSmsConfigInfo **) params_ptr->data )[ i ]->language;
...
)
{
  ...
  do
  {
    ...
    qmi_transport_error = qmi_client_sms_send_sync(QMI_WMS_GET_TRANSPORT_NW_REG_INFO_REQ_V01,
                                                   NULL,
                                                   0,
                                                   &nw_reg_response_msg,
                                                   sizeof(nw_reg_response_msg),
                                                   ModemEndPoint::SYNC_REQ_UNRESTRICTED_TIMEOUT);
    res = qcril_qmi_util_convert_qmi_response_codes_to_ril_result(qmi_transport_error, &nw_reg_response_msg.resp);
    if (res != RIL_E_SUCCESS)
    {
...
)
{
  ...
  req_data.data = (void*) resp_c_struct.get();
  /*-----------------------------------------------------------------------*/
...
)
{
  ...
  buf_ptr = (char *) qcril_malloc( QCRIL_SMS_BUF_MAX_SIZE );
  if ( buf_ptr == NULL )
  {
...
)
{
    ...
    ( event_report_ind->call_control_info.alpha_id_len > 0 ) )
  {
    ...
    if ( *buf_str )
    {
        qcril_default_unsol_resp_params( instance_id, RIL_UNSOL_STK_CC_ALPHA_NOTIFY, &unsol_resp1 );
...
)
{
  ...
  do
  {
    ...
    if (sum_len > 0)
    {
      mwi_msg_summary_ptr = (ims_MwiMessageSummary*) qcril_malloc(sizeof (ims_MwiMessageSummary) *
                                             sum_len);
      mwi_msg_summary_dptr = (ims_MwiMessageSummary**) qcril_malloc(sizeof (ims_MwiMessageSummary*) *
                                             (sum_len+1));
        ...
        qmi_mwi_msg->message_waiting_detail_info_len > 0)
    {
      ...
      mwi_msg_details_ptr  = (ims_MwiMessageDetails*) qcril_malloc(sizeof (ims_MwiMessageDetails)  * detail_len);
      mwi_msg_details_dptr = (ims_MwiMessageDetails**) qcril_malloc(sizeof (ims_MwiMessageDetails*) * (detail_len+1));
...
{
    SmsModule *module = getSmsModule();
    module->qcrilHalSmsModuleCleanup();
