...
)
{
  ...
  if ( qcril_reqlist_query_by_req_id( req_id, &instance_id, &req_info ) == E_SUCCESS )
  {
    ...
    else
    {
      ...
      switch(msg_id)
      {
        case QMI_IMS_SETTINGS_SET_IMS_SERVICE_ENABLE_CONFIG_RSP_V01:
...
)
{
  ...
  if (NULL != params_ptr)
  {
    resp = (ims_settings_set_ims_service_enable_config_rsp_msg_v01*)params_ptr->data;
...
)
{
  ...
  do
  {
    ...
    qmi_req.rtt_data_len = strnlen((char*)ims_rtt_msg_ptr->rttMessage.arg,
...
)
{
  ...
  if (NULL != params_ptr)
  {
    resp = (ims_settings_send_rtt_resp_msg_v01*)params_ptr->data;
...
void qcril_qmi_imss_rtt_rcvd_data_ind_hdlr(void *ind_data_ptr)
{
  ...
  if( rtt_message_incoming != NULL)
  {
    ...
    {
      strlcpy((char*)resp.rttMessage.arg, (char*)rtt_message_incoming->rtt_data,
                                    rtt_msg_len);
      QCRIL_LOG_INFO("Rtt incoming Message %s",(char*)resp.rttMessage.arg);
