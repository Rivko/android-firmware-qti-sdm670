...
#include "modules/data/SetRatPrefMessage.h"
...
uint8_t qcril_qmi_imss_update_modem_version(void);
  ...
  do
  {
        ...
        ((ims_StatusForAccessTech**)ims_in_data_ptr->accTechStatus.arg)[0]->has_status)
    {
           ...
           ims_RadioTechType_RADIO_TECH_LTE))
      {
        ...
        {
          // Store the VoLTE enabled status
          reqlist_entry.req_datalen = sizeof(enabled);
          reqlist_entry.req_data = &enabled;
        }
      }
      ...
      if (qmi_client_provisioning_req.wifi_call_valid)
      {
      ...
      {
        if (feature_voice_dom_pref_on_toggle_ims_cap &&
            qmi_req.volte_enabled_valid && qmi_req.volte_enabled)
        {
          // Make sure to send the response only when we set voice_domain_preference and
          // volte_enabled.
          res = qcril_qmi_imss_nas_set_voice_domain_preference(NAS_VOICE_DOMAIN_PREF_PS_PREF_V01,
              reqlist_entry.req_id);
          break;
        }
...
)
{
  ...
  do
  {
    ...
    if (ims_volte_user_pref_ptr->has_mode)
    {
      ...
      if (qcril_reqlist_new(QCRIL_DEFAULT_INSTANCE_ID, &reqlist_entry) == E_SUCCESS)
      {
        res = qcril_qmi_imss_nas_set_voice_domain_preference(
...
)
{
    ...
    do
    {
        ...
        reqlist_entry.req_data = (ims_WifiCallingInfo*)qcril_malloc(sizeof(ims_WifiCallingInfo));
...
)
{
  ...
  do
  {
    ...
    {
        config_params.config_item_value_len = strlen((const char*)ims_config_msg_ptr->stringValue.arg);
...
)
{
  ...
  if (params_ptr)
  {
    resp = (ims_settings_set_reg_mgr_config_rsp_msg_v01*)params_ptr->data;
    if (resp)
    {
...
)
{
  ...
  do
  {
    ...
    if (req_info.request == QCRIL_EVT_IMS_SOCKET_REQ_SET_SERVICE_STATUS)
    {
      ...
      if (feature_voice_dom_pref_on_toggle_ims_cap &&
          (req_info.req_datalen == sizeof(uint8_t) && req_info.req_data != NULL))
      {
        uint8_t volte_enabled = *((uint8_t *)req_info.req_data);
            ...
            (volte_enabled && (ril_err != RIL_E_SUCCESS)))
        {
          ...
          qcril_qmi_imss_nas_set_voice_domain_preference(NAS_VOICE_DOMAIN_PREF_CS_ONLY_V01,
              req_info.req_id);
          send_response = FALSE;
          break;
        }
...
)
{
  ...
  if (NULL != params_ptr)
  {
    resp = (ims_settings_set_qipcall_config_rsp_msg_v01*)params_ptr->data;
    ...
    if (params_ptr->event_id == QCRIL_EVT_IMS_SOCKET_REQ_SET_SERVICE_STATUS)
    {
      qcril_qmi_imss_handle_ims_set_service_status_event_resp(params_ptr, ril_err);
    }
    else
    {
...
)
{
    ...
    if (NULL != params_ptr)
    {
        resp = (ims_settings_get_qipcall_config_rsp_msg_v01*)params_ptr->data;
...
)
{
    ...
    if (NULL != params_ptr)
    {
        resp = (ims_settings_set_client_provisioning_config_rsp_msg_v01*)params_ptr->data;
...
)
{
    ...
    if (NULL != params_ptr)
    {
        resp = (ims_settings_get_client_provisioning_config_rsp_msg_v01*)params_ptr->data;
...
)
{
  ...
  if (NULL != params_ptr)
  {
    resp = (ims_settings_get_handover_config_rsp_msg_v01*)params_ptr->data;
...
)
{
   ...
   if (NULL != params_ptr)
   {
      resp = (ims_settings_set_handover_config_rsp_msg_v01*)params_ptr->data;
       ...
       config_params->config_item_value_type == item_value_type)
    {
      ...
      switch(item_value_type)
      {
        ...
        case QCRIL_QMI_RADIO_CONFIG_ITEM_IS_STRING:
          ...
          ims_resp.stringValue.arg = (void*)qcril_malloc(config_params->config_item_value_len + 1);
          if(ims_resp.stringValue.arg != NULL)
          {
            strlcpy((char*)ims_resp.stringValue.arg,
  ...
  if(params_ptr != NULL)
  {
    if(config_item > QCRIL_QMI_RADIO_CONFIG_INVALID)
    {
        if(params_ptr->event_id == QCRIL_EVT_IMS_SOCKET_REQ_GET_IMS_CONFIG)
        {
          QCRIL_LOG_INFO("get");
          resp_handler =
              qcril_qmi_radio_config_validate_and_find_get_config_resp_handler((qcril_qmi_radio_config_item_type)config_item, msg_id);
        }
        else if(params_ptr->event_id == QCRIL_EVT_IMS_SOCKET_REQ_SET_IMS_CONFIG)
        {
          QCRIL_LOG_INFO("set");
          resp_handler =
              qcril_qmi_radio_config_validate_and_find_set_config_resp_handler((qcril_qmi_radio_config_item_type)config_item, msg_id);
        }
...
)
{
  ...
  if ( qcril_reqlist_query_by_req_id( req_id, &instance_id, &req_info ) == E_SUCCESS )
  {
        ...
        (req_info.request == QCRIL_EVT_IMS_SOCKET_REQ_GET_IMS_CONFIG))
    {
      ...
      qcril_qmi_imss_dispatch_config_response_helper(&req_data, config_item, msg_id);
    }
    else
    {
      req_data.t = req_info.t;
...
//===========================================================================
// cril_qmi_imss_store_wifi_call_preference_to_req_info
//===========================================================================
...
)
{
    ...
    qcril_reqlist_public_type             req_info;
    qcril_instance_id_e_type              instance_id = QCRIL_DEFAULT_INSTANCE_ID;
    qcril_ims_setting_wfc_preference_type *wfc_pref    = NULL;
    ...
    if ((qcril_reqlist_query_by_req_id(req_id, &instance_id, &req_info ) == E_SUCCESS) &&
            req_info.req_data)
    {
        wfc_pref = (qcril_ims_setting_wfc_preference_type *)qcril_malloc(
                sizeof(qcril_ims_setting_wfc_preference_type));
        if (wfc_pref)
        {
            *wfc_pref = qcril_qmi_imss_convert_imss_to_ril_wfc_preference(wifi_call_preference);
            config_data = (qcril_qmi_radio_config_req_data_type *)req_info.req_data;
            config_data->data_len = sizeof(qcril_ims_setting_wfc_preference_type);
...
)
{
    ...
    if (wfc_preference && req_info && req_info->req_data)
    {
        ...
        if (config_data->data)
        {
            *wfc_preference = *(qcril_ims_setting_wfc_preference_type *)(config_data->data);
...
{
    qcril_reqlist_public_type       req_info;
    qcril_ims_setting_wfc_preference_type wifi_call_preference  = QCRIL_IMS_SETTING_WFC_PREF_NONE;
    int                             dsd_rat_preference = DSD_RAT_PREFERENCE_INACTIVE_V01;
    ims_WifiCallingInfo            *ims_wci_data_ptr = NULL;
    boolean                         is_wfc_off = FALSE;
    RIL_Errno                       ril_err          = RIL_E_GENERIC_FAILURE;
    ...
    do
    {
        ...
        // Check from cache or modem, if WFC is off
        if (1 == modem_version)
        {
          qcril_qmi_imss_get_ims_service_enable_config();
        }
        ...
        if (params_ptr->event_id == QCRIL_EVT_IMS_SOCKET_REQ_SET_WIFI_CALLING_STATUS)
        {
            ...
            QCRIL_LOG_INFO("has_preference = %d, preference = %d; has_status = %d,\
                    status = %d", ims_wci_data_ptr->has_preference, ims_wci_data_ptr->preference,
                    ims_wci_data_ptr->has_status, ims_wci_data_ptr->status);
            // Use the value from req buffer as higher priority
            if (ims_wci_data_ptr->has_status)
            {
                is_wfc_off = (ims_wci_data_ptr->status == ims_WifiCallingStatus_WIFI_STATUS_OFF);
            }
            // If WFC is off, force rat_pref as celluar only. Otherwise map the rat_pref
            // from wfc_pref
            if (is_wfc_off)
            {
                dsd_rat_preference = DSD_RAT_PREFERENCE_CELLULAR_ONLY_V01;
            }
            else if (!ims_wci_data_ptr->has_preference ||
        ...
        {
            // If WFC is off, force rat_pref as celluar only. Otherwise map the rat_pref
            // from wfc_pref that is stored in req info.
            if (is_wfc_off)
            {
                dsd_rat_preference = DSD_RAT_PREFERENCE_CELLULAR_ONLY_V01;
            }
            else if (!qcril_qmi_imss_retrieve_wifi_call_preference_from_req_info(
...
RIL_Errno qcril_qmi_imss_set_ims_test_mode_enabled(uint8_t ims_test_mode_enabled, uint16 req_id)
{
  ...
  QCRIL_LOG_FUNC_ENTRY();
  QCRIL_LOG_INFO("ims_test_mode_enabled = %s", ims_test_mode_enabled ? "TRUE" : "FALSE");
...
//===========================================================================
static void qcril_qmi_imss_set_voice_domain_preference_follow_up(uint16 req_id)
{
  RIL_Errno res = RIL_E_GENERIC_FAILURE;
  qcril_reqlist_public_type req_info;
  qcril_instance_id_e_type instance_id = QCRIL_DEFAULT_INSTANCE_ID;
  ...
  if (qcril_reqlist_query_by_req_id(req_id, &instance_id, &req_info) == E_SUCCESS)
  {
    ...
    {
      QCRIL_LOG_INFO("IMS_REQ SET_VOLTE_PREF - res: %s",
          (res == RIL_E_SUCCESS ? "SUCCESS" : "FAILURE"));
    }
    else if(req_info.request == QCRIL_EVT_IMS_SOCKET_REQ_SET_SERVICE_STATUS)
    {
          ...
          req_info.req_datalen == sizeof(uint8_t) && req_info.req_data != NULL)
      {
        ...
        if (volte_enabled)
        {
          uint32 user_data = QCRIL_COMPOSE_USER_DATA(QCRIL_DEFAULT_INSTANCE_ID,
              QCRIL_DEFAULT_MODEM_ID, req_info.req_id);
          if (modem_version == 1)
          {
            // for new IMSS modem!
            ...
            memset(&qmi_req, 0, sizeof(qmi_req));
            qmi_req.volte_enabled_valid = TRUE;
            qmi_req.volte_enabled = TRUE;
            res = qcril_qmi_imss_send_set_ims_service_enable_config_req(&qmi_req, user_data);
          }
          else
          {
            // for old IMSS modem!
    ...
    {
      QCRIL_LOG_INFO("Unknown event");
    }
    if (send_response)
    {
      QCRIL_LOG_INFO("Send response: res: %s", (res == RIL_E_SUCCESS ? "SUCCESS" : "FAILURE"));
      qcril_send_empty_payload_request_response(QCRIL_DEFAULT_INSTANCE_ID, req_info.t,
          req_info.request, res);
    }
  }
  else
  {
...
//===========================================================================
// qcril_qmi_imss_set_voice_domain_preference_follow_up_hdlr
//===========================================================================
void qcril_qmi_imss_set_voice_domain_preference_follow_up_hdlr(uint16 req_id, RIL_Errno result)
{
  ...
  if (qcril_reqlist_query_by_req_id(req_id, &instance_id, &req_info) == E_SUCCESS)
  {
    ...
    qcril_qmi_imss_set_voice_domain_preference_follow_up(req_id);
  }
  else
  {
    QCRIL_LOG_ERROR("Reqlist entry is not found for req_id = %d", req_id);
  }
}
...
//===========================================================================
// qcril_qmi_imss_nas_set_voice_domain_preference
//===========================================================================
...
{
  ImsSetVoiceDomainPreferenceRequestCallback cb("VoiceDomainPref-Token", req_id);
  auto msg = std::make_shared<NasSetVoiceDomainPreferenceRequest>(voice_domain_pref, &cb);
  msg->dispatch();
...
void qcril_qmi_imss_broadcast_wfc_settings (struct ims_cached_info_type ims_info) {
  ...
  std::shared_ptr<ImsWfcSettingsStatusInd> msg = std::make_shared<ImsWfcSettingsStatusInd>(imssWfcStatus);
