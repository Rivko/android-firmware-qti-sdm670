...
)
{
  ...
  if (original_request_ptr->request_id == UIM_OEM_HOOK_ENTER_DEPERSONALIZATION_REQ)
  {
...
)
{
  ...
  uint32_t                                 data_len = 0;
  qmi_uim_remote_unlock_operation_type     rsu_op_type = QMI_UIM_REMOTE_UNLOCK_INVALID_OPERATION;
  int                                      timer_value = 0;
  ...
  if (params_ptr->qmi_rsp_data.qmi_err_code == QMI_SERVICE_ERR_NONE ||
      params_ptr->qmi_rsp_data.qmi_err_code == QMI_SERVICE_ERR_NO_EFFECT)
  {
    response = QCRIL_UIM_REMOTE_SIMLOCK_RESP_SUCCESS;
  }
  else if (params_ptr->qmi_rsp_data.qmi_err_code == QMI_SERVICE_ERR_INCOMPATIBLE_STATE)
  {
  ...
  /* Unlock operation in reponse is invalide, get the operation type
     from cache */
  if (params_ptr->qmi_rsp_data.rsp_data.remote_unlock_rsp.operation ==
       QMI_UIM_REMOTE_UNLOCK_INVALID_OPERATION)
  {
    rsu_op_type = original_request_ptr->data.remote_unlock_op;
  }
  else
  {
    rsu_op_type = params_ptr->qmi_rsp_data.rsp_data.remote_unlock_rsp.operation;
  }
...
)
{
  ...
  if(in_ptr == NULL || in_ptr[0] == NULL)
  {
    qcril_uim_response(params_ptr->instance_id, params_ptr->t,
                       ...
                       TRUE, (char *)"NULL pointer in PIN data");
    QCRIL_ASSERT(0);
    return;
  }
  ...
  /* PIN id */
  if (params_ptr->event_id_android == RIL_REQUEST_ENTER_SIM_PIN)
  {
    pin_params.pin_id = QMI_UIM_PIN_ID_PIN1;
  }
  else if (params_ptr->event_id_android == RIL_REQUEST_ENTER_SIM_PIN2)
  {
  ...
  if (strlen((const char*)in_ptr[0]) == 0)
  {
    ...
    QCRIL_LOG_INFO( "Empty PIN string, fetching retries. Request_id: %d", params_ptr->event_id_android);

    err = qcril_uim_fetch_retries(params_ptr->event_id_android,
                                  pin_params.session_info.session_type,
                                  &num_retries);

    /* Generate response */
    qcril_uim_response(params_ptr->instance_id, params_ptr->t, err,
                       (void*)&num_retries, sizeof(num_retries), TRUE, NULL);
    return;
  }
  ...
  if (original_request_ptr == NULL)
  {
    qcril_uim_response(params_ptr->instance_id, params_ptr->t,
                       ...
                       (char *)"error allocating memory for original_request_ptr");
    return;
  }
  ...
  res = qcril_uim_send_qmi_async_msg(QCRIL_UIM_REQUEST_VERIFY_PIN,
                                     (void*)&pin_params,
...
)
{
  ...
  if (strlen((const char*)in_ptr[0]) == 0)
  {
    ...
    QCRIL_LOG_INFO( "Empty PUK string, fetching retries. Request_id: %d", params_ptr->event_id_android);
...
)
{
  ...
  if (res < 0)
  {
    qcril_uim_response(params_ptr->instance_id, params_ptr->t, QCRIL_UIM_RIL_E_MODEM_ERR,
                       NULL, 0, TRUE, (char *)"error in qmi_uim_set_pin_protection");
    /* Clean up any original request if allocated */
    if (original_request_ptr)
    {
      qcril_free(original_request_ptr);
...
)
{
  ...
  if (err != RIL_E_SUCCESS)
  {
    qcril_uim_response(params_ptr->instance_id, params_ptr->t, err, NULL, 0,
                       TRUE, (char *)"error extracting pin1 status");
    return;
  }
  ...
  qcril_uim_response(params_ptr->instance_id, params_ptr->t, err, &ret_value,
                     sizeof(ret_value), TRUE, NULL);
...
)
{
  ...
  if (params_ptr->event_id_android == UIM_OEM_HOOK_ENTER_DEPERSONALIZATION_REQ)
  {
    memcpy(&oemhook_req, params_ptr->data, sizeof(qcril_enter_deperso_req_msg));
  ...
  else if (params_ptr->event_id_android == RIL_REQUEST_ENTER_NETWORK_DEPERSONALIZATION)
  {
    ...
    if(depersonalization_code == NULL)
    {
      qcril_uim_response(params_ptr->instance_id, params_ptr->t,
                         QCRIL_UIM_RIL_E_INVALID_ARGUMENTS, NULL, 0,
                         TRUE, (char *)"NULL pointer in perso data");
      return;
    }
  ...
  {
    qcril_uim_response(params_ptr->instance_id, params_ptr->t,
                       RIL_E_REQUEST_NOT_SUPPORTED, NULL, 0, TRUE,
                       (char *)"Unsupported request_id");
    return;
  }
           ...
           (qcril_uim.card_status.card[slot].application[i].perso_state == QMI_UIM_PERSO_STATE_CODE_REQUIRED))
        {
           if (params_ptr->event_id_android == UIM_OEM_HOOK_ENTER_DEPERSONALIZATION_REQ)
           {
             ...
             qcril_uim_response(params_ptr->instance_id, params_ptr->t, RIL_E_SUCCESS,
                                (void*)&oemhook_resp, sizeof(oemhook_resp),
                                TRUE, (char *)"sending num perso retries");
           }
           else
           {
             num_retries = qcril_uim.card_status.card[slot].application[i].perso_retries;
             qcril_uim_response(params_ptr->instance_id, params_ptr->t, RIL_E_SUCCESS,
                                (void*)&num_retries, sizeof(num_retries), TRUE,
                                (char *)"sending num perso retries");
           }
           return;
        }
      }
...
)
{
  ...
  RIL_PersonalizationReq               * reactivate_req_ptr   = nullptr;
  ...
  reactivate_req_ptr = (RIL_PersonalizationReq *)params_ptr->data;

  if (reactivate_req_ptr == nullptr || reactivate_req_ptr->controlKey == nullptr)
  {
    qcril_uim_response(params_ptr->instance_id, params_ptr->t, QCRIL_UIM_RIL_E_INTERNAL_ERR,
                       NULL, 0, TRUE, (char *)"NULL request pointer");
    return;
  }
  ...
  switch ((int)reactivate_req_ptr->persoType)
  {
    case RIL_PERSOSUBSTATE_SIM_NETWORK:
  ...
  /* CK value */
  perso_params.ck_data.data_ptr = (unsigned char *)reactivate_req_ptr->controlKey;
  perso_params.ck_data.data_len = strlen(reactivate_req_ptr->controlKey);
...
)
{
  ...
  if ((ret != 0) || (!rsp_data.rsp_data.get_configuration_rsp.perso_status_valid))
  {
    qcril_uim_response(params_ptr->instance_id, params_ptr->t, QCRIL_UIM_RIL_E_INTERNAL_ERR,
                       ...
                       TRUE, (char *)"Unable to retrieve status");
    return ;
  }
...
)
{
  ...
  {
    remote_unlock_params.simlock_data.data_ptr = (uint8_t *)qcril_malloc(sizeof(uint8) * simlock_len);
...
)
{
      ...
      qcril_uim.temp_unlock_status)
  {
    ...
    qcril_hook_unsol_response((qcril_instance_id_e_type)slot,
                              QCRIL_EVT_HOOK_UNSOL_SIMLOCK_TEMP_UNLOCK_EXPIRED,
                              NULL,
                              0);
