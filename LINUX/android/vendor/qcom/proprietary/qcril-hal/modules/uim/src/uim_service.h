...
class UimImpl : public V1_1::IUim, public hidl_death_recipient {
  ...
  Return<void> setCallback(const sp<IUimResponse>& responseCallback,
                           const sp<IUimIndication>& indicationCallback);
  Return<void> UimRemoteSimlockRequest(int32_t token, UimRemoteSimlockOperationType simlockOp,
...
public:
  ...
  void uimRemoteSimlockResponse(int token, UimRemoteSimlockResponseType resp,
    V1_1::UimRemoteSimlockOperationType op_type, uint8_t * rsp_ptr, uint32_t rsp_len,
    UimRemoteSimlockVersion version, UimRemoteSimlockStatus status, int timer_value);
};
