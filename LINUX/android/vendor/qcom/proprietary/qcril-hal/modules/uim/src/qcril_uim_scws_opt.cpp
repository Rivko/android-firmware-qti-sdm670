  ...
  size_t                               * line_len)
{
  ...
  {
    eol_ptr = (uint8 *)memmem(*begin_ptr,
                           data_len,
                           QCRIL_SCWS_CRLF,
                           QCRIL_SCWS_CRLF_LEN);
  ...
  if (eol_ptr == NULL)
  {
    /* If CRLF not found, obtain pointer to LF,
       if present */
    eol_ptr = (uint8 *)memmem(*begin_ptr, data_len, QCRIL_SCWS_LF, QCRIL_SCWS_LF_LEN);
  ...
  size_t                                 data_len)
{
      ...
      analyzer_ptr->request_buffer_len > 0)
  {
    ...
    if (data_len <= SIZE_MAX - analyzer_ptr->request_buffer_len)
    {
      ...
      merged_data_buffer =
        (uint8 *)qcril_malloc(merged_data_buffer_len);
    }
  ...
  while(begin_ptr < data_ptr + data_len)
  {
    ...
    if (line_ptr == NULL)
    {
      ...
      analyzer_ptr->request_buffer =
          (uint8 *)qcril_malloc(analyzer_ptr->request_buffer_len * sizeof(uint8));
  ...
  while(begin_ptr <= data_ptr + data_len)
  {
    ...
    if (!analyzer_ptr->response_header_end_found)
    {
      ...
      else
      {
        ...
        analyzer_ptr->response_buffer =
          (uint8 *)qcril_malloc(analyzer_ptr->response_buffer_len * sizeof(uint8));
