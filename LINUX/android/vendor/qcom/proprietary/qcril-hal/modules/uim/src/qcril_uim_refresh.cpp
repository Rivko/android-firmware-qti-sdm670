...
)
{
  ...
  QCRIL_LOG_DEBUG( "qcril_uim_refresh_read_pbr_record_resp: token=%d qmi_err_code=0x%X \n",
                    qcril_log_get_uim_token_id((RIL_Token)original_request_ptr->token),
...
)
{
  ...
  switch (refresh_ind_ptr->refresh_event.refresh_mode)
  {
    ...
    case QMI_UIM_REFRESH_MODE_RESET:
      ...
      qcril_uim_update_gstk_card_event(slot, TRUE);
      break;
...
(
  ...
  uint8                             slot,
  uint8                             index,
  RIL_SimRefreshResponse_v7       * ril_refresh_resp_ptr
)
{
  ...
  if (refresh_instance_id == qcril_instance_id)
  {
    ...
    qcril_hook_unsol_response(refresh_instance_id, QCRIL_EVT_HOOK_UNSOL_SIM_REFRESH,
                             (char *)(&ril_hook_refresh_resp), sizeof(ril_hook_refresh_resp) );
  }
...
)
{
  ...
  if (refresh_instance_id == qcril_instance_id)
  {
    ...
    unsol_resp.resp_len = sizeof(RIL_SimRefreshResponse_v7);
...
)
{
  ...
  RIL_SimRefreshResponse_v7       ril_refresh_resp;
  ...
  {
    memset(&ril_refresh_resp, 0, sizeof(RIL_SimRefreshResponse_v7));
