...
typedef void (*qmi_ril_prop_updated_cb)(const char *property_name, size_t name_size, const char *value, size_t value_size);

/**
  * @function qmi_ril_wait_for_properties
  * @brief Block until any of the properties in the list are updated.
  * @param state The return value from a previous call to this function, or NULL if the first time.
  * @param cb The callback to be called when any of the listed properties gets updated.
  *             This is to be followed by a sequence of "const char *" arguments,
  *             corresponding to the names of each of the properties to be monitored,
  *             and is finalized with a NULL, to signal the end of the list
  * @example
  *
  */
