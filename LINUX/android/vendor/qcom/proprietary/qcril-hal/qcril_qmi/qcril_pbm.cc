...
#define PBM_CACHE_LOCK()    do {\
        ...
        pthread_mutex_lock(&pbm_common_info.pbm_info_mutex);\
    } while (0)

#define PBM_CACHE_UNLOCK()  do {\
        pthread_mutex_unlock(&pbm_common_info.pbm_info_mutex);\
...
)
{
  ...
  pthread_mutexattr_init(&pbm_common_info.pbm_info_mutex_attr);
  pthread_mutex_init(&pbm_common_info.pbm_info_mutex, &pbm_common_info.pbm_info_mutex_attr);
...
RIL_Errno qcril_qmi_pbm_query_centralized_ecc_support()
{
    ...
    // if centralized ecc support present, set property for disabling ril controlled ecc
    if (pbm_cache_info.pbm_centralized_ecc_supported)
    {
        qcril_qmi_nas_centralized_pbm_ecc_support(FALSE);
    }

    QCRIL_LOG_INFO("centralized ecc support - %d", pbm_cache_info.pbm_centralized_ecc_supported);
    return ril_req_res;
}
...
)
{
  ...
  if (  ind_data_ptr != NULL )
  {
    ...
    if ( !is_prop_val_full && emerg_ind->mcc_emer_nums_valid )
    {
      ...
      {
          for ( i = 0; i < emerg_ind->mcc_emer_nums[slot].emer_nums_len && !is_prop_val_full; i++ )
          {
            len = emerg_ind->mcc_emer_nums[slot].emer_nums[i].emer_num_len;
            if ( ( cnt + 1 + len ) <= ( PROPERTY_VALUE_MAX - 1 ) )
            {
              memset(temp_emer_num, 0, sizeof(temp_emer_num));
              memcpy(temp_emer_num, emerg_ind->mcc_emer_nums[slot].emer_nums[i].emer_num, len);

              QCRIL_LOG_INFO("mcc emergency number %s", temp_emer_num);

              if( qcril_other_is_number_found( temp_emer_num, prop_val ) )
              {
                QCRIL_LOG_INFO("mcc emergency number %s already present in the accumulated list %s", temp_emer_num, prop_val);
                continue;
              }
              ...
              for ( j = 0; j < len; j++ )
              {
                prop_val[ cnt++ ] = emerg_ind->mcc_emer_nums[slot].emer_nums[i].emer_num[j];
              }
            }
            else
            {
        ...
        QCRIL_LOG_INFO("Updating emergency number list from customer mcc, new list: \"%s\"", prop_val);
    }
...
)
{
    ...
    for(element_index = 0; element_index < adn_record_ptr->record_elements; element_index++)
    {
        ...
        for(email_index = 0; email_index < adn_resp_ptr->email_elements; email_index++)
        {
            ...
            if(adn_resp_ptr->email[email_index] != NULL)
            {
        ...
        for(anr_index = 0; anr_index < adn_resp_ptr->anr_elements; anr_index++)
        {
            ...
            if(adn_resp_ptr->ad_number[anr_index] != NULL)
            {
    ...
    hook_response = (char*)qcril_malloc(response_length);
    if ( !hook_response )
    {
    ...
    qcril_hook_unsol_response(instance_id, QCRIL_EVT_HOOK_UNSOL_ADN_RECORDS_IND,
                     (char *)hook_response, response_length);
}
...
{
    PbmModule *module = getPbmModule();
    module->qcrilHalPbmModuleCleanup();
