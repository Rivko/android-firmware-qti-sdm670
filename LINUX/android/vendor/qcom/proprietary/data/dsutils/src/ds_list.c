...
/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2007-2008, 2014, 2016 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
)
{
    ...
    */
    ...
    if (node) {
        ...
        if (node == *tail) {
            if (NULL != node->next) {
                ds_log_err("ds_dll_delete: already tail! node->next:[0x%p]", node->next);
                return NULL;
            }
...
void ds_dll_delete_all(ds_dll_el_t * head)
{
  ds_dll_el_t * node = NULL;
  void *data;
...
/*===========================================================================
  FUNCTION  ds_dll_delete_node
===========================================================================*/
/*!
@brief
  Deletes the node pointed by "node" from the list.
...
ds_dll_el_t* node)
{
  ...
  if ((!node) || (!head) || (!tail) || (!*tail) || ((*tail)->next)) {
    goto error;
  }
  ...
  if (node == *tail) {
    if (NULL != node->next) {
      ds_log_err("ds_dll_delete_node: already tail! node->next:[0x%p]", node->next);
      return;
    }
