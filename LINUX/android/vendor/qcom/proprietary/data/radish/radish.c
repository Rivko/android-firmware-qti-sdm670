...
#define RADISH_GRO_DISABLE 0
#define RADISH_GRO_ENABLE 1
int radish_upstream_iface_gro_enabled;
...
static int radish_query_gro_feature(const char * dev)
{
  int fd;
  int rval = RADISH_GRO_DISABLE;
  struct ifreq ifr;
  ...
  if (!dev)
  {
    LOGE("%s(): dev is NULL", __func__);
    goto error;
  }
  LOGD("%s(): dev is %s", __func__, dev);
  ...
  len = strlcpy(ifr.ifr_name, dev, sizeof(ifr.ifr_name));
  if (len > sizeof(ifr.ifr_name))
  {
    goto error;
  }

  value.cmd = ETHTOOL_GGRO;
  value.data = 0;
  ...
  {
    LOGE("%s(): SIOCETHTOOL ioctl failed", __func__);
    close(fd);
    goto error;
  }
  else
  {
    LOGD("%s(): querying GRO succeeded, value %d", __func__, value.data);
  }

  close(fd);

  rval = value.data ? RADISH_GRO_ENABLE: RADISH_GRO_DISABLE;

 error:
  return rval;
}

static int radish_ifioctl_set_flags( const char * dev, short flags, short fmask )
{
  int fd;
  int rval = RADISH_ERROR;
  struct ifreq ifr;
  size_t len;
  ...
  if((fd = socket( AF_INET, SOCK_DGRAM, 0 )) < 0)
  {
    LOGE("ifioctl_set: socket failed:\n");
    goto error;
  }
  ...
  if( ioctl(fd, SIOCGIFFLAGS, &ifr) < 0 ) {
    LOGE("ifioctl_get: SIOCGIFFLAGS ioctl failed:\n");
    close(fd);
    goto error;
  }
  ...
  */
  ...
  if( ioctl( fd, SIOCSIFFLAGS, &ifr ) < 0 ) {
    LOGE("ifioctl_set: SIOCSIFFLAGS ioctl failed:\n");
    close(fd);
    goto error;
  }
...
static void radish_configure_gro_feature(const char * dev, int enable)
{
  int fd;
  int rval = RADISH_ERROR;
  struct ifreq ifr;
  ...
  {
    LOGD("%s(): configuring GRO succeeded", __func__);
  }
...
)
{
  ...
  if( radish_ifioctl_set_flags( RADISH_BRIDGE_INTERFACE_NAME, IFF_UP, IFF_UP ) < 0 ) {
...
)
{
  ...
  while ( (opt = getopt_long(argc, argv, "t:k:b:i:p:xd:", radish_opts, NULL)) != -1)
  {
    switch (opt)
    {
      ...
      case 'd': {
        server_cfg->dhcp6_fw = FALSE;
        server_cfg->rdnss_ipv6_addr = strdup(optarg);
        if (!server_cfg->rdnss_ipv6_addr) {
          LOGE("Error duplicating string - %s, not updating "
               "RNDSS IP address in RA . F'wing dhcp packets", strerror(errno));
          server_cfg->dhcp6_fw = TRUE;
        }
        break;
      }
 ...
 * up this table and routes the ipv6 traffic based on this table.*/
      ...
      case 'i': {
          ...
          {
            char xlat_iface[MAX_IFACE_LEN];
            unsigned xlat_iface_idx;
            ...
            if (radish_query_gro_feature(cur_iface) == RADISH_GRO_ENABLE)
            {
              radish_upstream_iface_gro_enabled = RADISH_GRO_ENABLE;
              snprintf(xlat_iface, MAX_IFACE_LEN, "v4-%s", cur_iface);
              xlat_iface_idx = if_nametoindex(xlat_iface);
              if ((!xlat_iface_idx))
              {
                LOGD("Couldn't find xlat interface %s, not modifying GRO flags", xlat_iface);
              }
              else
              {
                LOGD("XLAT is enabled, xlat iface is %s. Disable GRO", xlat_iface);
                radish_configure_gro_feature(cur_iface, RADISH_GRO_DISABLE);
              }
            }
            else
            {
              LOGD("GRO is disabled, not modifying GRO flags");
              radish_upstream_iface_gro_enabled = RADISH_GRO_DISABLE;
            }
...
)
{
  ...
  if (setsockopt(sock,
                 ...
                 IPV6_MULTICAST_LOOP,
                 &radish_false,
                 sizeof(radish_false)) < 0) goto error;

  if ( (res = setsockopt(sock,
                         IPPROTO_IPV6,
                         IPV6_RECVPKTINFO,
                         &radish_true,
                         sizeof(radish_true))) < 0)
  {
...
)
{
  ...
  struct sock_filter filter[] =
  {
     ...
     * opcode 0x6  = BPF_RET
     * opcode 0x74 = BPF_ALU | BPF_RSH | BPF_K */
  ...
  struct sock_filter filter_no_dhcp6_fw[] =
  {
    /* offsets are adjusted assuming
     * message is captured at IPv6
     * header */
  ...
  if(server_cfg.dhcp6_fw == FALSE)
  {
    fprog.filter = filter_no_dhcp6_fw;
    fprog.len = sizeof(filter_no_dhcp6_fw)/sizeof(struct sock_filter);
  }
  else
  {
    fprog.filter = filter;
...
)
{
  ...
  struct sockaddr_in6 router_dns;
  ...
  rlist_foreach(o, &pnd->parsed_options)
  {
    ...
    /* Here we are modifying the RDNSS address to that of bridge0's Link layer
     * address so that dnsmaspv6 running on the bridge can resolve DNS requests
     * locally.
     */

    if (popt->option->h.type == ND_OPT_TYPE_RNDSS &&
          (server_cfg.dhcp6_fw == FALSE))
    {

      router_dns.sin6_family = AF_INET6;
      router_dns.sin6_addr = popt->option->dns.addr;
      log_sockaddr(&router_dns);

      ret = inet_pton(AF_INET6, server_cfg.rdnss_ipv6_addr, &(popt->option->dns.addr));

      if (ret <= 0)
      {
        if (ret == 0)
            LOGE("%s: DNS address passed is in invaild format", __func__);
        else
            LOGE("%s: inet_pton failed to convert - %s", __func__, strerror(errno));
      }

      /* Checking if there are two DNS addreses in the RDNSS. */
      if (popt->option->h.length == (sizeof(popt->option->dns)/sizeof(uint64_t)))
      {
        ret = inet_pton(AF_INET6, server_cfg.rdnss_ipv6_addr, &(popt->option->dns.sec_addr));
        if (ret <= 0)
        {
          if (ret == 0)
              LOGE("%s: DNS address passed is in invaild format", __func__);
...
)
{
  ...
  radish_modify_link_local_route(RADISH_DELETE_OPERATION);
    ...
    if( radish_ifioctl_set_flags( RADISH_BRIDGE_INTERFACE_NAME, ~IFF_UP, IFF_UP ) < 0 ) {
      LOGE("Cannot Set the bridge interface state to DOWN");
    }
    ...
    if('\0' != *g_cur_iface)
    {
      ...
      if (radish_upstream_iface_gro_enabled == RADISH_GRO_ENABLE)
      {
        radish_configure_gro_feature(g_cur_iface, RADISH_GRO_ENABLE);
      }
    }
  }
