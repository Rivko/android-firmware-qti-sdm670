...
)
{
  ...
  if(base_rate == 0)
  {
    base_rate = NETMGR_TC_MAX_DATARATE;
  }
  if(datarate == 0)
  {
  ...
  if( NULL == parent )
  {
    snprintf(cmd, sizeof(cmd),
             ...
             "htb prio %d rate %lubit ceil %lubit burst %lu",
             dev_name,
...
)
{
  ...
  if (node == NULL)
  {
    netmgr_log_high("netmgr_tc_find_flow_info: flow 0x%lx not found",(unsigned long) flow_id);
    NETMGR_LOG_FUNC_EXIT;
    return NULL;
  }
  ...
  if (flow_info == NULL)
  {
    netmgr_log_err("netmgr_tc_find_flow_info: flow 0x%lx data not found",(unsigned long) flow_id);
  }
...
)
{
  ...
  for (i = 0; i < flow_info->qos_flow.num_filter; i++)
  {
    ...
    */
    ...
    snprintf(cmd, sizeof(cmd), "%s %s -t mangle -I %s %d -m mark --mark 0x%08x -j RETURN",
             ipt_tool,
             iptables_opts,
    ...
    ret = netmgr_ds_exec(cmd, NULL);
    if (ret != DS_EXEC_OK)
        goto fail;

    memset( cmd, 0x0, sizeof(cmd) );
    snprintf(cmd, sizeof(cmd), "%s %s -t mangle -I %s %d -j %s_0x%08x.%d",
             ipt_tool,
             iptables_opts,
             NETMGR_TC_CHAIN_NAME_QOS_FILTER_POSTROUTING,
             2 * filter_index + 1,
    ...
    if (ret != DS_EXEC_OK) {
        memset( cmd, 0x0, sizeof(cmd) );
        snprintf(cmd, sizeof(cmd), "%s %s -t mangle -D %s %d",
                 ipt_tool,
                 iptables_opts,
                 NETMGR_TC_CHAIN_NAME_QOS_FILTER_POSTROUTING,
                 2 * filter_index + 1);
        netmgr_ds_exec(cmd, NULL);
    }
...
)
{
  ...
  memset( cmd, 0x0, sizeof(cmd) );
  length = (unsigned int)snprintf(cmd, maxlength, "%s %s -t mangle -A %s_0x%08x.%d",
...
)
{
  ...
  /* Create a chain */
  memset( cmd, 0x0, sizeof(cmd) );
  snprintf(cmd, maxlength, "%s %s -t mangle -N %s_0x%08x.%d",
           NETMGR_TC_IPTABLES_TOOL_V4,
           iptables_opts,
           NETMGR_TC_CHAIN_NAME_PREFIX,
           flow_id,
...
error:
  /* Deleting the chain deletes all the rules.  This chain must not be referenced
     by any other rules. */
  memset( cmd, 0x0, sizeof(cmd) );
  snprintf(cmd, sizeof(cmd), "%s %s -t mangle -X %s_0x%08x.%d",
           NETMGR_TC_IPTABLES_TOOL_V4,
           iptables_opts,
           NETMGR_TC_CHAIN_NAME_PREFIX,
           flow_id,
...
)
{
  ...
  /* The mark value is the flow ID. */
  length += (unsigned int)snprintf(cmd + length, maxlength - length, " -o %s -j MARK --set-mark 0x%08x",dev_name,flow_id);
...
)
{
  ...
  if( NETMGR_IS_DEFAULT_FLOW( qos_flow->flow_id ) )
  {
    ...
    do {
      /* Even if we fail to install class or filter for tcp ack prioritization,
         do not clean up the root and default classes */
      if (netmgr_main_get_tcpackprio_enabled())
      {
        result = netmgr_tc_create_class(link,
                                        rclass_handle,
...
)
{
  ...
  for (i = 0; i < flow_info->qos_flow.num_filter; i++)
  {
    ipt_tool = (NETMGR_QOS_IP_VERSION_6 == flow_info->qos_flow.filter_list[i].
                 qos_filter.ip_version)? NETMGR_TC_IPTABLES_TOOL_V6 :
                 NETMGR_TC_IPTABLES_TOOL_V4;

    /* Remove the reference to the chain from the POSTROUTING table */
    memset( cmd, 0x0, sizeof(cmd) );
    snprintf(cmd, sizeof(cmd), "%s %s -t mangle -D %s -j %s_0x%08x.%d",
             ipt_tool,
             iptables_opts,
             NETMGR_TC_CHAIN_NAME_QOS_FILTER_POSTROUTING,
             NETMGR_TC_CHAIN_NAME_PREFIX,
    ...
    memset( cmd, 0x0, sizeof(cmd) );
    snprintf(cmd, sizeof(cmd), "%s %s -t mangle -D %s -m mark --mark 0x%08x -j RETURN",
             ipt_tool,
             iptables_opts,
             NETMGR_TC_CHAIN_NAME_QOS_FILTER_POSTROUTING,
             flow_id);
    if (netmgr_ds_exec(cmd, NULL) != DS_EXEC_OK)
    {
      netmgr_log_high("Error removing RETURN rule. Ignoring");
    }

    /* Remove all the rules from the chain */
    memset( cmd, 0x0, sizeof(cmd) );
    snprintf(cmd, sizeof(cmd), "%s %s -t mangle -F %s_0x%08x.%d",
             ipt_tool,
             iptables_opts,
             NETMGR_TC_CHAIN_NAME_PREFIX,
             flow_id,
...
)
{
  ...
  if (NETMGR_IS_DEFAULT_FLOW(flow_id))
  {
    ...
    while (NULL != (node = ds_dll_deq(netmgr_tc_cfg.links[link].flow_list, NULL, &data)))
    {
      ...
      if (flow_info)
      {
        netmgr_log_med("Freeing flow_id=0x%lx",(unsigned long) flow_info->qos_flow.flow_id);
  ...
  else
  {
    ...
    if (node == NULL)
    {
      netmgr_log_err("flow %lx not found",(unsigned long) flow_id);
      goto done;
    }
    ...
    if (flow_info == NULL)
    {
      netmgr_log_err("flow %lx data not found",(unsigned long) flow_id);
      goto done;
    }
    ...
    else
    {
        ...
        qos_modify_cache_flow_buf.flow_id = flow_id;
        pthread_mutex_lock(&(netmgr_qos_modify_cache.modify_mutex));
        ...
        {
            netmgr_log_err("flow %lx not found in QoS modify cache", flow_id);
        }
...
)
{
  ...
  char * ipt_tool = NULL;
  const char * dev_name = NULL;
  uint32_t flow_id = NETMGR_QMI_PRIMARY_FLOW_ID;
  unsigned long burst;
  ...
  if(NULL == dev_name)
  {
    ...
    if (netmgr_tc_cfg.links[link].default_class)
    {
      snprintf(cmd, sizeof(cmd), "tc class change dev %s parent %d:%d classid %d:%d htb prio 7 rate %ubit ceil %lubit burst %lu",
               dev_name,
               netmgr_tc_cfg.links[link].root_class->major,
               netmgr_tc_cfg.links[link].root_class->minor,
               netmgr_tc_cfg.links[link].default_class->major,
    ...
    if (netmgr_main_get_tcpackprio_enabled())
    {
        if (netmgr_tc_cfg.links[link].tcp_ack_class)
        {
          ...
          if (netmgr_ds_exec(cmd, NULL) != DS_EXEC_OK)
          {
            netmgr_log_high("Error change default class. Ignoring");
          }
        }
    }
    ...
    if (netmgr_tc_cfg.links[link].root_class)
    {
      snprintf(cmd, sizeof(cmd), "tc class change dev %s parent root classid %d:%d htb prio 0 rate %ubit ceil %lubit burst %lu",
               dev_name,
               netmgr_tc_cfg.links[link].root_class->major,
               netmgr_tc_cfg.links[link].root_class->minor,
               ul_cfg_tput,
...
)
{
  ...
  length = snprintf(cmd,
                    maxlength,
                    "%s %s -t mangle -A POSTROUTING -j %s",
                    ipt_tool,
                    iptables_opts,
                    NETMGR_TC_CHAIN_NAME_QOS_FILTER_POSTROUTING);
...
)
{
  ...
  length = snprintf(cmd,
                    sizeof(cmd),
                    "%s %s -t mangle -D POSTROUTING -j %s",
                    ipt_tool,
                    iptables_opts,
                    NETMGR_TC_CHAIN_NAME_QOS_FILTER_POSTROUTING);
...
)
{
  ...
  if(create_chain == TRUE)
  {
    ...
    length = snprintf(cmd,
                    maxlength,
                    "%s %s -t mangle -A %s -o %s -s %s -j MARK --set-mark 0x%x",
                    ipt_tool,
                    iptables_opts,
                    NETMGR_TC_CHAIN_NAME_QOS_RESET_POSTROUTING,
                    dev_name,
  ...
  else
  {
    ...
    length = snprintf(cmd,
                    maxlength,
                    "%s %s -t mangle -D %s -o %s -s %s -j MARK --set-mark 0x%x",
                      ipt_tool,
                      iptables_opts,
                      NETMGR_TC_CHAIN_NAME_QOS_RESET_POSTROUTING,
                      dev_name,
...
/*===========================================================================
  FUNCTION netmgr_tc_set_iptables_command_options
===========================================================================*/
/*!
@brief
...
/*=========================================================================*/
void
netmgr_tc_set_iptables_command_options(char* opts)
{
  int rc = 0;
...
)
{
  ...
  {
      return (long int)( ((netmgr_qmi_qos_modify_info_t*)first)->flow_id -
              ((netmgr_qmi_qos_modify_info_t*)second)->flow_id );
  }
  else
  {
