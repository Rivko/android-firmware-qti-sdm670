      ...
      */
      for (i = 0; (unsigned int)i < info->nfd; ++i)
      {
        if( FD_ISSET((fdmap + i)->fd, &info->fdset) )
        {
          ...
          {
            netmgr_log_err("No function available to process"
                           "packets. This should not happen");
          }
        }
      }
    }
...
)
{
  ...
  if ((*p_sk_fd = socket(AF_NETLINK, SOCK_RAW | SOCK_CLOEXEC, proto)) < 0) {
  ...
  while( RTA_OK(rtah, buflen ) ) {
    ...
    switch( rtah->rta_type ) {
      ...
      case NETMGR_RTATYPE_FLOWINFO:
        ...
        netmgr_log_med( "Attribute: FlowID 0x%08lx\n", (unsigned long)(event_info->flow_info.flow_id) );
...
)
{
  ...
  char dup_data[_K_SS_MAXSIZE - sizeof(unsigned short)];
  ...
  if( (msg_type == RTM_NEWADDR) &&
      (addr_info->metainfo.ifa_family == AF_INET6) &&
      (addr_info->metainfo.ifa_scope == RT_SCOPE_UNIVERSE) &&
      (IFA_F_TENTATIVE   & addr_info->metainfo.ifa_flags))
  {
    netmgr_log_med("%s() Tentative V6 Address Newaddr received. Do not propagate", __func__);
    return NETMGR_FAILURE;
  }
#endif
    ...
    switch( rtah->rta_type ) {
      ...
      case IFA_ADDRESS: /* same as IFLA_ADDRESS */
        if(is_prefix == FALSE)
        {
          ...
          if(addr_info->metainfo.ifa_family == AF_INET)
          {
            is_prefix = TRUE;
          }
        }
        ...
        {
          netmgr_log_med("Received another address.");
          memcpy( &dup_data,
                RTA_DATA(rtah),
                sizeof(dup_data) );
          NETMGR_LOG_IPV4_ADDR( med, "Attribute: PrefixIPV4 ", dup_data);
        }
        break;
  ...
  while( NLMSG_OK(nlh, buflen) ) {
    switch( nlh->nlmsg_type ) {
      ...
      case RTM_DELADDR:
        ...
        /* Process the Netlink attributes */
        if( NETMGR_SUCCESS !=
            netmgr_nl_decode_rtm_addr( buffer, buflen, &msg_ptr->addr_info, msg_ptr->type ) ) {
...
/*=========================================================================*/
int
netmgr_nl_send_msg( int fd, pid_t pid, void* buffer, uint16 buflen )
{
  struct sockaddr_nl * nladdr = NULL;
  struct msghdr * msgh = NULL;
  ...
  nladdr->nl_family = AF_NETLINK;
  nladdr->nl_pid = pid;
  nladdr->nl_groups = ((pid == NETMGR_NL_MULTICAST_SEND) ? NETMGR_NL_GRP_EVENTS : NETMGR_NL_MULTICAST_SEND);
