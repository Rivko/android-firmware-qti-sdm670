...
static int qdi_nl_get_route_socket(void)
{
  ...
  if ((sock_fd = socket(AF_NETLINK, SOCK_RAW | SOCK_CLOEXEC, NETLINK_ROUTE)) < 0)
  {
...
)
{
  ...
  while (NLMSG_OK(nlh, buflen))
  {
    ...
    ifa_index = (int)ifa->ifa_index;
    QDI_LOG_LOW("qdi_nl_parse_getaddr_resp_msg: ifa_index %d", (int)ifa->ifa_index);
    if (-1 == ioctl(fd, SIOCGIFNAME, &ifr))
    {
      struct if_nameindex *ini;
      int i = -1;
      QDI_LOG_ERROR("qdi_nl_parse_getaddr_resp_msg: failed to get iface name, trying with if_nameindex\n");
      ...
      if (ini != NULL)
      {
        ...
        {
          if (ini[i].if_index == ifa_index)
          {
            memset(dev_name, 0, IFNAMSIZ);
            dev_index = ini[i].if_index;
            QDI_LOG_LOW("qdi_nl_parse_getaddr_resp_msg: got matching iface %s index %d", ini[i].if_name, dev_index);
            if (strlcpy(dev_name, ini[i].if_name, IFNAMSIZ) >= IFNAMSIZ)
            {
              QDI_LOG_ERROR("qdi_nl_parse_getaddr_resp_msg: string truncation");
              memset(dev_name, 0, IFNAMSIZ);
            }
            break;
          }
        ...
        if (dev_index == -1)
        {
          QDI_LOG_ERROR("qdi_nl_parse_getaddr_resp_msg: failed to get iface name from if_nameindex\n");
          ifr.ifr_name[0] = '\0';
          memset(dev_name, 0, IFNAMSIZ);
        }
      }
      ...
      {
        QDI_LOG_ERROR("qdi_nl_parse_getaddr_resp_msg: failed to get iface name from NULL if_nameindex\n");
        ifr.ifr_name[0] = '\0';
        memset(dev_name, 0, IFNAMSIZ);
      }
    }
    ...
    QDI_LOG_LOW( "Metainfo: Flags=[0x%x]%s %s %s %s %s %s %s %s \n",
                  ifa->ifa_flags,
                  (IFA_F_TEMPORARY   & ifa->ifa_flags)?"TEMPORARY ":"",
                  (IFA_F_HOMEADDRESS & ifa->ifa_flags)?"HOMEADDRESS ":"",
                  (IFA_F_OPTIMISTIC  & ifa->ifa_flags)?"OPTIMISTIC ":"",
                  (IFA_F_NODAD       & ifa->ifa_flags)?"NODAD ":"",
                  (IFA_F_DADFAILED   & ifa->ifa_flags)?"DADFAILED ":"",
                  (IFA_F_DEPRECATED  & ifa->ifa_flags)?"DEPRECATED ":"",
                  (IFA_F_TENTATIVE   & ifa->ifa_flags)?"TENTATIVE ":"",
                  (IFA_F_PERMANENT   & ifa->ifa_flags)?"PERMANENT ":"" );
    ...
    while (RTA_OK(rta, rtattrlen))
    {
      switch (rta->rta_type)
      {
        case IFA_ADDRESS:
          ...
          if (0 == strncmp(ifname, dev_name, IFNAMSIZ))
          {
            QDI_LOG_LOW("Matched becuase ifname %s size %d dev_name %s size %d", ifname, strlen(ifname), dev_name, strlen(dev_name));
                 ...
                 (unsigned char)scope == ifa->ifa_scope))
            {
              ...
              {
                if (IFA_F_DADFAILED & ifa->ifa_flags)
                {
                  QDI_LOG_LOW("Deprecated or DAD Failed RA was found");
                  break;
                }

                if((ifa->ifa_scope == RT_SCOPE_UNIVERSE) && (IFA_F_TENTATIVE & ifa->ifa_flags))
                {
                  retry = TRUE;
                  QDI_LOG_MED("Address is Tentative, don't notify clients");
                }
            ...
            QDI_LOG_LOW("Not matched becuase ifname %s size %d dev_name %s size %d", ifname, strlen(ifname), dev_name, strlen(dev_name));
          }
          break;
...
)
{
  ...
  scope = (WDS_TECHNOLOGY_NAME_MODEM_LINK_LOCAL_V01 == tech_name) ?
           RT_SCOPE_LINK : QDI_NL_SCOPE_ALL;

  /* Receive the RTM_GETADDR response and parse it */
  if (QDI_FAILURE == (reti = qdi_nl_parse_getaddr_resp_msg(sock_fd,
                                                            iface,
                                                            ip_family,
                                                            all_addr_info,
                                                            &num_addrs,
  ...
  {
    ret = QDI_RETRY;
    QDI_LOG_MED("qdi_nl_get_addr_info: V6 address is Tentative. Wait for Preferred address\n");
  }
  else
  {
    ret = QDI_SUCCESS;
