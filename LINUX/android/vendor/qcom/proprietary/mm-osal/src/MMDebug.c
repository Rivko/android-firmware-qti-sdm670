...
int MM_Debug_Initialize()
{
  ...
  property_get("mmosal.debug.config", sValue, "");
  ...
  if (nPropLen >= 6)
  {
    ...
    for (index = 0; index < MAP_SIZE; index++)
    {
      //!It is just to know what are the logmask set otherthan default value
      // it is need because we are not resetting earlier configure log masks
      //So, it is required to know what are the logmask enabled.
      if (ssidMaskMap[index] != 24)
        LOGE("%d %d", index + MSG_SSID_APPS, ssidMaskMap[index]);
    }
  }
