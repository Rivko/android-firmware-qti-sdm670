...
*//** @file  MMMalloc.h
   ...
   Copyright (c) 2008 - 2009, 2015 Qualcomm Technologies, Inc.
   All Rights Reserved.
...
/**
 * This macro calls MM_calloc in order to allocate memory from the heap,
 * intialize it to 0 and track memory allocations for debugging purposes. A
 * pointer to the allocated memory is returned.
 *
 * bkCnt - the number fo blocks of bkSz to allocate
 * bkSz - the size of each block
 */
#define MM_Calloc(bkCnt,bkSz)   MM_calloc(bkCnt,bkSz, __FILE__, __LINE__)
 ...
 * This function allocates the memory of requested size.
 *
 *  This function should not be called directly, instead, call MM_Calloc().
 *
 * @param[in] nelem - number of memory blocks
 * @param[in] elsize - memory size of each block requested
 * @param[in] file - the file name for debugging
 * @param[in] line - line number for debugging
 *
 * @return void* a pointer to memory
 */
 void * MM_calloc(size_t nElem, size_t elSize, const char *file, int line);
