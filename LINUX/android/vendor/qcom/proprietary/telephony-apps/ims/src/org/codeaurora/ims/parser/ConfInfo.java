...
public class ConfInfo {
            ...
            if (version != null && !version.equalsIgnoreCase(Element.CONF_NA))
            {
                ...
                for (int index = 0; index < length; index++) {
                    ...
                    log("getConfUriList[" + index + "] -> userEntity="
                            + Log.pii(participantInfo[INDEX_USER])
                            + ", Display Text=" + Log.pii(participantInfo[INDEX_DISPLAY_TEXT])
                            + ", endPoint=" + Log.pii(participantInfo[INDEX_ENDPOINT])
     ...
     */
    protected void updateNotification(List<Element> oldList, Element oldElement,
