...
public class CallBarring extends PreferenceActivity implements DialogInterface.OnClickListener,
    ...
    private void setPreferenceEnabled (boolean config) {
        mListOutgoing.setEnabled(config);
        mListIncoming.setEnabled(config);
        mDialogChangePSW.setEnabled(config);
        mDialogCancelAll.setEnabled(config);
    }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference.getKey().equals(CALL_BARRING_OUTGOING_KEY)) {
            showCBDialog(OUTGOING_SET_DIALOG);
        } else if (preference.getKey().equals(CALL_BARRING_INCOMING_KEY)) {
            showCBDialog(INCOMING_SET_DIALOG);
        }
    ...
    private void showCBDialog (int msg) {
        Log.d(LOG_TAG, "showCBDialog: " + msg + " mIsFront: " + mIsFront);
        if (mIsFront) showDialog(msg);
    }
    ...
    public void checkDataStatus() {
        ...
        Log.d(LOG_TAG, "isUtEnabled = " + mCallBarringHelper.isUtEnabled() +
                ", checkMobileDataForCf= " + checkMobileDataForCf() + " defaultDataSub is: " +
    ...
    private String convertToStatusString(int value) {
        if (value == CB_STATE_OFF)
            return getResources().getString(R.string.mobile_data_off);

        String statusString = "";
        if ((value & CB_BAOC) != 0)
            statusString += getResources().getString(R.string.call_barring_outgoing_call);
        if ((value & CB_BAOIC) != 0)
            statusString += getResources().getString(R.string.call_barring_international_call);
        if ((value & CB_BAOICxH) != 0)
            statusString += getResources().getString(R.string.call_barring_international_call_roaming);
        if ((value & CB_BAIC) != 0)
            statusString += getResources().getString(R.string.call_barring_all);
        if ((value & CB_BAICr)!= 0)
            statusString += getResources().getString(R.string.call_barring_roaming);
    ...
    private void queryAllCBOptions() {
        showCBDialog(INITIAL_BUSY_DIALOG);
                    ...
                    if (status != MSG_OK) {
                        ...
                        if (ar.exception != null) {
                            ...
                            if (ce.getCommandError() ==
                                ...
                                showCBDialog(FDN_CHECK_FAILURE);
                            } else if (ce.getCommandError() ==
                                    CommandException.Error.RADIO_NOT_AVAILABLE){
                                showCBDialog(RADIO_OFF_ERROR);
                            } else {
                                showCBDialog(EXCEPTION_ERROR);
                            }
