...
class BaseSigner(SecPluginIntf_Abs):
    ...
    def get_signature(self):
        ...
        if self.using_ecdsa:
            signature = ecdsa_functions.sign(self.hash_to_sign, self.certs[self.ATTEST].priv_key)
        else:
            signature = rsa_functions.sign(self.hash_to_sign, self.certs[self.ATTEST].priv_key, padding=self.padding,
                                           hash_algo=self.hash_algo, salt_len=self.PAD_PSS_SALT_1)
