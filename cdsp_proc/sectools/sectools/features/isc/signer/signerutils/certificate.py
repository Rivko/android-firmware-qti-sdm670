...
class Certificate(object):
    ...
    def _get_sha_algo(self):
        ...
        attr_pattern_list = attr_pattern.findall(self.certificate_text)
    ...
    def _get_soc_vers(self):
        ...
        attr_pattern = re.compile(r'OU\s*?=\s*?{0} ([a-fA-F\d ]+) {1}'.format(ou_id, self.SIGNATTR_SOC_VERS))
    ...
    def _get_multi_serial_numbers(self):
        ...
        attr_pattern = re.compile(r'OU\s*?=\s*?{0} ([a-fA-F\d ]+) {1}'.format(ou_id, self.SIGNATTR_SN))
