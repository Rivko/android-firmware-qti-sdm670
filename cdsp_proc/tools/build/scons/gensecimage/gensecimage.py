...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.qc/1.1/gensecimage/gensecimage.py#4 $
#  $DateTime: 2014/08/05 22:30:42 $
#  $Author: pwbldsvc $
...
# int types
#
DEFAULT_SIZE_WITH_PAD = 256*1024
header_size = 40
preamble_size = 0
...
min_size_with_pad = DEFAULT_SIZE_WITH_PAD
page_size = 0
num_of_pages = 0
...
# Multiplied by 1024 is "k" is at the end
#----------------------------------------------------------------------------
def convertStringToInt(inputstring):
  if (inputstring.endswith("k")):
    returnvalue = int(inputstring[:len(inputstring)-1], 10) * 1024
  else:
    returnvalue = int(inputstring, 10)
  return returnvalue
...
#----------------------------------------------------------------------------
def create_preamble_file(source_file_name, preamble_file_name,
    max_size_of_verify_buffer, small_preamble,
    flash_auto_detect_max_page, flash_auto_detect_min_page,
    preamble_type, num_of_pages=None):
  # Initialize
  ...
  if (read_buff[0] != mbn_tools.FLASH_CODE_WORD) or (read_buff[1] != mbn_tools.MAGIC_NUM):
    log("Invalid preamble!\n")
    raise RuntimeError, "Preamble not valid"

  # 2kB Preamble does not contain this magic number
  if (has_2K_preamble is False) and (read_buff[2] != mbn_tools.AUTODETECT_PAGE_SIZE_MAGIC_NUM) \
    and (read_buff[2] != mbn_tools.AUTODETECT_PAGE_SIZE_MAGIC_NUM64) \
    and (read_buff[2] != mbn_tools.AUTODETECT_PAGE_SIZE_MAGIC_NUM128):
    log("Preamble not valid!\n")
    raise RuntimeError, "Preamble not valid"

  boot_sbl_header = mbn_tools.Sbl_Hdr(init_val = int('0x0',16)) #get preamble size
  gen_dict = {'IMAGE_KEY_MAX_SIZE_OF_VERIFY_BUFFER' : max_size_of_verify_buffer,
            'IMAGE_KEY_FLASH_AUTO_DETECT_MAX_PAGE' : flash_auto_detect_max_page,
            'IMAGE_KEY_FLASH_AUTO_DETECT_MIN_PAGE' : flash_auto_detect_min_page,
            'IMAGE_KEY_BOOT_SMALL_PREAMBLE' : small_preamble}
  mbn_tools.image_preamble(gen_dict, preamble_file_name, boot_sbl_header, num_of_pages)

  log("\nWrote preamble of %d bytes into %s\n" %(boot_sbl_header.image_src, preamble_file_name))

  return boot_sbl_header.image_src
...
def is_elf(filename):
  ...
  # Compare ELF magic number
  if (hdr == (mbn_tools.ELFINFO_MAG0 +
                mbn_tools.ELFINFO_MAG1 +
                mbn_tools.ELFINFO_MAG2 +
                mbn_tools.ELFINFO_MAG3)):
    isElf = True
...
def process_signing(cert_dir):
  ...
  if image_format == 'mbn':
    ...
    if ota_enabled is True:
      mbn_tools.pad_SBL1_image(0, signed_fname, min_size_with_pad, page_size, num_of_pages)
...
def read_ini_file(section_name):
  ...
  if is_elf(source_image) is True:
      if (image_format == 'mbn'):
          raise RuntimeError, \
            "Source image is in ELF format. Please set file_type=htelf/rawelf in gensecimage.cfg."

      if elf_contain_hashtable(source_image) is True:
          if (image_format == 'rawelf'):
      ...
      if (image_format == 'rawelf') or (image_format == 'htelf'):
          raise RuntimeError, \
            "Source image is not in ELF format. Please do not set file_type=htelf/rawelf in gensecimage.cfg."
  ...
  if (config.has_option(section_name,'testsig_serialnum') == True):
     ...
     if len(testsig_serialnum_str.lstrip("0x"))>8:
  ...
  if (image_format == 'mbn'):
    ...
    if (config.has_option(section_name,'page_size') == True):
      page_size = convertStringToInt(config.get(section_name,'page_size'))

    if (config.has_option(section_name,'num_of_pages') == True):
      num_of_pages = convertStringToInt(config.get(section_name,'num_of_pages'))

    if (config.has_option(section_name,'min_size_with_pad') == True):
      min_size_with_pad = convertStringToInt(config.get(section_name,'min_size_with_pad'))
  else:
...
def process_section (section_name, stage):
    ...
    if (image_format == 'mbn'):
      ...
      if ((stage == "presign") or (stage== "qpsa:sign") or (stage=="cass:sign")):
        ...
        size_of_preamble = 0
        if has_preamble is True:
          size_of_preamble = create_preamble_file(source_image, preamble_fname,
                8192, small_preamble, flash_auto_detect_max_page,
                flash_auto_detect_min_page, preamble_type, num_of_pages)
...
def usage ():
  ...
  print "                    --config=resources/8974/8974_LA_gensecimage.cfg"
  print "     gensecimage.py --stage=qpsa:sign --section=all "
  print "                    --config=resources/8974/8974_LA_gensecimage.cfg"
  print "     gensecimage.py --stage=qpsa:sign --section=all "
  print "                    --config=resources/8974/8974_LA_gensecimage.cfg"
  print "                    --signconfig=resources/8974/8974_LA_signingattr_qpsa.cfg"
  print "                    --postsignconfig=resources/8974/8974_LA_postsigncmd.cfg"
  print ""
  print "========================================================"
