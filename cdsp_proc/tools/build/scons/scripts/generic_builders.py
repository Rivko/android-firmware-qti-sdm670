...
#------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.qc/1.1/scripts/generic_builders.py#17 $
...
def generate(env):
    ...
    # BuildTimingActions
    build_timing_actions_generate(env)
...
#--------------------------------------------------------------------------
# build_timing_actions_generate, start_timing_action, end_timing_action
# This functions will create builders for start and the end timing nodes
# by calling "MarkTimeStart" and "MarkTimeEnd" methods from utlis.py
#---------------------------------------------------------------------------

def build_timing_actions_generate(env):
    '''
       build_timing_action_generate
    '''
    blder_start_act = SCons.Action.Action(start_timing_action, None)
    blder_end_act = SCons.Action.Action(end_timing_action, None)

    env.Append(BUILDERS = {'GetStartTimingNode' : env.Builder(action=blder_start_act)})
    env.Append(BUILDERS = {'GetEndTimingNode' : env.Builder(action=blder_end_act)})

def start_timing_action(target, source, env):
    env.MarkTimeStart(str(source[0]))

def end_timing_action(target, source, env):
    env.MarkTimeEnd(str(source[0]))
