...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.qc/1.1/scripts/llvm_dam.py#3 $
#  $DateTime: 2018/04/16 17:35:03 $
#  $Change: 15941762 $
...
# Add Shared Library:
#-------------------------------------------------------------------------------
def add_shared_library(env, targets, library, sources, pack=False, \
   ...
   if env.IsKeyEnable(targets):
      ...
      analysis = env.AnalyzeLibrary(library, sources, env['SHLIBSUFFIX'], pack, **args)

      if analysis.build_as == 'library':
          library_node = env.SharedLibrary(analysis.library, analysis.sources)
...
def add_bin_shared_library(env, targets, library, sources, \
   pack_exception=None, **kargs):
   '''
   add_bin_shared_library

   Same as add_shared_library, but builds without source files in customer
   builds.
   '''
   return add_shared_library(env, targets, library, sources, pack=True, \
...
def generate(env):
   ...
   env.AddMethod(add_bin_shared_library, "AddBinarySharedLibrary")
   ...
   env.Replace(LLVMINCLUDECXX = os.environ['LLVMINCLUDECXX'])
   ...
   env.PrintInfo('LLVMINCLUDECXX          = ${LLVMINCLUDECXX}')
   ...
   if do_64:
      ...
      do_32 = True
      env.Replace(ARM_CPU = "-mcpu=cortex-a7")
      LLVMFlags += " -target armv7m-none-musleabi -w -mfloat-abi=softfp -mfpu=none -mcpu=cortex-a7 -mno-unaligned-access  "

      LLVMFlags += " -fms-extensions " #for anon union/struct
      if UseClangAs:
        env.Replace(AS = LLVMBIN + "/clang${EXE_EXT} -target armv7m-none-musleabi -fintegrated-as -mfloat-abi=softfp -mcpu=cortex-a7  -x assembler -c ")
      env.Replace(TCC = LLVMBIN + "/clang${EXE_EXT} -mthumb " + LLVMFlags + build_dflags )
      env.Replace(TCPP = LLVMBIN + "clang${EXE_EXT} -mthumb " + LLVMFlags)

   # ARM SDT ARM 32-bit inst. set ANSI C compiler
   env.Replace(ARMCC = LLVMBIN + "/clang${EXE_EXT} -marm " + LLVMFlags + " -xc ")
   env.Replace(CC = env['TCC'])

   env.Replace(ARMCPP = LLVMBIN + "/clang++${EXE_EXT} -mthumb " + LLVMFlags)
   ...
   if do_64:
      env.Replace(LINK = LLVMBIN + "/ld.qcld${EXE_EXT} -march aarch64  ")
   else:
      env.Replace(LINK = LLVMBIN + "/clang++${EXE_EXT} -target armv7m-none-musleabi -fuse-ld=qcld -lc++")
   ...
   env.Replace(BINTOOL = "${LLVMTOOLPATH}/llvm-elf-to-hex.py")
...
#       8916
   ...
   if env['PROC'] in ['arm9']:
     ...
     env.Replace(ARMCC_PREPROCESS_CMD = "-E -xc++")  #-xc or xc++?
   ...
   env.Replace(ARMLD_NOREMOVE_CMD = "--no-gc-sections")
   ...
   env.Replace(ARMBIN_NODEBUG_CMD = "")
   env.Replace(ARMBIN_NOLINKVIEW_CMD = "")
   ...
   env.Replace(ARM_INTRWORK="") # ARM/Thumb interworking support
   ...
   env.Replace(CCFLAGS = "$ARMCC_ARCH $ARMCC_APCS $ARMCC_CODE $ARM_DBG $ARMCC_OPT $BUILD_CCFLAGS $BUILD_FLAGS $CC_DFLAGS $CFLAGS  -fno-exceptions -fno-unwind-tables ")
   env.Replace(CXXFLAGS = "$ARMCC_ARCH $ARMCXX_APCS $ARMCXX_CODE $ARM_DBG $ARMCXX_OPT $BUILD_FLAGS $CC_DFLAGS $CFLAGS -fno-exceptions -fno-unwind-tables ")
   env.Replace(ASCPPFLAGS = "$ARMCC_ARCH $ARMCC_APCS $ARMASM_OPT $BUILDFLAGS $ASM_DFLAGS $CC_DFLAGS")
   env.Replace(ASMFLAGS = "$CC_DFLAGS $ARMASM_CPU $ARMCC_APCS $ARMASM_OPT $BUILDFLAGS $ASM_DFLAGS")
   env.Replace(ASFLAGS = "--cpu $ARM_CPU $ARMCC_APCS $ASM_DBG $CC_DFLAGS")
   if UseClangAs:
      env.Replace(ASFLAGS = " ")
   env.Replace(ARFLAGS = "$ARMAR_LIB_REPLACE $ARMAR_LIB_QUITE")
   env.Replace(LINKFLAGS = "-fuse-baremetal-sysroot -fno-use-baremetal-crt -static  -Wl,-no-undefined -Wl,-T,${TARGET.base}.scl -Wl,-Map,${TARGET.base}.map -Wl,-gc-sections  ${LIBDIRPREFIX} ${LLVMLIB}")
   ...
   cxxcom = "${TEMPFILE('$CXX $CXXFLAGS $CCFLAGS -I$LLVMINCLUDE -I$LLVMINCLUDECXX $_CCCOMCOM $INCPATHS $ARMCC_COMPILE_CMD $SOURCES $ARMCC_OUTPUT_CMD $TARGET')}"
   ...
   env.Replace(LINKCOM =    "${TEMPFILE('$LINK $LINKFLAGS $_LIBDIRFLAGS $SOURCES -Wl,--start-group $_LISTFILES -Wl,--end-group    $ARMLD_OUTPUT_CMD $TARGET')}")
