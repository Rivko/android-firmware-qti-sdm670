...
#------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.qc/1.1/scripts/utils.py#28 $
#  $DateTime: 2018/05/10 18:28:15 $
#  $Author: pwbldsvc $
#  $Change: 16142230 $
...
def mark_time_start (env, description):
    ...
    if not description in time_keeper:
        ...
        # Warnning in case of descriptionalreay existing in time_keeper
        env.PrintWarning("Mark Time Start already has same description (%s)" % description)
...
def get_timing_nodes(env, description):
    '''get_timing_nodes'''
    
    # Setup src node
    src_node = env.Value(description)

    # Setup target start and end node
    now = str(datetime.now())
    start_target = 's_' + description + now
    end_target = 'e_' + description + now
