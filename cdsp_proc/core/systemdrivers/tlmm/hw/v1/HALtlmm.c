...
$Header: //components/rel/core.qdsp6/2.1.c4/systemdrivers/tlmm/hw/v1/HALtlmm.c#1 $
...
void HAL_tlmm_ConfigGpioLowPower ( uint32 nGpioNumber )
{
  ...
  if ( nGpioNumber < tHALTLMMInit.nNumGpio )
  {
    ...
    if ( nLowPowerCfg & HAL_TLMM_LP_CFG_APPLIED )
    {
      ...
      HAL_tlmm_WriteConfig(nGpioNumber,nLowPowerCfg);
    }
  }
}
...
)
{
  ...
  if (tGpio != NULL)
  {
    tGpio->nFunc = HAL_FUNC_FROM_REG(nConfig);
    tGpio->nDir = HAL_DIR_FROM_REG(nConfig);
    tGpio->nPull = HAL_PULL_FROM_REG(nConfig);
    tGpio->nDrive = HAL_DRVSTR_FROM_REG(nConfig);
