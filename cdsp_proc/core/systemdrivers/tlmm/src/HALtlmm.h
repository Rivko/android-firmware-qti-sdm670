...
$Header: //components/rel/core.qdsp6/2.1.c4/systemdrivers/tlmm/src/HALtlmm.h#1 $
...
typedef struct
{
  uint32 nFunc;        /* Function select for a GPIO (0-15). */
  uint32 nDir;         /* Direction select for a GPIO (INPUT/OUTPUT). */
  uint32 nPull;        /* Pull Value (PULL-DOWN, PULL-UP, KEEPER, NO-PULL */
  uint32 nDrive;       /* Drive strength (2-16 mA in even increments). */
  uint32 nOutVal;      /* Output value of an GPIO configured as an output. */
}HAL_tlmm_GpioType;
...
#define HAL_OUT_VAL(config)  (((config)&0x1)>>0x15)

#define HAL_DRVSTR_FROM_REG(config) (((config)&0x1C0)>>6)
#define HAL_PULL_FROM_REG(config) ((config)&0x3)
#define HAL_DIR_FROM_REG(config) (((config)&0x200)>>9)
#define HAL_FUNC_FROM_REG(config) (((config)&0x3C)>>2)
...
void HAL_tlmm_GetLowPowerConfig ( uint32 nGpioNumber, HAL_tlmm_GpioType* Config );
