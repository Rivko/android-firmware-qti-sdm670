...
$Header: //components/rel/core.qdsp6/2.1.c4/systemdrivers/clock/hal/hw/v1/HALclkGeneric.c#1 $
...
)
{
  ...
  nVal  = inpdw(nAddr);
  if (eConfig == HAL_CLK_CONFIG_CORE_MEM_ON)
  {    
    nVal |= HAL_CLK_BRANCH_CTRL_REG_CLK_FORCE_MEM_CORE_ON_FMSK;
  }
  else if (eConfig == HAL_CLK_CONFIG_CORE_MEM_OFF)
  {    
    nVal &= ~HAL_CLK_BRANCH_CTRL_REG_CLK_FORCE_MEM_CORE_ON_FMSK;
  }
  else if(eConfig == HAL_CLK_CONFIG_PERIPH_MEM_ON)
  {    
    nVal |= HAL_CLK_BRANCH_CTRL_REG_CLK_FORCE_MEM_PERIPH_ON_FMSK;
  }
  else if(eConfig == HAL_CLK_CONFIG_PERIPH_MEM_OFF)
  {    
    nVal &= ~HAL_CLK_BRANCH_CTRL_REG_CLK_FORCE_MEM_PERIPH_ON_FMSK;
  }
  else if(eConfig == HAL_CLK_CONFIG_HW_CTL_ON)
  {    
    nVal |= HAL_CLK_BRANCH_CTRL_REG_CLK_HW_CTL_FMSK;
  }
  else if(eConfig == HAL_CLK_CONFIG_HW_CTL_OFF)
  {    
    nVal &= ~HAL_CLK_BRANCH_CTRL_REG_CLK_HW_CTL_FMSK;
  }
  else
...
{
  uint32 nCmdCGRVal;
  uintnt nCmdCGRAddr = pmClockDomainDesc->nCGRAddr;
