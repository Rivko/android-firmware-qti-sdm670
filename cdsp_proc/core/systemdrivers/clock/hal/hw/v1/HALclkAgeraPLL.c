...
$Header: //components/rel/core.qdsp6/2.1.c4/systemdrivers/clock/hal/hw/v1/HALclkAgeraPLL.c#1 $
 ...
 */
#define HAL_CLK_UPDATED_CONFIG_CTL_LO_VAL    0x20000954

/*
 * HAL_CLK_UPDATED_CONFIG_CTL_U_VAL
 */
#define HAL_CLK_UPDATED_CONFIG_CTL_HI_VAL    0x000003D6
...
)
{
  ...
  /*
   * Program L/Alpha values.  Note:  The Agera PLL fractional divider register is
   * 16-bits wide and pConfig->nFracVal variable is 32 bits, so the value must be 
   * massaged to fit the 16-bit field value.
   */
   
  nM = (pConfig->nFracVal & 0xFFFF0000) >> 16;
  nN = pConfig->nFracVal & 0xFFFF;

  /*
   * If the M or N value is greater than the field width, something is wrong.  Return
   * error.
   */
  if ((nM >= 256) || (nN >= 256))
  {
  ...
  HAL_CLK_PLL_HWIO_OUT(PLL_ALPHA_VAL, pSource, ((nM << 8) | nN));
...
)
{
  ...
  pConfig->PostDivs.Agera.nMain = nPostDiv;
