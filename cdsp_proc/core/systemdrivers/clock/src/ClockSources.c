...
/*
  ...
  $Header: //components/rel/core.qdsp6/2.1.c4/systemdrivers/clock/src/ClockSources.c#1 $
...
)
{
  ...
  if (!CLOCK_FLAG_IS_SET(pSource, BYPASS_DEPENDENCIES))
  {
    ...
    Clock_VoltageRequest(pSource->pRail, &pSource->VRegRequest, &VRegRequest);
  }
...
)
{
  ...
  /*
   * TODO:
   *   For the time being stub both RUMI and VIRTIO when STUB_HW_RUMI
   *   is set.
   */
  if ((pSource->nFlags & CLOCK_FLAG_STUB_HW_RUMI) &&
      (DalPlatformInfo_Platform() == DALPLATFORMINFO_TYPE_RUMI ||
...
)
{
  ...
  if (!pSource->NPAHandle || !pSource->NPAHandleSuppressible)
  {
    DALSYS_LogEvent(DALDEVICEID_CLOCK, DALSYS_LOGEVENT_FATAL_ERROR,
      "Unable to create NPA handles for %s.", pSource->szName);
    return;
  }
