...
$Header: //components/rel/core.qdsp6/2.1.c4/systemdrivers/hal/tlmmtile/src/HALtlmmtile.c#1 $
...
uint32 HAL_tlmmtile_Set ( uint32 nGPIO, HAL_tlmmtile_RegType nReg, HAL_tlmmtile_FieldType nField, uint32 nVal )
{
  ...
  if ( gHalTlmm.pGpio[nGPIO] == NULL || nGPIO ==124 )
  {
    /*
  ...
  if(gHalTlmm.pGpio[nGPIO] == 0xe357c000 )
  {
    DALSYS_LogEvent(DALDEVICEID_TLMM, 
          DALSYS_LOGEVENT_FATAL_ERROR, 
          "Write to register 0x357C000");
  }
