...
05/16/17   ah      Separate checks for rings and queues.
                           ...
                           U_LONG size)
{
   ...
   if (gpii != NULL)
   {
      gpi_log(GPI_DEBUG, 4, "nbr_avail_ring_elem - GPII ID: %d Chan %d Size %d has %d available!!\n",
              gpii->id, chan, size, avail_elem);
   }
   else
   {
      gpi_log(GPI_DEBUG, 3, "nbr_avail_ring_elem - Queue %d Size %d has %d available!!\n",
...
boolean en_q(gpi_work_q *q, gpi_ring_elem *elem)
{
   ...
   {
      gpi_log(GPI_WARNING, 0, "en_q - WARN - Queue full!\n");
...
U_LONG get_ring_ptr(gpii_info *gpii, uint8 chan, GPI_RING_PTR_TYPE ptr_type)
{
   ...
   switch (ptr_type)
   {
      ...
      default:
         ptr = 0;
         gpi_log(GPI_ERROR, 0, "get_ring_ptr - unsupported ptr type\n");
   }
