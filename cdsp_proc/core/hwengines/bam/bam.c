...
===============================================================================
 ...
 $Header: //components/rel/core.qdsp6/2.1.c4/hwengines/bam/bam.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
10/13/17   PC      Added return value check for BAM ISR registration.
...
exit:
   ...
   if (install_isr)
   {
       if(BAM_SUCCESS != bam_osal_isrinstall(&bam->irqhandle,bam->bam_cfg.bam_irq,bam_isr,(void *)bam))
       {
    	   bam_osal_syncenter(&bam_drv_sync);

    	   bam_osal_debug(MSG_ERR,"bam_init: BAM ISR registration failed\n");
    	   goto err;
       }
   }
