...
$Header: //components/rel/core.qdsp6/2.1.c4/debugtools/sys_m/src/sys_m_smsm_non_mpss.c#1 $
...
DALResult sys_m_smsm_task(DALSYSEventHandle hEvent, void* tid)
{
    ...
    for (;;)
    {
        ...
        TMS_SHUTDOWN_MSG_HIGH_2("SMP2P rx state current 0x%x received 0x%x", sys_m_rx_state, rx_state_received);
        ...
        {
            TMS_SHUTDOWN_MSG_HIGH(" SMP2P graceful shutdown request");
