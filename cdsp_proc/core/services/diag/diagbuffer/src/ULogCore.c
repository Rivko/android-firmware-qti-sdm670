...
$Header: //components/rel/core.qdsp6/2.1.c4/services/diag/diagbuffer/src/ULogCore.c#1 $
============================================================================*/
 ...
 * @param logName : The log name (24 chars max, including NULL). ULog is case insensitive, duplicate ulogs with 
 *                  same name but different upper/lowercase combinations will just be appended
...
ULogHandle ULogCore_HandleGet(char *logName)
{
  ...
  while (tempLog != NULL)
  {
    /* Case insensitive comparison for ULog name, duplicate ulogs with same name 
	   but different upper/lowercase combinations will just be appended */
    if (strncasecmp(tempLog->name, logName, ULOG_MAX_NAME_SIZE + 1) == 0)
    {
      // We have a match.
      Core_MutexUnlock(ulogContext.gLockHandle);
