...
/*===========================================================================
  ...
  $Header: //components/rel/core.qdsp6/2.1.c4/storage/tftp/common/src/tftp_protocol.c#1 $ $DateTime: 2018/07/30 01:21:36 $ $Author: pwbldsvc $

when         who   what, where, why
----------   ---   ---------------------------------------------------------
2017-06-05   rp    Improve TFTP log stats.
2017-05-31   rp    Port to SLPI image.
2016-12-27   rp    Fix incorrect receiving of user data on tftp packet timeout
       ...
       proto->retry_count++)
  {
    ...
    switch (proto_res)
    {
      ...
      case TFTP_PROTOCOL_EOT:
      {
        ...
        TFTP_ASSERT (rx_pkt->opcode == TFTP_PKT_OPCODE_TYPE_ERROR);
        error_pkt = &rx_pkt->pkt_fields.error_pkt;
        TFTP_LOG_INFO ("Recd END OF TRANSFER pkt. Code = %d, Msg = %s",
                      error_pkt->error_code, error_pkt->error_string);
        goto End;
      }
                                 ...
                                 const char *error_msg)
{
  ...
  {
    TFTP_LOG_INFO ("Sending End-Of-Transfer. Code = %d, Msg = %s",
                             ...
                             uint16 block_number, int is_last_ack)
{
  ...
  if (is_last_ack == 1)
  {
    if(new_timeout == 0)
    {
      TFTP_LOG_INFO ("Sent last ACK: *NOT* waiting on reply.\n");
      goto End;
    }
    ...
    if (proto_res == TFTP_PROTOCOL_SUCCESS)
    {
      TFTP_LOG_INFO ("Received data resending last ACK. Pkt Loss ..resend!\n");
                          ...
                          void *cb_func_param)
{
  ...
  proto->cur_block_number = 0;
  proto->cur_buf_posn = 0;
  ...
  while (!proto->final_pkt_sent)
  {
    for (retry_count = 0; retry_count < proto->max_pkt_retry_count;
         ++retry_count)
    {
      cur_block_number = proto->cur_block_number;
      cur_buf_posn = proto->cur_buf_posn;
      cur_window_size = 0;
      run_tx_loop = 1;
      ...
      switch (proto_res)
      {
        case TFTP_PROTOCOL_SUCCESS:
        {
          ...
          proto->cur_block_number = cur_block_number;
          proto->cur_buf_posn = cur_buf_posn;
          *sent_data_size = cur_buf_posn;
        }
        break;
  ...
  while (!proto->final_pkt_received && !proto->insufficient_recv_buffer)
  {
          ...
          ++retry_count)
    {
          ...
          (proto_res == TFTP_PROTOCOL_WRONG_PACKET))
      {
        ...
        proto_res = tftp_protocol_send_ack_pkt (proto,proto->cur_block_number,
                                                is_last_ask);
