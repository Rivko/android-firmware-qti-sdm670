...
$Header: //components/rel/core.qdsp6/2.1.c4/api/power/npa_remote_resource.h#1 $
============================================================================*/
...
/**
 * <!-- npa_remote_get_qdi_client_handle -->
 *
 * @brief Returns the QDI handle corresponding to the NPA client that
 * is a proxy for a client in a userPD.
 * Must be invoked from within update/driver functions of the resource
 * in the rootPD. Caller must not strip request attributes from the client.
 * The routine asserts that the client has the NPA_REQUEST_REMOTE_CLIENT
 * attribute set and that the client is infact a proxy.
 *
 * This routine is intended to be used within update and driver functions
 * where the resource author has need to copy from/into non-NPA data/buffers
 * in the userPD associated with the client.
 *
 * @param client: a NPA client that is a proxy for a client in a userPD
 */
