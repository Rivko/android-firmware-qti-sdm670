...
/**@ingroup func_qurt_system_hvx_regs_get
  ...
  @param[in]
    ...
    pBuf         Pointer to the buffer for HVX register saving.
                 ...
                 HVX regsters are saved. The address of the 5th byte should be 128 bytes aligned. 
                 For example, a buffer can be declared at first as,
                    unsigned char vbuf[QURT_HVX_VREG_BUF_SIZE+128];
                    unsigned char *pBuf;
                 and then align the buffer pointer to,
                    pBuf = vbuf;
                    pBuf += (128 - 4 - (unsigned)pBuf%128);
