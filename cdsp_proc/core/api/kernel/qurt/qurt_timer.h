...
/** 
 The sleep timer error margin for Qtimer is 1000 ticks ~52us.
*/
...
{ 
    ...
    unsigned int        group;  /**<  Group number of the timer; the criterion used to disable or enable the set
       of timers.  */
    pfn_t pFn;  /**< this will be a callback other than signal set */
    /** @endcond */
}
qurt_timer_attr_t;
...
/**@ingroup func_qurt_timer_restart
   ...
   A restarted timer expires after the specified duration, with the starting time being when the function is called.
...
/**@ingroup func_qurt_timer_get_attr
  ...
  @note1hang After a timer is created, the attributes assigned to the thread cannot be changed by the user.
...
/**@ingroup func_qurt_timer_sleep
  ...
  @note1hang The maximum sleep duration is 36 hours. The error margin of the sleep timer 
             is approximately 90 microseconds (due to a setup time of two ticks and resolution
             of one tick).  
