...
$Header: //components/rel/core.qdsp6/2.1.c4/api/dal/DDITimer.h#1 $
==============================================================================*/
...
struct DalTimer
{
   ...
   DALResult (*Register)(uint32 ClientHandle, DalDeviceHandle * _h, const DALSYSEventHandle  hEvent, uint32  time);
   DALResult (*UnRegister)(uint32 ClientHandle, DalDeviceHandle * _h, const DALSYSEventHandle  hEvent);
   DALResult (*GetTimerCount)(uint32 ClientHandle, DalDeviceHandle * _h, void * timer_count, uint32 size);
};
...
{
   return ((DalTimerHandle *)_h)->pVtbl->Register(DALDEVICE_LOCAL_CLIENTID,  _h, hEvent, time);
}
static __inline DALResult
DalTimer_UnRegister(DalDeviceHandle * _h, const DALSYSEventHandle  hEvent)
{
   return ((DalTimerHandle *)_h)->pVtbl->UnRegister(DALDEVICE_LOCAL_CLIENTID,  _h, hEvent);
}
//arg expected timer_count - pointer to long long , size = sizeof long long
static __inline DALResult
DalTimer_GetTimerCount(DalDeviceHandle * _h, void * timer_count, uint32 size)
{
   return ((DalTimerHandle *)_h)->pVtbl->GetTimerCount(DALDEVICE_LOCAL_CLIENTID,  _h, timer_count, size);
}
#endif
