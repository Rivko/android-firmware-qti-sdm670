...
int thread_create(void *pc, void *stack, unsigned int stack_size, char *arg, int prio){
    ...
    snprintf(tname, sizeof(tname),"App1Th%d",id);
...
void test(void *arg){
    ...
    for( j = 0; j < 2; j++ ){
        ...
        for( i = 0; i < MAX_THREADS; i++ ){
            ...
            free(stack_start[i]);
            //printf( "%d count %d t%d exit return %p status %p\n", myid, j, i, r, status);
            // if( r == 0 ){
...
extern int qurt_debug_monitor_ctrl(int on_off, void *p_cfg, size_t config_size);
...
int main(int argc, char **argv)
{
    ...
    assert(-1 == qurt_debug_monitor_ctrl(0, NULL, 0));
