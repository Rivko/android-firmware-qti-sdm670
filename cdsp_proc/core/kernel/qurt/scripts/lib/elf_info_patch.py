...
def _build_eip_info(physaddr, f, infiles, island_moves, clade_registers, opt_mappings, mappings=None, q6cfg_records=[]):
    ...
    private_asids = getattr(infiles[0].cfg, 'private_asids', [])
    if private_asids:
        private_asids += [0]          # Zero terminate it
        eip_data += encode_eip_record(PRIVATE_ASID, '<'+'B'*len(private_asids), *private_asids)
    for i in infiles[1:]:
        if hasattr(i,'SECURE_ASID'):
            #
            #  It's a secure program to be started at boot time.  The ELF mappings are already
            #   configured; just encode here the information needed to start the process.
            #
            eip_data += encode_eip_record(SECURE_PROCESS_INFO,
                                          '<LLLL%ds' % (len(i.SECURE_NAME)+1),
                                          i.SECURE_SID,
                                          i.SECURE_ASID,
                                          i.program_entry_point,
                                          i.SECURE_SLOT,
                                          str(i.SECURE_NAME))
            continue
