...
def generate(env, target='staticv5'):
   ...
   if target=='mpqurt':
      ...
      hexagon_hw_ver=65
   elif target=='computev65_SRM':
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/computev65_SRM'
      hexagon_hw_ver=65
      ...
      env['CONFIG_DIR'] = env.subst('$QURT_TOP')+'/config/sdkv65'
      hexagon_hw_ver=65
   ...
   env.Replace(BUILD_DIR = env.subst('${BUILD_DIR}/${TRGT}${TARGET_MOD_SUFFIX}'))
