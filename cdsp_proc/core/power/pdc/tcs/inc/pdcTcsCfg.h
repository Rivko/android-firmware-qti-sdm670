...
=============================================================================
$Header: //components/rel/core.qdsp6/2.1.c4/power/pdc/tcs/inc/pdcTcsCfg.h#1 $
...
typedef struct pdc_tcs_resource_s
{
  ...
  uint32  base_addr;    /* CMD DB base address */
}pdc_tcs_resource;

/* Command data */
typedef struct pdc_tcs_resource_data_s
{
  uint32    vlvl;         /* VLVL resource value for each TCS */
  uint32    options;      /* Control flags (see above) */
  uint32    addr_offset;  /* Aditional required offset from base 
                           * address */
  int32     hlvl;         /* Runtime populated hlvl based on given vlvl */
