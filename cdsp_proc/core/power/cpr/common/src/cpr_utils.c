 ...
 * $Header: //components/rel/core.qdsp6/2.1.c4/power/cpr/common/src/cpr_utils.c#1 $
 * $Change: 16759227 $
 */
...
const cpr_margin_data* cpr_utils_get_margins(const uint8 railCPRRev,
        const cpr_margin_cfg* marginList)
{
    if(marginList != NULL)
    {
        for(int i = 0; i < marginList->count; i++)
        {
            if (railCPRRev >= marginList->data[i].cprRevMin &&
                    railCPRRev <= marginList->data[i].cprRevMax) {
