 ...
 * $Header: //components/rel/core.qdsp6/2.1.c4/power/cpr/common/src/cpr_rail.c#1 $
 * $Change: 16759227 $
 */
#include <stdbool.h>
#include <string.h>
...
static void cpr_rail_update_lower_modes(cpr_rail* rail, const disablement_params_t *params)
{
    ...
    for(int modeIdx = 0; modeIdx < rail->vp->modesCount-1; modeIdx++)
    {
        ...
        CPR_LOG_WARNING( "Restrict %s rail %d mode: (openloop mode: %d) (closedloop mode: %d)",
                         rail->name, mode, restricted_openloop_mode, restricted_closedloop_mode );
    }
}
 ...
 * celing, floor to open loop ceiling, floor.
 */
