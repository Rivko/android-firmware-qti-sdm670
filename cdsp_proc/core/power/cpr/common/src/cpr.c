 ...
 * $Header: //components/rel/core.qdsp6/2.1.c4/power/cpr/common/src/cpr.c#1 $
 * $Change: 16759227 $
 */
#include "cpr.h"
#include "cpr_logs.h"
...
*
*/
static bool init_cpr_config(void)
{
    CPR_LOG_INIT();
    ...
    /*if(cpr_info.chipVersion == 0x10000)
        return FALSE;*/
...
*
*/
static bool lock_and_init_data(boolean requireInit, boolean requireCfg)
{
    CPR_IMAGE_LOCK_CORE();
    bool cpr_config_state;

    if(requireInit && !cpr_info.dataInit)
    {
        cpr_info.dataInit = true; // set here to prevent recursive call

       cpr_config_state = init_cpr_config();
       if(!cpr_config_state)
           return FALSE;
    }
...
void cpr_enable_closed_loop(uint64 tick)
{
    cpr_image_enable_closed_loop();
}

void cpr_disable_closed_loop()
{
    cpr_image_disable_closed_loop();
}
...
void cpr_init()
{
    ...
    cpr_init_state = lock_and_init_data(true, true);
...
void cpr_get_supported_modes(cpr_domain_id railId, const cpr_voltage_mode** modes, uint32* modesCount)
{
     ...
     * need only data init.
     */
    cpr_init_state = lock_and_init_data(true, false);
