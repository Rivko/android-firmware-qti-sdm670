 ...
 * $Header: //components/rel/core.qdsp6/2.1.c4/power/cpr/common/hal/v3/src/cpr_hal.c#1 $
 * $Change: 16759227 $
 *
 */
#include "cpr_hal.h"
...
static void cpr_hal_init_thread(cpr_hal_handle* hdl, cpr_hal_thread *thread, boolean configureHW)
{
    ...
    if(configureHW)
    {
        ...
        for(int i = 0; i < tcfg->sensorsCount; i++) {
            CPR_HWIO_OUT( HWIO_CPR_SENSOR_THREAD_n_ADDR( base, i ), thread->id );
        }
...
void cpr_hal_init_controller(cpr_hal_handle* hdl, boolean configureHW)
{
    ...
    if(configureHW)
    {
        ...
        cpr_hal_set_count_repeat(hdl, 20);
...
void cpr_hal_enable_up_interrupt(cpr_hal_handle* hdl, boolean enable)
{
    ...
    CPR_LOG_TRACE("cpr_hal_enable_up_interrupt: Enable = %u", enable);
}
...
void cpr_hal_enable_down_interrupt(cpr_hal_handle* hdl, boolean enable)
{
    ...
    CPR_LOG_TRACE("cpr_hal_enable_down_interrupt: Enable = %u", enable);
}
...
void cpr_hal_enable_mid_interrupt(cpr_hal_handle* hdl, boolean enable)
{
    ...
    CPR_LOG_TRACE("cpr_hal_enable_mid_interrupt: Enable = %u", enable);
}
...
void cpr_hal_enable_cpr_done_interrupt(cpr_hal_handle* hdl, boolean enable)
{
    ...
    CPR_LOG_TRACE("cpr_hal_enable_cpr_done_interrupt: Enable = %u", enable);
}
...
void cpr_hal_write_cprf_voltages(uint32 mode, uint32 vmax, uint32 vmin)
{
    ...
    HWIO_RPMH_CPRF_CPRFm_VOLTAGE_LIMITS_n_OUTI2(MSS_CPRF_ID, mode, val);
    HWIO_RPMH_CPRF_CPRFm_SAFE_VOLTAGE_n_OUTI2(MSS_CPRF_ID, mode, vmax);

    CPR_LOG_TRACE("cpr_hal_write_cprf_voltages: mode=%d vmax=%d vmin=%d",
            mode, vmax, vmin);
}
...
void cpr_hal_enable_cprf_interface( cpr_hal_handle* hdl, boolean enable )
{
    ...
    CPR_LOG_TRACE("cpr_hal_enable_cprf_interface: Enable = %u", enable);
//    CPR_ASSERT(val == enable);
}
...
void cpr_hal_enable_hw_closed_loop( cpr_hal_handle* hdl )
{
    ...
    CPR_LOG_TRACE("CPR HW Closed loop is Enabled.");
}
...
void cpr_hal_set_cprf_defaults ( const char* name, uint8 cprf_id )
{
    ...
    uint32 CPRFm_STEP_SETTINGS = 0x25;
    uint32 CPRFm_TIMEOUT = 0xf01; //200us
    uint32 CPRFm_FORCE_SETTINGS = 0x1;
    uint32 CPRFm_CTRL = 0x3b;
