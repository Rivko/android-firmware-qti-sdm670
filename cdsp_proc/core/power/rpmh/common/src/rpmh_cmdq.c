 ...
 */ 
rpmh_cmd_t *new_cmd (rpmh_cmdq_t *self)
{
  rpmh_cmd_t *cmd = (rpmh_cmd_t *)inq_pop (self->free_cmds);
  if (cmd == NULL)
  {
    cmd = (rpmh_cmd_t *)rpmh_malloc (sizeof (rpmh_cmd_t));
 ...
 */ 
void delete_cmd(rpmh_cmdq_t *self, rpmh_cmd_t *cmd)
{
...
rpmh_cmdq_t *rpmh_cmdq_create (uint32 drv_index)
{
  ...
  rpmh_cmdq_t *self = (rpmh_cmdq_t *)rpmh_malloc (sizeof(rpmh_cmdq_t));
...
void rpmh_cmdq_destroy (rpmh_cmdq_t *self)
{
  ...
  while ((msg = inq_pop (self->sleep_amcs)) != NULL) 
  {
    rpmh_free (msg);
  } 
  inq_destroy(self->sleep_amcs);

  while ((msg = inq_pop (self->wake_amcs)) != NULL) 
  {
    rpmh_free (msg);
  }	    
 ...
 * @param drv_index : The DRV (index into the DRV table) corresponding to the read cmd 
 *
 * @return The command id for the request 
 */ 
uint32 rpmh_cmdq_send_read (rpmh_cmdq_t *self, uint32 address, rpmh_client_t *client, boolean queue, uint32 drv_index)
{
  ...
  if (!queue) 
  {
    if (tcs_send(cmd, client->drv_id, drv_index))
    {
      //command sent    
 ...
 */ 
void rpmh_cmdq_flush_sleep_wake_amcs (rpmh_cmdq_t *self, RSCSW_DRV_MAPPING drv_id, uint32 drv_index, rpmh_set_enum set)
{
  inq_iterator_t it;
  rpmh_cmd_t *cmd;
  ...
  while ((cmd = (rpmh_cmd_t *)inq_it_deref (&it)) != NULL)
  {
    ...
    while (!tcs_send (cmd, drv_id, drv_index))
    {
                               ...
                               rpmh_command_set_internal_t *command_set, boolean *rc_idle)
{
  *rc_idle = TRUE;
  ...
  rpmh_core_verify(cmd->num_rcs <= self->num_cmds_per_tcs);
  rpmh_core_verify(command_set->num_commands <= self->num_cmds_per_tcs);
 ...
 * <!-- merge_cmds -->
 *
 * @brief Merge the contents of one command into another
 *
 * @param merge_cmd : The cmd to be merged
 * @param dest_cmd   : The cmd that will contain both its current contents and 'merge_cmd' contents
 * @param max_cmds   : The max # of commands allowed in a single request
 */ 
static void merge_cmds(rpmh_cmd_t *merge_cmd, rpmh_cmd_t *dest_cmd, uint32 max_cmds)
{
  rpmh_core_verify((dest_cmd->num_rcs + merge_cmd->num_rcs) <= max_cmds);	
  rpmh_core_verify(merge_cmd->set == dest_cmd->set);	
  rpmh_core_verify(merge_cmd->amc == dest_cmd->amc);	
  rpmh_core_verify((merge_cmd->req_id == 0) && (dest_cmd->req_id == 0));	
  rpmh_core_verify(merge_cmd->client == dest_cmd->client);
  for(int i = 0; i < merge_cmd->num_rcs; i++)	  
  {
    dest_cmd->details[dest_cmd->num_rcs].address = merge_cmd->details[i].address;
    dest_cmd->details[dest_cmd->num_rcs].type    = merge_cmd->details[i].type;
    dest_cmd->num_rcs++;
  }
}
 ...
 * <!-- combine_cmds -->
 *
 * @brief Insert the contents of one command into any command in the given queue with available space
 *
 * @param queue     : The cmdq to check for merging
 * @param merge_cmd : The cmd to be inserted
 * @param drv_id    : The RSC + DRV associated with this cmd merge. Used to check max # cmds allowed
 *
 * @return TRUE if merge_cmd was able to merge into the a command in the queue, FALSE if not
 */ 
static boolean combine_cmdq_cmds(inq_t *queue, rpmh_cmd_t *merge_cmd, RSCSW_DRV_MAPPING drv_id)
{
  inq_iterator_t it;
  rpmh_cmd_t *cmd;
  uint32 num_cmds = tcs_size (drv_id);
  inq_it_begin (queue, &it);
  while ((cmd = (rpmh_cmd_t *)inq_it_deref (&it)) != NULL)
  {
    rpmh_core_verify(num_cmds >= cmd->num_rcs);
    if(merge_cmd->num_rcs <= (num_cmds - cmd->num_rcs))
    {
      //We found room! Combine the commands, and return
      merge_cmds(merge_cmd, cmd, num_cmds);    
      return TRUE;
    } 
    inq_it_next (&it);
  }
 ...
 * @param client      : The client who initiated the command set
 * @param command_set : The command set with all the request details
 * @param drv_id      : The RSC+DRV mapping on which to send
 * @param drv_index   : The DRV (index into the DRV table) corresponding to the cmd set 
 *
 * @return The command id for the request 
 */ 
uint32 rpmh_cmdq_send_cmd_set(rpmh_cmdq_t *self, rpmh_client_t *client, 
                              rpmh_command_set_internal_t *command_set, RSCSW_DRV_MAPPING drv_id, uint32 drv_index)
{
  uint32 ret_req_id = 0;
  ...
  switch (command_set->set)
  {
    ...
    case RPMH_SET_WAKE:
      ...
      //try to combine this new command with an already existing one. 
      if(combine_cmdq_cmds(queue, cmd, drv_id))
      {
        //if successful, delete the redundant command
        delete_cmd (self, cmd);	      
      }
      else
      {	       
        //if there was no room to combine, append this cmd seperately	      
    ...
    case RPMH_SET_ACTIVE:
      ...
      if ((rc_idle == TRUE) && is_amc_free(drv_index))
      {    
        //request sent    
        inq_append (self->in_flight, cmd);
        tcs_send(cmd, client->drv_id, drv_index);
      }
      else
      {
 ...
 *
 * @param self      : The cmdq associated with the send
 * @param drv_index : The DRV (index into the DRV table) corresponding to the send 
 *
 * @return TRUE if sent, false if not because the resource was busy or there were no AMCs
 */ 
boolean rpmh_cmdq_send_cmd_at_head (rpmh_cmdq_t *self, uint32 drv_index)
{
  int i;	
  boolean idle = TRUE;
  inq_iterator_t it;
  ...
  if ((cmd = (rpmh_cmd_t *)inq_it_deref (&it)) != NULL)
  {
    ...
    if(idle == TRUE)
    {
      ...
      if (tcs_send (cmd, cmd->client->drv_id, drv_index) == TRUE)
      {
...
static void pull_from_queue (inq_t *q, rpmh_cmd_t *looking_for, rpmh_cmd_t **cmd)
{
  ...
  RPMH_LOG_0("\t!Inconcsistent cmdq state");
