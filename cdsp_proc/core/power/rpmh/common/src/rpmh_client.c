...
extern qurt_mutex_t g_uRPMhMutex;
...
struct rpmh_s
{
  ...
  boolean                   disable_sleep_rsc_clock_toggle;  //used for debug - leave the RSC clocks on
  boolean                   disable_tcs_stuck_check;         //used for debug - don't check for stuck TCSs
} rpmh;
...
void rpmh_finish_amc (RSCSW_DRV_MAPPING hw_drv)
{
  ...
  if (drv_id != RSC_DRV_VIRTUAL_MAX)
  {
    ...
    RPMH_LOG_QDSS_2("\tfinished req: (CLIENT: %s)(REQ_ID: %d)", finished_client->name, req_id);
  ...
  boolean post = rpmh_cmdqs_empty();
  rpmh_in_flight_parity(pre, post);

  RPMH_LOG_2 ("rpmh_finish_amc_complete pre %d post %d", pre, post);
  rpmh_unlock (TRUE);
}
...
void rpmh_finish_sleep_wake_amc (RSCSW_DRV_MAPPING drv_id, rpmh_set_enum set)
{
  ...
  //clean up at the TCS level. This also cleans up at the resource and queue level	
  tcs_finish_amc (rpmh.rpmh_cmdq[drv_index], drv_id, drv_index, set); 
  rpmh_unlock (FALSE);
}
...
/**
 * <!-- rpmh_update_command_priority -->
 *
 * @brief Update the priority field for a given resource command 
 *
 * @param handle   : The client handle associated with the update
 * @param address  : The address of the resource command on which to update priority
 * @param priority : The new priority value
 *
 * @return the resource command pointer
 */ 
void rpmh_update_command_priority (rpmh_client_handle client_handle, uint32 address, rpmh_rc_priority_enum priority)
{
  rpmh_core_verify(priority < RPMH_RC_PRIORITY_MAX);

  rpmh_client_t *client = (rpmh_client_t *)client_handle;
  rpmh_resource_command_t *rc = get_rc (address);
  rpmh_resource_command_update_priority(rc, client->drv_id, priority); 
}
...
uint32 rpmh_issue_command_set (rpmh_client_handle client_handle, rpmh_command_set_t *cmd_set)
{
  ...
  boolean pre = cmdq_empty(rpmh.rpmh_cmdq[drv_index]);
  ...
  rpmh_core_verify (command_set->num_commands <= IMAGE_TCS_SIZE);
  ...
  for (int i = 0; i < command_set->num_commands; i++)
  {
    ...
    RPMH_LOG_3 ("\tresource_command (ADDRESS: 0x%08x)(DATA: 0x%08x)(COMPLETION? %d)", command.address, command.data, command.completion);
  ...
  {	    
    ...
    {
        if(!rpmh.disable_tcs_stuck_check)
        {
	    //make sure no TCSs are "stuck"
            rpmh_core_verify (!is_tcs_stuck (drv_index));
        }
  ...
  else if(is_request_explicit)
  {
    ...
    {
      explicit_cmds->cmd_sets = (rpmh_command_set_internal_t *)rpmh_realloc (explicit_cmds->cmd_sets,
  ...
  boolean post = cmdq_empty(rpmh.rpmh_cmdq[drv_index]);
  rpmh_in_flight_parity(pre, post);

  RPMH_LOG_3 ("issue_command_set_complete (REQ_ID: %d) pre %d post %d", req_id, pre, post);
...
static void log_command_set_internal (rpmh_command_set_internal_t *cmds)
{
  int i;	
  for (i = 0; i < cmds->num_commands; i++)
  {
    RPMH_LOG_3 ("\t\t (CMD ADDRESS: 0x%08x) (CMD DATA: 0x%08x) (COMPLETION? %d)", 
                  cmds->commands[i].address, cmds->commands[i].data, cmds->commands[i].completion);
  }
}
...
{
    uint32 drv_index = lookup_drv_index (drv_id);
    if (tcs_slots_available(drv_index, set, cmd_set->num_commands) != -1)
    {
      RPMH_LOG_1 ("\tadding command set to sleep/wake TCS (SET: %d)", set);
      log_command_set_internal(cmd_set);
      rpmh_core_verify (tcs_prepare_sleep_add_cmd_set((rpmh_command_set_internal_t*)cmd_set, drv_index));
    }
    else
    {
      ...
      rpmh_cmdq_send_cmd_set (rpmh.rpmh_cmdq[drv_index], NULL, 
                              (rpmh_command_set_internal_t*)cmd_set, drv_id, drv_index);
      log_command_set_internal(cmd_set);
    }
    return;
}
...
static void add_explicit_cmd_set_for_sleep(rpmh_rc_priority_enum priority_level, RSCSW_DRV_MAPPING drv_id)
{
  ...
  {
    if((client->explicit_cmds != NULL) && (client->drv_id == drv_id))
    {
...
void rpmh_enter_sleep (RSCSW_DRV_MAPPING drv_id)
{
...
#ifdef RPMH_SENSORS_ISLAND_MODE_SWITCH
  ...
  {
    tcs_update_mode(drv_index, (uint32)SLPI_VIRTUAL_TCS_CONFIG_SLEEP, rpmh.rpmh_cmdq[drv_index]);
  }
#endif //RPMH_SENSORS_ISLAND

  //clean up from the previous sleep before sending again
  tcs_clean_sleep_wake (drv_index);

  if(drv->in_irq_num)
  {
    //disable interrupt
    rpmh_disable_isr(drv->in_irq_num);
  }
  ...
  tcs_prepare_sleep_commit (drv_id, drv_index);

  RPMH_LOG_0 ("\tflushing sleep AMCs");
  rpmh_cmdq_flush_sleep_wake_amcs (rpmh.rpmh_cmdq[drv_index], drv_id, drv_index, RPMH_SET_SLEEP);

  //disable RSC clocks
  for (int i = 0; i < DRV_CONFIG_DATA->num_rsc_sleep_clks; i++)
  {
    if(!rpmh.disable_sleep_rsc_clock_toggle)
    {
...
void rpmh_exit_sleep (RSCSW_DRV_MAPPING drv_id)
{
  ...
  if(drv_id == RSC_DRV_VIRTUAL_SENSORS)
  {
    tcs_update_mode(drv_index, (uint32)SLPI_VIRTUAL_TCS_CONFIG_ACTIVE, rpmh.rpmh_cmdq[drv_index]);
  }
#endif //RPMH_SENSORS_ISLAND


  RPMH_LOG_1 ("rpmh_exit_sleep, flushing wake AMCs (DRV_ID: 0x%08x)", drv_id);
  rpmh_cmdq_flush_sleep_wake_amcs (rpmh.rpmh_cmdq[drv_index], drv_id, drv_index, RPMH_SET_WAKE);
  ...
  {
    //enable interrupt
    rpmh_enable_isr (drv->in_irq_num);
  }
...
void rpmh_rsc_clocks_init(void)
{
  ...
  RPMH_LOG_0 ("rpmh_rsc_clocks_init");
  ...
  RPMH_LOG_0 ("rpmh_rsc_clocks_init_complete");
}
