...
//---------------------------------------------------------------------------
// SLPI_CX_WAKEUP_WORKAROUND Definitions
//---------------------------------------------------------------------------

#ifdef RPMH_SLPI_CX_WAKEUP_WORKAROUND  
#include "DDIChipInfo.h"
const char *cx_name = "cx.lvl";
uint32 cx_addr = 0;
#endif //RPMH_SLPI_CX_WAKEUP_WORKAROUND
...
//defined in urpmh.c
extern unpa_client rpmh_hwsolver_client[TCS_TOTAL_RESOURCE_NUM];
unsigned int rpmh_hwsolver_nclients;

/**
 * <!-- rpmh_hwsolver_avail_cb -->
 * @brief Callback function when the HWSolver is initialized
 */
static void rpmh_hwsolver_avail_cb(void *context, unsigned int event_type, void *data, 
                                   unsigned int size)
{
  pdc_votes_t *pdc_votes = (pdc_votes_t *)context;
  rpmh_core_verify_ptr(pdc_votes);
  
  rpmh_core_verify(rpmh_hwsolver_nclients < TCS_TOTAL_RESOURCE_NUM);
  
  pdc_votes->hwsolver_voter = unpa_initClient(pdc_votes->voter_name, UNPA_CLIENT_REQUIRED,
                                                HWSOLVER_MODE_DISABLE_NODE_NAME,
												&rpmh_hwsolver_client[rpmh_hwsolver_nclients++]);
  rpmh_core_verify_ptr(pdc_votes->hwsolver_voter);
...
{
  self->pdc_votes = (pdc_votes_t *)rpmh_malloc (sizeof (pdc_votes_t));
  rpmh_core_verify_ptr(self->pdc_votes);
  memset (self->pdc_votes, 0, sizeof (pdc_votes_t));

  self->pdc_votes->votes = (uint32 *)rpmh_malloc (sizeof (uint32) * TCS_NUM_TOTAL);
  ...
  for (int i = 0; i < TCS_NUM_TOTAL; i++)
  {
    ...
    for(int j = 0; j < NUM_COMMANDS_PER_TCS; j++)
    {
      ...
      {
        RPMH_LOG_2 ("\trpmh_pdc_init_val: (CMD NUM: %d) (VALUE: %d)", j, tcs[j].data.hlvl);
        //save the data for this resource in this tcs     
        self->pdc_votes->votes[i] = tcs[j].data.hlvl;
      }
    }
  }
...
#ifdef USES_HWSOLVER  
  npa_resource_available_cb(HWSOLVER_PRESENT_NODE_NAME, rpmh_hwsolver_avail_cb, (void*)self->pdc_votes);
#endif  
}
...
static void rpmh_resource_command_pdc_compare_and_update (rpmh_resource_command_t *self, rpmh_vote_t *vote, rpmh_set_enum set)
{
  ...
  if(set == RPMH_SET_SLEEP)
  {
    for(int i = 0; i < TCS_NUM_TOTAL_SLEEPS; i++)
    {
      if (vote->data > pdc_votes->votes[i])
      {
        sleep_tcs_vote |= (1 << i);      
      }
    }  
...
#ifdef USES_HWSOLVER
    if(NULL != pdc_votes->hwsolver_voter)
    { /* Vote to disable or re-enable PDC-involving LPMs */
      uint32 to_disable = (sleep_tcs_vote ? (HWSOLVER_SLEEP_MODE2|HWSOLVER_SLEEP_MODE3) : 0);
      unpa_issueRequest(pdc_votes->hwsolver_voter, to_disable);
    }
#endif

    if(NULL != pdc_votes->sleep_voter)
    {
      npa_issue_required_request(pdc_votes->sleep_voter, sleep_tcs_vote);
    }
  }
  
  if(vote->data <= pdc_votes->votes[TCS_NUM_WAKE0])
  {
    vote->dirty = RPMH_RC_DIRTY_MOL;
  }
}
...
drv_votes_t *rpmh_resource_command_get_voter (rpmh_resource_command_t *self, RSCSW_DRV_MAPPING drv_id)
{
  ...
  if (curr_drv == NULL)
  {
    ...
    curr_drv->priority = (rpmh_rc_priority_enum)cmd_db_query_priority(self->address, get_global_drv_id(drv_index));
...
/**
 * <!-- rpmh_resource_command_update_priority -->
 *
 * @brief Retrieve this voters priority for the given resource command
 *
 * @param self   : The resource command pointer
 ...
 */
void rpmh_resource_command_update_priority(rpmh_resource_command_t *self, 
                                           RSCSW_DRV_MAPPING drv_id, rpmh_rc_priority_enum priority)
{
  drv_votes_t *curr_drv = rpmh_resource_command_get_voter (self, drv_id);	
  rpmh_core_verify_ptr(curr_drv);
  curr_drv->priority = priority;
}
  ...
  if (curr_drv)
  {
    ...
    switch (vote->dirty)
    {
      ...
      case RPMH_RC_DIRTY_USE_ACTIVE:
	    //if its below MOL vote, let it proceed down to the MOL case
	    if(active_vote->dirty != RPMH_RC_DIRTY_MOL)
	    {
      ...
      case RPMH_RC_DIRTY_MOL:
	    //its either a valid vote, or its an RC_DIRTY_USE_ACTIVE case for the wake set
        rpmh_core_verify ((vote->valid) || (set == RPMH_SET_WAKE));
        rpmh_core_verify_ptr (self->pdc_votes->votes);

#ifdef RPMH_SLPI_CX_WAKEUP_WORKAROUND  
       //When this workaround is enabled, CX will be part of the PDC TCSs. This means for RSC votes
       //  we will use MOL instead of < MOL. However, we want to allow CX below MOL votes from the RSC
       //  if we are entering island mode (else CX would never be allowed to go to RET or OFF from island.
       //  In order to achieve this, check for CX, chip version (we only want this on Napali V1), and island status 
       uint32 cx_addr = cmd_db_query_addr(cx_name);
       rpmh_core_verify(cx_addr != 0);
       DalChipInfoVersionType version = DalChipInfo_ChipVersion ();

       if((version < CHIPINFO_VERSION(2, 0)) && (self->address == cx_addr) && (tcs_island_mode_status()))
       {
         *data = vote->data;
       }
       else
...
static void resource_command_dirty_update (rpmh_resource_command_t *self, drv_votes_t *curr_drv)
{
  if(curr_drv)
  {
                     ...
                     ((active_vote->data) != curr_drv->vote_at_rpmh.data)))
    {
      ...
      //vote is dirty. If this is a PDC resource, don't sent < MOL
      if(self->pdc_votes != NULL)
      {
        rpmh_resource_command_pdc_compare_and_update (self, active_vote, RPMH_SET_ACTIVE);
      }
    }
    else
    {
    ...
    if (sleep_vote->dirty != RPMH_RC_CLEAN_EXPLICIT_VOTE)
    {
          ...
          ((active_vote->valid) && (active_vote->data != sleep_vote->data))))
      {
        ...
        {
          //vote is dirty. If this is a PDC resource, don't sent < MOL
          rpmh_resource_command_pdc_compare_and_update (self, sleep_vote, RPMH_SET_SLEEP);
        }
      }	
      else
      {
        if((self->pdc_votes != NULL) && (active_vote->valid))
        {
          //the sleep vote might be clean, but if there is a valid active vote we still need to update PDC      
          //Do so using the active value, as the sleep value is clean. This will be the following cases:
          //  Sleep is invalid, but active is valid
          //  Sleep is valid, but active vote == sleep vote
          rpmh_resource_command_pdc_compare_and_update (self, active_vote, RPMH_SET_SLEEP);
        }
    ...
    if (wake_vote->dirty != RPMH_RC_CLEAN_EXPLICIT_VOTE)
    {
          ...
          ((sleep_vote->valid) && (wake_vote->data != sleep_vote->data)))) 
      {
        ...
        {
          //vote is dirty. If this is a PDC resource, don't sent < MOL
          rpmh_resource_command_pdc_compare_and_update (self, wake_vote, RPMH_SET_WAKE);
        }
