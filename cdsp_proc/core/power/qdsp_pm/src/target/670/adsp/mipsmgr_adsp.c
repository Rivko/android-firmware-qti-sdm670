...
$Header: //components/rel/core.qdsp6/2.1.c4/power/qdsp_pm/src/target/670/adsp/mipsmgr_adsp.c#1 $
*/
#include <limits.h>
#include <stdlib.h>
...
static uint32 mipsMgr_mppsv3(mipsMgrCtxType *self)
{
	...
	// If the below message appears in the Log,The aggregation will be incorrect. 
	// This error will occur if there is no audioCpp or voiceCpp table in the config file.
	if(audioCppFactors == NULL || voiceCppFactors == NULL)
	{		
		ADSPPM_LOG_PRINTF_2(ADSPPM_LOG_LEVEL_ERROR,
			"Unable to retrieve audioCppFactors(%u)/voiceCppFactors(%u) from Config File",audioCppFactors,voiceCppFactors); 
		return 0;
	}
