...
/*
@file: mipsmgr_slpi.c
@brief: QDSP_PM slpi specific mips manager functions.

$Header: //components/rel/core.qdsp6/2.1.c4/power/qdsp_pm/src/target/845/slpi/mipsmgr_slpi.c#1 $
*/
...
void mipsMgr_CalcMips(mipsMgrCtxType *self)
{
	if(NULL != self) //Assuming lock already obtained by caller
	{
        ...
        self->mppsAggregateData.classData[SLPI_INDEX].numDominantThreads = mipsMgr_CalcNumDominantThreads(
            self->mppsAggregateData.classData[SLPI_INDEX].mppsTotal,
            self->mppsAggregateData.classData[SLPI_INDEX].maxMppsVote,
            self->mppsAggregateData.classData[SLPI_INDEX].secondMaxMppsVote);
        ...
        pMipsToClockData->mipsData.mipsPerThread = MAX(
            ...
            self->mppsAggregateData.classData[SLPI_INDEX].adspFloorClock / ACM_GetHwThreadNumber());
	}
}
