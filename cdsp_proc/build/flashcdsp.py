...
#  flashadsp.py <Build_location>
#
#  $Header: //components/rel/dspbuild.adsp/5.0.c3/flashcdsp.py#1 $
#  
#  $Author: pwbldsvc $
#  $Change: 16823152 $
...
def flash_adsp_build(real_location, chipset):
	...
	rm_cdsp_FI = "adb wait-for-device shell rm -rf %s/image/cdsp.*"%firmware_partition
	# m_adsp_EF = "adb shell rm -rf /etc/firmware/cdsp*"
	rm_shared = "adb wait-for-device shell rm -rf %s/cdsp/.*"%dsp_partition
	push_cdsp_FI = "adb wait-for-device push " + real_location + "\\obj\\qdsp6v5_ReleaseG\\" + chipset + "\\signed\\LA\\system\\etc\\firmware\\. %s/image"%firmware_partition
	push_json  = "adb wait-for-device push " + real_location + "\\build\\ms\\servreg\\" + chipset + "Q\\. %s/image"%firmware_partition
	push_shared  = "adb wait-for-device push " + real_location + "\\build\\dynamic_modules\\" + chipset + "\\. %s/cdsp"%dsp_partition
	...
	runadbcommand(rm_cdsp_FI)
	runadbcommand(rm_shared)
	runadbcommand(push_cdsp_FI)
