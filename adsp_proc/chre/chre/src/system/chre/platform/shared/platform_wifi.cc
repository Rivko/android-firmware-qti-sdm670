...
//    return mWifiApi->getCapabilities();
    uint32_t wifiCapabilities = mWifiApi->getCapabilities();
    LOGD("Real WiFi capabilities 0x%X, reporting capabilities: 0x%X", wifiCapabilities,
    (CHRE_WIFI_CAPABILITIES_SCAN_MONITORING | CHRE_WIFI_CAPABILITIES_ON_DEMAND_SCAN));
