...
#ifdef T8996
smr_client_hndl gPlatformChreMdServiceSmrClientHandle;

/* CHRE Motion Detection sensor ID */
uint8_t chreNoMotionSensorId = SNS_SMGR_ID_OEM_SENSOR_09_V01;
uint8_t chreAnyMotionSensorId = SNS_SMGR_ID_OEM_SENSOR_10_V01;
uint8_t chreNoMotionInstanceId = 0;
uint8_t chreAnyMotionInstanceId = 0;
uint8_t chreMdDataType = 0; //00 - SNS_SMGR_DATA_TYPE_PRIMARY
#endif
 ...
 * Handles sensor data provided by the SMGR framework.
 *
 * @param chreMdIndMsg Decoded motion detection indication message
 */
void handleChreMdIndication(
    const sns_oem_3_report_ind_msg_v01& chreMdIndMsg) {

  sns_oem3_event_type_e_v01 requestType = chreMdIndMsg.request_type;
  uint8_t sensorId;

  if (requestType == SNS_OEM_3_ANY_MOTION_V01) {
    sensorId = chreAnyMotionSensorId;
  } else if (requestType == SNS_OEM_3_NO_MOTION_V01) {
    sensorId = chreNoMotionSensorId;
  } else {
    LOGW("Received unknown md request_type %d", requestType);
    return;
  }
  ...
  if (sensorType == SensorType::Unknown) {
    LOGW("Received sensor sample for unknown sensor %" PRIu8, sensorId);
  } else {
    SensorSampleType sampleType = getSensorSampleTypeFromSensorType(sensorType);
    if (sampleType == SensorSampleType::Occurrence) {
      size_t memorySize = sizeof(chreSensorDataHeader);
      memorySize += sizeof(chreSensorOccurrenceData::chreSensorOccurrenceSampleData);
      chreSensorOccurrenceData *event = static_cast<chreSensorOccurrenceData *>(memoryAlloc(memorySize));
      if (event != nullptr) {
        // Compensate for header timestamp's 32-bit rollovers
        uint64_t slpiTime = SystemTime::getMonotonicTime().toRawNanoseconds();
        uint64_t baseTime = getNanosecondsFromSmgrTicks(chreMdIndMsg.timestamp);
        while (slpiTime > baseTime + kTickRolloverOffset / 2) {
          baseTime += kTickRolloverOffset;
        }
        memset(&(event->header.reserved), 0, sizeof(event->header.reserved));
        event->header.baseTimestamp = baseTime;
        event->header.sensorHandle = getSensorHandleFromSensorType(sensorType);
        event->header.readingCount = 1;

        event->readings[0].timestampDelta = 0;

        // Schedule a deferred callback to update on-change sensor's last
        // event in the main thread.
        if (sensorTypeIsOnChange(sensorType)) {
          updateLastEvent(sensorType, (void *)event);
        }

        EventLoopManagerSingleton::get()->getEventLoop().postEvent(
            getSampleEventTypeForSensorType(sensorType), (void *)event,
            smgrSensorDataEventFree);
      }
    }
  }
 ...
 * @see smr_client_ind_cb
 */
void platformSensorChreMdIndicationCallback(
    smr_client_hndl handle, unsigned int messageId, void *decodedInd,
    unsigned int decodedIndLen, void *callbackData) {
  switch (messageId) {
    case SNS_OEM_3_REPORT_IND_V01: {
      CHRE_ASSERT(decodedIndLen >= sizeof(sns_oem_3_report_ind_msg_v01));
      auto *chreMdInd =
          static_cast<sns_oem_3_report_ind_msg_v01 *>(decodedInd);
      handleChreMdIndication(*chreMdInd);
      break;
    }
    default:
      LOGW("Received unhandled sensor service message: 0x%x", messageId);
...
#ifdef T8996
  if (sensorId == chreNoMotionSensorId || sensorId == chreAnyMotionSensorId) {
    LOGW("MointorRequest not supported for chre md");
    return;
  }
#endif
...
bool makeBufferingReq(uint8_t sensorId, uint8_t dataType, uint8_t calType,
  ...
  is_T8996 = true;
#endif
  if ((sensorId == chreNoMotionSensorId || sensorId == chreAnyMotionSensorId) && is_T8996) {
    if (request.getMode() == SensorMode::Off) {
      auto sensorRequest = MakeUniqueZeroFill<sns_oem_3_disable_req_msg_v01>();
      auto sensorResponse = MakeUnique<sns_oem_3_disable_resp_msg_v01>();

      if (sensorRequest.isNull() || sensorResponse.isNull()) {
        LOGE("Failed to allocate chre md disable msg");
      } else {
        sns_oem_3_disable_req_msg_v01 *chreMdRequest = sensorRequest.get();
        memset(chreMdRequest, 0, sizeof(*chreMdRequest));

        if (sensorId == chreNoMotionSensorId) {
          chreMdRequest->instance_id = chreNoMotionInstanceId;
        } else {
          chreMdRequest->instance_id = chreAnyMotionInstanceId;
        }

        smr_err status = getSmrHelper()->sendReqSync(
            gPlatformChreMdServiceSmrClientHandle, SNS_OEM_3_DISABLE_REQ_V01,
            &sensorRequest, &sensorResponse);

        if (status != SMR_NO_ERR) {
          LOGE("Error requesting chreMd data: %d", status);
        } else if (sensorResponse->resp.sns_result_t != SNS_RESULT_SUCCESS_V01) {
          LOGE("ChreMd data request failed with error: %d",
               sensorResponse->resp.sns_err_t);
        } else {
          if (sensorId == chreNoMotionSensorId) {
            chreNoMotionInstanceId = 0;
          } else {
            chreAnyMotionInstanceId = 0;
          }
          success = true;
        }
      }
      ...
      auto sensorRequest = MakeUniqueZeroFill<sns_oem_3_enable_req_msg_v01>();
      auto sensorResponse = MakeUnique<sns_oem_3_enable_resp_msg_v01>();

      if (sensorRequest.isNull() || sensorResponse.isNull()) {
        LOGE("Failed to allocate chre md enable msg");
      } else {
        sns_oem_3_enable_req_msg_v01 *chreMdRequest = sensorRequest.get();
        memset(chreMdRequest, 0, sizeof(*chreMdRequest));

        if (sensorId == chreNoMotionSensorId) {
          chreMdRequest->event_request = SNS_OEM_3_NO_MOTION_V01;
        } else {
          chreMdRequest->event_request = SNS_OEM_3_ANY_MOTION_V01;
        }

        smr_err status = getSmrHelper()->sendReqSync(
            gPlatformChreMdServiceSmrClientHandle, SNS_OEM_3_ENABLE_REQ_V01,
            &sensorRequest, &sensorResponse);
        ...
        if (status != SMR_NO_ERR) {
          ...
          if (sensorId == chreNoMotionSensorId) {
            chreNoMotionInstanceId = sensorResponse->instance_id;
          } else {
            chreAnyMotionInstanceId = sensorResponse->instance_id;
          }
          success = true;
        }
      }
...
#ifdef T8996
void addChreMdSensor(DynamicVector<Sensor> *sensors) {
  // Add oem3 algorithm for chre motion detection
  sns_smgr_sensor_datatype_info_s_v01 chreMdSensorInfo;
  memset(&chreMdSensorInfo, 0, sizeof(sns_smgr_sensor_datatype_info_s_v01));
  chreMdSensorInfo.SensorID = chreAnyMotionSensorId;
  chreMdSensorInfo.DataType = chreMdDataType;
  chreMdSensorInfo.SensorName_len = 10;
  strcpy(chreMdSensorInfo.SensorName, "any_motion");

  addSensor(chreMdSensorInfo, SNS_SMGR_CAL_SEL_FULL_CAL_V01, sensors);

  memset(&chreMdSensorInfo, 0, sizeof(sns_smgr_sensor_datatype_info_s_v01));
  chreMdSensorInfo.SensorID = chreNoMotionSensorId;
  chreMdSensorInfo.DataType = chreMdDataType;
  chreMdSensorInfo.SensorName_len = 9;
  strcpy(chreMdSensorInfo.SensorName, "no_motion");
...
#ifdef T8996
  // sns_oem_3_v01
  qmi_idl_service_object_type chreMdSvcObj =
      SNS_OEM_3_SVC_get_service_object_v01();
  if (chreMdSvcObj == nullptr) {
    FATAL_ERROR("Failed to obtain the SNS_OEM_3_service instance");
  }

  result = getSmrHelper()->waitForService(chreMdSvcObj);
  if (result != SMR_NO_ERR) {
    FATAL_ERROR("Failed while waiting for SNS_OEM_3_service service");
  }

  status = smr_client_init(
      chreMdSvcObj, SMR_CLIENT_INSTANCE_ANY,
      platformSensorChreMdIndicationCallback, nullptr /* ind_cb_data */,
      kSmrInitTimeoutMs, nullptr /* err_cb */, nullptr /* err_cb_data */,
      &gPlatformChreMdServiceSmrClientHandle, isSlpiUimgSupported());
  if (status != SMR_NO_ERR) {
    FATAL_ERROR("Failed to initialize SMGR client: %d", status);
  }
#endif
...
#ifdef T8996
  err = getSmrHelper()->releaseSync(
      gPlatformChreMdServiceSmrClientHandle);
  if (err != SMR_NO_ERR) {
    LOGE("Failed to release SMGR internal client: %d", err);
  }
  gPlatformChreMdServiceSmrClientHandle = nullptr;
#endif
