...
#include "qurt.h"
extern int HAP_timer_sleep(unsigned long long sleep_duration);
}  // extern "C"
...
/**
 * Cleans up all CHRE resources including all CHRE threads to gracefully exit.
 *
 */
void cleanupChreRsrc(void) {
  ...
  while (gThreadRunning) {
    HAP_timer_sleep(kThreadStatusPollingIntervalUsec);
  }
  gThreadHandle = 0;
...
/**
 * when daemon abruptly crashes, the reverse monitor fastrpc thread exits by
 * invoking the tls destructor callback. Please note the qurt_tls_create_key
 * must be called from the reverse monitor fastrpc thread to set the destructor
 * callback.
 */
void onHostProcessTerminated(void * /*data*/) {
  ...
  /*
   * Set to false, to avoid a deadlock between other threads waiting to acquire qurt_tls_lock, while exiting
   * trigerred by the destructor callback(func) vs this thread which acquired the qurt_tls_lock before
   * calling the destructor callback(this func).
   */
  gThreadRunning = false;
  cleanupChreRsrc();
}
