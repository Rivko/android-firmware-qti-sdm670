...
enum class SystemCallbackType : uint16_t {
  ...
  QcBleScanMonitorStateChange,
  QcBleHandleScanEvent,
...
class EventLoopManager : public NonCopyable {
 public:
  ...
  /**
   * @return Returns a reference to the qc ble request manager. 
   *         This allows interacting with the platform qc ble
   *         subsystem and manages the requests from various
   *         nanoapps.
   */
  QcBleRequestManager& getQcBleRequestManager() {
    return mQcBleRequestManager;
  }
 ...
 private:
  ...
  //! The QcBLeRequestManager that handles requests for nanoapps. This manages
  //! the state of the qc ble subsystem that the runtime subscribes to.
  QcBleRequestManager mQcBleRequestManager;
