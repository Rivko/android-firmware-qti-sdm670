...
===============================================================================*/
/*=============================================================================
Copyright (c) 2014-18 Qualcomm Technologies Incorporated.
All rights reserved.
Qualcomm Confidential and Proprietary
=============================================================================*/
/*=============================================================================
Edit History
$Header: //components/rel/core.qdsp6/2.1/debugtools/sys_m/src/sys_m_listen_root.c#5 $
$DateTime: 2020/09/15 01:57:49 $
$Change: 26165507 $
$Author: pwbldsvc $
===============================================================================*/
...
#include "timer.h"
#define SYS_M_QURT_SEM_DWON_TIMEOUT_MSEC 100 /* 100 milliseconds*/
...
struct device_s
{
   ...
   timer_type qurt_sem_down_timeout;
};
...
static void SYS_M_LISTEN_RELEASE(qurt_qdi_obj_t *obj)
{
   ...
   if (NULL != device_p)
   {
      ...
      timer_undef(&device_p->qurt_sem_down_timeout);
...
void sys_m_listner_sem_down_timeout_cb(void * data)
{
   struct device_s* device_p = (struct device_s*)data;
   if(device_p){
     qurt_sem_up(&device_p->sem_ack);
   }
   TMS_MSG_HIGH_1("sys_m_listner_sem_down_timeout_cb : %d", data);
}
      ...
      */
      case QDI_OPEN:
      {
         ...
         if (pn.asid != device_p->pid)
         {
            ...
            timer_def_osal( &device_p->qurt_sem_down_timeout,
                           &timer_non_defer_group,/*non-deferrable timer*/ 
                           TIMER_FUNC1_CB_TYPE,
                           sys_m_listner_sem_down_timeout_cb,
                          (time_osal_notify_data)device_p);
         }
...
static void sys_m_listen_write(SYS_M_NAME event)
{
   ...
   for (pid = SYS_M_DEVICE_PID_0; pid < SYS_M_DEVICE_PID_MAX; pid++)
   {
          ...
          sizeof(device_p->event) > strlcpy(device_p->event, event, sizeof(device_p->event)))
      {
		 ...
		 timer_set_64(&device_p->qurt_sem_down_timeout,
		              SYS_M_QURT_SEM_DWON_TIMEOUT_MSEC,
					  0,
					  T_MSEC);
      }
   }
   ...
   for (pid = SYS_M_DEVICE_PID_0; pid < SYS_M_DEVICE_PID_MAX; pid++)
   {
       ...
       && SYS_M_DEVICE_PID_NA != device_p->pid)
      {
         ...
         timer_clr_64(&device_p->qurt_sem_down_timeout, T_NONE);
