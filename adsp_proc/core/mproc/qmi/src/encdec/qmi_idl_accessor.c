    ...
    *c_struct_len = 0;
    ...
    case 6:
      ...
      while (inheritance_obj) 
      {
        ...
        for (msg_type = QMI_IDL_REQUEST; msg_type < QMI_IDL_NUM_MSG_TYPES; msg_type++)
        {
          int max_msgs, i;

          max_msgs = inheritance_obj->n_msgs[msg_type];
          for (i = 0; i < max_msgs; i++)
          {
            const qmi_idl_service_message_table_entry *p_srvmsg;
            const qmi_idl_message_table_entry *p_msg;
            uint16_t table_index, type_index;
              
            p_srvmsg = &inheritance_obj->msgid_to_msg[msg_type][i];
            ...
            /* Use the table and type index to get the message table entry */
            p_msg = &(inheritance_obj->p_type_table->p_referenced_tables[table_index]->
                      p_messages[type_index]);

            if (*c_struct_len < p_msg->c_struct_sz)
            {
              *c_struct_len = p_msg->c_struct_sz;
            }
          }
        }
        inheritance_obj = qmi_idl_get_inherited_service_object(inheritance_obj);
