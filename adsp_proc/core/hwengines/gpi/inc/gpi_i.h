...
--------   ---     ------------------------------------------------------------ 
09/13/17   pc      Added support to save irq type for each qup instance
03/08/17   ah      Adding new definitions.
...
typedef struct _isr_debug_info
{
   uint32  gpii_id;
   uint32  clock_off;
   uint32  irq_type_zero;
   uint32  int_not_recognized;
   uint32  clock_off_end_isr;
   U_LONG irq_type;
}isr_debug_info;

typedef struct _isr_debug_regs
{
   U_LONG irq_type;
   U_LONG GSI_CH_IRQ;
   U_LONG EV_CH_IRQ;
...
typedef struct gpi_ctxt
{
   ...
   isr_debug_info isr_debug[5];
   uint32 isr_error;
   isr_debug_regs isr_reg;
}gpi_ctxt;
...
#define REVERSE_RING_PTR(base, ptr, size)                                                 \
                (((ptr - base) != 0)? (ptr - sizeof(gpi_ring_elem)):                      \
                                   (ptr + ((size - 1) * sizeof(gpi_ring_elem))))
