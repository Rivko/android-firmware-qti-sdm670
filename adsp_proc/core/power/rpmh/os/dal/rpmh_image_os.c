 ...
 * <!-- rpmh_enable_isr -->
 *
 * @brief Enable the corresponding ISR   
 *
 * @param irq : the interrupt # to enable
 */
void rpmh_enable_isr (uint32 irq)
{
  
  if (rpmh_os.isr_handle)
  {
    rpmh_core_verify (DalInterruptController_InterruptEnable (rpmh_os.isr_handle, irq) == DAL_SUCCESS);
  }
}
 ...
 * <!-- rpmh_disable_isr -->
 *
 * @brief Disable the corresponding ISR   
 *
 * @param irq : the interrupt # to disable
 */
void rpmh_disable_isr (uint32 irq)
{
  
  if (rpmh_os.isr_handle)
  {
    rpmh_core_verify (DalInterruptController_InterruptDisable (rpmh_os.isr_handle, irq) == DAL_SUCCESS);
  }
}

/**
 * <!-- rpmh_in_flight_parity -->
 *
 * @brief Update sleep if cmdq parity has changed
 *
 * @param cmdq status before an update
 * @param cmdq status after an update
 */ 
inline void rpmh_in_flight_parity(boolean pre, boolean post)
{
  if(pre != post)
  {
      RPMH_LOG_0 ("\t In flight parity");
