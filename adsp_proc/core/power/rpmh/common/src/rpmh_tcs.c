...
void tcs_update_mode(uint32 drv_index, uint32 new_mode, rpmh_cmdq_t *cmdq)
{
  ...
  for(int i = 0; i < drv->num_tcs; i++)
  {
    ...
    if (i < drv->modes[new_mode]->num_amcs)
    {
      ...
      rpmh_core_verify (RscHalEnableAMCFinishedIRQ (drv->hw_drv, tcs->id) == HAL_STATUS_SUCCESS);
    }
    else
    {
      ...
      rpmh_core_verify (RscHalDisableAMCFinishedIRQ (drv->hw_drv, tcs->id) == HAL_STATUS_SUCCESS);
...
void log_cmd_set (rpmh_cmd_t *cmds)
{
  int i;	
  if(cmds->amc)
  {
    RPMH_LOG_1 ("\tREQ_ID: %d", cmds->req_id);
    if (cmds->client)
    {
      RPMH_LOG_1 ("\tCLIENT_NAME: %s", cmds->client->name);
    } 
  }
  RPMH_LOG_1 ("\tNUM_RESOURCE_COMMANDS: %d", cmds->num_rcs);
  for (i = 0; i < cmds->num_rcs; i++)
  {
    RPMH_LOG_2 ("\t\t(CMD #: %d) (CMD ADDRESS: 0x%08x)", i, cmds->details[i].address);
  }
}
...
boolean is_tcs_stuck (uint32 drv_index)
{
  ...
  while(bmsk != 0)
  {
    ...
    rpmh_core_verify (RscHalIsTCSIdle (drv->hw_drv, tcs_id, &is_idle) == HAL_STATUS_SUCCESS);
    uint64 current_time = rpmh_get_timestamp ();   
    if ((!is_idle) && (current_time > (rpmh_tcs[drv_index][tcs_index].time_last_sent + TCS_TIMEOUT_THRESHOLD)))
    {
      RPMH_LOG_2 ("!!!!TCS STUCK!!!!", drv->drv_id, tcs_id);      
      log_cmd_set(rpmh_tcs[drv_index][tcs_index].cmds);      
      return TRUE;
    }
 ...
 *
 * @param drv_index : The DRV (index into the DRV table) on which to check for an available tcs slot 
 ...
 */
int tcs_slots_available (uint32 drv_index, rpmh_set_enum set, uint32 num_slots)
{
  uint32 start = 0;
  uint32 end = 0;
  const drv_config_t *drv = &DRV_CONFIG_DATA->drvs[drv_index]; 
  ...
  {
    case RPMH_SET_SLEEP:
      start = drv->modes[tcs_drv_cfg[drv_index].mode]->sleep_start;
      end = drv->modes[tcs_drv_cfg[drv_index].mode]->wake_start;
      break;

    case RPMH_SET_WAKE:
      start = drv->modes[tcs_drv_cfg[drv_index].mode]->wake_start;
      end = drv->num_tcs;
      break;
    default:
      rpmh_core_verify (FALSE);
 ...
 * @param drv_index : The DRV (index into the DRV table) on which to add a cmd_set 
 *
 * @return TRUE if we were able to add the command, FALSE if not (no room in TCSs)
 */
boolean tcs_prepare_sleep_add_cmd_set (rpmh_command_set_internal_t *cmd_set, uint32 drv_index)
{
  const drv_config_t *drv = &DRV_CONFIG_DATA->drvs[drv_index]; 
  tcs_t *tcs = NULL;

  //find proper tcs pointer
  int tcs_slot = tcs_slots_available (drv_index, cmd_set->set, cmd_set->num_commands);
  ...
  if (!tcs->cmds)
  {
    tcs->cmds = (rpmh_cmd_t*)rpmh_malloc (sizeof(rpmh_cmd_t));
...
static uint32 get_island_tcs_index(RSCSW_DRV_MAPPING drv_id, rpmh_set_enum set)
{
  ...
  for(int i = drv->tcs_offset; i < (drv->modes[tcs_drv_cfg[drv_index].mode]->num_amcs + drv->tcs_offset); i++)
  {
    use_this_tcs = TRUE;
    ...
    {
      if((urpmh_cmd_sets[j].type != ISLAND_EMPTY) && (urpmh_cmd_sets[j].tcs_num == i) && (urpmh_cmd_sets[j].cmd_set.set == set))
      {
...
//returns whether or not the TCS layer thinks we are in island mode
boolean tcs_island_mode_status(void)
{
  return island_mode;
}
...
static void tcs_send_internal (rpmh_cmd_t *cmd, RSCSW_DRV_MAPPING drv_id, uint32 drv_index, uint32 tcs_index)
{
  ...
  for (i = 0; i < cmd->num_rcs; i++)
  {
    ...
    {    
      ...
      RPMH_LOG_QDSS_6( "(SET: %d)(AMC: %d)(ADDRESS: 0x%x)(DATA: 0x%x)(COMPLETION?: %d)(req_id: %u)", cmd->set, cmd->amc, 
                         cmd->details[i].address, 
                         data, cmd->details[i].type, 
                         cmd->req_id);  
    }      
  }
...
{
  const drv_config_t *drv = &DRV_CONFIG_DATA->drvs[drv_index]; 
  rpmh_core_verify (drv->modes[tcs_drv_cfg[drv_index].mode]->num_amcs > 0);
  for (int i = 0; i < drv->modes[tcs_drv_cfg[drv_index].mode]->num_amcs; i++)
  {
    if (rpmh_tcs[drv_index][i].state == TCS_AMC_IDLE)
    {
      rpmh_tcs[drv_index][i].state = TCS_AMC_WAIT_FOR_DONE;    
      tcs_drv_cfg[drv_index].busy_bmsk |= (1 << (rpmh_tcs[drv_index][i].id - drv->tcs_offset));
      tcs_send_internal (cmd, drv_id, drv_index, i);
      return TRUE;
    }
  }    
 ...
 *
 * @param drv_index : The DRV (index into the DRV table) on which to clean sleep/wake TCSs 
 *
 * @return TRUE if the TCS was able to be sent, FALSE if not
 */
void tcs_clean_sleep_wake (uint32 drv_index)
{
  const drv_config_t *drv = &DRV_CONFIG_DATA->drvs[drv_index]; 

  for (int i = drv->modes[tcs_drv_cfg[drv_index].mode]->sleep_start; i < drv->num_tcs; i++)
  { 
    tcs_t *tcs = &(rpmh_tcs[drv_index][i]);
 ...
 * @return TRUE if the TCS was able to be sent, FALSE if not
 */
void tcs_prepare_sleep_commit (RSCSW_DRV_MAPPING drv_id, uint32 drv_index)
{
  int i;	
  const drv_config_t *drv = &DRV_CONFIG_DATA->drvs[drv_index]; 

  for (i = drv->modes[tcs_drv_cfg[drv_index].mode]->sleep_start; i < drv->num_tcs; i++)
  { 
    tcs_t *tcs = &(rpmh_tcs[drv_index][i]);
	
	//disable commands to make sure we clear out any stale commands/TCSs
    rpmh_core_verify (RscHalToggleTCSCmd (drv->hw_drv, tcs->id, 0) == HAL_STATUS_SUCCESS);
    if ((tcs->cmds != NULL) && (tcs->cmds->num_rcs > 0))
    {
      tcs_send_internal (tcs->cmds, drv_id, drv_index, i);
    }
  }
}
