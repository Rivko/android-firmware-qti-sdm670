...
int32 rscHalGetNumTcs (size_t baseAddress, uint8 drvId, uint32 *numTcs)
{
  int32 status = HAL_STATUS_SUCCESS;
...
int32 rscHalGetNumCmdsPerTcs (size_t baseAddress, uint8 drvId, uint32 *numCmdsPerTcs) 
{
  int32 status = HAL_STATUS_SUCCESS;
...
int32 RscHalTriggerTCS (RSCSW_DRV_MAPPING drvMapId, uint8 tcsId)
{
  ...
  if ((status = RscHalIsTCSIdle(drvMapId,tcsId, &IsTCSIdle)) == HAL_STATUS_SUCCESS)
  {
    if(IsTCSIdle ==  FALSE)
    {
      RPMH_LOG_1 ("\tTCS Id %d is not Idle in RscHalTriggerTCS\n", tcsId);
...
int32 RscHalConvertTCStoAMC (RSCSW_DRV_MAPPING drvMapId, uint8 tcsId)
{
  ...
  if ((status = RscHalIsTCSIdle(drvMapId,tcsId, &IsTCSIdle)) == HAL_STATUS_SUCCESS)
  {
    if(IsTCSIdle ==  FALSE)
    {
      RPMH_LOG_1 ("\tTCS Id %d is not Idle in RscHalConvertTCStoAMC\n", tcsId);
      status = HAL_TCS_NOT_IDLE;
      return status;
    }
  }
  ...
  if ((status = isContextInitialized(ctxt, drvMapId)) == HAL_STATUS_SUCCESS)
  {
    ...
    {
      case DRV_ID_0:
#ifdef RPMH_AMC_MODE_EN_ORDERING_WA	
        //make sure TRIGGER bit is set to 0 when changing mode
        RSC_HAL_HWIO_TCS_OUT_WRITE_MSK (PLATFORM,
                                        _RSCC_TCSm_DRV0_CONTROL,
                                        baseAddress,
                                        tcsId,
                                        RSC_HAL_BIT_MSK(PLATFORM,_RSCC_TCSm_DRV0_CONTROL, _AMC_MODE_TRIGGER_BMSK), 
                                        AMC_CLEAR_BIT << RSC_HAL_BIT_SHFT(PLATFORM,_RSCC_TCSm_DRV0_CONTROL, _AMC_MODE_TRIGGER_SHFT));
#endif //RPMH_AMC_MODE_EN_ORDERING_WA	
...
int32 RscHalConvertAMCtoTCS (RSCSW_DRV_MAPPING drvMapId, uint8 tcsId)
{
  ...
  if ((status = RscHalIsTCSIdle(drvMapId,tcsId, &IsTCSIdle)) == HAL_STATUS_SUCCESS)
  {
    if(IsTCSIdle ==  FALSE)
    {
      RPMH_LOG_1 ("\tTCS Id %d is not Idle in RscHalConvertAMCtoTCS\n", tcsId);
      status = HAL_TCS_NOT_IDLE;
      return status;
    }
  }
...
int32 RscHalToggleTCSCmd (RSCSW_DRV_MAPPING drvMapId, uint8 tcsId, uint32 cmdIdMask)
{
  ...
  if ((status = RscHalIsTCSIdle(drvMapId,tcsId, &IsTCSIdle)) == HAL_STATUS_SUCCESS)
  {
    if(IsTCSIdle ==  FALSE)
    {
      RPMH_LOG_1 ("\tTCS Id %d is not Idle in RscHalToggleTCSCmd\n", tcsId);
      status = HAL_TCS_NOT_IDLE;
      return status;
    }
  }
...
/**
* Function Name:    RscHalDisableAMCFinishedIRQ
* Description:      RscHalDisableAMCFinishedIRQ is used to disable completion/finish interrupt on a TCS/AMC.
* Input:            RSCSW_DRV_MAPPING DrvId 
*                   TCSId 
* Return:           HAL_STATUS_SUCCESS if success ow error code 
**/

int32 RscHalDisableAMCFinishedIRQ (RSCSW_DRV_MAPPING drvMapId, uint32 tcsId)
{
  ...
  if ((ctxt->DrvConfigArray[drvMapId].NumTcs ==  0) || 
       (IS_TCS_VALID (tcsId,ctxt->DrvConfigArray[drvMapId].NumTcs) < HAL_STATUS_SUCCESS))
  {
    //ULOG_RT_PRINTF_1 (rpm_log, "\tInvalid TCS Id, %d, in RscHalEnableAMCFinishedIRQ",tcsId);
    status = HAL_INVALID_TCS_ID; 
    return status;
  }
  ...
  if ((status = isContextInitialized(ctxt, drvMapId)) == HAL_STATUS_SUCCESS)
  {
    ...
    {
      case DRV_ID_0:
        readVal = RSC_HAL_HWIO_IN_READ (PLATFORM,
                                        _RSCC_TCS_AMC_MODE_IRQ_ENABLE_DRV0,
                                        baseAddress);
        readVal &= (~(AMC_ENABLE_FINISH_IRQ << tcsId));
...
#endif
      default:
        //ULOG_RT_PRINTF_1(rpm_log, "\tInvalid DRVId, %d, passed to RscHalEnableAMCFinishedIRQ",ctxt->DrvConfigArray[drvMapId].DrvId);
        status = HAL_INVALID_DRV_ID;
        break;
     }                  
...
int32 rscHalSetupDrv0Tcs (size_t baseAddress, uint8 tcsId, RscHalMessageBody *pCommandInfo)
{
...
int32 rscHalSetupDrv1Tcs (size_t baseAddress, uint8 tcsId, RscHalMessageBody *pCommandInfo)
{
...
#endif
#ifdef DRV_2_VALID	
int32 rscHalSetupDrv2Tcs (size_t baseAddress, uint8 tcsId, RscHalMessageBody *pCommandInfo)
{
...
int32 RscHalSetupTCS (RscHalMessage *pMsg)
{
  ...
  if ((status = RscHalIsTCSIdle(drvMapId,pMsg->TcsId, &IsTCSIdle)) == HAL_STATUS_SUCCESS)
  {
    if(IsTCSIdle ==  FALSE)
    {
      ...
      RPMH_LOG_1 ("\tTCS Id %d is not Idle in RscHalSetupTCS\n", pMsg->TcsId);
      status = HAL_TCS_NOT_IDLE;
      return status;
    }
  }
