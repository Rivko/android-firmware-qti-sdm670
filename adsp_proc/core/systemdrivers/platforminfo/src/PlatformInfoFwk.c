...
/*
  ...
  $Header: //components/rel/core.qdsp6/2.1/systemdrivers/platforminfo/src/PlatformInfoFwk.c#2 $
  $DateTime: 2017/10/17 03:24:30 $
  $Author: pwbldsvc $
...
static uint32
PlatformInfo_DeviceDetach(uint32 ClientHandle, DalDeviceHandle* h)
{
...
static DALResult
PlatformInfo_DeviceInit(uint32 ClientHandle, DalDeviceHandle *h)
{
   return DAL_SUCCESS;
}

static DALResult
PlatformInfo_DeviceDeInit(uint32 ClientHandle, DalDeviceHandle *h)
{
   return DAL_SUCCESS;
}

static DALResult
PlatformInfo_DevicePowerEvent(uint32 ClientHandle, DalDeviceHandle *h, DalPowerCmd PowerCmd,
...
static DALResult
PlatformInfo_DeviceOpen(uint32 ClientHandle, DalDeviceHandle* h, uint32 mode)
{
   return DAL_SUCCESS;
}

static DALResult
PlatformInfo_DeviceClose(uint32 ClientHandle, DalDeviceHandle* h)
{
   return DAL_SUCCESS;
}

static DALResult
PlatformInfo_DeviceInfo(uint32 ClientHandle, DalDeviceHandle* h, DalDeviceInfo* pInfo, uint32 infoSize)
{
  DalDeviceInfo Info;
...
static DALResult
PlatformInfo_DeviceSysRequest(uint32 ClientHandle, DalDeviceHandle* h,DalSysReq ReqIdx,
...
static DALResult DALPlatformInfo_GetPlatform (uint32 ClientHandle, DalDeviceHandle * h, DalPlatformInfoPlatformType * pType)
{
  DalPlatformInfoPlatformType Type;
  DALResult                   eResult;

  eResult = PlatformInfo_GetPlatform(HANDLE2CTXT(h), &Type);
  if (!pType || DalSys_CopyToUser(ClientHandle, pType, &Type, sizeof(Type)))
  {
    return DAL_ERROR_INVALID_ADDRESS;
  }
...
static DALResult DALPlatformInfo_GetPlatformInfo (uint32 ClientHandle, DalDeviceHandle * h, DalPlatformInfoPlatformInfoType * pInfo)
{
  DalPlatformInfoPlatformInfoType Info;
  DALResult                       eResult;

  secure_memset(&Info, 0, sizeof(Info));

  eResult = PlatformInfo_GetPlatformInfo(HANDLE2CTXT(h), &Info);
  if (!pInfo || DalSys_CopyToUser(ClientHandle, pInfo, &Info, sizeof(Info)))
  {
    return DAL_ERROR_INVALID_ADDRESS;
  }
...
static DALResult DALPlatformInfo_GetKeyValue(uint32 ClientHandle, DalDeviceHandle * h, DalPlatformInfoKeyType eKey, uint32 *pnValue)
{
  uint32    nValue;
  DALResult eResult;

  eResult = PlatformInfo_GetKeyValue(HANDLE2CTXT(h), eKey, &nValue);
  if (!pnValue || DalSys_CopyToUser(ClientHandle, pnValue, &nValue, sizeof(nValue)))
  {
    return DAL_ERROR_INVALID_ADDRESS;
  }
...
static DALResult DALPlatformInfo_CDTConfigPreDDR (uint32 ClientHandle, DalDeviceHandle * h, void *pPlatformInfo)
{
...
static DALResult DALPlatformInfo_CDTConfigPostDDR (uint32 ClientHandle, DalDeviceHandle * h, void *pPlatformInfo)
{
  if (ClientHandle != DALDEVICE_LOCAL_CLIENTID)
  {
    return DAL_ERROR_NOT_SUPPORTED;
...
PlatformInfo_InitInterface(PlatformInfoDALClientCtxt* pclientCtxt)
{
   ...
   pclientCtxt->DalPlatformInfoHandle.dwVtblen = sizeof(vtbl)/sizeof( void (*)(void));
