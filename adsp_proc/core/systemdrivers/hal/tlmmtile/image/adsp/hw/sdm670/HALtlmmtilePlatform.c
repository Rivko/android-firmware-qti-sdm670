...
$Header: //components/rel/core.qdsp6/2.1/systemdrivers/hal/tlmmtile/image/adsp/hw/sdm670/HALtlmmtilePlatform.c#3 $
...
{
  uint32 nTileAddr, nTileIter, ncount,status;
  ...
  for ( nTileIter = 0; nTileIter < HALTLMMTILENUMTILES; nTileIter++ )
  {
    ...
    status = in_dword(nTileAddr + pDesc->GpioIdStatus.nOffset) & pDesc->GpioIdStatus.nGPIOIdStatusBmsk;
    pStatusregsgs[0] = status;
    if(nGpio == 124)
    {
        for(ncount = 1; ncount < 10; ncount++)
        { 
          pStatusregsgs[ncount] = (in_dword(nTileAddr + pDesc->GpioIdStatus.nOffset) & pDesc->GpioIdStatus.nGPIOIdStatusBmsk);
          if(pStatusregsgs[ncount] != status)
          {
            DALSYS_LogEvent(DALDEVICEID_TLMM, 
            DALSYS_LOGEVENT_FATAL_ERROR, 
            "Statusregister incorect for gpio 124 %d ",status);
           }
        }
