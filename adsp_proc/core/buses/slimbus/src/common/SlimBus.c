...
$Header: //components/rel/core.qdsp6/2.1/buses/slimbus/src/common/SlimBus.c#4 $

when       who     what, where, why
--------   ---     --------------------------------------------------------
09/22/20   PR      Fix Framer wakeup and HW enumeration race condition
09/07/18   DPK     Fix for the TLB miss issue when port closing and under/over flow.
                   Retuned previous issue.
07/20/18   DPK     Fix for the TLB miss issue when port closing and under/over flow.
...
SBResult SlimBusCommonDpc(SlimBusDevCtxt *pDevCtxt, struct SlimBusIsrCtxt *pIsrCtxt, SlimBusLinkNodeType *pSignalList)
{
    ...
    for (i = 0; i < pDevCtxt->numPorts; i++)
    {
      ...
      if ( uPgdIntStatus & uIntMask )
      {
        ...
        else if ( eEvents & (SLIMBUS_EVENT_O_FIFO_OVERRUN|SLIMBUS_EVENT_O_FIFO_UNDERRUN) )
        {
          ...
          /* Signal any client that is waiting for port descriptors to flush */
          if ((pPort->uSignalDescEmptyCnt > 0) && (SB_HANDLE_DEALLOCATED != pPort->eState))
          {
            uint32 numDesc = 0;
         ...
         !pDevCtxt->pSched->bMasterWorkerBusy ) )
  {
    ...
    if(SB_PAUSE_RESUMING_FROM_PAUSE == pDevCtxt->pSched->ePause)
	{
