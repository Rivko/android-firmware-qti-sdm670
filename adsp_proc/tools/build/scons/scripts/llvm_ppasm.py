...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.qc/1.1/scripts/llvm_ppasm.py#2 $
#  $DateTime: 2017/04/17 17:54:04 $
#  $Author: pwbldsvc $
#  $Change: 13042105 $
...
def main():
   ...
   if options.output_filename is not None:
      if options.output_filename.find('.scl') > 0:
         isSCLFile = True
      elif options.output_filename.find('.c') > 0:
         isCFile = True
   ...
   for line in data:
     ...
     if in_macro or (not in_includes and re_bad_line.match(line)) is None:
        ...
        elif options.gnu_comment and not isXMLFile and not isCFile:
