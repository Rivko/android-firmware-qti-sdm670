...
#----------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.qc/1.1/scripts/version_builder.py#9 $
#  $DateTime: 2017/03/06 17:39:26 $
#  $Author: pwbldsvc $
#  $Change: 12598194 $
#
...
def qc_version_builder(target, source, env):
    ...
    if xml_image_tree is not None:
        xml_image_tree_build_id = xml_image_tree.find('build_id')
    else:
        env.PrintError("")
        env.PrintError("-------------------------------------------------------\
        	------------------------")
    ...
    if (xml_image_tree_build_id is not None) and (xml_image_tree_build_id.text is not None):
        build_id = xml_image_tree_build_id.text.strip()
