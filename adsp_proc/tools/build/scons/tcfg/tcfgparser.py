...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/scons.qc/1.1/tcfg/tcfgparser.py#8 $
#  $DateTime: 2015/11/19 03:27:05 $
#  $Change: 9446675 $
...
class TargetConfigurationDb(Printable):
   ...
   #--------------------------------------------------------------------
   def ParseBuildFile (self, build_file_name, pl = None, search_paths = None):
      ...
      try:
         ...
         self.BuildFileName = build_file_name
         self.CurBuildsFile = build_file_name
         ...
         while token != build_file.eof:
            ...
            if token == 'define':
               ...
               newbe = BuildElement(build_file, self.GlobalDefines, self.CurBuildsFile)
               try:
               ...
               pl_to_include = ''
               conditional_token = ''
               # Check the next token to see if this is a conditional include
               # Important: reading this next token must be done using a call
               #    to get_token() that does NOT combine newline tokens. To
               #    combine tokens, the token past the last newline (next non-
               #    newline) must be found.  This token is then pushed back 
               #    onto the stack, but the stack is not specific to the file 
               #    stream.  If a new file is included when this token is on 
               #    the stack, it will be returned first before the tokens 
               #    from that file.
               conditional_token = build_file.get_token(combine_newline=False)
               if conditional_token.lower() == 'if':
                  pl_to_include = build_file.get_token()
                  pl_to_include = pl_to_include.lower()
               else:
                  build_file.push_token(conditional_token)

               if pl_to_include == '' or pl_to_include == pl:
                  if search_paths:
   ...
   #--------------------------------------------------------------------
   def GenConfigData (self, build_id, pl = None):
      ...
      try:
         ...
         if pl:
            pl = pl.lower()
         bid_config_data = BidConfigData(pl)
         ...
         return_data.tcfg_flags = self.CollectTcfgFlags(bid_config_data)
   ...
   def ExtractBidConfigData (self, bid_config_data):
      ...
      for tag in self.TagMap.Elements:
         print "tcfgparser: Visiting", tag.name + ":", tag.bid_val.upper(), "from:", os.path.basename(self.BuildFileName)
         BEType = self.BuildElementTypes[tag.name]
         BE     = BEType.Elements[tag.bid_val]
      ...
      for attr in curbe.attributes:
         if attr.image in bid_config_data.images_allowed:
            if attr.type == 'define':
               ...
               bid_config_data.mod_tcfg.add(attr)
            elif (attr.type == 'use' and
                  attr.value not in bid_config_data.unuses and
                  attr.value in self.AllBuildElements):
               BE = self.AllBuildElements[attr.value]
               print "tcfgparser: Visiting SoftComponent:", attr.value
               self.RecurseBEsForConfigData(BE, bid_config_data)
...
#      #(still need targ<bid>.h because of mbn_builder dependency)
#      if type == 'targ' and not bid_config_data.hattrs.has_data():
#         # Don't generate targ<bid>.h if there is no data for it.
#         return

      # Allow target.builds to be elsewhere, and always generate
      # header files in the current directory (which will be build\ms)
      header_file_name = type.lower() + build_id.lower() + ".h"
      header_file_guard = type.upper() + build_id.upper() + "_H"
      header_file_guard = header_file_guard.replace('.', '_')
      try:
...
Copyright (c) %s by Qualcomm Technologies, Inc.  All Rights Reserved.
Qualcomm Technologies Proprietary and Confidential.        
=========================================================================== */

""" % (type.upper() + build_id.upper(), date.today().year))

      if type == 'cust':    #(still need targ<bid>.h because of mbn_builder dependency) and bid_config_data.hattrs.has_data():
         header_file.write("#ifndef TARG" + build_id.upper().replace('.', '_') + "_H\n")
   ...
   # CollectTcfgFlags
   # 
   # Local method to create a list of TCFG_* flags for the build id to
   # return to the caller.
   # 
   # Parameters:
   #   self:            self reference
   ...
   # 
   # Returns:
   #   List of strings containing the names of the TCFG_* flags.
   #-------------------------------------------------------------------
   def CollectTcfgFlags (self, bid_config_data):
      tcfg_flags = dict()
      for attr in bid_config_data.mod_tcfg:
         tcfg_flags[attr.name] = attr.value
      return tcfg_flags
...
class TagElement(Printable):
   ...
   def __init__ (self, token, build_file):
      ...
      if len(token) > 1:
         index = token[1]           # Digit indicating index
      else:
         index = '-'
      ...
      if self.start >= 0:
         ...
         self.end = None           # Go to end of string
      self.name  = ""
...
class BuildElement(Printable):
   ...
   #   No return value.
   #------------------------------------------------------------------
   def __init__ (self, build_file, GlobalDefines, build_file_name):
      ...
      while token.lower() not in ('end_block', '[', build_file.eof):
         ...
         # not sure if they're used.  If so, they'll have to be added here.
            ...
            attr = AttributeUse(build_file, build_file_name)
            self.attributes.append(attr)
         ...
         elif token.lower() == 'tcfg':
            attr = AttributeTcfg(build_file)
            self.attributes.append(attr)
...
class AttributeUse(AttributeParent):
   ...
   #   No return value.
   #------------------------------------------------------------------
   def __init__ (self, build_file, build_file_name):
      AttributeParent.__init__(self)
      self.BuildFileName = build_file_name
      self.type = 'use'
...
# Class AttributeTcfg
# 
# A sub-class of AttributeParent that represents the 'tcfg' statements
# from the .builds file.
#-----------------------------------------------------------------------
class AttributeTcfg(AttributeParent):
   ...
   # Description:
   #   Constructor for class.  Initializes member variables and parses
   #   the 'tcfg' attribute definition in the .builds file.
   # 
   # Parameters:
   #   self:          Self reference
   #   build_file:    A token stream for the .builds file.
   ...
   def __init__ (self, build_file):
      ...
      self.type = 'tcfg'
      self.name = build_file.get_token(preserve_case = True)
      temp = build_file.get_token()
      if temp != '=':
          raise Exception("Invalid token encountered when parsing tcfg attribute: '" + temp + "'.  Expected '='.")
      self.value = build_file.get_multi_token(['\n'],
                                              preserve_case = True,
                                              see_whitespace = False)
      AttributeParent.ParseIfImage(self, build_file)
...
class BidConfigData(Printable):
   ...
   class BidAttrLists:
      ...
      #
      # Description:
      #   Return True if the object has any data.
      # 
      # Parameters:
      #   self:          Self reference
      # 
      # Return Value:
      #   True if the object has any data
      #------------------------------------------------------------------
      def has_data (self):
         return self.defines or self.undefs or self.undef_firsts or self.includes
...
class Tokenizer(shlex.shlex):
   ...
   #   combine_newline Boolean indicating whether or not newlines that are found
   #                   should be grouped together.  If True, the next token will
   #                   continue to be read once a newline is found until a non-
   #                   newline is found.  
   # 
   # Return Value:
   #   String containing the next token
   #------------------------------------------------------------------
   def get_token (self, preserve_case=False, expand_dict=None,
                  see_whitespace = False, combine_newline = True):

      token = self.get_token_skip_comments()

      # Do not group newlines together if combine_newline is False (stop at the 
      # first newline found)
      if token == '\n' and combine_newline == False:
         return token
