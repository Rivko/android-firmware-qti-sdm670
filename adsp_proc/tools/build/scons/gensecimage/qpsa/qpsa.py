...
#=============================================================================
#
#  $Header: //components/rel/scons.qc/1.1/gensecimage/qpsa/qpsa.py#3 $
#  $DateTime: 2014/03/20 16:28:52 $
#  $Author: coresvc $
#  $Change: 5511367 $
...
for some_file in file_list:
    if os.path.exists(some_file):
        log_str="found file '{0}'\n".format(some_file)
        LOG.write(log_str)
        print log_str
        os.unlink(some_file)
        if os.path.exists(some_file):
            error_str="\n\nFailed to delete file '%s' before copying\n\n" % some_file
            LOG.write(error_str)
            LOG.close()
            raise(error_str)
        log_str="file '{0}' deleted successfully\n".format(some_file)
        LOG.write(log_str)
        print log_str
...
if check_file_exists("qpsa.zip") == True:
    ...
    for file in openssl_configfiles:
        ...
        if check_file_exists(file_to_copy) == False:
            ...
            LOG.close()
            sys.exit(1)
        shutil.copyfile("alt_resources/default/openssl/%s" % file, "./%s" % file)
