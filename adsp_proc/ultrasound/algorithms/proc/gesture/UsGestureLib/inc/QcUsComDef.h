...
typedef  signed long long  Word40;          /* 40-bit Long Signed integer    */
...
typedef  signed __int64   Word40;           /* 40-bit Long Signed integer    */
#endif
...
/* Align addr so it starts at size boundary. When aligning pointers make sure to cast addr
   to an integer type */
#define ALIGN_US(addr, size) ((addr + size - 1) & ~(size - 1))
