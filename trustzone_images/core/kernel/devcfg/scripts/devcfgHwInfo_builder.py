...
def create_seginfo_dict(env, config):
   rootenv = env.get('IMAGE_ENV')
   devcfg_xml_tag_list = rootenv.get('DEVCFG').getFeature('devcfgXMLTagList')
   prop_info = {}
   for elem in devcfg_xml_tag_list:
      # DALPROP_PropsInfo_8996_xml
      segment_base_addr = 'DALPROP_PropsInfo_' + str(elem).replace('_xml','') + config
...
def devcfg_create_prop_info_builder(target, source, env):
   ...
   soc_dicts            = env.get('DEVCFG').getFeature('socHwVersion')
   tcsr_soc_hw_ver_addr = env.get('DEVCFG').getFeature('tcsrSsocHwVersionAddr')
