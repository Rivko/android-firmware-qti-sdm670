...
#
# Copyright (c) 2014-17 by QUALCOMM, Technologies Inc.
# All Rights Reserved.
...
''')
   ...
   for tag in qsee_env.get('DEVCFG').getFeature('devcfgXMLTagList'):
      ...
      file_name_ext = "DevCfg_" + id + "_data_oem_tz.o"
      lcsf.write("      KEEP(*" + file_name_ext + " (.rodata* .data* .sdata*))\n")
      file_name_ext = "DevCfg_" + id + "_oem_tz.o"
      lcsf.write("      KEEP(*" + file_name_ext + " (.rodata* .data* .sdata*))\n")
   for file in tz_source_list:
      file_path = qsee_env.RelPath(str(file), "${BUILD_ROOT}", False).replace('\\','\\\\')
      lcsf.write("      KEEP(*" + file_path + " (.rodata* .data* .sdata*))\n")
...
''')
   ...
   for tag in hyp_env.get('DEVCFG').getFeature('devcfgXMLTagList'):
      id = str(tag).split("_")[0]
      file_name_ext = "DevCfg_" + id + "_data_oem_hyp.o"
      lcsf.write("      KEEP(*" + file_name_ext + " (.rodata* .data* .sdata*))\n")
      file_name_ext = "DevCfg_" + id + "_oem_hyp.o"
      lcsf.write("      KEEP(*" + file_name_ext + " (.rodata* .data* .sdata*))\n")
   for file in hyp_source_list:
      file_path = hyp_env.RelPath(str(file), "${BUILD_ROOT}", False).replace('\\','\\\\')
      lcsf.write("      KEEP(*" + file_path + " (.rodata* .data* .sdata*))\n")
