...
$Header: //components/rel/core.tz/2.0.2/api/dal/DALSys.h#2 $
=============================================================================*/
...
/*
  @brief Add memory to an active heap at run time

  Allows user to add extra memsection to an active heap at run-time.
  This extends the amount of memory available on the heap.

  In order for successfull completion the heap must be initialized and active.

  @param dwSize        : size of memsection being added to the heap
         section_start : start address of memsection being added to the heap

  @see DALSYS_Malloc
  @return Return Code, DAL_SUCCESS on successful completion, DAL_ERROR otherwise
*/
DALResult
DALSYS_HeapAddSection(uint32 dwSize,  void *section_start);
/*
@brief get present statistics of DAL heap

Provide present DAL heap statistics.
Refer mem_heap_totals_type
to know detail information on statistcis that will be provided

In order for successfull completion the heap must be initialized and active.

@param totals  : pointer to mem_heap_totals_type struct 

@see DALSYS_Malloc
@return Return Code, DAL_SUCCESS on successful completion, DAL_ERROR otherwise
*/
DALResult
DALSYS_HeapGetTotals(mem_heap_totals_type *totals);
/*
