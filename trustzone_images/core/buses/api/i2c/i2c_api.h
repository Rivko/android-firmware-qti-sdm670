...
typedef struct
{
    ...
    boolean     smbus_mode;                 /**< TRUE if SMBUS mode is used. */
...
typedef struct
{
    ...
    uint32  cmd;            /**< If mode is I3C_BROADCAST_CCC or I3C_DIRECT_CCC,
                                 this field holds the CCC command. If mode is HDR_DDR,
                                 this field holds the HDR Command. */
...
/**
  ...
  specific. In most generic cases, the value is a timer tick at a resolution of
  about 13 microseconds.

  @param[in] i2c_handle           Handle to the I2C instance.

  @return
  0 -- Timestamp not supported. \n
  N -- A non-zero timestmamp N, where N is the ticks read from the QTIMER.
*/
uint32
