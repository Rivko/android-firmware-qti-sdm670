...
#===============================================================================
# Edit History
# $Header: //components/rel/core.tz/2.0.2/bsp/build/scripts/tms_builder_rcinit.py#1 $
...
__version__ = "$Header: //components/rel/core.tz/2.0.2/bsp/build/scripts/tms_builder_rcinit.py#1 $"
...
RCINIT_USES_FUNC_ONLY_VARIANT_FLAG = 'USES_RCINIT_FUNC_ONLY_VARIANT'
...
THREAD_STACK_TYPE = 'stack_mem_type'           # TECH AREA supplied
...
RCINIT_INFO_STACK_TYPE = 'stack_type'
...
RCINIT_INFO_PATCH_DEFINED = 'patch_defined'     # internal key
RCINIT_DEFAULT_POLICY_NAME = 'default'          # internal key
...
RCINIT_STACK_TYPE_NONE = 'RCINIT_STACK_TYPE_NONE'       # internal key
RCINIT_STACK_TYPE_STATIC = 'RCINIT_STACK_TYPE_STATIC'   # internal key
RCINIT_STACK_TYPE_DYNAMIC = 'RCINIT_STACK_TYPE_DYNAMIC' # internal key
RCINIT_STACK_TYPE_AUTO = 'RCINIT_STACK_TYPE_AUTO'       # internal key
...
RCINIT_INFO_LIST_PATCH = 'RCINIT_INFO_COLLECTOR_PATCH'  # internal key

#env_rcinit_pol[]
RCINIT_POLICY_DICT = 'RCINIT_POLICY_COLLECTOR'  # internal key
RCINIT_POLICY_INFO_DICT = 'RCINIT_POLICY_INFO_COLLECTOR'  # internal key
...
def generate(env):
   ...
   env_rcinit[RCINIT_INFO_LIST_PATCH] = []
   ...
   # Function-only variant policy info dictionary
   env_rcinit_pol[RCINIT_POLICY_INFO_DICT] = {}
   ...
   env.AddMethod(tool_method_add_func_playbook, 'AddRCInitFuncPlaybook')
   ...
   env.AddMethod(tool_method_patch_task, 'PatchRCInitTask')
   env.AddMethod(tool_method_patch_function, 'PatchRCInitFunc')
...
def tool_emitter(target, source, env):
   ...
   for s in source:
      if s != None:
         ...
         if source_ext == RCINIT_EXT_PL:
            source_patch = source_stem + RCINIT_EXT_PL + '_patch'
            if os.path.exists(source_patch):
               env.Precious(source_patch)
   ...
   if target != None:
      for t in target:
         if t != None:
            ...
            target_patch = target_stem + target_ext + '_patch'
            target_unmet = target_stem + '_unmet_dep' + RCINIT_EXT_LOG
            ...
            if target_ext == RCINIT_EXT_PB:     # Always PB
               ...
               output = File(target_stem + '_unmet_dep' + RCINIT_EXT_LOG)
               env.Clean(target_unmet, output)  # Explicit Clean, Command Line -c
               targets.append(output)           # Target accepted

               output = File(target_stem + '_unmet_dep' + RCINIT_EXT_LOG + RCINIT_EXT_HASH)
               env.Clean(target_unmet, output)  # Explicit Clean, Command Line -c
               targets.append(output)           # Target accepted
               ...
               output = File(target_stem + RCINIT_EXT_PL + '_patch')
               env.Clean(target_patch, output)  # Explicit Clean, Command Line -c
               targets.append(output)           # Target accepted
               ...
               output = File(target_stem + RCINIT_EXT_PL + '_patch' + RCINIT_EXT_HASH)
               env.Clean(target_patch, output)  # Explicit Clean, Command Line -c
               targets.append(output)           # Target accepted
...
def tool_action(target, source, env):
   ...
   RCINIT_STACK_SIZE_AUTO = 0                         # internal stats
   ...
   # Keep count of unmet dependencies for future printing
   unmetDependenciesCount = 0
   ...
   for playlist in playlists:
      ...
      if playlist_ext == RCINIT_EXT_PL and playlist_stem != target_stem:
         ...
         playlist_patch = playlist_stem + playlist_ext + '_patch'
         if os.path.exists(playlist_patch):
            try:
               with open(playlist_patch, 'r') as playlist_file:
                  for line in playlist_file:
                     entry = eval(line)
                     if isinstance(entry, OrderedDict):
                        if entry.has_key(RCINIT_INFO_TYPE):
                           if entry[RCINIT_INFO_TYPE] == RCINIT_INFO_TASK or  entry[RCINIT_INFO_TYPE] == RCINIT_INFO_FUNCTION :
                              tool_method_patch_task_func(env, None, entry)
                           else:
                              env.TmsError(RCINIT_POC_IO + ' Specified Playlist incorrect internal format, no matching type {:s}'.format(playlist_patch))
                        else:
                           env.TmsError(RCINIT_POC_IO + ' Specified Playlist incorrect internal format, no type {:s}'.format(playlist_patch))
                     else:
                        env.TmsError(RCINIT_POC_IO + ' Specified Playlist incorrect internal format, no matching instance {:s}'.format(playlist_patch))
            except:
               raise Exception(RCINIT_POC_HBA + ' Fails with ' + playlist_patch)

   if source != None:
      for threads in env_rcinit[RCINIT_INFO_LIST]:
         if threads not in env_rcinit_pol[RCINIT_NOPOLICY_ORDERED_LIST][env.TmsToInt(threads[RCINIT_SEQUENCE_GROUP])] :
            env_rcinit_pol[RCINIT_NOPOLICY_ORDERED_LIST][env.TmsToInt(threads[RCINIT_SEQUENCE_GROUP])].append(threads)
            for policy in threads[RCINIT_POLICY_OPTIN]:
               env_rcinit_pol[RCINIT_POLICY_DICT][policy][env.TmsToInt(threads[RCINIT_SEQUENCE_GROUP])].append(threads)
...
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
   ...
   if env.GetUsesFlag('USES_RCINIT_FUNC_ONLY_VARIANT'):
       PlaybookFuncOutput = []

       PlaybookFuncOutput.append('/*build_id     {:s}*/\n'.format(env.get('BUILD_ID')))
       PlaybookFuncOutput.append('/*version_info {:s}*/\n'.format(__version_info__))

       PlaybookFuncOutput.append('''
#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif\n''')
       ...
       #RCINIT internal struct with enums for current policy and group here

       PlaybookFuncOutput.append('\ntypedef enum {')
       #walk through each policy name
       for policy in env_rcinit_pol[RCINIT_POLICY_DICT]:
          ...
          if policy not in env_rcinit_pol[RCINIT_POLICY_USE_LIST]:
             env.TmsWarning('Policy: ' + policy + ' is not in use')
             continue
          PlaybookFuncOutput.append('\n   {:s},'.format(policy.upper()))
       ...
       for i in range(RCINIT_SEQUENCE_GROUP_MAX):
          PlaybookFuncOutput.append('\n   RCINIT_GROUP_{:d},'.format(i))

       PlaybookFuncOutput.append('\n   RCINIT_GROUP_TERM')
       PlaybookFuncOutput.append('\n}rcinit_group_t;\n')

       PlaybookFuncOutput.append('\ntypedef struct {')
       PlaybookFuncOutput.append('\n   rcinit_policy_t policy;')
       PlaybookFuncOutput.append('\n   rcinit_group_t group;')
       PlaybookFuncOutput.append('\n}rcinit_autogen_internal_s;\n')

       PlaybookFuncOutput.append('\nrcinit_autogen_internal_s rcinit_autogen_internal;\n')
          
       PlaybookFuncOutput.append('\nvoid rcinit_init_main (void);\n')
...
/*handle*/     RCINIT_NULL
   ...
   HEAP_QUOTA_FIELD, entry[RCINIT_INFO_GROUP], entry[RCINIT_INFO_STACK_TYPE]))
      ...
      # API is DAL Workloop
         ...
         if entry[RCINIT_INFO_STACK_TYPE] == RCINIT_STACK_TYPE_STATIC and entry[RCINIT_INFO_ENTRY] != RCINIT_NULL:
...
static rcinit_stack_t {0:s}[({1:s}+sizeof(_rcxh_scope_t))/sizeof(rcinit_stack_t)] __attribute__((aligned(RCINIT_STACK_ALIGN), section("{2:s}")));\n'''.format(STACK_NAME, entry[RCINIT_INFO_STACK], entry[RCINIT_INFO_STACK_STATIC], entry[RCINIT_INFO_NAME]))
         elif entry[RCINIT_INFO_STACK_TYPE] == RCINIT_STACK_TYPE_AUTO and entry[RCINIT_INFO_ENTRY] != RCINIT_NULL:
            RCINIT_STACK_SIZE_AUTO = RCINIT_STACK_SIZE_AUTO + env.TmsToInt(entry[RCINIT_INFO_STACK]) + RCINIT_STACK_OVERHEAD
         else:
...
/*handle*/     &rcinit_rw_{0:s}
   ...
   #generate Unmet Dependencies Log
   PlaybookUnmetDepLog = []
   PlaybookUnmetDepLog.append('\n****************************************\n')
   PlaybookUnmetDepLog.append('RCINIT UNMET DEPENDENCIES OUTPUT LOG')
   PlaybookUnmetDepLog.append('\n****************************************\n')
   ...
   for grpLvl in env_rcinit_pol[RCINIT_NOPOLICY_ORDERED_LIST]:
      ...
      grpDict = {}
      for item in grpLvl:         
         #remove empty/stray space string elements from currDependencies
         currDependencies = set(x for x in set(item[RCINIT_DEPENDENCIES]) if x and x != ' ')
         
         filteredDependencies = list(set(item[RCINIT_DEPENDENCIES]) & grpNames)
         
         #unmet dependencies: compare lengths of currDependencies and filteredDependencies
         if len(filteredDependencies) != len(currDependencies):                      
             #if currDependencies has an element not found in grpNames, it is an unmet dependency
             unmetDependenciesStr = str(list(currDependencies - grpNames))
             env.TmsWarning('Unmet dependencies for ' + str(item[RCINIT_INFO_NAME]) + ': ' + unmetDependenciesStr)
             unmetDependenciesCount += 1
             
             #append to Unmet dependencies list here with item[RCINIT_INFO_NAME]
             PlaybookUnmetDepLog.append('\n{:s}: {:s}'.format(item[RCINIT_INFO_NAME], unmetDependenciesStr))
             PlaybookUnmetDepLog.append('\n')
   ...
   if unmetDependenciesCount != 0:
      PlaybookUnmetDepLog.append('\n****************************************\n')
      PlaybookUnmetDepLog.append('\nResolve {:d} unmet dependencies for above init_names\n'.format(unmetDependenciesCount))
   PlaybookUnmetDepLog.append('\n****************************************\n')
   ...
   #Policies for rcinit functions-only variant
   if env.GetUsesFlag('USES_RCINIT_FUNC_ONLY_VARIANT'):
      env.TmsInfo('RCINIT Policy count: ' + str(len(env_rcinit_pol[RCINIT_POLICY_USE_LIST])))
      PolicyInfoList = env_rcinit_pol[RCINIT_POLICY_INFO_DICT].items()
      env.TmsInfo('RCINIT Policies: ' + str(PolicyInfoList))
  
      #generate rcinit functions-only
      for policy in env_rcinit_pol[RCINIT_POLICY_DICT]:
         #skip if policy is not used in this build
         if policy not in env_rcinit_pol[RCINIT_POLICY_USE_LIST]:
            env.TmsWarning('Policy: ' + policy + ' not in use')
            continue
      
         #variable to hold policy defined start group
         policy_start = 0

         #output for policy determiner function (exclude 'default')
         if policy != RCINIT_DEFAULT_POLICY_NAME:
            PolicyInfo = env_rcinit_pol[RCINIT_POLICY_INFO_DICT].get(str(policy), None)
            policy_start = int(PolicyInfo[0]) #change to policy defined start group
            #extern declaration
            PlaybookFuncOutput.append('\n/* {:s} policy determiner function */\n'.format(policy))
            PlaybookFuncOutput.append('extern boolean {:s}(void);\n'.format(PolicyInfo[1]))

         PlaybookFuncOutput.append('\n')

         for i in range(policy_start, RCINIT_SEQUENCE_GROUP_MAX):
            #extern declarations
            for rcinitInfo in env_rcinit_pol[RCINIT_POLICY_DICT][policy][i]:
               #skip if tasks or policies are encountered
               if rcinitInfo[RCINIT_INFO_API] == THREAD_API_FUNC and rcinitInfo[RCINIT_INFO_ENTRY] != RCINIT_NULL:
                  PlaybookFuncOutput.append('extern void {:s}(void);\n'.format(rcinitInfo[RCINIT_INFO_ENTRY]))
               elif rcinitInfo[RCINIT_INFO_API] != THREAD_API_FUNC and rcinitInfo[RCINIT_INFO_API] != THREAD_API_POLICY and env.GetUsesFlag('USES_RCINIT_FUNC_ONLY_VARIANT'):
                  env.TmsWarning('Skipping task: ' + rcinitInfo[RCINIT_INFO_NAME] + ' in function-only RCINIT variant')
         
            PlaybookFuncOutput.append('\n')
         
            func_added_to_group = False
            PlaybookFuncOutput.append('void rcinit_group_{:s}_{:d} (void)\n{{\n'.format(policy, i))
            for rcinitInfo in env_rcinit_pol[RCINIT_POLICY_DICT][policy][i]:
               #skip if tasks or policies are encountered
               if rcinitInfo[RCINIT_INFO_API] == THREAD_API_FUNC and rcinitInfo[RCINIT_INFO_ENTRY] != RCINIT_NULL:
                  PlaybookFuncOutput.append('   {:s}();\n'.format(rcinitInfo[RCINIT_INFO_ENTRY]))
                  func_added_to_group = True 

            if func_added_to_group:
               PlaybookFuncOutput.append('   rcinit_autogen_internal.group = RCINIT_GROUP_{:d};\n'.format(i))
            PlaybookFuncOutput.append('}}\n\n'.format(0))
         
         PlaybookFuncOutput.append('\n/* ******************************** */\n') 

      #Main RCINIT entry point function   
      PlaybookFuncOutput.append('\nvoid rcinit_init_main (void)\n{\n')
      for policy in env_rcinit_pol[RCINIT_POLICY_DICT]:
         #skip if policy is not used in this build
         if policy not in env_rcinit_pol[RCINIT_POLICY_USE_LIST]:
         ...
         #Tune main entry point for policies
         if policy != RCINIT_DEFAULT_POLICY_NAME: 
            policy_start = 0
            PolicyInfo = env_rcinit_pol[RCINIT_POLICY_INFO_DICT].get(str(policy), None)
            policy_start = int(PolicyInfo[0]) #change to policy defined start group
         
            PlaybookFuncOutput.append('\n{:s}:\n'.format(policy)) #label for policy entry
            for i in range(policy_start, RCINIT_SEQUENCE_GROUP_MAX):
               PlaybookFuncOutput.append('   rcinit_group_{:s}_{:d}();\n'.format(policy, i))
            PlaybookFuncOutput.append('goto end;\n')
         else:
            for i in range(RCINIT_SEQUENCE_GROUP_MAX):
               for entry in list(PolicyInfoList):
                  PolicyName = list(entry)[0] #extract policy name
                  PolicyData = list(entry)[1] #extract policy data list
                  PolicyInfoGroup = PolicyData[0] #extract group from data list
                  env.TmsDebug('PolicyInfoList: ' + PolicyName + ' ' + str(PolicyData) + ' ' + str(PolicyInfoGroup))
                  #output policy determiner functions based on group match
                  if i == int(PolicyInfoGroup):
                     PolicyInfoEntry = PolicyData[1]
                     PlaybookFuncOutput.append('\n   if (TRUE == {:s}())\n   {{'.format(PolicyInfoEntry))
                     PlaybookFuncOutput.append('\n      rcinit_autogen_internal.policy = {:s};'.format(PolicyName.upper()))
      ...
      PlaybookFuncOutput.append('\nend:\n   rcinit_autogen_internal.group = RCINIT_GROUP_TERM;\n}}\n\n'.format(0))
   
   #done generating rcinit functions-only
   ...
   PlaybookOutput.append('\n')
   PlaybookOutput.append('rcinit_info_t**** RCINIT_PLAYBOOK_MAP_GROUP = (void*)rcinit_internal_groups;\n')
   PlaybookOutput.append('rcinit_internal_name_map_t* RCINIT_PLAYBOOK_MAP_NAME = (void*)rcinit_internal_name_map;\n')

   # Const Memory
   PlaybookOutput.append('''
DLL_API_GLOBAL const rcinit_autogen_internal_t rcinit_autogen_internal = {
/*name_map*/          (void*)rcinit_internal_name_map,
/*name_map_size*/     rcinit_internal_name_map_size,
/*policy_map*/        (void*)rcinit_internal_policy_map,
/*policy_map_size*/   rcinit_internal_policy_map_size,
/*policy_list*/       (void*)rcinit_internal_policy_list,
/*group_map*/         (void*)rcinit_internal_groups 
};\n''')
   PlaybookOutput.append('\n#if defined(__cplusplus)\n}\n#endif\n\n')
   ...
   PlaybookOutputLog.append('\n')
   PlaybookOutputLog.append('Build Stack Allocation ESTIMATES {:s} Total, {:s} Dynamic, {:s} Static, {:s} Auto\n'.format(
      ...
      str(roundup(RCINIT_STACK_SIZE_AUTO, RCINIT_STACK_ROUNDUP))))
   ...
   PlaybookOutputLog.append('Patched entries are denoted by "' + RCINIT_OUTPUT_PATCH + '" \n')
   ...
   for i in range(RCINIT_SEQUENCE_GROUP_MAX):
      ...
      for entry in env_rcinit_pol[RCINIT_NOPOLICY_ORDERED_LIST][i]:
         ...
         if entry[RCINIT_INFO_TYPE] == RCINIT_INFO_FUNCTION or entry[RCINIT_INFO_TYPE] == RCINIT_INFO_POLICY:
            if entry[RCINIT_INFO_PATCH_DEFINED] != None: 
               PlaybookOutputLog.append(debuglinefmt.format(
                  RCINIT_OUTPUT_PATCH + RCINIT_OUTPUT_LEN + entry[RCINIT_INFO_NAME] if len(entry[RCINIT_INFO_NAME]) >
                  RCINIT_NAME_MAXLEN else RCINIT_OUTPUT_PATCH + entry[RCINIT_INFO_NAME],
   ...
   #form function-only output log
   if env.GetUsesFlag('USES_RCINIT_FUNC_ONLY_VARIANT'):
       PlaybookFuncOutputLog = []
       PlaybookFuncOutputLog.append('\n****************************************\n')
       PlaybookFuncOutputLog.append('RCINIT FUNCTION-ONLY OUTPUT LOG\n')
       
       PlaybookFuncOutputLog.append('\nUses: ')
       if env.GetUsesFlag('USES_RCINIT_FUNC_ONLY_VARIANT'):
          PlaybookFuncOutputLog.append('USES_RCINIT_FUNC_ONLY_VARIANT ')
       if env.GetUsesFlag('USES_RCINIT_OPTION_PLAYBOOK') or env.GetUsesFlag('USES_RCINIT_OPTION_PLAYBOOK_SHARED_OBJECT') or env.GetUsesFlag('USES_RCINIT_PLAYBOOK'):
          PlaybookFuncOutputLog.append('PLAYBOOK ')
       PlaybookFuncOutputLog.append('\n')

       PlaybookFuncOutputLog.append('\nPolicies in use: ' + str(env_rcinit_pol[RCINIT_POLICY_DICT].keys()) + '\n')

       PlaybookFuncOutputLog.append('\nNOTES\n')
       PlaybookFuncOutputLog.append('Specific field does not apply is denoted by "' + RCINIT_OUTPUT_NA + '".\n')
       PlaybookFuncOutputLog.append('TECH AREA supplied unique name too long for NHLOS space requirements is denoted by "' + RCINIT_OUTPUT_LEN + '"; shorten it.\n')
       PlaybookFuncOutputLog.append('"Started" field of "' + RCINIT_OUTPUT_NA + '" denotes TECH AREA function or task is not executed at runtime startup,\n')
       PlaybookFuncOutputLog.append('reguardless of group assignment, and may indicate dynamic use case is expected. Consult with TECH AREA.\n')
       ...
       PlaybookFuncOutputLog.append('\n')
       debuglinefmt = '{: <25.25s} {: <12.12s} {: <22.22s} {: <45.45s} {: <12.12s} {: <12.12s} {: <60.60s} {: <40.40s} {: <25.25s}\n'
       PlaybookFuncOutputLog.append(debuglinefmt.format('Name', 'Hash', 'NHLOS API', 'NHLOS Priority', 'Stack', 'StackDyn', 'Function Dependency', 'Policy', 'Started'))
       PlaybookFuncOutputLog.append('\n-----------------------------------------------------------------------------------\n')

       for i in range(RCINIT_SEQUENCE_GROUP_MAX):

          PlaybookFuncOutputLog.append('*** RCINIT_GROUP_{:d}\n'.format(i))
          PlaybookFuncOutputLog.append('\n')
          ...
          for entry in env_rcinit_pol[RCINIT_NOPOLICY_ORDERED_LIST][i]:
             ...
             if entry[RCINIT_INFO_TYPE] == RCINIT_INFO_FUNCTION or entry[RCINIT_INFO_TYPE] == RCINIT_INFO_POLICY:
                if entry[RCINIT_INFO_PATCH_DEFINED] != None: 
                   PlaybookFuncOutputLog.append(debuglinefmt.format(
                      RCINIT_OUTPUT_PATCH + RCINIT_OUTPUT_LEN + entry[RCINIT_INFO_NAME] if len(entry[RCINIT_INFO_NAME]) >
                      RCINIT_NAME_MAXLEN else RCINIT_OUTPUT_PATCH + entry[RCINIT_INFO_NAME],
                      entry[RCINIT_INFO_HASH], RCINIT_OUTPUT_NA, RCINIT_OUTPUT_NA, RCINIT_OUTPUT_NA,
                      RCINIT_OUTPUT_NA,
       ...
       PlaybookFuncOutputLog.append('\n')
       debuglinefmt = '{: <25.25s} {: <60s}\n'
       PlaybookFuncOutputLog.append(debuglinefmt.format('Name', 'Defined At'))
       rcinit_ents = sorted(env_rcinit[RCINIT_INFO_LIST], key=lambda k: k[RCINIT_INFO_NAME])
       for entry in rcinit_ents:
          if entry[RCINIT_INFO_API] == THREAD_API_FUNC or entry[RCINIT_INFO_API] == THREAD_API_POLICY:
             PlaybookFuncOutputLog.append(
       ...
       #append output log to end of file
       PlaybookFuncOutput.append('/*\n')
       for line in PlaybookFuncOutputLog:
          PlaybookFuncOutput.append(line)
       PlaybookFuncOutput.append('*/\n')
   
   # Output what was requested from builder based on extension and flags

   if target_ext == RCINIT_EXT_PB:
      #Tune output for function-only RCINIT variant
      if env.GetUsesFlag('USES_RCINIT_FUNC_ONLY_VARIANT'):
         tool_update_file_c(env_rcinit, target_full, PlaybookFuncOutput)
      else:
         tool_update_file_c(env_rcinit, target_full, PlaybookOutput)
      # Output the unmet dependencies log if necessary 
      if unmetDependenciesCount != 0:
         tool_update_file_log(env_rcinit, target_unmet, PlaybookUnmetDepLog)
         
   # Intermediate Output
   elif target_ext == RCINIT_EXT_PL:
      tool_update_file_rcpl(env_rcinit, target_full, env_rcinit[RCINIT_INFO_LIST], None)
      tool_update_file_rcpl(env_rcinit, target_patch, env_rcinit[RCINIT_INFO_LIST_PATCH], 'PatchPlaylist')
   ...
   # Diagnostic Output
      ...
      if env.GetUsesFlag('USES_RCINIT_FUNC_ONLY_VARIANT'):
         tool_update_file_log(env_rcinit, target_full, PlaybookFuncOutputLog)
      else:
...
def tool_method_add_func_playbook(env, build_ids, output, input): 
   ''' Method Call, Calls RCInitBuilder '''
   env.TmsDebug('tool_method_add_func_playbook('+str(output)+')')

   env.AddUsesFlags(RCINIT_USES_FUNC_ONLY_VARIANT_FLAG)
   ...
   if output_ext == '': # Assign RCINIT_EXT_PB
      ...
      env.RequirePublicApi([ 'TMS', 'DEBUGTOOLS', 'DAL', 'DEBUGTRACE', 'SERVICES', 'SYSTEMDRIVERS', 'KERNEL', ], area = 'core')
...
def tool_method_add_task(env, build_ids, input_dict):
   ...
   if input_dict.has_key(THREAD_STACK_TYPE):
      output_dict[THREAD_STACK_TYPE] = input_dict[THREAD_STACK_TYPE]
      output_dict[RCINIT_INFO_STACK_TYPE] = input_dict[THREAD_STACK_TYPE]

      if input_dict.has_key(THREAD_STACK_STATIC) and input_dict[THREAD_STACK_TYPE]!= RCINIT_STACK_TYPE_STATIC:
         env.TmsError(RCINIT_POC_TA + ' Only Static stack type tasks can have static stack {:s}'.format(output_dict[RCINIT_INFO_NAME]))
   #Default Behavior
   else:
      if env.GetUsesFlag('USES_RCINIT_OPTION_STATIC_STACK'):
         output_dict[THREAD_STACK_TYPE] = RCINIT_STACK_TYPE_STATIC
         output_dict[RCINIT_INFO_STACK_TYPE] = RCINIT_STACK_TYPE_STATIC
      else:
         output_dict[THREAD_STACK_TYPE] = RCINIT_STACK_TYPE_DYNAMIC
         output_dict[RCINIT_INFO_STACK_TYPE] = RCINIT_STACK_TYPE_DYNAMIC
...
def tool_method_patch_task(env, build_ids, input_dict):
   ''' Method Call, Adds RCINIT Task
   Validates input and stores to 'globals' construction environment
   Runs before tool_emitter() function '''
   ...
   output_dict[RCINIT_INFO_PATCH_DEFINED] = { 'file' : None, 'line' : None }
   output_dict[RCINIT_SEQUENCE_GROUP] = None
   output_dict[RCINIT_INFO_GROUP] = None 
   if input_dict.has_key(RCINIT_INFO_PATCH_DEFINED):
      output_dict[RCINIT_INFO_PATCH_DEFINED] = input_dict[RCINIT_INFO_PATCH_DEFINED]
   else:
      output_text = filename.replace('\\','/').lower().replace(env.get('BUILD_ROOT', None).lower()+'/', '')
      output_dict[RCINIT_INFO_PATCH_DEFINED] = { 'file' : output_text, 'line' : line_number }
   ...
   # Process input on SCONS Command Options
   if env.GetOption('clean') or env.GetOption('no_exec'):
      env.TmsDebug('TARGET CLEAN/NO_EXEC RCInitPatchTask {:s} "{:s}" {:d}'.format(output_dict[RCINIT_INFO_NAME], output_dict[RCINIT_INFO_PATCH_DEFINED]['file'], output_dict[RCINIT_INFO_PATCH_DEFINED]['line']))
      return None # RETURN, IGNORED
   # Ignore input on specified BUILD_ID not being active
   elif build_ids is not None and not env.IsTargetEnable(build_ids):
      env.TmsDebug('TARGET NOT ACTIVE RCInitPatchTask {:s} "{:s}" {:d}'.format(output_dict[RCINIT_INFO_NAME], output_dict[RCINIT_INFO_PATCH_DEFINED]['file'], output_dict[RCINIT_INFO_PATCH_DEFINED]['line']))
      return None # RETURN, IGNORED

   #for threads in env_rcinit[RCINIT_INFO_LIST_PATCH]:
   #   if threads.has_key(RCINIT_INFO_NAME) and output_dict[RCINIT_INFO_NAME] == threads[RCINIT_INFO_NAME]:
   #      env.TmsError(RCINIT_POC_TA + ' Create unique name for {:s}'.format(output_dict[RCINIT_INFO_NAME]))
   #   if threads.has_key(RCINIT_INFO_HASH) and output_dict[RCINIT_INFO_HASH] == threads[RCINIT_INFO_HASH]:
   #      env.TmsError(RCINIT_POC_TA + ' Create unique name for hash {:s}'.format(output_dict[RCINIT_INFO_NAME]))
   ...
   if input_dict.has_key(RCINIT_POLICY_OPTIN):
      ...
      output_dict[RCINIT_POLICY_OPTIN] = None
      output_dict[RCINIT_INFO_POLICY] = None

   #read in sequence level
   if input_dict.has_key(RCINIT_SEQUENCE_GROUP):
      if input_dict[RCINIT_SEQUENCE_GROUP] not in RCINIT_SEQUENCE_GROUP_KEYS.keys():
         output_dict[RCINIT_SEQUENCE_GROUP] = RCINIT_SEQUENCE_GROUP_KEYS[input_dict[RCINIT_SEQUENCE_GROUP]]
         output_dict[RCINIT_INFO_GROUP] = RCINIT_SEQUENCE_GROUP_KEYS[input_dict[RCINIT_SEQUENCE_GROUP]]
   ...
   if input_dict.has_key(THREAD_API):
      ...
      output_dict[THREAD_API] = None
      output_dict[RCINIT_INFO_API] = None

   #validation
   error = 0
   if output_dict.has_key(RCINIT_INFO_API):
      if output_dict[RCINIT_INFO_API] == THREAD_API_DAL and not env.GetUsesFlag('USES_DAL'):
         error = 1
      elif output_dict[RCINIT_INFO_API] == THREAD_API_POSIX and not env.GetUsesFlag('USES_POSIX'):
         error = 1
   ...
   if input_dict.has_key(THREAD_STACK_TYPE):
      ...
      if input_dict.has_key(THREAD_STACK_STATIC) and input_dict[THREAD_STACK_TYPE]!= RCINIT_STACK_TYPE_STATIC:
         env.TmsError(RCINIT_POC_TA + ' Only Static stack type tasks can have static stack {:s}'.format(output_dict[RCINIT_INFO_NAME]))
   #Cannot assign Default Behavior while patching
   #else:
   #   if env.GetUsesFlag('USES_RCINIT_OPTION_STATIC_STACK'):
   #      output_dict[THREAD_STACK_TYPE] = RCINIT_STACK_TYPE_STATIC
   #      output_dict[RCINIT_INFO_STACK_TYPE] = RCINIT_STACK_TYPE_STATIC
   ...
   #if this is a new policy, create dictionary/lists
   if output_dict[RCINIT_POLICY_OPTIN] != None : 
      for policy in output_dict[RCINIT_POLICY_OPTIN]:
         if policy not in env_rcinit_pol[RCINIT_POLICY_DICT].keys():
            env_rcinit_pol[RCINIT_POLICY_DICT][policy] = []
            for i in range(RCINIT_SEQUENCE_GROUP_MAX):
               env_rcinit_pol[RCINIT_POLICY_DICT][policy].append([])
   #add entry to each policy  list
   #Moved to tool_action
   #env_rcinit_pol[RCINIT_NOPOLICY_ORDERED_LIST][env.TmsToInt(output_dict[RCINIT_SEQUENCE_GROUP])].append(output_dict)
   #for policy in output_dict[RCINIT_POLICY_OPTIN]:
      #env_rcinit_pol[RCINIT_POLICY_DICT][policy][env.TmsToInt(output_dict[RCINIT_SEQUENCE_GROUP])].append(output_dict)

   #add to environment descriptor
   env_rcinit[RCINIT_INFO_LIST_PATCH].append(output_dict)
   env.TmsDebug('RCInitPatchTask {:s} "{:s}" {:d}'.format(output_dict[RCINIT_INFO_NAME], output_dict[RCINIT_INFO_PATCH_DEFINED]['file'], output_dict[RCINIT_INFO_PATCH_DEFINED]['line']))
...
def tool_method_add_function(env, build_ids, input_dict):
   ...
   output_dict[RCINIT_INFO_AFFINITY] = None
   output_dict[RCINIT_INFO_STACK] = None
   output_dict[RCINIT_INFO_STACK_TYPE] = RCINIT_STACK_TYPE_NONE
   output_dict[RCINIT_INFO_PRIORITY] = None
   output_dict[RCINIT_INFO_DEFINED] = None
   output_dict[RCINIT_INFO_PATCH_DEFINED] = None
   if input_dict.has_key(RCINIT_INFO_DEFINED):
...
def tool_method_patch_function(env, build_ids, input_dict):
   ''' Method Call,  RCINIT Function
   Validates input and stores to 'globals' construction environment
   Runs before tool_emitter() function '''
   ...
   #read in name
   if not input_dict.has_key(INIT_NAME):
      env.TmsError(RCINIT_POC_TA + ' Patch Function must have a name')
   ...
   # Process input on SCONS Command Options
   if env.GetOption('clean') or env.GetOption('no_exec'):
      env.TmsDebug('TARGET CLEAN/NO_EXEC RCInitPatchFunc {:s} "{:s}" {:d}'.format(output_dict[RCINIT_INFO_NAME], output_dict[RCINIT_INFO_PATCH_DEFINED]['file'], output_dict[RCINIT_INFO_PATCH_DEFINED]['line']))
      return None # RETURN, IGNORED
   # Ignore input on specified BUILD_ID not being active
   elif build_ids is not None and not env.IsTargetEnable(build_ids):
      env.TmsDebug('TARGET NOT ACTIVE RCInitPatchFunc {:s} "{:s}" {:d}'.format(output_dict[RCINIT_INFO_NAME], output_dict[RCINIT_INFO_PATCH_DEFINED]['file'], output_dict[RCINIT_INFO_PATCH_DEFINED]['line']))
      return None # RETURN, IGNORED
   ...
   #add to environment descriptor
   env_rcinit[RCINIT_INFO_LIST_PATCH].append(output_dict)
   env.TmsDebug('RCInitPatchFunc {:s} "{:s}" {:d}'.format(output_dict[RCINIT_INFO_NAME], output_dict[RCINIT_INFO_PATCH_DEFINED]['file'], output_dict[RCINIT_INFO_PATCH_DEFINED]['line']))
...
def tool_method_add_policy(env, build_ids, input_dict):
   ...
   #add policy group and entry point to list in a dictionary (name = unique key)
   #save for function-only variant policy info
   PlaybookFuncPolicyInfoList = [output_dict[RCINIT_SEQUENCE_GROUP], output_dict[RCINIT_INFO_ENTRY]]
   env_rcinit_pol[RCINIT_POLICY_INFO_DICT][output_dict[RCINIT_INFO_NAME]] = PlaybookFuncPolicyInfoList

   env.TmsInfo('Added RCINIT Policy: ' + output_dict[RCINIT_INFO_NAME] + ': '+ str(PlaybookFuncPolicyInfoList))
...
def tool_method_patch_task_func(env, build_ids, input_dict):
   ''' Patches RCINIT entries in 'globals' construction environment
   Runs before tool_emitter() function '''
   ...
   for threads in env_rcinit[RCINIT_INFO_LIST]:
      if threads.has_key(RCINIT_INFO_NAME) and input_dict[RCINIT_INFO_NAME] == threads[RCINIT_INFO_NAME]:
         if threads.has_key(RCINIT_INFO_TYPE) and input_dict.has_key(RCINIT_INFO_TYPE) and  input_dict[RCINIT_INFO_TYPE] != None:
            threads[RCINIT_INFO_TYPE] =input_dict[RCINIT_INFO_TYPE]
         if threads.has_key(RCINIT_INFO_HASH) and input_dict.has_key(RCINIT_INFO_HASH) and  input_dict[RCINIT_INFO_HASH] != None:
            threads[RCINIT_INFO_HASH] =input_dict[RCINIT_INFO_HASH]
         if threads.has_key(RCINIT_INFO_GROUP) and input_dict.has_key(RCINIT_INFO_GROUP) and  input_dict[RCINIT_INFO_GROUP] != None:
            threads[RCINIT_INFO_GROUP] =input_dict[RCINIT_INFO_GROUP]
         if threads.has_key(RCINIT_INFO_API) and input_dict.has_key(RCINIT_INFO_API) and  input_dict[RCINIT_INFO_API] != None:
            threads[RCINIT_INFO_API] =input_dict[RCINIT_INFO_API]
         if threads.has_key(RCINIT_INFO_ENTRY) and input_dict.has_key(RCINIT_INFO_ENTRY) and  input_dict[RCINIT_INFO_ENTRY] != None:
            threads[RCINIT_INFO_ENTRY] =input_dict[RCINIT_INFO_ENTRY]
         if threads.has_key(RCINIT_INFO_POLICY) and input_dict.has_key(RCINIT_INFO_POLICY) and  input_dict[RCINIT_INFO_POLICY] != None:
            threads[RCINIT_INFO_POLICY] =input_dict[RCINIT_INFO_POLICY]
         if threads.has_key(RCINIT_INFO_DEPENDENCY) and input_dict.has_key(RCINIT_INFO_DEPENDENCY) and  input_dict[RCINIT_INFO_DEPENDENCY] != None:
            threads[RCINIT_INFO_DEPENDENCY] =input_dict[RCINIT_INFO_DEPENDENCY]
         if threads.has_key(RCINIT_INFO_AFFINITY) and input_dict.has_key(RCINIT_INFO_AFFINITY) and  input_dict[RCINIT_INFO_AFFINITY] != None:
            threads[RCINIT_INFO_AFFINITY] =input_dict[RCINIT_INFO_AFFINITY]
         if threads.has_key(RCINIT_INFO_STACK) and input_dict.has_key(RCINIT_INFO_STACK) and  input_dict[RCINIT_INFO_STACK] != None:
            threads[RCINIT_INFO_STACK] =input_dict[RCINIT_INFO_STACK]
         if threads.has_key(RCINIT_INFO_STACK_STATIC) and input_dict.has_key(RCINIT_INFO_STACK_STATIC) and  input_dict[RCINIT_INFO_STACK_STATIC] != None:
            threads[RCINIT_INFO_STACK_STATIC] =input_dict[RCINIT_INFO_STACK_STATIC]
         if threads.has_key(RCINIT_INFO_STACK_TYPE) and input_dict.has_key(RCINIT_INFO_STACK_TYPE) and  input_dict[RCINIT_INFO_STACK_TYPE] != None:
            threads[RCINIT_INFO_STACK_TYPE] =input_dict[RCINIT_INFO_STACK_TYPE]
         if threads.has_key(RCINIT_INFO_PRIORITY) and input_dict.has_key(RCINIT_INFO_PRIORITY) and  input_dict[RCINIT_INFO_PRIORITY] != None:
            threads[RCINIT_INFO_PRIORITY] =input_dict[RCINIT_INFO_PRIORITY]
         #if threads.has_key(RCINIT_INFO_DEFINED) and input_dict.has_key(RCINIT_INFO_DEFINED) and  input_dict[RCINIT_INFO_DEFINED] != None:
            #threads[RCINIT_INFO_DEFINED] =input_dict[RCINIT_INFO_DEFINED]
         if threads.has_key(RCINIT_INFO_PATCH_DEFINED) and input_dict.has_key(RCINIT_INFO_PATCH_DEFINED) and  input_dict[RCINIT_INFO_PATCH_DEFINED] != None:
            threads[RCINIT_INFO_PATCH_DEFINED] =input_dict[RCINIT_INFO_PATCH_DEFINED]  
         if threads.has_key(THREAD_NAME) and input_dict.has_key(THREAD_NAME) and  input_dict[THREAD_NAME] != None:
            threads[THREAD_NAME] =input_dict[THREAD_NAME] 
         if threads.has_key(INIT_NAME) and input_dict.has_key(INIT_NAME) and  input_dict[INIT_NAME] != None:
            threads[INIT_NAME] =input_dict[INIT_NAME]
         if threads.has_key(RCINIT_POLICY_OPTIN) and input_dict.has_key(RCINIT_POLICY_OPTIN) and  input_dict[RCINIT_POLICY_OPTIN] != None:
            threads[RCINIT_POLICY_OPTIN] =input_dict[RCINIT_POLICY_OPTIN]
         if threads.has_key(INIT_ENTRY) and input_dict.has_key(INIT_ENTRY) and  input_dict[INIT_ENTRY] != None:
            threads[INIT_ENTRY] =input_dict[INIT_ENTRY] 
         if threads.has_key(RCINIT_SEQUENCE_GROUP) and input_dict.has_key(RCINIT_SEQUENCE_GROUP) and  input_dict[RCINIT_SEQUENCE_GROUP] != None:
            threads[RCINIT_SEQUENCE_GROUP] =input_dict[RCINIT_SEQUENCE_GROUP]
         if threads.has_key(THREAD_API) and input_dict.has_key(THREAD_API) and  input_dict[THREAD_API] != None:
            threads[THREAD_API] =input_dict[THREAD_API]
         if threads.has_key(THREAD_STACK) and input_dict.has_key(THREAD_STACK) and  input_dict[THREAD_STACK] != None:
            threads[THREAD_STACK] =input_dict[THREAD_STACK]
         if threads.has_key(THREAD_PRIORITY_ALIAS) and input_dict.has_key(THREAD_PRIORITY_ALIAS) and  input_dict[THREAD_PRIORITY_ALIAS] != None:
            threads[THREAD_PRIORITY_ALIAS] =input_dict[THREAD_PRIORITY_ALIAS]
         if threads.has_key(THREAD_AFFINITY) and input_dict.has_key(THREAD_AFFINITY) and  input_dict[THREAD_AFFINITY] != None:
            threads[THREAD_AFFINITY] =input_dict[THREAD_AFFINITY]  
         if threads.has_key(THREAD_PRIORITY) and input_dict.has_key(THREAD_PRIORITY) and  input_dict[THREAD_PRIORITY] != None:
            threads[THREAD_PRIORITY] =input_dict[THREAD_PRIORITY]  
         if threads.has_key(RCINIT_INFO_PRIORITY_ALIAS) and input_dict.has_key(RCINIT_INFO_PRIORITY_ALIAS) and  input_dict[RCINIT_INFO_PRIORITY_ALIAS] != None:
            threads[RCINIT_INFO_PRIORITY_ALIAS] =input_dict[RCINIT_INFO_PRIORITY_ALIAS]
         if threads.has_key(THREAD_ENTRY) and input_dict.has_key(THREAD_ENTRY) and  input_dict[THREAD_ENTRY] != None:
            threads[THREAD_ENTRY] =input_dict[THREAD_ENTRY]
         if threads.has_key(RCINIT_DEPENDENCIES) and input_dict.has_key(RCINIT_DEPENDENCIES) and  input_dict[RCINIT_DEPENDENCIES] != None:
            threads[RCINIT_DEPENDENCIES] =input_dict[RCINIT_DEPENDENCIES]


         #tool_info(env, 'Updated and patched thread' + str(threads))
...
def tool_method_add_heapquota(env, build_ids, input_dict):
   ...
   heapq_rcpl_dict[RCINIT_INFO_NAME] = input_dict[THREAD_NAME].lower()
   ...
   heapq_thread_name = input_dict[THREAD_NAME].lower()
   heapQuotaDict = {}
...
def tool_update_file_rcpl(env, target, content, listtype):
   ''' Output PL File '''
   ...
   md5 = hashlib.md5()
   sorted_entry = sorted(content, key=lambda k: k[RCINIT_INFO_NAME])
   for entry in sorted_entry:
      # defines a unique entry
      unique_entry = ''
      if entry[RCINIT_INFO_NAME] != None :
         unique_entry += entry[RCINIT_INFO_NAME]
      if entry[RCINIT_INFO_TYPE] != None :
         unique_entry += ','+entry[RCINIT_INFO_TYPE]
      if entry[RCINIT_INFO_API] != None :
         unique_entry += ','+entry[RCINIT_INFO_API] 
      if entry[RCINIT_INFO_ENTRY] != None :
         unique_entry += ','+entry[RCINIT_INFO_ENTRY]
      if entry[RCINIT_INFO_GROUP] != None :
         unique_entry += ','+entry[RCINIT_INFO_GROUP]
      if entry[RCINIT_INFO_PRIORITY] != None :
         unique_entry += ','+entry[RCINIT_INFO_PRIORITY]
      if entry[RCINIT_INFO_GROUP] != None :
         unique_entry += ','+entry[RCINIT_INFO_PRIORITY_ALIAS]
