...
from sectools.features.isc.advanced_defines import PLATFORM_BINDING_INDEPENDENT, PLATFORM_BINDINGS
...
from sectools.features.isc.signer.qti_remote_signer import ENV_SIGNING_SERVER_HOST_TAG, ENV_SIGNING_SERVER_PORT_TAG

# Name & version of the tool
SECTOOLS_BUILDER_TOOL_NAME = 'Sectools Builder'
SECTOOLS_BUILDER_TOOL_VERSION = 'v3.2'
...
class BuilderInputPackage(object):
    ...
    def __init__(self,
        ...
        self.verbatim_config = convert_to_bool(verbatim_config)
    ...
    def __str__(self):
        ...
        return "~~~~~~~~~~~~~~~BuilderInputPackage~~~~~~~~~~~~~~~" + "\n" + \
               ...
               "port: " + to_string(self.port) + "\n" + \
               "host: " + to_string(self.host) + "\n" + \
               "serial_number: " + to_string(self.serial_number) + "\n" + \
               "verbatim_config: " + to_string(self.verbatim_config) + "\n" + \
               "platform_binding: " + to_string(self.platform_binding) + "\n" + \
               "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
def build(is_scons_build, environment, **kwargs):
    ...
    # builder was called from the command line
        ...
        if not args.port and ENV_SIGNING_SERVER_PORT_TAG in os.environ:
            args.port = os.environ[ENV_SIGNING_SERVER_PORT_TAG]
        if not args.host and ENV_SIGNING_SERVER_HOST_TAG in os.environ:
            args.host = os.environ[ENV_SIGNING_SERVER_HOST_TAG]
...
class BuilderParser(CoreOptionParser):
    ...
    def c_add_options(self):
        ...
        self.add_option('--host', metavar='<str>', default=None, help="The hostname of the server to connect to for remote signing.")
        self.add_option('--port', metavar='<int>', default=None, help="The port of the server to connect to for remote signing.")
        self.add_option("--serial_number", metavar="<str>", default=None, help="Value to override config file's serial_number value.")
        self.add_option("--verbatim_config", action="store_true", help=SUPPRESS_HELP)
        self.add_option("--platform_binding", metavar="<" + comma_separated_string(PLATFORM_BINDINGS) + ">",
                        dest="platform_binding", help="Specifies whether image should be bound to JTAG ID (msm_part), "
                                                      "SOC HW Version (soc_hw_version/soc_vers), "
                                                      "or neither. " + PLATFORM_BINDING_INDEPENDENT + " only applies "
...
"""
    ...
    environment, target_base_dir, source, sign_id, jtag_id, soc_hw_version, app_id, sectools_install_base_dir,
    install_file_name, config, pilsplitter_target_base_dir, image_entry, relocatable, soc_vers, signer, qti_sign,
    uie_qti, target_image_type_filter, hash_table_algo, signature_digest_algo, num_root_certs, max_num_root_certs,
    selected_cert_config, config_type, client_id, lib_id, multi_serial_numbers, port, host, serial_number, and
    platform_binding are utilized by builder. The remaining parameters are ignored and are only present to preserve
    backwards compatibility with build scripts using previous versions of Sectools Builder.

    target_image_type_filter allows target filtering so only images of the specified type will be present in the
    returned target list. This has no effect on which files are actually generated. That images which are generated is
    still controlled by build policy and additional secimage policies.

    config_type should be used instead of config. config_type allows the user to specify an integration config
    file without needing to provide its file path.

    The port and host params can be utilized if multiple instances of the Builder need to executed simultaneously
    but different signing servers need to be used for signing various images.  Port and host only effect signing
    if the signer is set to "qti_remote".
...
def register_build_method(environment,
    ...
    if not port and ENV_SIGNING_SERVER_PORT_TAG in os.environ:
        port = os.environ[ENV_SIGNING_SERVER_PORT_TAG]
    if not host and ENV_SIGNING_SERVER_HOST_TAG in os.environ:
