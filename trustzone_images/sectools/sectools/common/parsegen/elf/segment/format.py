    ...
    for idx in range(elf_header.e_phnum):
        ...
        if len(program_header_data) < header_class.get_size():
            raise RuntimeError('Data length "' + str(len(program_header_data)) + '" is less than ' +
                               str(ELFCLASS_DESCRIPTION[elf_header.e_ident_class]) +
                               ' Elf Program Header length "' + str(header_class.get_size()) + '"')
...
def repr_phdrs(phdrs, sno_range=None):
    ...
    for idx, phdr in enumerate(phdrs):
        ...
        retval.insert_data(idx+1, COLUMN_TYPE, str(PT_DESCRIPTION.get(phdr.p_type, phdr.p_type)), justify=retval.LEFT)
        ...
        retval.insert_data(idx+1, COLUMN_FLAGS, PF_PERM_DESCRIPTION.get(phdr.f_perm, hex_addr(phdr.p_flags, stats[COLUMN_FLAGS])))
