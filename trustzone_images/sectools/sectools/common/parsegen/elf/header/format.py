...
from sectools.common.parsegen.elf.header import ident
...
class Elf_Ehdr_Common(StructBase):
    ...
    def validate(self):
        ...
        if self.e_ident_mag not in ident.ELFMAG_DESCRIPTION.keys():
    ...
    def _repr_properties(self):
        properties = [
                      (ident.ELFMAG_STRING, ident.ELFMAG_DESCRIPTION, ensure_str(self.e_ident_mag)),
                      (ident.ELFCLASS_STRING, ident.ELFCLASS_DESCRIPTION, self.e_ident_class),
                      (ident.ELFDATA_STRING, ident.ELFDATA_DESCRIPTION, self.e_ident_data),
                      (ident.ELFVER_STRING, ident.ELFVER_DESCRIPTION, self.e_ident_ver),
                      (ident.ELFOSABI_STRING, ident.ELFOSABI_DESCRIPTION, self.e_ident_osabi),
                      (ident.ELFABIVER_STRING, ident.ELFABIVER_DESCRIPTION, self.e_ident_abiver),
...
ELF_HDRS = {
                ident.ELFCLASS32 : Elf32_Ehdr,
                ident.ELFCLASS64 : Elf64_Ehdr,
           }


def get_ehdr(_class):
    header_class = ELF_HDRS.get(_class, None)
    if header_class is None:
        supported_classes = [str(key) + '-' + str(ident.ELFCLASS_DESCRIPTION[key]) for key in ELF_HDRS.keys()]
        raise RuntimeError('File contains invalid class: ' + str(_class) + '\n'
...
def extract_ehdr(data):
    ...
    if len(data) < Elf_Ehdr_Common.get_size():
        raise RuntimeError('Data length "' + str(len(data)) +
                           '" is less than common Elf Header length "' +
                           Elf_Ehdr_Common.get_size() + '"')

    # Get the appropriate elf header class
    header = Elf_Ehdr_Common(data)
    header_class = get_ehdr(header.e_ident_class)
    ...
    if len(data) < header_class.get_size():
        raise RuntimeError('Data length "' + str(len(data)) + '" is less than ' +
                           str(ident.ELFCLASS_DESCRIPTION[header.e_ident_class]) +
                           ' Elf Header length "' + header_class.get_size() + '"')

    # Extract the header
    header = header_class(data)
    return header

def create_empty_ehdr(_class):
    header_class = get_ehdr(_class)
    header = header_class()

    from sectools.common.parsegen.elf import header as hdr
    header.e_ident_mag = hdr.ident.ELFMAG
    header.e_ident_class = _class
    header.e_ident_data = hdr.ident.ELFDATA2LSB
    header.e_ident_ver = hdr.ident.ELFVERCURRENT
    header.e_ident_osabi = hdr.ident.ELFOSABI_NONE
    header.e_type = hdr.type.ET_NONE
    header.e_machine = hdr.machine.EM_ARM
    header.e_version = hdr.version.EV_CURRENT
    header.e_phoff = header.get_size()
    header.e_ehsize = header.get_size()

    header = header_class(header.pack())
    return header
