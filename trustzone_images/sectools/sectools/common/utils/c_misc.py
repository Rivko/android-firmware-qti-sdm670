...
from past.builtins import long, xrange, basestring
...
from six import ensure_binary, PY2, int2byte
...
def store_data_to_file(file_path, data):
    ...
    if (isinstance(data, bytes)):
        ...
        raise RuntimeError('Data to write must be of byte-string type')
...
def hexdump(src, length=16):
    if PY2:
        FILTER = ''.join([(len(repr(int2byte(x))) == 3) and int2byte(x) or '.' for x in range(256)])
    else:
        FILTER = ''.join([(len(repr(int2byte(x))) == 4) and int2byte(x).decode('utf-8') or '.' for x in range(256)])
        src = ensure_binary(src)
    lines = []
    for c in xrange(0, len(src), length):
        chars = src[c:c+length]
        hex = ' '.join(["%02x" % ensure_int(x) for x in chars])
        printable = ''.join(["%s" % ((ensure_int(x) <= 127 and FILTER[ensure_int(x)]) or '.') for x in chars])
