...
class DictHandler(BaseHandler):
    ...
    def __init__(self, data_handler):
        ...
        self._i_detailer = DictDetailer(self)
    ...
    @property
    def _detailer(self):
        """Returns the :class:`DictDetailer` object for the data type."""
...
class DictValidator(BaseValidator):
    ...
    def validate(self, i_value, i_format):
        ...
        try:
            ...
            err = "{}{}\n{}".format(
                      ('' if attr_key is None else (attr_key + '->')),
                      str(e),
...
class DictTransferMgr(BaseTransferMgr):
    ...
    def transfer(self, i_value_from, i_value_to, i_structure, i_structure_parent):
        ...
        try:
            ...
            err = '{}{}\n{}'.format(
                ('' if i_structure_key is None else (i_structure_key + '->')),
                str(e),
                traceback.format_exc())
            raise type(e)(type(e)(err))
...
class DictCodeGenerator(BaseCodeGenerator):
    ...
    def generate_code(self, attr_key, attr_value, _classes=None):
        """See :meth:`.Data_Handler.BaseCodeGenerator.generate_code`"""
        return_val = ''

        # Initial _classes to empty dictionary
        if _classes is None:
            _classes = {}
                ...
                for choice, choice_format in choice_group.items():
                    if choice in attr_value.keys():
                        if choice_format != attr_value[choice]:
                            raise RuntimeError(
                                'Choice key "' + choice + '" already exists in the dictionary under: "' +
                                str(attr_key) + '"')
                    else:
...
class DictDetailer(BaseDetailer):
    """Detailer for Dict data.
    ...
    def detail(self, i_format):
        ...
        detail = DataDetail(DataType.Complex, self.data_handler, i_format)
        detail.children = {}

        # Check if the i_value is optional
        if DictHandlerAttr_Optional in dict_attrs:
            detail.is_optional = True
        ...
        try:
            ...
            for i_format_key, i_format_value in i_format.items():
                ...
                # Update the value using the normalized value
                detail.children[i_format_key] = self.generic_detail(i_format_value)

        except Exception as e:
            err = "{}{}\n{}".format(
                ('' if i_format_key is None else (i_format_key + '->')),
                str(e),
                traceback.format_exc())
            raise type(e)(type(e)(err))
