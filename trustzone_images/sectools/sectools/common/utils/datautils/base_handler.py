...
class BaseHandler(object):
    ...
    @property
    def _detailer(self):
        """Returns the :class:`BaseDetailer` object for the data type."""
        raise NotImplementedError('Data handler: "' + self.__class__.__name__ + '" does not support data detailing.')
    ...
    def _detailer(self, value):
        raise RuntimeError('Detailer must not be set directly for data handler: ' + self.__class__.__name__)
    ...
    def detail(self, *args, **kwargs):
        """Implements the detailing logic for the type of data supported
        by this class. Used by :meth:`.DataHandler.detail`."""
        return self._detailer.detail(*args, **kwargs)
...
class BaseNormalizer(with_metaclass(abc.ABCMeta, BaseFeature)):
...
class BaseValidator(with_metaclass(abc.ABCMeta, BaseFeature)):
...
class BaseTransferMgr(with_metaclass(abc.ABCMeta, BaseFeature)):
...
class BaseCodeGenerator(with_metaclass(abc.ABCMeta, BaseFeature)):
...
class BaseDetailer(with_metaclass(abc.ABCMeta, BaseFeature)):
    """Abstract Detailer base class to be used by the type-specific
    detailers.

    :params: Same as :class:`BaseFeature`.
    """

    def get_detailer(self, i_format):
        """Gets the detailer corresponding to the i_format"""
        return self._generic_get_data_handler(i_format)._detailer

    def generic_detail(self, *args, **kwargs):
        """Calls :meth:`.DataHandler.detail`."""
        return self.data_handler.data_handler.detail(*args, **kwargs)

    def generic_detailer(self, *args, **kwargs):
        """Calls :meth:`.DataHandler.detailer`."""
        return self.data_handler.data_handler.detailer(*args, **kwargs)

    @abc.abstractmethod
    def detail(self, i_format):
        """See :meth:`.DataHandler.detail`

        .. note::
            This method must be implemented by all derived classes.
        """
        raise NotImplementedError('Detailer ' + str(self.__class__.__name__) + ' must implement detail')
