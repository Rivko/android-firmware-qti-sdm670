...
def nice_string_to_binary(data, sep=':'):
    return b''.join([int2byte(int(c, 16)) for c in sep.join(data.strip().split('\n')).split(sep)])
...
def remove_duplicates_ordered(*attributes):
    """ Get a unique list of attributes.

    Remove the duplicates, None, and 0, while maintaining order.

    Input arguments:
    attributes: a list of objects of any type

    Return value:
        A list of unique non-zero attributes.
    """
    unique_attributes = []
...
def comma_separated_string(input_list, final_separator="or"):
    """
    ...
    if len(str_list) == 1:
        ...
        return (" " + final_separator + " ").join(str_list)
    else:
        return ", ".join(str_list[:-1]) + ", " + final_separator + " " + str_list[-1]
