...
def run_command(cmd, expected_retcode=0, large_output=False, log=True,
                error_prefix=True, ignore_retcode=False, ret_binary=True):
    if log:
    ...
    if large_output:
        try:
            ...
            stderr_data = output = e.output
            retcode = e.returncode
    else:
        retcode, output, stderr_data = CoreSubprocess.simpleExecuteCommand(cmd)

    if not ignore_retcode and retcode != expected_retcode:
        if error_prefix:
            error_message = cmd[0] + ' retcode: ' + str(retcode) + '\nOutput: ' + str(stderr_data)
        else:
            error_message = stderr_data
        raise RuntimeError(error_message)

    return output if ret_binary else ensure_str(output)
        ...
        try:
            ...
            try:
                ...
                while True:
                    ...
                    end_time = time.time()
                    if ((not communication.is_alive()) or ((self.timeout is not None) and ((end_time - start_time) > self.timeout))):
                        break
                if communication.is_alive():
                    self._toolTimedout = True
                    self.killProcess(process)
                    communication.join(2)
                    assert communication.is_alive() == False
