...
from sectools.common.utils.c_misc import is_TA, is_cmnlib
...
class BaseCASSSigningRequest(SecPluginIntf_Abs):
    ...
    def get_secboot_version(cls):
        """
        :return: Secboot version that is supported by this class
        """
        raise NotImplementedError("Plugin CASS signing request class must implement get_secboot_version.")

    def __init__(self, hash_to_sign, data_to_sign, image_path, signing_attributes):
        self.description = "image_to_sign={0}".format(image_path)
        self.digest = binascii.b2a_hex(hash_to_sign)
        ...
        self._init_attribute_dict(hash_to_sign, signing_attributes)
    ...
    def _apply_attribute_additions(self):
        if self.attributes_to_add and len(self.attributes_to_add) > 0:
            for key, value in self.attributes_to_add.items():
    ...
    def convert_to_xml(self):
        ...
        if len(self.attribute_dict) > 0:
            ...
            for key in self.attribute_dict:
                ...
                attr.text = ensure_str(self.attribute_dict[key])
        digest = SubElement(sr, 'Digest')
        digest.text = ensure_str(self.digest)
        return sr
