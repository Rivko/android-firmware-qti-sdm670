...
from sectools.common.utils.c_path import store_data_to_file, copy_file
...
from sectools.features.isc.secimage import secimage
...
from sectools.features.isc.secimage.secimage_cfgparser import secimage_cfgparser
...
def launch_secimage(chipset=None, output_dir=None, sign_id=None,
                    ...
                    debug=False, quiet=False, cfg_params=None, config=None, authority=None):
    ...
    if cfg_params is not None:
        for cfg_tag, cfg_value in cfg_params.items():
            if cfg_value is not None:
                args += ['--cfg_' + cfg_tag, str(cfg_value)]
    if authority is not None and authority == AUTHORITY_QTI:
        args += ['--qti_signing']

    # Launch secimage
    parsed_args = secimage.parse_args(args)
    return secimage.main(parsed_args, return_isc=return_isc)[1]

def update_config(chipset, update_cb, file_path):
    """Helps update the secimage config file
    """
    cfg_dir = cfgparser.ConfigDir(TOP_DIR, cfgparser=secimage_cfgparser())
    cfg_path = cfg_dir.get_chipset_config_path(chipset)
    cfg = cfgparser.ConfigParser(cfg_path, cfgparser=secimage_cfgparser())
    update_cb(cfg)
...
def generatesigned(chipset, output_dir, sig_package, sign_id=None,
    ...
    if (sig_package != sig_package_exp):
        ...
        ret, err = copy_file(sig_package, sig_package_exp)
        if not ret:
            raise RuntimeError(err)
...
def validateimage(chipset, sign_id=None, root_cert_hash=None, imagefile=None,
    ...
    try:
        ...
        if il[0].status.validate_sign.state != StatusInfo.SUCCESS:
            retcode = 1
            errstr = 'Image signing validation failed. ' + il[0].status.validate_sign.error
