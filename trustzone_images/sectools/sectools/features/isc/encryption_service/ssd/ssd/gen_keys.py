...
# place ext in the sys path so we can resolve the 'six' library
CUR_DIR = os.path.dirname(os.path.realpath(__file__))
TOP_LEVEL_DIR = os.path.abspath(os.path.join(CUR_DIR, '..', '..', '..', '..', '..', '..'))
sys.path.insert(1, os.path.join(TOP_LEVEL_DIR, 'ext'))

from six import ensure_str

packaged_openssl_binary_path = os.path.join(TOP_LEVEL_DIR, 'bin', 'WIN', 'openssl.exe')
if sys.platform.startswith('linux'):
    packaged_openssl_binary_path = ensure_str(
        Popen(["which", "openssl"], stdout=PIPE).communicate()[0].strip())
