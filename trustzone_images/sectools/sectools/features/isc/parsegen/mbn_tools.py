...
sys.path.insert(1, os.path.abspath(os.path.join(CUR_DIR, '..', '..', '..', '..')))
...
def create_elf_header( output_file_name,
   ...
   if (is_elf_64_bit is True):
      ...
      elf_fp.write(b''.rjust(9, int2byte(ELFINFO_RESERVED)))
      ...
      elf_fp.write(b''.rjust(4, int2byte(ELFINFO_RESERVED)))
      ...
      elf_fp.write(b''.rjust(6, int2byte(ELFINFO_RESERVED)))
...
def generate_code_hash(env, elf_in_file_name):
   ...
   for i in range(elf_header.e_phnum):
      ...
      if (curr_phdr.p_align == 8 and
         ...
         hash_data = struct.unpack("I" * int(curr_phdr.p_filesz / 4), hash_data)
   ...
   code_seg_idx = -1
   code_seg_pages = int(hash_phdr.p_filesz / DP_HASH_SIZE)

   for i in range(elf_header.e_phnum):
      curr_phdr = phdr_table[i]
      curr_pages = int((curr_phdr.p_filesz + DP_PAGE_SIZE - 1) / DP_PAGE_SIZE)
   ...
   while (bytes_left > 0):
      ...
      while (len(page) < bytes_in_page):
         page = page + bytes(elf_in_fp.read(bytes_in_page - len(page)))
...
def getSegmentFlag(seg_info):
   ...
   if seg_info is None or len(seg_info) is 0:
      raise RuntimeError('Invalid segment information passed: ' + str(seg_info))
...
def generate_global_dict(env):
   ...
   cust_file_name = find_file(env, cust_h, "${INC_ROOT}/build/ms")
   targ_file_name = find_file(env, targ_h, "${INC_ROOT}/build/ms")
...
#----------------------------------------------------------------------------
# Non-memoized replacement for env.FindFile
#----------------------------------------------------------------------------
def find_file(env, file_name, dirs):
    file_name = env.subst(file_name)
    dirs = env.Flatten(dirs)
    for dir_name in dirs:
        dir_name = env.subst(dir_name)
        candidate = os.path.abspath(os.path.normpath(os.path.join(dir_name, file_name)))
        if os.path.isfile(candidate):
            return str(env.File(candidate).abspath)

    raise IOError("Required file '{}' not found".format(file_name))
...
def insert_SBL1_magicCookie (env, target):
    ...
    if (length <= VIRTUAL_BLOCK_SIZE):
        ...
        mc1 = int2byte(int(a[8:10],16)) + int2byte(int(a[6:8],16)) + int2byte(int(a[4:6],16)) + int2byte(int(a[2:4],16))

        b = str(hex(MAGIC_NUM))
        mc2 = int2byte(int(b[8:10],16)) + int2byte(int(b[6:8],16)) + int2byte(int(b[4:6],16)) + int2byte(int(b[2:4],16))

        c = str(hex(SBL_VIRTUAL_BLOCK_MAGIC_NUM))
        mc3 = int2byte(int(c[8:10],16)) + int2byte(int(c[6:8],16)) + int2byte(int(c[4:6],16)) + int2byte(int(c[2:4],16))
