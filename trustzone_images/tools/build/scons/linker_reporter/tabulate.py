...
def tabulate(tabular_data, headers=[], tablefmt="simple",
    ...
    return _format_table(tablefmt, headers, rows, minwidths, aligns, subtotal)
...
def _format_table(fmt, headers, rows, colwidths, colaligns, subtotal):
    ...
    if padded_rows and fmt.linebetweenrows and "linebetweenrows" not in hidden:
        ...
        for row in padded_rows[:-(sub_items + 1)]:
        ...
        lines.append(_build_row(padded_rows[-(sub_items + 1)], padded_widths, colaligns, fmt.datarow))
    elif sub_items > 0:
        for row in padded_rows[:-sub_items]:
            lines.append(_build_row(row, padded_widths, colaligns, fmt.datarow))
    else:
    ...
    lines.append('')
    lines.append(_build_line(padded_widths, colaligns, fmt.linebelow).replace(' ', '-'))

    # Adds a subtotal section at the end of the main table
    if subtotal is not None:
        for row in padded_rows[-sub_items:]:
            lines.append(_build_row(row, padded_widths, colaligns, fmt.datarow))
        lines.append('')
