...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 09/02/16   vk      Fix KlockWork warnings
 07/13/15   al      Changing TPL_NOTIFY to TPL_CALLBACK
 08/07/14   vk      Fix LLVM warning
 02/19/13   vk      Fix warning
 01/09/13   vk      Fix KlockWork warnings
 06/08/12   jz      Added SimpleTextIn Ex support, fixed Esc key handling
=============================================================================*/
  ...
  )
{
  ...
  BytesRead = SerialPortRead ((UINT8 *)&Char, 1);
  if (BytesRead == 0) {
    return EFI_NOT_READY;
  }
...
/* EfiSimpleTextInputExProtocol wrapper */
EFI_STATUS
EFIAPI
TextInResetEx (
  IN EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL  *This,
  ...
  )
{
  ...
  //
  if ((UINTN) ((CHAR16 *) Destination -  Source) < StrLen (Source)) {
    ASSERT (FALSE);
    return NULL;
  }

  if((UINTN) ((CHAR8 *) Source - Destination) < StrLen (Source)) {
    ASSERT (FALSE);
    return NULL;
  }
...
{
  /* Change supported rows and columns to be defined by PCD */
  UINT32 NumCols = PcdGet32 (PcdConOutColumn);
  UINT32 NumRows = PcdGet32 (PcdConOutRow);
