...
USB_KEYBOARD_LAYOUT_PACK_BIN  mUsbKeyboardLayoutBin = {
  ...
  USB_KEYBOARD_KEY_COUNT, // DescriptorCount
  {
    ...
    //EfiKeyC12 fix for TG3 keyboard that returns 0x32 for "\" and "|" instead of 0x31.
    //USB HID spec says: 0x31 Keyboard \ and |.
    // 0x32 Keyboard Non-US # and ~. (Footnote: Typical language mappings: US: \|)
...
UINT8 ModifierValueToEfiScanCodeConvertionTable[] = {
  ...
  SCAN_HOME,       // EFI_LEFT_LOGO_MODIFIER. Map the SCAN_HOME to left Windows key
  SCAN_HOME,       // EFI_RIGHT_LOGO_MODIFER. Map the SCAN_HOME to right Windows key
  ...
  )
{
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
    ...
    USB_ASSERT_RETURN ((KeyboardLayout != NULL), NULL);
  ...
  )
{
  ...
  USB_ASSERT (UsbKeyboardDevice->KeyConvertionTable != NULL);
  ...
  )
{
  ...
  UsbKeyboardDevice->KeyConvertionTable = AllocateZeroPool ((NUMBER_OF_VALID_USB_KEYCODE) * sizeof (EFI_KEY_DESCRIPTOR));
  USB_ASSERT_RETURN ((UsbKeyboardDevice->KeyConvertionTable != NULL), EFI_OUT_OF_RESOURCES);
  ...
  )
{
  ...
  USB_ASSERT (Queue->Buffer[0] != NULL);
...
{
  USB_ASSERT (ItemSize == Queue->ItemSize);
  //
...
{
  USB_ASSERT_RETURN ((Queue->ItemSize == ItemSize), EFI_DEVICE_ERROR);
