...
/*=============================================================================
 ...
 01/20/14   ck       Port EDKII KB change
 08/10/12   cching   Add few changes on top of Windows key fix
 08/09/12   mliss    Windows key fix to return the desired KeyShiftState
  ...
  )
{
  ...
  USB_ASSERT_GOTO ((UsbKeyboardDevice != NULL), ErrorExit1);
...
{
  USB_ASSERT_RETURN ((RegsiteredData != NULL && InputData != NULL), FALSE);
