...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/06/17   jb      Fix mask error on addresses >4GB
 12/05/14   bh      Security fix for integer overflow
 11/30/14   bh      fix 64-bit compiler errors
 10/14/14   tj		Kw Fix 
 03/13/12   niting  Cleanup
  ...
  while ((Ptr->Length != 0) || (Ptr->Union.ContinuationPointer != (EFI_PHYSICAL_ADDRESS) (UINTN) NULL)) {
    ...
    if (Ptr->Length == 0) {
      ...
      if (CapsuleSize == 0) {
        ...
        //Add this subtract, we must account for the first Block descriptor's length.
        CapsuleSize = CapsuleHeader->CapsuleImageSize - Ptr->Length;
      } else {
  ...
  while (TempBlockDesc->Union.ContinuationPointer != (EFI_PHYSICAL_ADDRESS) (UINTN) NULL) {
    if (TempBlockDesc->Length == 0) {
      ...
      if (IsOverlapped (
        ...
        DEBUG ((EFI_D_INFO, "Capsule relocate descriptors from/to/size  0x%llx 0x%8x 0x%llx\n", 
                TempBlockDesc->Union.DataBlock, RelocBuffer, TempBlockDesc->Length));
      }
    }
    TempBlockDesc++;
  }
  ...
  )
{
  ...
  if ((BlockListBuffer == NULL) || (BlockDescriptorList == NULL))
  {
  ...
  while (BlockListBuffer[Index] != 0) {
    if (Index == 0) {
      ...
      //
      //Because this function strings together every scattergatherlist
      //  in our ScatterGatherList Array, we should clear all 
      //  but the first element in the ScatterGatherList array.  
      //  
      //  If we were to call this function multiple times on the same
      //  scattergatherlist array without clearing out all but the first
      //  element, this function would create a circular ScatterGather list.
      //       
      BlockListBuffer[Index] = (EFI_PHYSICAL_ADDRESS)(UINTN) 0;
    }
    Index ++;
  }
  ...
  )
{
  ...
  UINT8                          *DestPtr;
  UINT8                          *RelocPtr;
  UINT64                         CapsuleOffset[MAX_SUPPORT_CAPSULE_NUM]; 
  UINT64                         *AddDataPtr;
  UINT32                         CapsuleTimes; 
  UINT64                         SizeLeft; 
  ...
  if (CapsuleSize >= (MAX_ADDRESS - (sizeof (EFI_CAPSULE_PEIM_PRIVATE_DATA) + sizeof(CapsuleOffset) + sizeof(UINT64))))
  ...
  //
  NumDescriptors  += 3;
  CapsuleSize     += sizeof (EFI_CAPSULE_PEIM_PRIVATE_DATA) + sizeof(CapsuleOffset) + sizeof(UINT64);
  ...
  //
  DEBUG ((EFI_D_INFO, "Capsule Memory range from 0x%8x to 0x%8x\n", *MemoryBase, (UINTN)*MemoryBase + *MemorySize));
  if ((UINTN)*MemoryBase < (UINTN) MIN_COALESCE_ADDR) {
    if (((UINTN)*MemoryBase + *MemorySize) < (UINTN) MIN_COALESCE_ADDR) {

      //Return Size of Capsule-related data, plus one page.
      MemorySizeInPages = EFI_SIZE_TO_PAGES(CapsuleSize + DescriptorsSize)+1;
      *MemorySize = EFI_PAGES_TO_SIZE(MemorySizeInPages);
      return EFI_BUFFER_TOO_SMALL;
    } else {
  ...
  while ((CurrentBlockDesc->Length != 0) || (CurrentBlockDesc->Union.ContinuationPointer != (EFI_PHYSICAL_ADDRESS) (UINTN) NULL)) {
    ...
    while (TempBlockDesc->Length != 0) {
      ...
      if (IsOverlapped (
        ...
        DEBUG ((EFI_D_INFO, "Capsule reloc data block from 0x%llx to 0x%8x with size 0x%llx\n",
                TempBlockDesc->Union.DataBlock, RelocPtr, TempBlockDesc->Length));
    ...
    if (CapsuleTimes > 1) {
      ...
      if (CapsuleBeginFlag) {
        ...
        if (!IsCapsuleCorrupted (CapsuleHeader)) {
          ...
          DEBUG ((EFI_D_INFO, "Capsule coalesce block no.0x%8x from 0x%llx to 0x%8x with size 0x%llx\n",
                  CapsuleTimes, CurrentBlockDesc->Union.DataBlock, DestPtr, CurrentBlockDesc->Length));
          //
          // Cache the begin offset of this capsule
          //
          CapsuleOffset[CapsuleIndex++] = (UINT64) (UINTN) DestPtr - (UINT64)(UINTN)NewCapsuleBase - (UINT64)sizeof(EFI_CAPSULE_PEIM_PRIVATE_DATA);
  ...
  *MemoryBase = (VOID *) NewCapsuleBase;
  ...
  AddDataPtr = (UINT64*)(((UINTN) DataPtr + sizeof(UINT64) - 1) &~ (UINTN) (sizeof (UINT64) - 1));

  *AddDataPtr++ = CapsuleIndex;

  CopyMem (AddDataPtr, &CapsuleOffset[0], sizeof (UINT64) * CapsuleIndex);
