  ...
  )
{
  ...
  // Vbus Check and Enable would be done in ConfigUSB protocol function, logic to be decided
  // Make sure VBUS is on
  if (Xhc->UsbConfigProtocol) {
    Status = Xhc->UsbConfigProtocol->GetUsbVbusStatus (Xhc->UsbConfigProtocol, Xhc->CoreNum, &VbusSt);
    if (Status == EFI_UNSUPPORTED) {
      DEBUG ((EFI_D_INFO, "Vbus Status return not supported %r\n", Status));
    }
    else if(EFI_ERROR(Status)) {
      DEBUG ((EFI_D_ERROR, "PmicUsbProtocol->GetOtgStatus %r\n", Status));
    }
    else {
      if ((VBUS_STATUS_TYPE)VbusSt == VBUS_STATUS_ENABLED) {
        // reset OTG enablement count to zero
        OtgEnMsgCnt = 0;
      }
      else {
        Status = Xhc->UsbConfigProtocol->UsbEnableVbus (Xhc->UsbConfigProtocol, Xhc->CoreNum);
        if (Status == EFI_UNSUPPORTED) {
          DEBUG ((EFI_D_INFO, "Vbus Status enablement not supported %r\n", Status));
        }
        else {
  ...
  )
{
  ...
  Status = Xhc->PciIo->GetLocation(Xhc->PciIo, NULL, NULL, &DeviceNum, NULL);
  if (EFI_ERROR(Status)) {
    DeviceNum = (UINTN)-1;
  }
