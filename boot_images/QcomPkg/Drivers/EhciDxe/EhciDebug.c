...
/*=============================================================================
 ...
 04/27/15   ck       Fix incorrect data printing
 04/30/14   amitg    Fix Compiler warnings
  ...
  )
{
  QTD_HW                  *QtdHw;
  UINTN                   Index;
  //UINTN                   Count; //CK-DF Disable access to Page buffer since this is SMMU mapped
  CHAR8                   *pData;

  if (Msg != NULL) {
    DEBUG ((EFI_D_VERBOSE, Msg));
  }
  (void)pData;  // Make compiler happy
  ...
  for (Index = 0; Index < 5; Index++) {
    ...
    //CK-DF Disable access to Page buffer since this is SMMU mapped
    /*if(Index == 0 && QtdHw->TotalBytes) {
      pData = (VOID*) ((UINTN) (QtdHw->Page[Index]));
      DEBUG ((EFI_D_VERBOSE, " "));
      // Output the 64 bytes only to the terminal window
      for (Count = 0; Count < 64; Count++ )
        DEBUG ((EFI_D_VERBOSE, "%02x ", *pData++ & 0xFF));
      DEBUG ((EFI_D_VERBOSE, "\n"));
    }*/
  }
}
  ...
  )
{
  ...
  for (Index = 0; Index < 5; Index++) {
    ...
    //CK-DF Disable access to Page buffer since this is SMMU mapped
    /*if(Index == 0 && QhHw->TotalBytes) {
      pData = (VOID*) ((UINTN) QhHw->Page[Index]);
      DEBUG ((EFI_D_VERBOSE, " "));
      for (Count = 0; Count < QhHw->TotalBytes; Count++ )
        DEBUG ((EFI_D_VERBOSE, "%02x ", *pData++ & 0xFF));
      DEBUG ((EFI_D_VERBOSE, "\n"));
    }*/
  }
