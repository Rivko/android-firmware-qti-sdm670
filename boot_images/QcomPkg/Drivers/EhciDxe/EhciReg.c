...
Portions Copyright (c) 2007 - 2009, Intel Corporation. All rights reserved.<BR>
  ...
  )
{
  ...
  USB_ASSERT_GOTO_SETSTS ((Ehc->CapLen != 0), Exit, EFI_INVALID_PARAMETER, Status);
...
Exit:
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "EhcReadOpReg: Error - %r at %d\n", Status, Offset));
    Data = 0xFFFF;
  }
...
Exit:
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "EhcWriteOpReg: Error: %r at %d\n", Status, Offset));
  }
}
  ...
  )
{
  ...
  // Reset core
  //
  Status = Ehc->PciIo->GetLocation (Ehc->PciIo, &Segment, &Bus, &DevCoreNum, &Function);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  DEBUG ((EFI_D_INFO, "ConfigUsb::EhcResetHC, configuration type  x%08x\r\n",coreType));

  if (coreType != USB_CONFIG_HSIC)
  {
      DEBUG ((EFI_D_INFO, "NOT USB CONFIG HSIC\r\n"));
    ...
    DEBUG ((EFI_D_INFO, "IS USB CONFIG HSIC\r\n"));
  }

  Status = Ehc->UsbConfig->ResetUsb (Ehc->UsbConfig, coreType, FALSE);
  ...
  )
{
  ...
  // 0. Before enable the controller, ensure completion of memory access
  // since the qtd ahd qh have been initialized previously in EhcInitSched
