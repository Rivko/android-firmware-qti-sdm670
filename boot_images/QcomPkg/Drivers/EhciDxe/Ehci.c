...
// Async Poll time out retrieved from the client
UINTN gAsyncBulkPollInterval;
UINTN gAsyncControlPollInterval;


// For use in Usb port test in EchiUrb.c
INTN UsbPortTestIndex = USB_PORT_TEST_VAR_UNDEFINED;
...
/**
  Sets the timer for checking the completion of Async Bulk transfer URB
  ...
  )
{
    ...
    Status = gBS->SetTimer (Ehc->CheckAsyncBulkInTimer, TimerRelative, PollTimer * EHC_1_MILLISECOND_100NS);
  ...
  )
{
    ...
    Status = gBS->SetTimer (Ehc->CheckAsyncControlTimer, TimerRelative, PollTimer * EHC_1_MILLISECOND_100NS);
...
/**
  Retrieve USB_PORT_TEST_PARAM_QTD_INDEX for USB port testing.

  @param [in]  Ehc      EHCI device instance
...
**/
INTN
EhcCheckPortTestVariable (
  IN USB2_HC_DEV            *Ehc
  )
{
  EFI_STATUS  Status;
  INTN        Var = USB_PORT_TEST_VAR_UNDEFINED;

  if (NULL == Ehc) {
    return USB_PORT_TEST_VAR_UNDEFINED;
  }

  Status = Ehc->UsbPortTest.GetParam(
            &Ehc->UsbPortTest,
  ...
  )
{
  ...
  // update the port test index
  UsbPortTestIndex = EhcCheckPortTestVariable (Ehc);
  ...
  )
{
  ...
  portsc = (State & PORTSC_PSPD_BMSK) >> PORTSC_PSPD_SHFT;

  //default is EFI_USB_SPEED_FULL (== 0)
  if (portsc == EFI_USB_SPEED_LOW) {
    PortStatus->PortStatus |= USB_PORT_STAT_LOW_SPEED;
  } else if (portsc == EFI_USB_SPEED_HIGH) {
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  if (Data) {
    WriteBackInvalidateDataCacheRange(Data, *DataLength); // ensure the HW sees the fresh data
  }
  if (Request) {
    WriteBackInvalidateDataCacheRange(Request, sizeof (EFI_USB_DEVICE_REQUEST));
  }
  ...
  if (Data && TransferDirection == EfiUsbDataIn) {
    WriteBackInvalidateDataCacheRange(Data, *DataLength); // ensure the HW sees the fresh data
  }
  ArmDataMemoryBarrier();
...
/**
  Submits control transfer to a control endpoint of a USB device.
  ...
  )
{
  ...
  // with the polling function EhcCheckAsyncControlResult()
  ...
  if (Urb == NULL) {
    DEBUG ((EFI_D_ERROR, "EhcAsyncControlTransfer: failed to create URB\n"));
  ...
  InsertHeadList (&Ehc->AsyncControlTransfers, &Urb->UrbList);

  // Start the timer to check for completion
  Status = EhcAsyncControlSetTimer(Ehc, PollingInterval);

  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Cannot Create CheckAsyncControlTimer\n"));
    EhcUnlinkQhFromAsync (Ehc, Urb->Qh);
  ...
  ON_EXIT:
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "EhcAsyncControlTransfer: error - %r\n", Status));
    }
  ...
  )
{
  ...
  DataProcessed = 0;
  TransferDirection = (((EndPointAddress & 0x80) != 0) ? EfiUsbDataIn : EfiUsbDataOut);

  // For RUMI, use smaller transfer size. If the transfer direction is RX,
  // the host cannot reduce the size because it can cause buffer overflow 
  // in the endpoint level.
  if (Ehc->PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_RUMI &&
      TransferDirection == EfiUsbDataOut) {
    TransferSize = EFI_PAGE_SIZE;
  } else {
  ...
  do {
    *TransferResult = EFI_USB_ERR_SYSTEM;
    ...
    if (DataProcessed + TransferSize <= *DataLength) {
      Len = TransferSize;
    }
    else {
      Len = *DataLength - DataProcessed;
    }
    ...
    Urb = EhcCreateUrb(
      ...
      *DataToggle,
      ...
      EHC_BULK_TRANSFER,
      NULL,
      (VOID *)(((UINTN)Data[0]) + DataProcessed),
      Len,
      NULL,
      NULL,
      1
    ...
    WriteBackInvalidateDataCacheRange((VOID *)(((UINTN)Data[0]) + DataProcessed), Len);
    ArmDataMemoryBarrier();

    EhcLinkQhToAsync(Ehc, Urb->Qh);
    Status = EhcExecTransfer(Ehc, Urb, TimeOut);

    WriteBackInvalidateDataCacheRange((VOID *)(((UINTN)Data[0]) + DataProcessed), Len); // ensure the HW sees the fresh data
    ArmDataMemoryBarrier();

    EhcUnlinkQhFromAsync(Ehc, Urb->Qh);

    DataProcessed += Urb->Completed;
    *TransferResult = Urb->Result;
  ...
  } while (DataProcessed < *DataLength && *TransferResult == EFI_USB_NOERROR);


  *DataLength = DataProcessed;
  ...
  )
{
  ...
  if (EhcIsHalt (Ehc) || EhcIsSysError (Ehc)) {
    DEBUG ((EFI_D_ERROR, "EhcAsyncBulkTransfer: HC is halted\n"));
  ...
  // To share the Async Bulk In Polling interval
  // with the polling function EhcCheckAsyncBulkInResult()
  // which checks if transfer completed successfully

  gAsyncBulkPollInterval = PollingInterval;
  ...
  if (Urb == NULL) {
    DEBUG ((EFI_D_ERROR, "EhcAsyncBulkTransfer: failed to create URB\n"));

    Status = EFI_OUT_OF_RESOURCES;
    goto ON_EXIT;
  }

  WriteBackInvalidateDataCacheRange(Data[0], *DataLength); // ensure the HW sees the fresh data
  ArmDataMemoryBarrier();
  EhcLinkQhToAsync (Ehc, Urb->Qh);

  InsertHeadList (&Ehc->AsyncBulkInTransfers, &Urb->UrbList);

  // Start the timer to check for completion
  Status = EhcAsyncBulkSetTimer(Ehc, PollingInterval);

  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Cannot Create CheckAsyncBulkInTimer\n"));
    EhcUnlinkQhFromAsync (Ehc, Urb->Qh);
    WriteBackInvalidateDataCacheRange(Data[0], *DataLength); // ensure the HW sees the fresh data
    ArmDataMemoryBarrier();
    EhcAckAllInterrupt (Ehc);
  ...
  ON_EXIT:
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((EFI_D_ERROR, "EhcBulkTransfer: error - %r\n", Status));
    }
  ...
  )
{
  ...
  // Check the comments in EhcMonitorAsyncRequests
  //
  WriteBackInvalidateDataCacheRange(Data, DataLength); // ensure the HW sees the fresh data
  ArmDataMemoryBarrier();
 ...
 )
{
  ...
  USB_ASSERT_RETURN ((Ehc->PciIo != NULL), EFI_INVALID_PARAMETER);
  ...
  )
{
  ...
  Ehc->Usb2Hc.AsyncControlTransfer      = EhcAsyncControlTransfer;
  Ehc->Usb2Hc.BulkTransfer              = EhcBulkTransfer;
  Ehc->Usb2Hc.AsyncBulkTransfer         = EhcAsyncBulkTransfer;
  ...
  // Init PortTest protocol
  CopyMem (&Ehc->UsbPortTest, &gEhciUsbPortTestTemplate, sizeof(QCOM_USB_PORT_TEST_PROTOCOL));

  // Populate PortTestIdentity string
  Status = Ehc->PciIo->GetLocation(Ehc->PciIo, NULL, NULL, &CoreNum, NULL);
  if (EFI_ERROR(Status)) {
    CoreNum = (UINTN)-1;
  }
  (VOID) UnicodeSPrint(
      Ehc->UsbPortTestIdentity,
      sizeof(Ehc->UsbPortTestIdentity),
      L"Ehci PortTest protocol on core %d",
      CoreNum
      );


  Ehc->CompanionDeviceNumber = CoreNum;

  InitializeListHead (&Ehc->AsyncIntTransfers);

  InitializeListHead (&Ehc->AsyncBulkInTransfers);

  InitializeListHead (&Ehc->AsyncControlTransfers);
  ...
  //
  // Create timer to check the transfer status in
  // a later time.
  //
  Status = gBS->CreateEvent (
      EVT_TIMER | EVT_NOTIFY_SIGNAL,
      TPL_CALLBACK,
      EhcCheckAsyncBulkInResult,
      Ehc,
      &Ehc->CheckAsyncBulkInTimer
  );
  ...
  Status = gBS->CreateEvent (
      EVT_TIMER | EVT_NOTIFY_SIGNAL,
      TPL_CALLBACK,
      EhcCheckAsyncControlResult,
      Ehc,
      &Ehc->CheckAsyncControlTimer
  );
  ...
  )
{
  ...
  EFI_PLATFORMINFO_PROTOCOL *PlatformInfo; 
  ...
  if (EFI_ERROR (Status))
  {
    DEBUG ((EFI_D_ERROR, "EhcDriverBindingStart: failed to locate UsbConfig protocol\n"));
    goto FREE_POOL;
  }  

  // Locate PlatformInfo protocol
  Status = gBS->LocateProtocol(
  ...
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "EhcDriverBindingStart: failed to locate PlatformInfo protocol\n"));
    goto FREE_POOL;
  }

  Status = PlatformInfo->GetPlatformInfo(PlatformInfo, &Ehc->PlatformInfo);  
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "EhcDriverBindingStart: failed to get PlatformInfo\n"));
    goto FREE_POOL;
  }
  ...
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &Controller,
                  &gEfiUsb2HcProtocolGuid,       &Ehc->Usb2Hc,
                  NULL
                  );
  ...
  Status = gBS->InstallMultipleProtocolInterfaces (
                &Controller,
                &gQcomUsbPortTestProtocolGuid, &Ehc->UsbPortTest,
                NULL
                );

  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "EhcDriverBindingStart: failed to install USB Port test Protocol\n"));
    goto FREE_POOL;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "EhcDriverBindingStop: failed to uninstall EfiUsb2HcProtocol\n"));
    return Status;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "EhcDriverBindingStop: failed to uninstall QcomUsbPortTestProtocol\n"));
    return Status;
  }
