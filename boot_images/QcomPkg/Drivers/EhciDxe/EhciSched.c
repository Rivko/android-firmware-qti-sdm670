...
extern  UINTN gAsyncBulkPollInterval;
extern  UINTN gAsyncControlPollInterval;
  ...
  )
{
  ...
  Ehc->MemPool = UsbHcInitMemPool(
                    PciIo,
  EHC_BIT_IS_SET(Ehc->HcCapParams, HCCP_64BIT),0);
  ...
  Bytes = EFI_PAGE_SIZE;
  Buf = UsbHcAllocateMem(Ehc->MemPool, Bytes);

  if (Buf == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ASSERT(((EFI_PHYSICAL_ADDRESS)Buf) % Bytes == 0);

  Ehc->PeriodFrame      = Buf;
  Ehc->PeriodFrameMap   = Map;

  PhyAddr = UsbHcGetPciAddressForHostMem(Ehc->MemPool, Buf, Bytes);

  // Remove: CK-DF
  DEBUG ((EFI_D_INFO, "EhcInitSched: PeriodFrame: host addr 0x%llx, device addr 0x%llx\n",
    Buf,
    PhyAddr
    ));
...
ErrorExit1:
  // CK - DF: Don't free Buf until IMEM usage is removed
  ...
  )
{
  ...
  if (Ehc->PeriodFrame != NULL) {
    PciIo = Ehc->PciIo;
    USB_ASSERT_RETURN ((PciIo != NULL), EFI_INVALID_PARAMETER);

    PciIo->Unmap (PciIo, Ehc->PeriodFrameMap);

    // CK-DF: Don't Free PeriodFrame until IMEM usage is removed
    /*PciIo->FreeBuffer (
             PciIo,
             EFI_SIZE_TO_PAGES (EFI_PAGE_SIZE),
             Ehc->PeriodFrame
             );*/
    UsbHcFreeMem (Ehc->MemPool, Ehc->PeriodFrame, 4096);
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((Ehc->ReclaimHead->NextQh == Qh), EFI_DEVICE_ERROR);
  ...
  )
{
  ...
  for (Index = 0; Index < EHC_FRAME_LEN; Index += Qh->Interval) {
    ...
    //
    USB_ASSERT_RETURN ((!EHC_LINK_TERMINATED (((UINT32*)Ehc->PeriodFrame)[Index])), EFI_INVALID_PARAMETER);
    ...
    USB_ASSERT_RETURN ((Next != NULL), EFI_INVALID_PARAMETER);
    ...
    if (Next->Interval == Qh->Interval) {
      ...
      // are all done.
      //
      USB_ASSERT_RETURN (((Index == 0) && (Qh->NextQh == NULL)), EFI_INVALID_PARAMETER);
  ...
  )
{
  ...
  USB_ASSERT_GOTO (((Ehc != NULL) && (Urb != NULL) && (Urb->Qh != NULL)), ON_EXIT_ERR);
...
/**
  Async Bulk transfer completion handler.
  ...
  )
{
  ...
  USB_ASSERT_GOTO((Context != NULL), Exit);

  Ehc     = (USB2_HC_DEV *) Context;

  Loop      = (EHC_ASYNC_BULK_POLL_TIMEOUT / (gAsyncBulkPollInterval * EHC_1_MILLISECOND))  + 1;

  if ((&Ehc->AsyncBulkInTransfers) == NULL) {
     DEBUG ((EFI_D_ERROR, "EhcCheckAsyncBulkInResult: AsyncBulkInTransfers list empty \n"));
     goto Exit;
  }


  EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Ehc->AsyncBulkInTransfers) {
    Urb = EFI_LIST_CONTAINER (Entry, URB, UrbList);

    // Check if the async bulk transfer completed
    // If not, reset the polling timer
    Finished = EhcCheckUrbResult (Ehc, Urb);

    if (!Finished) {
      if (Counter < Loop) {
        //
        // Logged under INFO since the message would be printed very often for the
        // network data log
        //
        DEBUG ((EFI_D_INFO, "EhcCheckAsyncBulkInResultr: transfer not finished counter %x loop %x\n",Counter, Loop));
        EhcDumpQh (Urb->Qh, NULL, FALSE);
        //Reset the polling timer
        gBS->SetTimer(Event, TimerRelative, gAsyncBulkPollInterval * EHC_1_MILLISECOND_100NS);
        Counter++;
      }
      else {
        //
        // We time out if the transfer did not finish for some reason
        // within the EHC_ASYNC_BULK_POLL_TIMEOUT
        //
        DEBUG ((EFI_D_ERROR, "EhcExecTransfer: transfer not finished, timed out\n"));
        EhcDumpQh (Urb->Qh, NULL, FALSE);
        Status = EFI_TIMEOUT;
        Finished = TRUE;
      }
        ...
        if (Urb->Result != EFI_USB_NOERROR){
          //
          // If the transfer finished, with or without error,
          // inform the client about the transfer result, provided
          // a callback function is set.
          //
          DEBUG ((EFI_D_ERROR, "EhcCheckAsyncBulkInResult: transfer failed with %x\n", Urb->Result));
          EhcDumpQh (Urb->Qh, NULL, FALSE);
          Status = EFI_DEVICE_ERROR;
        }
      }
      if (Finished) {
        ...
        if (Urb->Callback != NULL) {
          ...
          (Urb->Callback) (Urb->Data, Urb->Completed, Urb->Context, Urb->Result, Urb->DataToggle);
          OldTpl = gBS->RaiseTPL (EHC_TPL);
        }
  ...
  )
{
  ...
  Loop      = (EHC_ASYNC_CONTROL_POLL_TIMEOUT / (gAsyncControlPollInterval * EHC_1_MILLISECOND))  + 1;

  if ((&Ehc->AsyncControlTransfers) == NULL) {
     DEBUG ((EFI_D_ERROR, "EhcCheckAsyncControlResult: AsyncControlTransfers list empty \n"));
     goto Exit;
  }

  EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Ehc->AsyncControlTransfers) {
    Urb = EFI_LIST_CONTAINER (Entry, URB, UrbList);

    // Check if the async control transfer completed
    // If not, reset the polling timer
    Finished = EhcCheckUrbResult (Ehc, Urb);
    ...
    if (!Finished) {
      if (Counter < Loop) {
        ...
        // network data log
        //
        DEBUG ((EFI_D_INFO, "EhcCheckAsyncControlResult: transfer not finished counter %x loop %x\n",Counter, Loop));
        EhcDumpQh (Urb->Qh, NULL, FALSE);
        //Reset the polling timer
        gBS->SetTimer(Event, TimerRelative, gAsyncControlPollInterval * EHC_1_MILLISECOND_100NS);
        Counter++;
      }
      else {
        //
        // We time out if the transfer did not finish for some reason
        // within the EHC_ASYNC_CONTROL_POLL_TIMEOUT
        //
        DEBUG ((EFI_D_ERROR, "EhcExecTransfer: transfer not finished, timed out\n"));
        EhcDumpQh (Urb->Qh, NULL, FALSE);
        Status = EFI_TIMEOUT;
        ...
        if (Urb->Result != EFI_USB_NOERROR){
          ...
          // a callback function is set.
          //
          DEBUG ((EFI_D_INFO, "EhcCheckAsyncControlResult: transfer failed with %x\n", Urb->Result));
          EhcDumpQh (Urb->Qh, NULL, FALSE);
          Status = EFI_DEVICE_ERROR;
        }
      }
      if (Finished) {
        ...
        DEBUG ((EFI_D_INFO, "EhcCheckAsyncControlResult: transfer finished sucessfully with %x\n", Urb->Result));
        ...
        if (Urb->Callback != NULL) {
          ...
          //
          gBS->RestoreTPL (OldTpl);
          DEBUG ((EFI_D_INFO, "EhcCheckAsyncControlResult: transfer finished sucessfully, callback now \n"));
          (Urb->Callback) (Urb->Data, Urb->Completed, Urb->Context, Urb->Result, Urb->DataToggle);
          OldTpl = gBS->RaiseTPL (EHC_TPL);
        }

        if( EFI_ERROR(Status) ){
          DEBUG ((EFI_D_ERROR, "EhcCheckAsyncControlResult: transfer failed with %x\n", Status));
        }
      RemoveEntryList (&Urb->UrbList);
      EhcFreeUrb (Ehc, Urb);
      Counter = 0;
  ...
  )
{
  ...
  EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Ehc->AsyncIntTransfers) {
    ...
    if (Urb->Result == EFI_USB_NOERROR) {
      ...
      WriteBackInvalidateDataCacheRange(Urb->Data, Urb->Completed);
      ArmDataMemoryBarrier();
    ...
    if (Urb->Callback != NULL) {
      ...
      // Restore the old TPL, USB bus maybe connect device in
      // his callback. Some drivers may has a lower TPL restriction.
      // Toggle should not be sent for Interrupt Transfers
      gBS->RestoreTPL (OldTpl);
