...
/*=============================================================================
 ...
 06/14/13   mliss    Updated to support providing UsbPortTest protocol
 12/08/11   wufan    Add support for USB host mode electrical testing
...
//Global variable for Usb port test, defined in Ehci.c
extern INTN     UsbPortTestIndex;
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((Ehc != NULL), (EHC_QTD *) NULL);
  ...
  )
{
  ...
  USB_ASSERT_RETURN (((Urb != NULL) && (Urb->Qh != NULL)), EFI_INVALID_PARAMETER);

  //
  // EHCI follows the alternate next QTD pointer if it meets
  ...
  //
  // [HSECT] The following code block is for the support of USB host mode
  // electrical testing. The particular test case, Host Controller Packet Parameter,
  // requires the three transactions to get device descriptor happen in user controlled
  // intervals. The scheme adopted here is to have the test application initiate
  // the same Control Transfer three times, and a full QTD list is constructed
  // with code above. The code below will remove all except the QTD designated
  // in global variable USB_ET_QTD_INDEX_NAME. The test app will bump up the
  // global variable after each transfer.
  //
  if ( UsbPortTestIndex != USB_PORT_TEST_VAR_UNDEFINED ) {
        if (Ep->Type != EHC_CTRL_TRANSFER) {
          goto ON_ERROR;
        }
        ...
        Current = 0;
        EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Qh->Qtds) {
           Qtd = EFI_LIST_CONTAINER (Entry, EHC_QTD, QtdList);
           if ( Current == 0 && Qtd->QtdHw.Pid != QTD_PID_SETUP ) {
             goto ON_ERROR;
           }
           if ( Current != UsbPortTestIndex ) {
              RemoveEntryList (&Qtd->QtdList);
              UsbHcFreeMem (Ehc->MemPool, Qtd, sizeof (EHC_QTD));
           }
           Current += 1;
  ...
  // end support of USB host mode electrical testing
  ...
  )
{
  ...
  if (DevSpeed != EFI_USB_SPEED_HIGH) {
    USB_ASSERT_GOTO ((Hub != NULL), ON_ERROR);
