...
Copyright (c) 2009 - 2010, 2017, Intel Corporation. All rights reserved.<BR>
  ...
  )
{
  ...
  *NumberOfStrings = 0;
  ...
  while (*CharInStr != 0 || *(CharInStr+1) != 0) { 
    ...
    for (StrLen = 0 ; StrLen < (SMBIOS_STRING_MAX_LENGTH+1); StrLen++) {
      if (*(CharInStr+StrLen) == 0) {
        break;
      }  
    }

    if (StrLen == (SMBIOS_STRING_MAX_LENGTH+1)) {
      return EFI_INVALID_PARAMETER;
    }
    //
    // forward the pointer
