...
ELFINFO_RELOC_ETYPE       = '\x01\x00' #************************************************
ELFINFO_DYNAM_ETYPE       = '\x03\x00' #************************************************
ELFINFO_CORE_ETYPE       = '\x04\x00' #************************************************
ELFINFO_NONE_ETYPE       = '\x00\x00' #************************************************
...
#----------------------------------------------------------------------------
def create_elf_header_gui( output_file_name,
                       image_dest,
                       image_size,
                       elf_struct):
   ...
   if (elf_struct.is_elf_64_bit is True):
      # ELf header
      ...
      if elf_struct.e_type == 0:
        elf_fp.write(ELFINFO_NONE_ETYPE)
      elif elf_struct.e_type == 1:
        elf_fp.write(ELFINFO_RELOC_ETYPE)
      elif elf_struct.e_type == 3:
        elf_fp.write(ELFINFO_DYNAM_ETYPE)
      elif elf_struct.e_type == 4:
        elf_fp.write(ELFINFO_CORE_ETYPE)
      else:
      ...
      elf_fp.write(convert_int_to_byte_string(elf_struct.e_shoff, 8))
      ...
      elf_fp.write(convert_int_to_byte_string(ELF64_HDR_SIZE+ELF64_PHDR_SIZE, 8)) #offset
      elf_fp.write(convert_int_to_byte_string(image_dest, 8))
      elf_fp.write(convert_int_to_byte_string(image_dest, 8))
      ...
      elf_fp.write(convert_int_to_byte_string(elf_struct.e_shoff, 4))
      elf_fp.write(''.rjust(4, chr(ELFINFO_RESERVED)))
      ...
      # Program Header
      elf_fp.write(convert_int_to_byte_string(LOAD_TYPE, 4))
      elf_fp.write(convert_int_to_byte_string(ELF32_HDR_SIZE+ELF32_PHDR_SIZE, 4)) #offset
      elf_fp.write(convert_int_to_byte_string(image_dest, 4))
      elf_fp.write(convert_int_to_byte_string(image_dest, 4))
...
#----------------------------------------------------------------------------
# pboot_add_hash - DOES NOT COPY ORIGINAL ELF INTO OUTPUT FILE ****
#----------------------------------------------------------------------------
def pboot_add_hash_file(env, elf_in_file_name,
                        hash_tbl_file_name,
                        elf_out_file_name):
   ...
   # BELOW COMMENTED SO ONLY HASH TABLE IS COPIED
   # if hash_segment_found is True:
   #    # Copy input file to output file
   #    shutil.copyfileobj(elf_in_fp, elf_out_fp, os.path.getsize(elf_in_file_name))

      # Insert hash table into new file
      file_copy_offset(hash_tbl_fp, 0, elf_out_fp, 0, hash_size)
...
# array of ELF Program Header Objects
#----------------------------------------------------------------------------
def preprocess_elf_file_no_errcheck(elf_file_name):
