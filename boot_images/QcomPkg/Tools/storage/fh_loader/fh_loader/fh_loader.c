 ...
 *
 * Release Version: 17.07.28.15.44 (Jul 28 2017 @ 15:44)
 *
...
=============================================================================
 ...
 $Header: //components/rel/boot.xf/2.1/QcomPkg/Tools/storage/fh_loader/fh_loader/fh_loader.c#1 $
...
gcc fh_loader.c fh_loader_sha.c -o fh_loader -lrt
...
char *CommandLineOptions[] = {  "benchmarkdigestperformance", "benchmarkreads", "benchmarkwrites",
                                ...
                                "dontsorttags",
                                "erase=", "erasefirst",
                                "files=", "firmwarewrite", "fixgpt=", "flattenbuildto=", "flavor=", "forcecontentsxmlpaths", "forceoverwrite",
                                "getstorageinfo=", "getgptmainbackup=",
                                "interactive=", "labels=", "loglevel=", "lun=",
                                "mainoutputdir=", "maxdigesttablesizeinbytes=", "maxpayloadsizeinbytes=", "memoryname=",
                                "notfiles=", "notlabels=", "nop", "noprompt", "noreset", "num_sectors=",
                                "port=", "porttracename=",
                                "readbogusdata", "reset",
...
uint64 TotalTransferSize = 0, BuildSizeTransferSoFar = 0; // sum up all the transfers (reads and writes)
...
#define MAX_NUMBER_LABELS_FILTER 128
#define MAX_LABEL_SIZE 128  /* GPT spec defines the label name to be 72 bytes  */
struct single_label_filter
{
  int lun; /* LUN where the label belongs to. */
  char label[MAX_LABEL_SIZE];
};

struct multiple_label_filter
{
  struct single_label_filter list[MAX_NUMBER_LABELS_FILTER];
  size_t count;  /* Number of entries populated in the list above. */
};

struct multiple_label_filter filter_labels;    // --files=sbl1.mbn,tz.mbn
struct multiple_label_filter filter_not_labels;// --notfiles=sbl1.mbn,tz.mbn
...
void time_throughput_calculate (struct timeval *start_time, struct timeval *end_time, uint64 size_bytes, double NetworkElapsed, enum LOG_TYPES ThisLogType);
...
typedef struct
{
    ...
    char start_sector[MAX_ATTR_RAW_SIZE];    // special attributes
...
struct Attributes_Struct AllAttributes[] =
{
    ...
    */
    ...
    {"label",                     "", NULL,                         0, 0, 1,    sizeof (fh.attrs.label),  (char *) fh.attrs.label,   's', 0 },
...
/* Add a label to filter*/
int filter_label_add_entry(struct multiple_label_filter *data, int lun, char *label)
{
  size_t count = data->count;
  if (count >= MAX_NUMBER_LABELS_FILTER)
  {
    dbg (LOG_ERROR, "Too many labels to filter %d, will not add %d/%s to the list",
         (int)count, lun, label);
    return -1;
  }

  data->list[count].lun = lun;
  /* A portable way to do strlcpy */
  snprintf(data->list[count].label, sizeof(data->list[count].label), "%s",  label);

  data->count++;
  return 0;
}

/* param is the command line parameter, may be a comma delimited string. */
int filter_label_add_entries(struct multiple_label_filter *data, char *param)
{
  size_t size, s;
  int lun, result;
  char *start;
  char chunk[MAX_NUMBER_LABELS_FILTER + 2]; /* +2 to account for 'LUN_NUM/' */
  char *curr;
  ...
  {
    start = curr;
    /* Search for the end of the label. */
    while (*curr != '\0' && *curr != ',')
      curr++;
    ...
    {
      dbg (LOG_ERROR, "Label size too large %d %s", size, start);
      return -1;
    }

    memcpy (chunk, start, size);
    chunk[size] = '\0';
    s = 0;
    if(chunk[1] == '/')
    {
      ...
      {
        dbg (LOG_ERROR, "Invalid LUN number for label %c %s", chunk[0], start);
        return -1;
      }
    }
    else
    ...
    result = filter_label_add_entry(data, lun, &chunk[s]);
    if (result != 0)
    {
      return -1;
    }
...
{
  filter_labels.count = 0;
  filter_not_labels.count = 0;
  return 0;
}
...
int main (int argc, char *argv[])
{
    ...
    */
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        if ( strncmp (MyOpt, "labels=", MAX (strlen (MyOpt), strlen ("labels=") ) ) == 0 )
        {
          int result;
          char *cmd_arg = MyArg;
          result = filter_label_add_entries(&filter_labels, cmd_arg);
          continue;
        }

        if ( strncmp (MyOpt, "notlabels=", MAX (strlen (MyOpt), strlen ("notlabels=") ) ) == 0 )
        {
          int result;
          char *cmd_arg = MyArg;
          result = filter_label_add_entries(&filter_not_labels, cmd_arg);
          continue;
        }
    }
    ...
    for (i = 1; i < (SIZE_T) argc; i++) // skip 0 since argv[0] is fh_loader.exe
    {
        ...
        else if (strncmp (MyOpt, "fixgpt=", MAX (strlen (MyOpt), strlen ("fixgpt=") ) ) == 0)
        {
          ...
          {
            snprintf (temp_buffer, sizeof(temp_buffer), "lun=\"all\" " );
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
          }
          else
          {
            snprintf (temp_buffer, sizeof(temp_buffer), "lun=\"%d\" ", atoi(MyArg) );
            AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
          }

          snprintf (temp_buffer, sizeof(temp_buffer), "grow_last_partition=\"1\" " );
          AppendToBuffer (tx_buffer, temp_buffer, FIREHOSE_TX_BUFFER_SIZE);
...
SIZE_T  DetermineAttributes (char *Packet, SIZE_T  CurrentPacketLoc, SIZE_T MaxPacketSize)
{
    ...
    while (CurrentPacketLoc < PacketLength)
    {
        ...
        {
            if (strstr(NewAttrName, "num_partition_sectors") != NULL && strstr(NewAttrValue, "NUM_DISK_SECTORS") != NULL)
            {
                dbg (LOG_INFO, "Found NUM_DISK_SECTORS on num_partition_sectors, only allowed for <erase");
            }
            else
            {
...
int this_label_in_filter (struct multiple_label_filter *filter, char *label, int lun)
{
  size_t i, count;
  int filter_lun;

  count = filter->count;
  /* No filters are defined, so return -1. */
  if (count == 0)
  {
    return -1;
  }
  ...
  {
    int cmp;
    filter_lun = filter->list[i].lun;
    if (filter_lun == -1 || filter_lun == lun)
    {
      cmp = strncasecmp (filter->list[i].label, label,MAX_LABEL_SIZE);
      if (cmp == 0)
        return 1; // TRUE, load this entry
    }
  }
  return 0; // FALSE, do not load this entry
}
...
static firehose_error_t handleProgram()
{
    ...
    result = this_label_in_filter (&filter_labels, fh.attrs.label, fh.attrs.physical_partition_number);
    if (result == 0)
    {
      dbg (LOG_DEBUG, "'%s' is being SKIPPED since it was not in --labels provided by the user at the command line", fh.attrs.label);
      return FIREHOSE_SUCCESS;
    }

    result = this_label_in_filter (&filter_not_labels, fh.attrs.label, fh.attrs.physical_partition_number);
    if (result == 1)
    {
      dbg (LOG_INFO, "handleProgram() '%s' is being SKIPPED since it was in --notlabels provided by the user at the command line", fh.attrs.label);
      return FIREHOSE_SUCCESS;
    }
...
static firehose_error_t handleRead()
{
    ...
    if (num_filter_not_files > 0)
    {
        if (ThisFileIsInNotFilterFiles (fh.attrs.filename) )
        {
            dbg (LOG_INFO, "handleRead() '%s' is being SKIPPED since it was in --notfiles provided by the user at the command line", fh.attrs.filename);
            return FIREHOSE_SUCCESS;
        }
    }
    ...
    if (result == 1)
    {
      dbg (LOG_INFO, "handleRead() '%s' is being SKIPPED since it was in --notlabels provided by the user at the command line", fh.attrs.label);
      return FIREHOSE_SUCCESS;
    }
...
void ParseContentsXML (char *FileAndPath)
{
    ...
    while (1)
    {
        ...
        if (SaveThis)
        {
            ...
            for (i = 0; i < strlen (ContensXML[NumContensXML].Path); i++)
            {
                if ( ContensXML[NumContensXML].Path[i] == '$' && ContensXML[NumContensXML].Path[i+1] == '{')
                {
                    j = i;
                    found_special = 1;
                    break;
            ...
            if(found_special == 1)
            {
                for(; i < strlen (ContensXML[NumContensXML].Path); i++)
                {
                    if (ContensXML[NumContensXML].Path[i] == ':')
                    {
                        k = i + 1;
