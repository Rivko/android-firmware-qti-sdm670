/** @file
  ...
  Portions copyright (c) 2011-2012 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 02/07/12   ma       Added support for EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL testing.
 01/26/12   yg       Print without debug dependency
 07/30/11   sy       Adding TestInterface Library
 06/21/11   shl      Changed total test times to 20 to allow different combo
                       keys to be exercised.
...
**/
EFI_STATUS EFIAPI KeypadTestMain (
  ...
  )
{
  ...
  for (;;)
  {
    ...
    if(!EFI_ERROR (Status))
    {
      ...
      {
        AsciiPrint("Key Registration failed\n");
      }
      else
      {
  ...
  TestStatus("KeypadTest",Status);
  TEST_STOP("KeypadTest");
  return Status;
}
