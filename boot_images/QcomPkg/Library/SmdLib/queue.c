...
#if ((defined FEATURE_QUBE) || (defined FEATURE_WOA) || ((defined FEATURE_Q_WIN32_LOCKS) || (defined FEATURE_WINCE) || (defined FEATURE_UEFI)) && !defined(FEATURE_WINCE_BOOTLOADER))
...
void q_lock_init( q_type *q_ptr )
{
   ...
   ASSERT( status == EOK );
  #elif defined(FEATURE_WOA)
   KeInitializeSpinLock(&q_ptr->SpinLock);
  #elif defined(FEATURE_UEFI)
#if 0   
...
void q_lock_delete( q_type *q_ptr )
{
   ...
   int status;
#endif
   UNREFERENCED_PARAMETER(q_ptr);

   ASSERT( q_ptr != NULL );
  #if !defined(FEATURE_WINCE) && !defined(FEATURE_WOA) && !defined(FEATURE_UEFI)
   if(NULL == q_ptr->mutex)
   {
  ...
  #elif defined(FEATURE_UEFI)
#if 0   
  DALSYS_DestroyObject( ( DALSYSSyncHandle ) q_ptr->cs );
#endif  
  #else
...
static void q_lock( q_type *q_ptr )
{
   ...
   ASSERT( status == EOK );
  #elif defined(FEATURE_WOA)
   KeAcquireSpinLock(&q_ptr->SpinLock, &q_ptr->Irql);
  #elif defined(FEATURE_UEFI)
#if 0
  DALSYS_SyncEnter( q_ptr->cs );
#endif
  #else 
...
static void q_unlock( q_type *q_ptr )
{
   ...
   ASSERT( status == EOK );
  #elif defined(FEATURE_WOA)
   KeReleaseSpinLock(&q_ptr->SpinLock, q_ptr->Irql);
  #elif defined(FEATURE_UEFI)
#if 0
  DALSYS_SyncEnter( q_ptr->cs );
#endif
