...
static unsigned find_and_wait(rpm_mq_t *self, inq_t *q, unsigned msg_id)
{
    ...
    while((msg = (rpm_msg_t *)inq_it_deref(&it)) != NULL)
    {
        if(msg_id == msg->msg_id)
        {
            ...
            ULOG_RT_PRINTF_2(rpm_log, "rpm_msg_wait (msg_id: 0x%0.8x) (event: 0x%08x)", msg_id, PtrToUlong(completion_event));
            ...
            ULOG_RT_PRINTF_2(rpm_log, "rpm_msg_wait_complete (msg_id: 0x%0.8x) (event: 0x%08x)", msg_id, PtrToUlong(completion_event));
