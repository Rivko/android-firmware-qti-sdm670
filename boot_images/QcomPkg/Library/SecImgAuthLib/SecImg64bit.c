...
--------   ---      ------------------------------------
05/22/15   as       Fix 64b address pointersn
...
{
  Elf64_Phdr *cur= (Elf64_Phdr *) phdr;

  if (phdr == NULL || end == NULL)
    return NULL;

  while ((uint64*)cur < (uint64*)end)
  {
...
)
{
  ...
  Elf64_Phdr*       hash_prog_hdr = NULL;

  prog_tbl_sz = (uint32)(sizeof(Elf64_Phdr)) * 
                (img_data->elf_prg_hdr.hdr_64.elf_hdr->e_phnum);
  /* Interger overflow check */
  if(((uint64)img_data->elf_prg_hdr.hdr_64.prog_hdr + prog_tbl_sz) < 
     (uint64)img_data->elf_prg_hdr.hdr_64.prog_hdr)
  {
    return SEC_IMG_AUTH_HASH_SEG_INVALID_SIZE;
  }

  hash_prog_hdr = sec_img_auth_elf_find_seg_64(img_data->elf_prg_hdr.hdr_64.prog_hdr,
                                     (Elf64_Phdr*)((uint8*) img_data->elf_prg_hdr.hdr_64.prog_hdr +
                                     prog_tbl_sz),
                                     MI_PBT_HASH_SEGMENT);
...
)
{
  ...
  if (((start_addr < (UINT64_MAX_VALUE - bytes)) &&
       (address >= start_addr)             &&
...
)
{
  ...
  ph_count = img_data->elf.elf_prg_hdr.hdr_64.elf_hdr->e_phnum;
  ph_hdr = img_data->elf.elf_prg_hdr.hdr_64.prog_hdr;
  ...
  for(i = 0; i < ph_count; i++, ph_hdr++)
  {
   ...
   if(((sec_img_auth_is_valid_segment(img_data->elf.elf_format, ph_hdr)) && (ph_hdr->p_filesz != 0))&&
      (sec_img_auth_is_valid_area_64(whitelist_area, ph_hdr->p_paddr, ph_hdr->p_paddr + ph_hdr->p_memsz)))
   {
      /* Check if it falls within the range of one of the ELF segments */
      ret = sec_img_auth_check_address_64(img_data->elf.elf_prg_hdr.hdr_64.elf_hdr->e_entry,
            img_data->elf.elf_prg_hdr.hdr_64.prog_hdr[i].p_paddr,
            img_data->elf.elf_prg_hdr.hdr_64.prog_hdr[i].p_filesz);
      if(ret == SEC_IMG_AUTH_SUCCESS)
      {
        /* ELF entry falls in valid area */
        return SEC_IMG_AUTH_SUCCESS;
      }
      /* Check if it falls within the virtual range of one of the ELF segments */
      ret = sec_img_auth_check_address_64(img_data->elf.elf_prg_hdr.hdr_64.elf_hdr->e_entry,
            img_data->elf.elf_prg_hdr.hdr_64.prog_hdr[i].p_vaddr,
            img_data->elf.elf_prg_hdr.hdr_64.prog_hdr[i].p_filesz);
      if(ret == SEC_IMG_AUTH_SUCCESS)
      {
        /* ELF entry falls in valid area */
...
)
{
  ...
  /* For all loadable segments, make sure they fall in one of the whitelist areas */
   ...
   for(i = 0; i < elf->elf_prg_hdr.hdr_64.elf_hdr->e_phnum; i++)
   {
     if((sec_img_auth_is_valid_segment(elf->elf_format, 
	    (void*)&(elf->elf_prg_hdr.hdr_64.prog_hdr[i])))) 
     {
      /* This is a corrupted image */
      if ((elf->elf_prg_hdr.hdr_64.prog_hdr[i].p_filesz > 
           elf->elf_prg_hdr.hdr_64.prog_hdr[i].p_memsz) ||
      ...
      len =  elf->elf_prg_hdr.hdr_64.prog_hdr[i].p_memsz - 
      ...
      if ((sec_img_auth_is_valid_area_64(elf->whitelist_area, elf->elf_prg_hdr.hdr_64.prog_hdr[i].p_paddr, 
           elf->elf_prg_hdr.hdr_64.prog_hdr[i].p_paddr + elf->elf_prg_hdr.hdr_64.prog_hdr[i].p_memsz)) && (len))
      {

        MEMSET((void *)(uintnt)(elf->elf_prg_hdr.hdr_64.prog_hdr[i].p_paddr + 
                    elf->elf_prg_hdr.hdr_64.prog_hdr[i].p_filesz), 0, len);
      }
     }
    }
...
)
{
  ...
  uint64 hash_seg_addr = 0;
  uint64 hash_seg_hdr_addr = 0;
  ...
  {  
    ...
    elf_hdr_sz = sizeof(Elf64_Ehdr);
    prog_tbl_sz = (uint32)(sizeof(Elf64_Phdr)) * (((Elf64_Ehdr*)img_data->elf_hdr)->e_phnum);
    
    /* Check for integer overflow */
    if (prog_tbl_sz !=
      (uint64) (sizeof (Elf64_Phdr) * (uint64)((Elf64_Ehdr*) img_data->elf_hdr)->e_phnum))
    {
      ret = SEC_IMG_AUTH_PROG_TBL_INVALID_SIZE;
      break;
    }

    if(prog_tbl_sz > (MI_PBT_MAX_SEGMENTS * sizeof (Elf64_Phdr)))
    {
      ret = SEC_IMG_AUTH_PROG_TBL_INVALID_SIZE;
      break;
    }
    ...
    /* Interger overflow check */
    if(((uint64)img_data->prog_hdr + prog_tbl_sz) < (uint64)img_data->prog_hdr)
    {
      return SEC_IMG_AUTH_HASH_SEG_INVALID_SIZE;
    }

    ret = sec_img_auth_validate_hash_segment_entries_64((Elf64_Phdr*)img_data->prog_hdr,
          (Elf64_Phdr*)((uint8*) img_data->prog_hdr + prog_tbl_sz),
          MI_PBT_HASH_SEGMENT);
    ...
    hash_seg_hdr_addr = (uint64)img_data->hash_seg_hdr;
    hash_seg_hdr_sz = sizeof(mi_boot_image_header_type);
    
    hash_seg_addr = hash_seg_hdr_addr + sizeof (mi_boot_image_header_type);
    hash_array_sz = ((Elf64_Ehdr*)img_data->elf_hdr)->e_phnum * CEML_HASH_DIGEST_SIZE_SHA256;
    /* Check for integer overflow */
    ...
    img_auth->elf_prg_hdr.hdr_64.elf_hdr = img_data->elf_hdr;
    img_auth->elf_prg_hdr.hdr_64.prog_hdr = img_data->prog_hdr;
    img_auth->prog_hdr_num = img_auth->elf_prg_hdr.hdr_64.elf_hdr->e_phnum;
    ...
    if((mi_boot_hdr->signature_size > 0) && (img_auth->cert_sz > 0))
    {
      ...
      img_auth->sig_ptr = (uint8*)(uintnt)hash_seg_addr + hash_array_sz;
       ...
       if (((uint64)img_auth->sig_ptr  + img_auth->sig_sz) < (uint64)img_auth->sig_ptr)
       {
      ...
      if (((uint64)img_auth->cert_ptr  + img_auth->cert_sz) < (uint64)img_auth->cert_ptr)
      {
   ...
   ret = sec_img_auth_validate_hash_segment_len_64(img_auth);
   BREAKIF (ret != SEC_IMG_AUTH_SUCCESS);
