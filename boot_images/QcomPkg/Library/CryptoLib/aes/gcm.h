...
/*  After encryption or decryption operations the return value of
    ...
    or RETURN_ERR, the latter indicating an error. A return value
...
typedef struct
{
    ...
    secenc_aes_ctx_type aes[1]; /* AES encryption context (needs to be aligned to uint32)*/
...
ret_type gcm_encrypt_message(                   /* encrypt an entire message    */
            ...
            unsigned char cipher_text[],        /* the cipher text              */
            unsigned long cipher_text_len,      /* and its length in bytes      */
...
ret_type gcm_decrypt_message(                   /* decrypt an entire message    */
            ...
            unsigned char msg[],                /* the message buffer           */
            unsigned long msg_len,              /* and its length in bytes      */
            unsigned char cipher_text[],        /* the encrypted message buffer */
            unsigned long cipher_text_len,      /* and its length in bytes      */
            unsigned char tag[],                /* the buffer for the tag       */
            unsigned long tag_len,              /* and its length in bytes      */    
            gcm_ctx ctx[1]);                     /* the mode context             */
