...
--------   ---     ----------------------------------------------------------
01/03/18   st      Moving debug board check earlier(WA:CR2120731)
...
pm_sbl_chg_init (void)
{
      ...
      ( (pmic_model == PMIC_IS_PM660)) )
   {
	  ...
	  //WA: [660]moving debug board check earlier(CR2120731)
	  //WA: [660]WA for Erroneous USBIN auto-power-on(CR2142179)
...
pm_err_flag_type pm_system_init(void)
{
   ...
   {
     err_flag = pm_config_wlan_active_and_retention_level(); //somu needs update here
   }
