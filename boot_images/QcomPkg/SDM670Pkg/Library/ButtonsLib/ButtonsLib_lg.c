...
#include "ButtonsLibPrivate_lg.h"
...
EFI_STATUS ConfigureButtonGPIOs ( VOID )
{
   ...
   // volume up
   Status = EnableInput(PM_DEVICE_2, VOLUME_UP_BUTTON_GPIO);
   if ( EFI_ERROR (Status) )
   {
...
EFI_STATUS PollButtonArray( UINT8 *pButtonArray )
{
  ...
  *(pButtonArray + 0) = ButtonPressed;
   ...
   //for volume up
   ButtonPressed = FALSE;
   Status = ReadGpioStatus(PM_DEVICE_2, VOLUME_UP_BUTTON_GPIO, &ButtonPressed );
   if ( EFI_ERROR (Status) )
   {
