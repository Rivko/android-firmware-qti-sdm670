  ...
  )
{
  ...
  ASSERT(Source != Destination);	/* source and destination should not be same */
  ...
  )
{
  ...
  //
#if defined(QCOM_EDK2_PATCH)
  ASSERT(Source != Destination);	/* source and destination shoul not be same */

  if (Source > Destination) {
    ASSERT( (UINTN) ( (Destination + Length) < Source) ); /* Note: if Length > StrLen(Source) memory set to 0 till Length*/
  }
  else {
    ASSERT( (UINTN) ( (Source + StrLen(Source)) < Destination ) );
  }
#else
  ASSERT ((UINTN)(Destination - Source) > StrLen (Source));
...
#if defined(QCOM_EDK2_PATCH)
  Length = (StrLen(Source));
  ASSERT((UINTN) ((CHAR8*) Source != (CHAR8*) Destination) );

  if ( ((CHAR8*) Source) > Destination ) {
    ASSERT( (UINTN) ( (Destination + Length + 1)  < ( (CHAR8*)Source ) ) );
  }
  else {
    ASSERT( (UINTN) ( (CHAR8*) (Source + Length + 1)  <  Destination ) );
  }
#else
  ...
  )
{
  ...
  ASSERT(Source != Destination);	/* source and destination shoul not be same */
  if(Source > Destination) {
    ASSERT( (UINTN) ( (Destination + Length) < Source ) );
  }
  else {
    ASSERT( (UINTN) ( (Source + Length)  < Destination ) );
  }
#else
  ASSERT ((UINTN)(Destination - Source) > AsciiStrLen (Source));
  ASSERT ((UINTN)(Source - Destination) >= Length);
