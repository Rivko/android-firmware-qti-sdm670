...
STATIC BOOLEAN IsBBMSafe (UINT64);
UINT32 gBBM = 0;
  ...
  for (IndexLevel = RootTableLevel; IndexLevel <= PageLevel; IndexLevel++) {
    ...
    if ((IndexLevel != 3) && ((*BlockEntry & TT_TYPE_MASK) == TT_TYPE_TABLE_ENTRY)) {
      ...
      if (IndexLevel != PageLevel) {
        ...
        {
          gBBM++;
          DEBUG ((EFI_D_ERROR, "WARN: Not BBM Safe Region: 0x%x\n", RegionStart));

          WriteBackInvalidateDataCacheRange(BlockEntry, sizeof(UINT64));
          WriteBackInvalidateDataCacheRange(TranslationTable, sizeof(EFI_PAGE_SIZE));
          ArmDataSynchronizationBarrier();
...
{
  UINT64 FnAddr = (UINT64)GetBlockEntryListFromAddress;
  
  if (!ArmMmuEnabled())
    return TRUE;

  if (ABS(FnAddr - RegionStart) <= SIZE_2MB)
  {
    return FALSE; 
  }

  if (ABS((UINT64)&FnAddr - RegionStart) <= SIZE_2MB)
  {
    return FALSE; 
  }
  ...
  )
{
  ...
  do {
    ...
    do {
      ...
      TempBlockEntry = *BlockEntry;

      *BlockEntry = 0;
      ArmDataSynchronizationBarrier();
      WriteBackDataCacheRange (BlockEntry, 8);
      ArmInvalidateTlb();
