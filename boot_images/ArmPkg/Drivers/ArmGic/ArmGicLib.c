...
#define MAX_NUM_INTERRUPTS    (1020)
#endif
...
{
#if defined (QCOM_EDK2_PATCH)
  //The max number of interrupts is normally 32(IT Lines Number + 1)i
  //However a value of 0x1F in the IT Lines field indicates a max of 1020 instead
  UINTN MaxNumInterrupts = 32 * ((MmioRead32 (GicDistributorBase + ARM_GIC_ICDICTR) & 0x1F) + 1);
  MaxNumInterrupts = (MaxNumInterrupts > MAX_NUM_INTERRUPTS) ? MAX_NUM_INTERRUPTS : MaxNumInterrupts;
...
#if defined (QCOM_EDK2_PATCH)
UINTN
ArmGicV3GetAffinityRedistBase(
  UINTN GicRedistributorBase, UINTN MpId)
{
  UINT64  MpidrAffinity;
  UINT64  GicrAffinity;
  UINT64  GicrTyper;
  UINTN   Address;

  if(!GicRedistributorBase){
    return 0;
  }

/*  Construct the affinity as used by GICv3.
    Need to move affinity level 3 from bits 32-39 to 24-31
 */
  MpidrAffinity = (UINT64)MpId & 0xFF00FFFFFF;
  MpidrAffinity |= ((MpidrAffinity >> 8) & 0xFF000000);
  MpidrAffinity &= 0xFFFFFFFF;

  Address = GicRedistributorBase;
  do {
      GicrTyper = MmioRead64 (Address + ARM_GICR_TYPER);
      //Affinity is the top region of GICR_TYPER
      GicrAffinity = (GicrTyper >> 32) & 0xFFFFFFFF;

      if (MpidrAffinity == GicrAffinity) {
          return Address;
      }

      if (GicrTyper & ARM_GICR_TYPER_VLPI) {
        /* RD_Base, SGI_Base, VLPI_Base and reserved */
        Address += SIZE_64KB << 2;
      } else {
        /* RD_Base and SGI_Base */
        Address += SIZE_64KB << 1;
      }
