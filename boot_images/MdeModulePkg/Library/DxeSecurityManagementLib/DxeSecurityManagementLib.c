...
Portions Copyright (c) 2009 - 2015, Intel Corporation. All rights reserved.<BR>
  ...
  for (Index = 0; Index < mNumberOfSecurityHandler; Index ++) {
    if ((mSecurityTable[Index].SecurityOperation & EFI_AUTH_OPERATION_IMAGE_REQUIRED) == EFI_AUTH_OPERATION_IMAGE_REQUIRED) {
      ...
      if (SourceBuffer != NULL) {
        if (FileBuffer == NULL) {
          FileBuffer = (void*) AllocatePool (FileSize);
          if (FileBuffer == NULL) {
            return RETURN_OUT_OF_RESOURCES;
          }
          InternalMemCopyMem(FileBuffer, SourceBuffer, FileSize);
        }
      }
#endif
      if (FileBuffer == NULL) {
