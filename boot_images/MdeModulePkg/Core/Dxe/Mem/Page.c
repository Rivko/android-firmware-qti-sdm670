...
#if defined (ENABLE_XN) && defined (ENABLE_EXEC_CODE_READY_TO_BOOT) && defined (ENABLE_DEP_64)
    extern UINTN AllowRWXAllocForCode;    
    if ( ((MemoryType == EfiLoaderCode) || (MemoryType == EfiBootServicesCode) || (MemoryType == EfiRuntimeServicesCode)) && (AllowRWXAllocForCode) )
    {
      EFI_STATUS DepStatus;
      // DEBUG((DEBUG_ERROR, "Convert Start: 0x%016lx N: 0x%016lx T: 0x%x \n", Start, NumberOfPages, MemoryType));
      DepStatus = CoreSetMemorySpaceAttributes (Start, NumberOfPages*EFI_PAGE_SIZE, EFI_MEMORY_WB);
      if (DepStatus != EFI_SUCCESS)
          DEBUG((DEBUG_ERROR, "ERROR: Failed clearing XN %r \n", DepStatus));
    }
#endif
