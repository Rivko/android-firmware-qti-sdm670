  ...
  )
{
  ...
  if (!EFI_ERROR(PciGetBusRange (Descriptors, &MinBus, &MaxBus, NULL))) {
#else
...
#if defined (QCOM_EDK2_PATCH)
/**
  Get bus range from PCI resource descriptor list for Qcom targets.
...
{
#if defined (QCOM_EDK2_PATCH)
  return QcomPciGetBusRange(Descriptors, MinBus, MaxBus, BusRange);
#else
  while ((*Descriptors)->Desc != ACPI_END_TAG_DESCRIPTOR) {
    if ((*Descriptors)->ResType == ACPI_ADDRESS_SPACE_TYPE_BUS) {
      if (MinBus != NULL) {
