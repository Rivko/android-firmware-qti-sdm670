...
STATIC EFI_DXE_SERVICES  *gDS = NULL;
#else
...
#if defined (QCOM_EDK2_PATCH)
STATIC
VOID PrintMemCapabilities (EFI_MEMORY_DESCRIPTOR *MemMap)
{
  if (MemMap->Attribute & EFI_MEMORY_RUNTIME)
    AsciiPrint (" RT");
  else
    AsciiPrint (" --");

  if (MemMap->Attribute & EFI_MEMORY_NV)
    AsciiPrint (" NV");
  else
    AsciiPrint (" --");

  if (MemMap->Attribute & EFI_MEMORY_XP)
    AsciiPrint (" XP");
  else
    AsciiPrint (" --");

  if (MemMap->Attribute & EFI_MEMORY_RP)
    AsciiPrint (" RP");
  else
    AsciiPrint (" --");

  if (MemMap->Attribute & EFI_MEMORY_WP)
    AsciiPrint (" WP");
  else
    AsciiPrint (" --");

  if (MemMap->Attribute & EFI_MEMORY_UCE)
    AsciiPrint (" UCE");
  else
    AsciiPrint (" ---");

 if (MemMap->Attribute & EFI_MEMORY_WB)
    AsciiPrint (" WB");
  else
    AsciiPrint (" --");

  if (MemMap->Attribute & EFI_MEMORY_WT)
    AsciiPrint (" WT");
  else
    AsciiPrint (" --");
