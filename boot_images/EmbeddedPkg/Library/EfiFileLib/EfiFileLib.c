  ...
  )
{
  ...
  if (File->Type == EfiOpenLoadFile) {
    ...
    if((File->LoadFile == NULL) || (File->DevicePath == NULL)) {
      return 0;
    }
#endif
...
#if defined (QCOM_EDK2_PATCH)
    // Increase the file size, if necessary. If CurrentPosition > current file size, then the file needs to grow
    File->Size = (UINTN)File->CurrentPosition > (UINTN)File->Size ? (UINTN)File->CurrentPosition : (UINTN)File->Size;
    File->MaxPosition = (UINT64)File->Size;
#endif
    break;
