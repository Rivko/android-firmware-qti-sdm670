...
class MbnHdrV6(object):
    ...
    # Dictionary that maps matadata IDs to size and name
    METADATA_0_0_QTI = {1: ("I", "sw_id"),
                        2: ("I", "hw_id"),
                        3: ("I", "oem_id"),
                        4: ("I", "sw_size"),
                        5: ("I", "model_id"),
                        ...
                        7: ("I", "flags"),
                        8: ("I"*12, "soc_vers"),
                        9: ("I"*8, "multi_serial_numbers"),
                        10: ("I", "mrc_index"),
                        11: ("I", "anti_rollback_version")}
    ...
    def _unpack(self, data):
        ...
        if self.version == MBN_HDR_VERSION_3:
            raise RuntimeError("Cannot update MBN v3 to MBN v6")

        # Determine original size of header so rest of hash segment can be properly extracted
        if self.version == MbnHdr40BV5.get_version():
    ...
    def _unpack_metadata(self, data, end, authority=AUTHORITY_QTI):
        ...
        if authority == AUTHORITY_QTI:
            ...
            # Check if oem metadata is packed with zeros (zero filled when generating QTI signed double-signable image)
        ...
        for i in range(len(metadata_fields)):
            offset = end
            end = offset + struct.Struct(id_format).size
            extracted_id, = struct.unpack(id_format, data[offset:end])
            if extracted_id in metadata_fields.keys():
                field = metadata_fields[extracted_id]
            else:
                raise RuntimeError("MBN v6 header {0} metadata v{1}.{2} does not support a metadata value with ID {3}".format(authority, metadata_major_version, metadata_minor_version, extracted_id))
            field_format = field[0]
            field_name = field[1]
            offset = end
            end = offset + struct.Struct(field_format).size
    ...
    def pack(self):
        ...
        if self.metadata_size > 0:
            ...
            if self.contains_zero_filled_oem_metadata:
                # Pack oem metadata with zeros (zero filled when generating QTI-signed double-signable image)
    ...
    def _pack_metadata(self, data, metadata, metadata_fields, flag_fields, authority, major_version, minor_version):
        for field_id, field in metadata_fields.iteritems():
            ...
            elif field_name == "sw_size":
                if metadata[field_name] is None:
                    metadata[field_name] = self.get_size() + self.code_size
                data += struct.pack(field_format, metadata[field_name])
            else:
                # Pack flags
    ...
    def _get_metadata_size(self, authority=AUTHORITY_QTI):
        if authority == AUTHORITY_QTI:
            ...
            metadata_fields = MbnHdrV6.VERSION_TO_METADATA_MAP[(self.metadata_major_version, self.metadata_minor_version)]
        metadata_format = "".join([key[0] for key in metadata_fields.values()])
        metadata_ids_format = 'I' * len(metadata_fields)
        return struct.Struct(self._get_metadata_version_format() + metadata_format + metadata_ids_format).size
    ...
    def repr_ignore_attr():
        return ["original_size", "contains_zero_filled_oem_metadata", "metadata", "metadata_qti"]
