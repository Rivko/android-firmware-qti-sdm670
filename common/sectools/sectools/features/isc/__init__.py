...
class SecImageCore(object):
    ...
    def set_image_path(self, image_path, sign_id):
        ...
        if self._img_config_parser is not None and self._parsegen_config is not None:
            self._stager = stager.ImagePathsStager(image_path, self._img_config_parser, self._parsegen_config, self.authority, sign_id)
        else:
    ...
    def set_meta_build_path(self, meta_build_path, sign_id_list=[]):
        ...
        if self.meta_supports_sign_id(meta_build_path):
            self._stager = stager.MetaBuildStager(meta_build_path, self._config_dir_obj, self._parsegen_config, self.authority, sign_id_list)
        else:
            self._stager = stager.MetaBuildTmpStager(meta_build_path, self._config_dir_obj, self._img_config_parser, self._parsegen_config, self.authority, sign_id_list)
    ...
    def process(self,
        ...
        if not (verify_setup or integrity_check or sign or encrypt or decrypt or val_image or val_integrity_check or val_sign or val_encrypt):
        ...
        def _process(idx, image, sign_attr, integrity_check, sign, encrypt, decrypt, val_image, val_integrity_check, val_sign, val_encrypt):
            ...
            try:
                ...
                if integrity_check or sign or encrypt or decrypt:
                    parsegen = self._process_secure_operation(image, progress, security_policy_list, sign_attr, integrity_check, sign, encrypt, decrypt)
        ...
        for idx, image in enumerate(self.image_info_list):
            _process(idx, image, sign_attr, integrity_check, sign, encrypt, decrypt, val_image, val_integrity_check, val_sign, val_encrypt)

        progress.complete()

    def _process_secure_operation(self, image, progress, security_policy_list, sign_attr, i_integrity_check, i_sign, i_encrypt, i_decrypt, idx=0, prefix_override=None):
        from imageinfo import ImageInfo, StatusInfo
        # Check bounds
    ...
    def _create_parsegen_obj(self, image, validating=False, signing=False, prefix_override=None, sign_attr=False):
        c_path.create_debug_dir(image.dest_image.debug_dir_parsegen)
        from sectools.features.isc.parsegen import get_parser
        parsegen = get_parser(image, validating, signing, prefix_override, sign_attr)
