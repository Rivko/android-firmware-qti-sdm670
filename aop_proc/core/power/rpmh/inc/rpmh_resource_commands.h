...
{
  RPMH_RC_CLEAN = 0,
  RPMH_RC_CLEAN_EXPLICIT_VOTE = 1,    //used for explicit clients which don't exist on AOP. 
  RPMH_RC_LAST_CLEAN = 1,             //used for explicit clients which don't exist on AOP.
  RPMH_RC_DIRTY = 2,
...
typedef struct
{
  ...
  boolean            completion; //whether or not the last vote was response required
 ...
 * @param completion : If the latest command was completion based
