...
uint32 rpmh_cmdq_send_read (rpmh_cmdq_t *self, uint32 address, rpmh_client_t *client, boolean queue, uint32 drv_index)
{
  ...
  if (!queue) 
  {
    ...
    //RPMH_LOG_1(rpmh_log_holding_read_request, 1); //NO FREE AMCS
  }
  else
  {
    //RPMH_LOG_1(rpmh_log_holding_read_request, 0); //PDC WORKAROUND READ LIMIT
  }
                               ...
                               rpmh_command_set_internal_t *command_set, boolean *rc_idle)
{
  *rc_idle = TRUE;
  ...
  CORE_VERIFY(cmd->num_rcs <= self->num_cmds_per_tcs);
  CORE_VERIFY(command_set->num_commands <= self->num_cmds_per_tcs);
  ...
  switch (command_set->set)
  {
    ...
    case RPMH_SET_ACTIVE:
      ...
      else
      {
        ...
        //RPMH_LOG_1(rpmh_log_holding_request, rc_idle);
        inq_append (self->enqueued, cmd);
      }
      break;
    default:
