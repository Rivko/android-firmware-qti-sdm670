...
pdc_seq_result_t pdc_seq_handle_init (pdc_seq_handle handle)
{
  ...
  if ((handle == NULL) || (handle->cfg == NULL) || 
      (handle->modes == NULL) || (handle->offset == NULL))
  {
...
/* Simple PDC sequencer test basic sanity */
void pdc_seq_test(void)
{
  pdc_seq_handle h;
  pdc_seq_result_t res;

  pdc_seq_driver_init();

  pdc_seq_handles ("aop", 1, &h);
  res = pdc_seq_handle_init (h);
  pdc_seq_set_lpm (h, 2);
  pdc_seq_set_wakeup_time (h, 0x123456789ABCD);

  CORE_VERIFY (res == PDC_SEQ_SUCCESS);
}
#endif
