...
#define RSC_ADDR_TO_IPCB_ADDR(addr) ((((addr) & 0x70000) << 5) | ((addr) & 0xFFFF))
...
/**
 * <!-- cmd_db_query_addr_ipcb -->
 *
 * @brief Query command db for resource id address in iPCB format.
 *
 *  This is used to retrieve resource address based on resource
 *  id.
 *
 ...
 *  @return 0 for failure, non-zero for offset address
 */
uint32_t cmd_db_query_addr_ipcb(const char *res_id)
{
  uint32_t addr = cmd_db_query_addr(res_id);

  /* Convert to iPCB format from RSC format. */
  if (addr != 0)
  {
    addr = RSC_ADDR_TO_IPCB_ADDR(addr);
  }
