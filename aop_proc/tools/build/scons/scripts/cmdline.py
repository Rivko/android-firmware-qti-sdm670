...
#-------------------------------------------------------------------------------
#
#  $Header: //components/rel/aop.ho/1.1.c1/aop_proc/tools/build/scons/scripts/cmdline.py#1 $
...
def generate(env):
   ...
   vars.AddVariables(
      ...
      ('CHIPSET_VERSION', 'Full target chipset_version ', None, validator),
      ('CHIPSET_PMIC', 'Full target chipset_pmic ', None, validator),
   )
   ...
   if env['MSM_ID'] in ['670']:
        mbn_root =  BUILD_ROOT +'/build/ms/bin/' + env['BUILD_ID'] + '/' + env['CHIPSET_PMIC']  
   else:  
        mbn_root =  BUILD_ROOT +'/build/ms/bin/' + env['BUILD_ID']
